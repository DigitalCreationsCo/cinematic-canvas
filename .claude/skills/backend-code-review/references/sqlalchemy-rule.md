...
- Key modules:
  - Session management (canonical source): `px.services.deps.session_scope`, `px.services.deps.session_scope_readonly`
  - Portals wrappers (delegates to px): `portals.services.deps.session_scope`, `portals.services.deps.session_scope_readonly`
  - Injectable dependencies: `px.services.deps.injectable_session_scope`, `px.services.deps.injectable_session_scope_readonly`
  - Database service: `portals.services.database.service.DatabaseService`
  - Model CRUD functions: `src/backend/base/portals/services/database/models/*/crud.py`
- Import preference: Use `from portals.services.deps import session_scope` in Portals code for consistency. The Portals wrappers are thin `@asynccontextmanager` functions that delegate to `px.services.deps`. Only import directly from `px` when working inside the `px` package itself.
...
- Example:
  ...
  - Good:
    ...
    from portals.services.deps import session_scope_readonly
...
- Category: security
- Severity: critical
- Description: Reads and writes against user-owned tables must be scoped by `user_id` to prevent cross-user data leakage or corruption. Portals uses `user_id` (not `tenant_id`) for user-scoped data isolation. Every query on user-owned entities (flows, variables, folders, messages, API keys) must include a `user_id` filter.
...
- Severity: suggestion
- Description: Raw SQL via `text()` should be exceptional. ORM/Core expressions are easier to evolve, safer to compose, dialect-portable (SQLite + PostgreSQL), and more consistent with the codebase. Portals uses `sqlmodel.select()` for queries which provides both type safety and dialect portability.
...
- Category: quality
- Severity: critical
- Description: Multi-writer paths without explicit concurrency control can silently overwrite data. Choose the safeguard based on contention level, lock scope, and throughput cost instead of defaulting to one strategy. Portals's async architecture means multiple coroutines may attempt concurrent writes.
- Suggested fix:
