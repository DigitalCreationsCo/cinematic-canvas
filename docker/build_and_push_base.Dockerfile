...
RUN npm install \
    ...
    && cp -r build /app/src/backend/base/portals/frontend \
    && rm -rf /tmp/src/frontend

WORKDIR /app/src/backend/base
# portals-base ships the core framework only.  The extension bundles
# (lfx-duckduckgo, lfx-arxiv, lfx-ibm, lfx-docling) are intentionally NOT
# installed in this image: they are dependencies of the full ``portals``
# distribution, not of the lean ``portals-base`` core, and we keep that
# boundary at the image layer too.  Consumers who want those components
# should use the ``portals`` image, or ``pip install`` the bundle (e.g.
# ``lfx-duckduckgo``) alongside portals-base.
RUN --mount=type=cache,target=/root/.cache/uv \
    RUSTFLAGS='--cfg reqwest_unstable' \
...
# Pre-create PORTALS_CONFIG_DIR (the default location used by the docker_example
# compose file) with the non-root user as owner. When the official compose mounts
# a fresh named volume at /app/portals, Docker copies this directory's ownership
...
# as root:root and Portals crashes during startup with PermissionError on
# /app/portals/secret_key. See https://github.com/portals-ai/portals/issues/10437
RUN mkdir -p /app/portals && chown -R 1000:0 /app/portals && chmod -R g+rwX /app/portals

LABEL org.opencontainers.image.title=portals
