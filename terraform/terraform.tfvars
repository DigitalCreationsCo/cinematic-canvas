...
# Disk Configuration
boot_disk_image   = "projects/ml-images/global/images/c0-deeplearning-common-gpu-v20241209-debian-11-py310"
...
ssh_source_ranges = ["0.0.0.0/0"]  # Change to your IP: ["YOUR_IP/32"]
http_source_ranges = ["0.0.0.0/0"]  # Change for production security
...
whitelisted_ip_ranges = []  # Example: ["YOUR_OFFICE_IP/32"]

# Autoscaling Configuration (ZERO COST WHEN IDLE!)
autoscaling_min_replicas = 0  # Set to 0 for zero cost when idle, 1+ for instant response
autoscaling_max_replicas = 5  # Maximum concurrent instances
autoscaling_cpu_target = 70  # Scale up when CPU > 70%
...
autoscaling_initial_delay_sec = 600  # Wait 10 minutes for model loading before health checks
autoscaling_mode = "ON"  # ON, OFF, or ONLY_SCALE_OUT