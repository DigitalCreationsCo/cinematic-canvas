...
description: Refactor high-complexity React components in Portals frontend. Use when manual complexity assessment shows complexity > 50 or lineCount > 300, when the user asks for code splitting, hook extraction, or complexity reduction; avoid for simple/well-structured components, third-party wrappers, or when the user explicitly wants testing without refactoring.
---

# Portals Component Refactoring Skill

Refactor high-complexity React components in the Portals frontend codebase with the patterns and workflow below.
...
Since Portals does not have automated complexity analysis tools, assess components manually:
...
**Portals Convention**: Place hooks in a `hooks/` subdirectory or alongside the component as `use-<feature>.ts`. Portals uses kebab-case filenames with `use-` prefix.
...
**Portals Convention**: Place sub-components in subdirectories or as separate files in the same directory. UI primitives go in `components/ui/`, domain components in `components/core/`, reusable components in `components/common/`.
...
**Portals Convention**: Modals should be extracted with their state management.
...
**Portals Convention**: Extract form state and validation into hooks.
...
## Portals-Specific Refactoring Guidelines
...
**Portals Reference**: All stores in `src/frontend/src/stores/` follow this selector pattern.
