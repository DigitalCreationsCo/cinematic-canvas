---
name: "source-command-technical-update-skeumorphic-white-diagrams"
...
Use this skill when the user asks to run the migrated source command `technical-update-skeumorphic-white-diagrams`.
...
Reproduce the exact visual language of the attached reference diagram (`2026-04-08-authorization-diagram.png`). Every technical graphic MUST conform to all of the rules below without deviation.

**Canvas & Background**
- White canvas (`#FFFFFF`) with no border or outer frame.
- Subtle overall light — no dark mode, no gradients on the canvas itself.
...
- Arrow stroke: `#4D8EF5`
- Drop shadow: `rgba(77, 142, 245, 0.12)` — soft, low-spread, no hard edges.

**Panel & Card Anatomy**
- Group related concepts inside rounded-rectangle panels (`border-radius: 18px`).
- Each panel has: a bold ALL-CAPS header label in `#1A2B4A`, a `#EEF4FF` fill, and a `1px` solid border at `rgba(77,142,245,0.2)`.
- Inner cards (individual nodes) are white (`#FFFFFF`), `border-radius: 12px`, with the same soft blue drop shadow.
- Cards may contain: a blue-filled icon (top-center or left), 1–2 lines of label text (sentence case, grotesk), and an optional small monospaced code snippet block in light grey.

**Icons**
- Use simple, filled, single-color icons in `#4D8EF5` — no outlines, no multi-color.
- Icon subjects must be semantically appropriate: locks for security/auth, database cylinders for storage, cloud shapes for object storage, API circles for service endpoints, person silhouettes for users, gear+person for roles/teams.
- Icons are circular or square with a soft `#EEF4FF` background bubble behind them.

**Typography**
- Font: Inter or equivalent geometric grotesk. No serif. No decorative fonts.
- Panel headers: ALL CAPS, `font-weight: 700`, `~13px`, `#1A2B4A`.
- Card labels: Sentence case, `font-weight: 600`, `~12px`, `#1A2B4A`.
- Sub-labels / captions beneath cards: `font-weight: 400`, `~10px`, `#6B7FA3`, centered.
- Code snippet text (if used): monospaced, `~9px`, `#4D8EF5` on a `#F0F4FF` background block.

**Arrows & Connectors**
- All arrows are directional (single arrowhead, filled blue `#4D8EF5`), `2px` stroke.
- Connector lines are straight or right-angled (orthogonal); no curved/bezier paths.
- Short inline labels on connector lines (e.g. "Pass", "Deny", "Authenticated Token") in `font-weight: 500`, `~10px`, `#6B7FA3`.

**Layout**
- Strictly left-to-right flow for sequential/pipeline diagrams. Top-to-bottom for hierarchical diagrams.
- Generous whitespace between panels — never crowded.
- Maximum 5–6 named nodes per graphic. If more are needed, group into a labeled panel rather than adding nodes.
- No decorative elements, gradients, drop shadows on text, or photographic textures.
- Error-free English labels throughout. Proofread every text element before output.
