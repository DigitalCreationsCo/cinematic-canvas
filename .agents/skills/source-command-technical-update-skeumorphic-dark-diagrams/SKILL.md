---
name: "source-command-technical-update-skeumorphic-dark-diagrams"
...
Use this skill when the user asks to run the migrated source command `technical-update-skeumorphic-dark-diagrams`.
...
Technical graphics = Skeuomorphic dark UI aesthetic. Match this color scheme exactly: deep navy/near-black background (#0a0f1e range), component cards with rounded-rectangle borders in glowing teal/cyan (#00e5c8 range), soft cyan inner glow on card edges, dark card fills (#0d1a2a range), white or near-white sans-serif labels, and small teal iconography inside cards. Secondary labels (e.g. protocol names, sub-labels) in muted teal, smaller weight.

Match this structural anatomy exactly: Horizontal or left-to-right flow layout. Group related components into labeled rectangular sections with rounded corners and a faint border. Connect sections and sub-components with directional arrows. Each component node is a rounded card with an icon and a label. Use pass/deny branching where logic dictates. Section headers in uppercase, sans-serif, teal. Node labels in white, sentence case. Sub-labels (e.g. protocol type) below the icon in small muted teal text.

Graphics must be informative and clear — not dense or overwhelming. Less is more. High-quality vector-style output, error-free English text. No mention of "Google".
