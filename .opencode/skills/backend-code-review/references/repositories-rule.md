...
- Key directories:
  - Services: `src/backend/base/portals/services/` (each service has `service.py` and `factory.py`)
  - Model CRUD functions: `src/backend/base/portals/services/database/models/*/crud.py`
  - Service base class: `portals.services.base.Service`
  - Service factory base: `portals.services.factory.ServiceFactory`
  - Service registry: `portals.services.manager.ServiceManager`
  - Service types: `portals.services.schema.ServiceType`
  - Dependency helpers: `portals.services.deps` (provides `get_service()`, `get_xxx_service()` functions)
...
- Severity: suggestion
- Description: Portals uses a service layer pattern rather than a repository pattern. Each service under `src/backend/base/portals/services/` encapsulates business logic and data access for its domain. If a service already handles operations for a given model/table, all reads/writes/queries for that table should go through the existing service (or its associated CRUD module). Additionally, many models have CRUD utility functions in `src/backend/base/portals/services/database/models/<model>/crud.py` that should be reused rather than duplicated.
- Suggested fix:
  - First check `src/backend/base/portals/services/` and `src/backend/base/portals/services/database/models/<model>/crud.py` to verify whether the table/model already has a service or CRUD abstraction. If it exists, route all operations through it and add missing methods instead of bypassing it with ad-hoc SQLAlchemy queries.
...
- Severity: critical
- Description: Portals manages services through `ServiceManager` which uses `ServiceFactory` instances to create and configure services. Each service consists of:
  1. A base class or protocol (optional, for abstraction) inheriting from `portals.services.base.Service`
  2. A concrete implementation in `service.py`
  3. A factory class in `factory.py` inheriting from `portals.services.factory.ServiceFactory`
  4. A `ServiceType` enum entry in `portals.services.schema`
  5. A `get_xxx_service()` function in `portals.services.deps`
...
- Suggested fix: When introducing a new service, create all required files following the existing pattern. Register the factory in `ServiceManager.get_factories()` and add a convenience getter in `portals.services.deps`.
- Example:
  - Bad:
    ```python
  ...
  - Good:
    ```python
    # src/backend/base/portals/services/notification/service.py
    ...
    # src/backend/base/portals/services/notification/factory.py
    from portals.services.factory import ServiceFactory
    from portals.services.notification.service import NotificationService
...
- Example:
  - Bad:
    ...
    # src/backend/base/portals/services/flow_stats/service.py
  ...
  - Good:
    ...
    # src/backend/base/portals/services/database/models/flow/utils.py
...
- Example:
  - Bad:
    ...
    from portals.services.database.models.message.crud import get_messages_by_flow_id
