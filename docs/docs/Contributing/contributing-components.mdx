...
New components are added as objects of the [`Component`](https://github.com/portals-ai/portals/blob/main/src/px/src/px/custom/custom_component/component.py) class.

Dependencies are added to the [pyproject.toml](https://github.com/portals-ai/portals/blob/main/pyproject.toml) file.

## Contribute an example component to Portals

Anyone can contribute an example component. For example, to create a new data component called **DataFrame processor**, follow these steps to contribute it to Portals.

<PartialBasicComponentStructure />

5. Save the `dataframe_processor.py` to the `src/px/src/px/components` directory.
This example adds a data component, so add it to the `/data` directory.

6. Add the component dependency to `src/px/src/px/components/data/__init__.py` as `from .DataFrameProcessor import DataFrameProcessor`.
You can view the [/data/__init__.py](https://github.com/portals-ai/portals/blob/dev/src/px/src/px/components/data/__init__.py) in the Portals repository.

7. Add any new dependencies to the [pyproject.toml](https://github.com/portals-ai/portals/blob/main/pyproject.toml#L20) file.

8. Submit documentation for your component. For this example component, you would submit documentation to the [Data components page](https://github.com/portals-ai/portals/blob/main/docs/docs/Components/components-data.mdx).

9. Submit your changes as a pull request. The Portals team will review, suggest changes, and add your component to Portals.
