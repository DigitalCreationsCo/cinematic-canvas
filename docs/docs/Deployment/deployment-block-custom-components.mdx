...
Custom components and the component code editor allow Portals users to create and execute Python code in the Portals runtime.
While this freedom is a feature of Portals, in some deployments you might want to block the creation of custom Portals components to prevent arbitrary code execution.

To block custom component creation on a Portals server, set the following [environment variable](/environment-variables):

```bash
LANGFLOW_ALLOW_CUSTOM_COMPONENTS=false
```

When set to `false`, Portals blocks creating custom components and changing code in the visual editor.

When unset or `true`, Portals allows custom code.
Existing Portals installations will keep the default `true` behavior until you opt in to this restriction.

This environment variable is a beta feature, and should not be your only safeguard in production environments.

In the event that Portals could execute untrusted or LLM-generated code, run Portals in an isolated and containerized execution environment.
...
This environment variable replaces the default index bundled with the `px` package.
...
If both environment variables are set, Portals builds one combined set of components from the custom index _and_ from `LANGFLOW_COMPONENTS_PATH`. If the same component category name exists in both, `LANGFLOW_COMPONENTS_PATH` replaces that whole category from the custom index.
