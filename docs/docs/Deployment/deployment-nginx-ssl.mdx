---
title: Deploy Portals with Nginx and SSL
slug: /deployment-nginx-ssl
---

import Icon from "@site/src/components/icon";

Deploy Portals on a Linux-based server using Nginx as a reverse proxy, Let's Encrypt for SSL certificates, and Certbot for automated certificate management.

This setup encrypts all communications between users and your Portals server. SSL certificates ensure that sensitive data is protected from eavesdropping and tampering, and the automatic certificate management through Certbot eliminates the complexity of manual SSL configuration.
...
## Install Portals on your server

To install Portals on your server, do the following:
...
3. Install uv to manage Python packages.
    Portals recommends uv for faster installation.
    ```bash
    pip install uv
    ```

4. Create a virtual environment for Portals.
    ```bash
    uv venv portals-venv
    source portals-venv/bin/activate
    ```

5. Install Portals using uv.
    ```bash
    uv pip install portals
    ```

6. Optionally, start Portals.
    ```bash
    uv run portals run --host 127.0.0.1 --port 7860 &
    ```

    Test access at your `http://YOUR_PUBLIC_IP:7860`.
    Since Portals is running on `localhost`, you should not be successful.
...
Nginx is a reverse proxy that receives external requests and forwards them to your Portals server. In addition to SSL termination, Nginx includes features for load balancing and security. For more information, see the [Nginx documentation](https://nginx.org/en/docs/).
...
Replace **DOMAIN_NAME** with your actual domain name, such as `portals.example.com`.
    ```bash
...
3. Start Portals from the virtual environment.
    ...
    b. Start Portals in the background.
    ```bash
    uv run portals run --host 127.0.0.1 --port 7860 &
    ```
...
- [Deploy Portals on a remote server with Caddy](/deployment-caddyfile)
