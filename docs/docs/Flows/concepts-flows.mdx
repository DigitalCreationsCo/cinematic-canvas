...
Portals flows are fully serializable and can be saved and loaded from the file system where Portals is installed.

:::tip
To try building and running a flow in a few minutes, see the [Portals quickstart](/get-started-quickstart).
:::
...
* **Create a flow from a template**: Select a project, click **New Flow**, and then click the template that you want to use.
    ...
    You can also [contribute templates](/contributing-templates) to the Portals codebase.
...
You can also create a flow with the [Portals API](/api-flows), but the Portals team recommends using the [visual editor](/concepts-overview) until you are familiar with flow creation.
...
When you're ready to use Portals for application development, learn how to [trigger flows with the Portals API](/concepts-publish), explore more advanced configuration options like [custom dependencies](/install-custom-dependencies), and, eventually, [containerize your Portals application](/develop-application).

When you're ready to go to production or deploy a Portals MCP server for access over the public internet, see [Portals deployment overview](/deployment-overview).

#### Flow graphs

When a flow runs, Portals builds a Directed Acyclic Graph (DAG) object from the nodes (components) and edges (connections), and the nodes are sorted to determine the order of execution.
...
The **Projects** page is where you arrive when you launch Portals.
From here, you can manage flows and your projects' [MCP servers](/mcp-server).

Portals projects are like folders that you can use to organize related flows.
...
:::tip
To get back to the **Projects** page after editing a flow, click the project name or Portals icon in the Portals header.
:::
...
Saved flow versions are stored in the database configured for your Portals deployment.

When restoring a saved version, if **Save current draft before restoring** is enabled, then Portals creates a backup of your current work before the selected version replaces it.
...
When previewing a saved version, Portals temporarily pauses auto-save so the preview doesn't overwrite your current draft.
...
By default, flows and flow execution data are stored in the Portals database, and flow logs are stored with other Portals logs in the Portals config directory.
