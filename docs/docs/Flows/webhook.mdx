...
1. In Portals, open the flow where you want to use the **Webhook** component.
...
7. Send a POST request with `data` to the flow's `webhook` endpoint to trigger the flow.
    ...
    A successful response indicates that Portals started the flow.
...
To learn about triggering flows with payloads from external applications, see the video tutorial [How to Use Webhooks in Portals](https://www.youtube.com/watch?v=IC1CAtzFRE0).
...
To allow webhooks to run without authentication (not recommended; use only in trusted environments), in your Portals `.env` file, set `LANGFLOW_WEBHOOK_AUTH_ENABLE=False`. When disabled, requests to the webhook endpoint are treated as being sent by the flow owner.

When webhook authentication is enabled, you must provide a Portals API key with each webhook request.
...
- [Get started with the Portals API](/api-reference-api-examples)
