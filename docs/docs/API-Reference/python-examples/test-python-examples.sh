...
for file in "${PY_FILES[@]}"; do
  ...
  if [[ "$MODE" == "execute" ]]; then
    # api-openai-responses/* call Portals's HTTP API (e.g. /api/v1/responses) with x-api-key;
    ...
    if ! uv run python - "$file" "$PY_TIMEOUT_SECONDS" >/tmp/portals-python-example.out 2>/tmp/portals-python-example.err <<'PY'
import runpy
import signal
import sys
...
PY
    then
      ...
      print_failure_logs "/tmp/portals-python-example.out" "/tmp/portals-python-example.err"
      ((FAIL+=1))
      continue
    fi
  fi
