...
To enable Arize tracing, set the required Arize environment variables in your Portals deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating Portals and Arize are also available in the Arize documentation:

* [Portals tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/portals/portals-tracing)
* [Portals tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/portals/portals-tracing)
:::
...
2. In the root of your Portals application, edit your existing Portals `.env` file or create a new one.
...
4. Start your Portals application with your `.env` file:
...
1. In Portals, run a flow that has an LLM-driven component, such as an **Agent** component or any language model component.
...
3. To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each Portals execution generates two traces in Arize:

    * The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    * The `UUID` trace is the trace of the Portals components.
