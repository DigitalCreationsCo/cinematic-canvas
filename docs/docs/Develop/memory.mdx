...
Portals provides flexible memory management options for storage and retrieval of data relevant to your flows and your Portals server.
This includes essential Portals database tables, file management, and caching, as well as chat memory.

## Storage options and paths

Portals supports both local memory and external memory options.

Portals's default storage option is a [SQLite](https://www.sqlite.org/) database.
The default storage path depends on your operating system and installation method:

- **Portals Desktop**:
    - **macOS**: `/Users/<username>/.portals/data/database.db`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.PortalsDesktop\data\database.db`
- **Portals OSS**
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/portals/portals.db` (Python version can vary. Database isn't shared between virtual environments because it is tied to the venv path.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/portals/portals.db`

Portals offers a few alternatives to the default database path:

* **Config directory**: Set `LANGFLOW_SAVE_DB_IN_CONFIG_DIR=True` to store the database in your Portals config directory as set in [`LANGFLOW_CONFIG_DIR`](/logging).

* **External PostgreSQL database**: You can use an external PostgreSQL database for all of your Portals storage.
For more information, see [Configure external memory](#configure-external-memory)

    External storage can be useful if you want to preserve the data after uninstalling Portals or to share the same database between multiple virtual environments.

* **Separate chat memory**: You can selectively use external storage for chat memory only, separate from other Portals storage.
For more information, see [Store chat memory](#store-chat-memory).

* **No database**: To disable all database operations and run a no-op session, set `LANGFLOW_USE_NOOP_DATABASE=True` in your [Portals environment variables](/environment-variables).
This is useful for testing when you don't want to persist any data.

## Portals database tables

The following tables are stored in `portals.db`:

• **ApiKey**: Manages Portals API authentication keys. Component API keys are stored in the **Variables** table. For more information, see [API keys and authentication](/api-keys-and-authentication).

• **File**: Stores metadata for files uploaded to Portals's file management system, including file names, paths, sizes, and storage providers. For more information, see [Manage files](/concepts-file-management).
...
• **Flow**: Contains flow definitions, including nodes, edges, and components, stored as JSON or database records. For more information, see [Build flows](/concepts-flows).
    ...
    :::tip
    To automatically remove API keys and tokens from flow data before saving a flow to the database, set `LANGFLOW_REMOVE_API_KEYS=True` in your [Portals environment variables](/environment-variables).
...
For more information, see the database models in the [source code](https://github.com/portals-ai/portals/tree/main/src/backend/base/portals/services/database/models).

## Configure external memory {#configure-external-memory}

To replace the default Portals SQLite database with another database, set the `LANGFLOW_DATABASE_URL` environment variable to your database URL, and then start Portals with your `.env` file.
...
* `LANGFLOW_DATABASE_CONNECTION_RETRY`: Whether to retry lost connections to your Portals database. If `true`, Portals tries to connect to the database again if the connection fails. Default: `false`.
...
* `LANGFLOW_MIGRATION_LOCK_NAMESPACE`: Optional namespace identifier for PostgreSQL advisory lock during migrations. If not provided, a hash of the database URL will be used. Useful when multiple Portals instances share the same database and need coordinated migration locking
...
* `LANGFLOW_MIGRATION_LOCK_NAMESPACE`: Optional namespace for PostgreSQL advisory locks used during database migrations. This is useful when running multiple Portals instances that share the same PostgreSQL database. Each instance should use a unique namespace to avoid conflicts. If not set, Portals uses a default namespace. This setting only applies when using PostgreSQL as your database backend.

## Configure cache memory

The default Portals caching behavior is an asynchronous, in-memory cache:
...
Portals officially supports only the default asynchronous, in-memory cache, which is suitable for most use cases.
...
|----------|------|---------|-------------|
| `LANGFLOW_CACHE_TYPE` | String | `async` | Set the cache type for Portals's internal caching system. Possible values: `async`, `redis`, `memory`, `disk`. If you set the type to `redis`, then you must also set the `LANGFLOW_REDIS_*` environment variables. |
| `LANGFLOW_LANGCHAIN_CACHE` | String | `InMemoryCache` | Set the cache storage type for the LangChain caching system (a Portals dependency), either `InMemoryCache` or `SQLiteCache`. |
...
Chat-based flows with a **Chat Input** or **Chat Output** component produce chat history that is stored in the Portals `messages` table.
...
All messages are stored in [Portals storage](#storage-options-and-paths), and the component provides minimal memory configuration options, such as the number of messages to retrieve.
...
* **Message History component**: By default, this component stores and retrieves memories from Portals storage, unless you attach a third-party chat memory component. It provides a few more options for sorting and filtering memories, although most of these options are also built-in to the **Agent** component as configurable or fixed parameters.
...
Typically, this is necessary only if you have specific storage needs that aren't met by Portals storage.
For example, if you want to manage chat memory data by directly working with the database, or if you want to use a different database than the default Portals storage.

For more information and examples, see [**Message History** component](/message-history) and [Agent memory](/agents#agent-memory).

## See also

* [Portals file management](/concepts-file-management)
* [Portals logs](/logging)
