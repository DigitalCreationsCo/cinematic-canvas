...
Portals provides several components to help you store and retrieve vector data in your flows, including embedding models, vector stores, and knowledge bases.
...
Knowledge bases are stored local to your Portals instance.
The default storage location depends on your operating system and installation method:

- **Portals Desktop**:
    - **macOS**: `/Users/<username>/.portals/knowledge_bases`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.PortalsDesktop\knowledge_bases`
- **Portals OSS**:
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/portals/knowledge_bases` (Python version can vary. Knowledge bases aren't shared between virtual environments.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/portals/knowledge_bases`
