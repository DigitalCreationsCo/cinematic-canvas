...
To modify environment variables for Portals Desktop, set environment variables in a Desktop `.env` file, and then restart the app.
...
1. Create or edit `~/.portals/data/.env`.
2. Add your Portals environment variables, for example:

    ```text
    PORTALS_LOG_LEVEL=info
    LANGFLOW_DOCLING=true
    ```

3. Save the file.
4. Restart Portals Desktop.
...
1. Create or edit `%APPDATA%\com.PortalsDesktop\data\.env`.
2. Add your Portals environment variables, for example:
...
Windows supports two sources for Portals Desktop environment variables: a Portals application `.env` file, and Windows user environment variables.

The `.env` file at `%APPDATA%\com.PortalsDesktop\data\.env` is the recommended approach, but
Windows user variables are useful for single-sourcing API keys between Portals and other Windows applications.
If the same variable is defined in both the Portals application `.env` file and as a Windows user environment variable, the `.env` file takes precedence.
...
5. Click **OK**, and then **restart Portals Desktop**.
...
When running Portals in a production environment using Gunicorn, you can configure Gunicorn workers using the `GUNICORN_CMD_ARGS` environment variable.
...
| `LANGFLOW_COMPONENTS_INDEX_PATH` | String | Not set | File path or URL (`http://` or `https://`) to a prebuilt component index JSON file used to populate built-in components in the visual editor. When not set, Portals uses the included index. Useful for supplying a curated component index, for example in airgapped deployments. For more information, see [Block custom components](../Deployment/deployment-block-custom-components.mdx). |
