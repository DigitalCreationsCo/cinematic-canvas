...
You can configure Portals to collect tracing data about your flow executions and automatically send the data to Openlayer for analysis, monitoring, and evaluation.
...
:::tip
If you need a flow to test the Openlayer integration, see the [Portals quickstart](/get-started-quickstart).
:::
...
3. Set your Openlayer credentials as environment variables in the same environment where you run Portals.
...
## Start Portals and view traces in Openlayer

1. Start Portals in the same environment where you set the Openlayer environment variables:

    ```bash
    uv run portals run
    ```

2. Run a flow in Portals.

    Portals automatically collects and sends tracing data about the flow execution to Openlayer, including:
...
To disable the Openlayer integration, remove the `OPENLAYER_API_KEY` environment variable, and then restart Portals.
