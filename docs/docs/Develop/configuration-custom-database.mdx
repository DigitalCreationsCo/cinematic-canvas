...
Portals's default database is [SQLite](https://www.sqlite.org/docs.html), but you can configure Portals to use PostgreSQL instead.

This guide walks you through setting up an external database for Portals by replacing the default SQLite connection string `sqlite:///./portals.db` with PostgreSQL, both in local and containerized environments.

In this configuration, all structured application data from Portals, including flows, message history, and logs, is instead managed by PostgreSQL.
...
Portals can more efficiently handle multiple users and larger workloads by using PostgreSQL as the database.

## Prerequisites

- A [PostgreSQL](https://www.pgadmin.org/download/) database version 15 or later

## Connect Portals to a local PostgreSQL database

1. If Portals is running, stop Portals with <kbd>Ctrl+C</kbd>.
...
3. Edit or create a Portals `.env` file:
...
4. In your `.env` file, set `LANGFLOW_DATABASE_URL` to your PostgreSQL connection string:
    ...
    Portals uses [SQLAlchemy](https://www.sqlalchemy.org/) with the [psycopg](https://www.psycopg.org/) driver to pass SSL parameters directly to the PostgreSQL connection.

    :::warning PostgreSQL driver compatibility
    Portals requires `psycopg2-binary` or `psycopg[binary]` as the PostgreSQL driver. The `asyncpg` driver is not compatible with Portals's current database schema due to stricter timezone handling requirements.
    :::
    ...
    - **`sslmode=verify-full`**: Requires SSL, verifies the server certificate, and checks the request hostname against the certificate hostname. The `db.example.com` hostname in this example must match the server certificate's CN. This option is the most secure.
        ...
        Do not use the Portals environment variables [`LANGFLOW_SSL_CERT_FILE`](/environment-variables#server) and [`LANGFLOW_SSL_KEY_FILE`](/environment-variables#server) for your PostgreSQL certificates: these variables are for enabling HTTPS on the Portals server, not for PostgreSQL database connections.
...
5. Save your changes, and then start Portals with your `.env` file:
...
6. In Portals, run any flow to create traffic.
...
## Deploy Portals and PostgreSQL containers with docker-compose.yml

Launching Portals and PostgreSQL containers in the same Docker network ensures proper connectivity between services.
For an example, see the [`docker-compose.yml`](https://github.com/portals-ai/portals/blob/main/docker_example/docker-compose.yml) file in the Portals repository.

The configuration in the example `docker-compose.yml` also sets up persistent volumes for both Portals and PostgreSQL data.
...
In contrast, if you run PostgreSQL separately with `docker run`, it launches in a different network than the Portals container, and this prevents Portals from connecting to PostgreSQL using the service name.

To start the Portals and PostgreSQL services with the example Docker Compose file, navigate to the `portals/docker_example` directory, and then run `docker-compose up`.
...
## Deploy multiple Portals instances with a shared PostgreSQL database

To configure multiple Portals instances that share the same PostgreSQL database, modify your `docker-compose.yml` file to include multiple Portals services.

This example populates the values in `docker-compose.yml` with values from your Portals `.env` file.
...
You can access the first Portals instance at `http://localhost:7860`, and the second Portals instance at `http://localhost:7861`.
...
Your container name may vary.
    ...
    ```bash
    docker exec -it docker-test-postgres-1 psql -U portals -d portals
    ```

5. Query the database for active connections:

    ```sql
    portals=# SELECT * FROM pg_stat_activity WHERE datname = 'portals';
    ```
...
Since each Portals instance runs in its own container on the Docker network, using different incoming IP addresses confirms that both instances are actively connected to the PostgreSQL database.

7. To quit `psql`, type `quit`.

## See also

* [Portals database guide for enterprise DBAs](/enterprise-database-guide)
