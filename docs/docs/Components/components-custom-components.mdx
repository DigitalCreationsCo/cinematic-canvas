...
The `build_df` method processes data and returns a `Table`. The `-> DataFrame` type annotation helps Portals validate connections and provides color-coding in the visual editor. You can also set `self.status` to show progress messages in the UI.
