...
import RecursiveCharacterSource from "!!raw-loader!@portals/src/px/src/px/components/langchain_utilities/recursive_character.py";
...
:::tip
Portals provides keyboard shortcuts for the workspace.

In the Portals header, click your profile icon, select **Settings**, and then click **Shortcuts** to view the available shortcuts.
:::
...
* <Icon name="Component" aria-hidden="true" /> **Core components**: Portals's base components are grouped by purpose, such as **Inputs and Outputs** or **Data**.
...
For information about the programmatic representation of each data type, see [Portals data types](/data-types).
...
For example, the [**Recursive Character Text Splitter** component](https://github.com/portals-ai/portals/blob/main/src/px/src/px/components/langchain_utilities/recursive_character.py) is a child of the [`LCTextSplitterComponent`](https://github.com/portals-ai/portals/blob/main/src/px/src/px/base/textsplitters/model.py) class.
...
Component versions and states are stored in an internal Portals database. When you add a component to a flow, you create a detached copy of the component based on the information in the Portals database.
These copies are detached from the primary Portals database, and they don't synchronize with any updates that can occur when you upgrade your Portals version.
...
When editing a flow in the workspace, Portals notifies you if a component's workspace version is behind the database version so you can update the component's workspace version:
...
Components are updated to the latest available version, based on the version of Portals you are running.
