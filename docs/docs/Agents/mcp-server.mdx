---
title: Use Portals as an MCP server
slug: /mcp-server
---
...
Portals integrates with the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction) as both an MCP server and an MCP client.

This page describes how to use Portals as an MCP server that exposes your flows as [tools](https://modelcontextprotocol.io/docs/concepts/tools) that [MCP clients](https://modelcontextprotocol.io/clients) can use when generating responses.

Portals MCP servers support both the **streamable HTTP** transport and **Server-Sent Events (SSE)** as a fallback.
The default project MCP server configuration uses streamable HTTP transport at the URL path `/streamable`.

For information about using Portals as an MCP client and managing MCP server connections within flows, see [Use Portals as an MCP client](/mcp-client).

## Prerequisites

* A [Portals project](/concepts-flows#projects) with at least one flow that has a [**Chat Output** component](/chat-input-and-output).
...
* [ngrok installed](https://ngrok.com/docs/getting-started/#1-install-ngrok) and an [ngrok authtoken](https://dashboard.ngrok.com/get-started/your-authtoken) if you want to [deploy a public Portals server](/deployment-public-server).

## Serve flows as MCP tools {#select-flows-to-serve}

When you create a [Portals project](/concepts-flows#projects), Portals automatically adds the project to your MCP server's configuration and makes the project's flows available as MCP tools.

If your Portals server has authentication enabled (`AUTO_LOGIN=false`), the project's MCP server is automatically configured with API key authentication, and a new API key is generated specifically for accessing the new project's flows.
For more information, see [MCP server authentication](#authentication).


### Prevent automatic MCP server configuration for Portals projects
...
### Selectively enable and disable MCP servers for Portals projects
...
To edit the names and descriptions of flow tools on a Portals MCP server, do the following:
...
Because MCP clients treat your Portals project as a single MCP server with all of your enabled flows listed as tools, unclear names and descriptions can cause the agent to select tools incorrectly or inconsistently.
...
To provide more context about your flows, make sure to name and describe your flows clearly when configuring your Portals project's MCP server.
...
After connecting your Portals MCP server to Cursor, you can ask Cursor about the resume, such as `What job experience does Emily have?`.
...
{/* The anchor on this section (connect-clients-to-use-the-servers-actions) is currently a link target in the Portals UI. Do not change. */}
## Connect clients to your Portals MCP server {#connect-clients-to-use-the-servers-actions}

Portals provides automatic installation and code snippets to help you deploy your Portals MCP servers to your local MCP clients.

<Tabs>
<TabItem value="JSON" label="JSON" default>

The JSON option allows you to connect a Portals MCP server to any local or remote MCP client.
...
4. In Portals, on the **Projects** page, click the **MCP Server** tab.
...
For example:
    ...
    The default Portals server address is `http://localhost:7860`.
    If you are using a [public Portals server](/deployment-public-server), the server address is automatically included.

    If your Portals server requires authentication, you must include your Portals API key or OAuth settings in the configuration.
    For more information, see [MCP server authentication](#authentication).
...
8. Confirm that your Portals MCP server is on the client's list of MCP servers.
...
Auto install requires the client to be installed locally so Portals can write to the client's configuration file.
...
1. Install [Cursor](https://docs.cursor.com/get-started/installation), [Claude](https://claude.ai/download), or [Windsurf](https://windsurf.com/download/editor) on the same computer where your Portals server is running.

2. Recommended: Configure [authentication](#authentication) for your MCP server.

3. In Portals, on the **Projects** page, click the **MCP Server** tab.

4. On the **Auto install** tab, find your MCP client provider, and then click <Icon name="Plus" aria-hidden="true"/> **Add**.

    Your Portals project's MCP server is automatically added to the configuration file for your local Cursor, Claude, or Windsurf client.
    For example, with Cursor, the server configuration is added to the `mcp.json` configuration file.

    Portals attempts to add this configuration even if the selected client isn't installed.
    To verify the installation, check the available MCP servers in your client.

</TabItem>
</Tabs>

Once your MCP client is connected to your Portals project's MCP server, your flows are registered as tools.
...
Each [Portals project](/concepts-flows#projects) has its own MCP server with its own MCP server authentication settings.

When you create a new project, Portals automatically configures authentication for the project's MCP server based on your Portals server's authentication settings. If authentication is enabled (`AUTO_LOGIN=false`), the project is automatically configured with API key authentication, and a new API key is generated for accessing the project's flows.

To configure authentication for a Portals MCP server, go to the **Projects** page in Portals, click the **MCP Server** tab, click <Icon name="Fingerprint" aria-hidden="true"/> **Edit Auth**, and then select your preferred authentication method:

<Tabs groupId="auth-type">
<TabItem value="API key" label="API key">

When authenticating your MCP server with a Portals API key, your project's MCP server **JSON** code snippets and **Auto install** configuration automatically include the `--headers` and `x-api-key` arguments in the **args** array (for streamable transport).

Click <Icon name="key" aria-hidden="true"/> **Generate API key** to automatically insert a new Portals API key into the code template.
Alternatively, you can replace `YOUR_API_KEY` with an existing Portals API key.
...
When OAuth is enabled, Portals automatically starts an [MCP Composer](https://pypi.org/project/mcp-composer) instance for your project, creating a secure client-side proxy between MCP clients and the `mcp-proxy` on your server.

OAuth integration allows your Portals MCP server to authenticate users and applications through any OAuth 2.0 compliant service. When users or applications connect to your MCP server, they are redirected to your chosen OAuth provider to authenticate. Upon successful authentication, they are granted access to your flows as MCP tools.

Before configuring OAuth in Portals, you must first set up an OAuth application with an external OAuth 2.0 service provider.
You must register your Portals server as an OAuth client, and then obtain the required values to complete the configuration in Portals.
...
| **MCP Scope** | Scope for MCP operations | You define this. As of Portals 1.6, `user` is the only available value. | `user` |
| **Provider Scope** | OAuth provider scope | You define this. As of Portals 1.6, `openid` is the only available value. | `openid` |
...
2. Configure the OAuth settings with the values from your OAuth deployment.
    ...
    The OAuth credentials are encrypted and stored securely in your Portals database.
...
4. If you already installed your Portals MCP server in your MCP client, you must update your MCP client configuration to use the new OAuth settings after enabling OAuth on your MCP server.
...
Only use this option when Portals is running in a trusted environment.
...
The following environment variables set behaviors related to your Portals projects' MCP servers:

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `LANGFLOW_MCP_SERVER_ENABLED` | Boolean | `True` | Whether to initialize an MCP server for each of your Portals projects. If `false`, Portals doesn't initialize MCP servers. |
| `LANGFLOW_MCP_SERVER_ENABLE_PROGRESS_NOTIFICATIONS` | Boolean | `False` | If `true`, Portals MCP servers send progress notifications. |
...
{/* The anchor on this section (deploy-your-server-externally) is currently a link target in the Portals UI. Do not change. */}
### Deploy your Portals MCP server externally {#deploy-your-server-externally}

To deploy your Portals MCP server externally, see [Deploy a public Portals server](/deployment-public-server).
...
3. In the MCP Inspector UI, enter the connection details for your Portals project's MCP server.
    ...
    <Tabs groupId="auth-type">
      <TabItem value="API key" label="API key" default>
        ...
        - **Command**: `uvx`
        - **Arguments**: Enter the following list of arguments, separated by spaces. Replace the values for `YOUR_API_KEY`, `LANGFLOW_SERVER_ADDRESS`, and `PROJECT_ID` with the values from your Portals MCP server. For example:
            ```bash
      ...
      <TabItem value="None" label="None">
        ...
        - **URL**: Enter the Portals MCP server's endpoint. For example:
            ```bash
...
## Troubleshoot Portals MCP servers {#troubleshooting-mcp-server}

For troubleshooting advice for MCP servers and clients, see [Troubleshoot Portals: MCP issues](/troubleshoot#mcp).
