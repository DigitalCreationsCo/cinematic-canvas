...
Portals is a **visual flow builder first**. Every change should make sense to someone whose only interface is dragging nodes onto a canvas. Use these principles to scope work; if a proposed change fails one of them, stop and reconsider.
...
2. **Every backend feature must land on the canvas.** If a capability cannot be expressed as a component (or a property of one) that a non-Python user can wire up, it is an SDK feature and belongs in `px`, not `portals-base`. Backend changes that have no visual surface are almost always off-narrative.
...
4. **`px` is the runtime, `portals-base` is the platform, `portals` is the distribution.** Components and the graph engine live in `px` (`src/px/src/px/components/`). API, auth, persistence, and multi-user concerns live in `portals-base`. The `portals` package is the integration that ships everything together. New code goes in the lowest layer that can host it. Boundaries are enforced one-way: `portals` → `portals-base` → `px`. See [ARCHITECTURE.md](./ARCHITECTURE.md).
...
10. **Two audiences, one product: visual builders and Python devs.** When their needs conflict, the visual builder wins for `portals-base` features and the Python dev wins for `px` SDK features. Don't compromise the canvas to make the SDK prettier, and don't compromise the SDK to make the canvas easier.
