...
### 1. [client/src/components/PlaybackControls.tsx](client/src/components/PlaybackControls.tsx)
**Changes:**
- Updated interface to accept `mainVideoRef`, `timelineVideoRefs`, and `onPlayMainVideo`.
- Dependencies updated (`useCallback`, `useEffect` dependencies expanded).
- Added logic for audio source switching and synchronized play/pause/seek across main video ref.
- Enhanced loop handling to reset main video time.
- Volume control now applies to main video volume if no external audio is used.

### 2. [client/src/components/SceneDetailPanel.tsx](client/src/components/SceneDetailPanel.tsx)
**Changes:**
- Renamed `onPlayVideo` to `onPlayMainVideo`.
- Added `currentTime`, `isPlaying`, `audioUrl`, `mainVideoRef` props.
- Added three key `useEffect` blocks: time sync/mute control, play/pause control, and source reload handler.
- Play button now triggers `onPlayMainVideo()`.
- Disabled native video controls (`controls={false}`).

### 3. [client/src/components/Timeline.tsx](client/src/components/Timeline.tsx)
**Changes:**
- Added `currentTime`, `isPlaying`, `audioUrl`, and `onSetTimelineVideoRefs` props.
- Implemented `videoRefs` using `useRef` for local storage and propagation.
- Added `useEffect` to propagate refs via callback.
- Added `useEffect` to manage local play/pause of timeline videos when `audioUrl` is absent.
- Timeline video segments now use `<button>` wrapper, assign refs dynamically, and disable native controls (`controls={false}`).

### 4. [client/src/hooks/use-swr-api.ts](client/src/hooks/use-swr-api.ts)
**Changes:**
- Strict check added for `projectData` existence when returning app data structure.

### 5. [server/index.ts](server/index.ts)
**Changes:**
- Added explicit `dotenv.config()` call for environment loading in API server startup.

### 6. [server/routes.ts](server/routes.ts)
**Changes:**
- Project listing endpoint filters out prefixes matching "audio".

---

## Files Created

1. **[TEMPORAL_TRACKING_IMPLEMENTATION.md](TEMPORAL_TRACKING_IMPLEMENTATION.md)** - This file detailing implementation summary.
...
## Quick Reference: Client Synchronization

**Master Control:** The external audio track (or main video audio) dictates the playback `currentTime`.
**Synchronization Target:** All video elements (main view, timeline segments) update their `.currentTime` based on the master time.
**Muting Rule:** External audio takes precedence; local video audio is muted if external audio is present.

---

## Quick Reference: Server Changes

**API Server Startup**: Requires `.env` file or environment variables for configuration loading.
**Project Listing**: Excludes any GCS prefix matching `audio`.

---

## Next Steps

Proceed to update `IMPLEMENTATION_SUMMARY.md` (Todo #3) and commit changes (Todo #4).
