...
This page summarizes significant changes to Portals in each release.
For all changes, see the [Changelog](https://github.com/portals-ai/portals/releases/latest).

Due to strict SemVer requirements, Portals Desktop can have different patch versions than the core Portals OSS Python package, but the major and minor versions are aligned.

## Prepare to upgrade

:::warning
Whenever possible, the Portals team recommends installing new Portals versions in a new virtual environment or VM before upgrading your primary installation.
...
To avoid the impact of potential breaking changes and test new versions, the Portals team recommends the following upgrade process:
...
2. Install the new version:
   ...
   * **Portals OSS Python package**: Install the new version in a new virtual environment. For instructions, see [Install and run the Portals OSS Python package](/get-started-installation#install-and-run-the-portals-oss-python-package).
   * **Portals Docker image**: Run the new image in a separate container.
   * **Portals Desktop**: To upgrade in place, open Portals Desktop, and then click **Upgrade Available** in the Portals header. If you want to isolate the new version, you must install Portals Desktop on a separate physical or virtual machine, and then [import your flows](/concepts-flows-import) to the new installation.
...
For all changes, see the [Changelog](https://github.com/portals-ai/portals/releases).

### New features and enhancements

- Portals MCP client for Bob (IBM) and Claude Code

    The **Settings** page now includes <Icon name="Terminal" aria-hidden="true"/> **Portals MCP Client**, with instructions to connect **Bob (IBM)** and **Claude Code** to your Portals instance.
    For more information, see [Portals MCP client](../Agents/portals-mcp-client.mdx).
...
- MCP Tools: register servers first, and then add from the MCP sidebar
    ...
    **MCP Tools** is no longer added from the **Agents** component list. Register each MCP server under **Settings** > **MCP Servers*, or from the flow editor <McpIcon /> **MCP** sidebar.
    ...
    For more information, see [Use Portals as an MCP client](/mcp-client#use-the-mcp-tools-component).

- Flow DevOps Toolkit SDK

    Use the Flow DevOps Toolkit SDK to manage local and remote flows with Git-style terminal commands.
    For more information, see the [Portals Flow DevOps Toolkit SDK](../API-Reference/flow-devops-sdk.mdx).
...
- Renamed `Data` and `DataFrame` types
    ...
    For more information, see [Portals data types](/data-types).
...
- Build components with **Portals Assistant**

    **Portals Assistant** is a graph-aware co-pilot that can create components from natural language prompts, directly within the Portals workspace.
    For more information, see [Portals Assistant](../Flows/portals-assistant.mdx).
...
- Vulnerability reporting now uses HackerOne
    ...
    For the full policy and what to include in a report, see the [Portals Security Policy](https://github.com/portals-ai/portals/blob/main/SECURITY.md).
...
- Deploy flows to watsonx Orchestrate (beta)
    ...
    In Portals 1.9.1, Portals flows can now be published to IBM watsonx Orchestrate as tools that a watsonx Orchestrate agent can call.
    This workflow packages a selected flow version for use in IBM watsonx Orchestrate.
    For more information, see [Deploy Portals on watsonx Orchestrate](../Deployment/deployment-wxo.mdx).

    As of Portals 1.9.2, this feature is behind a feature flag. To enable it, set `LANGFLOW_FEATURE_WXO_DEPLOYMENTS=true` before starting Portals.
...
-   `portals-base` dependency structure refactored

    The `portals-base` package now uses granular optional dependency groups. As a result, many dependencies that were previously included in the `portals-base` installation were moved to optional extras.

    If you installed Portals with `uv pip install portals`, this isn't a breaking change. Installing `portals` automatically installs `portals-base[complete]`, which includes all optional dependencies and maintains the same functionality as before.

    However, if you installed Portals with `uv pip install portals-base` without specifying extra dependencies, this _is_ a breaking change.
    Some dependencies that were previously included by default are now available only through optional extras.
    Therefore, installing `portals-base` directly only installs the [core base dependencies](https://github.com/portals-ai/portals/blob/main/src/backend/base/pyproject.toml).

    If you installed `portals-base`, there are two ways to resolve dependency errors that result from this breaking change:

    * If you need the full set of dependencies, you must install `portals-base` with the `complete` extra:

        ```bash
        uv pip install "portals-base[complete]"
        ```

    * If you need specific dependencies, you must install `portals-base` with those optional dependency groups. For example:

       ```bash
       uv pip install "portals-base[postgresql,openai,chroma]"
       ```

    For more information about available optional dependency groups, see [Install optional dependency groups for `portals-base`](/install-custom-dependencies#install-optional-dependency-groups-for-portals-base).

-   Docker image does not include `uv` or `uvx`

    :::tip
    Starting with Portals 1.8.1, the official Docker images include `uv` and `uvx` again.
    If you're using Portals 1.8.0, follow the steps in this release note to add `uv` and `uvx` in a derived image.
    :::

    In Portals 1.8.0, the Portals Docker image does not include `uv` or `uvx` in the runtime image.
    This means that MCP server configurations, including the default Portals MCP server, that rely on commands like `uvx mcp-proxy` will fail inside the container with a `command not found` error.

    If you use MCP from within a Portals 1.8.0 Docker image, you must install `uv` in an image derived from the official `portalsai/portals` image.

    To install `uv` and `uvx` in a derived image based on the official `portalsai/portals` image, do the following:

    1. Create a `Dockerfile` in your project.
    2. Use the official Portals image as the base and install `uv` into the existing virtual environment:
        ```dockerfile
        FROM portalsai/portals:1.8.0
    ...
    For more information, see [Upgrade the Portals Docker image](/deployment-docker#upgrade-the-portals-docker-image).
...
- Docling installation behavior differs between Desktop and OSS
    ...
    In Portals 1.8.0, Docling behavior depends on how Portals is installed.

    For Portals Desktop installations, you need to set `LANGFLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for Portals Desktop](/environment-variables#set-environment-variables-for-portals-desktop).

    For Portals OSS installations with `uv pip install portals`, Docling is included with the Portals package.

    For Portals OSS installations with `uv pip install portals-base`, install Docling explicitly as an extra with `uv pip install "portals-base[docling]"`.
...
- Developer API: `/workflow` synchronous endpoints (Beta)
    ...
    The Developer API is part of a larger effort to improve Portals's APIs with enhanced capabilities and better developer experience.
...
- Traces and trace view
    ...
    Portals now records execution traces for flows and components.
...
- More configuration options for JWT-based session authentication
    ...
    Portals 1.8 offers additional configuration options for JWT algorithms, including support for RS256/RS512 algorithms, configurable keys, and token lifetimes. For more information, see [JWT authentication](/jwt-authentication).
...
- Docker AMD vs ARM image sizes
    ...
    Portals 1.8.0 addresses the AMD vs ARM Docker image size gap.
...
- New [**Openlayer** observability integration](/integrations-openlayer)
    ...
    Configures Portals to send tracing data to Openlayer for analysis, monitoring, and evaluation of your flow executions.
...
All Portals versions upgrading to 1.7.0 are affected.

Don't upgrade to Portals version 1.7.0.
...
1. Revert Portals to version 1.6.9:

   ```bash
   uv pip install portals==1.6.9
   ```
...
- Support for streamable HTTP transport for MCP clients and servers
    ...
    Portals now supports [streamable HTTP transport](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) for both MCP clients and servers.
    When using Portals as an [MCP client](/mcp-client), you can connect to MCP servers with streamable HTTP transport.
    When using Portals as an [MCP server](/mcp-server), clients can connect using streamable HTTP transport.
    SSE transport is still supported as a fallback for backwards compatibility.

- Webhook authentication

    Added the `LANGFLOW_WEBHOOK_AUTH_ENABLE` environment variable for authenticating requests to the [`/webhook` endpoint](/api-flows-run#webhook-run-flow). When `LANGFLOW_WEBHOOK_AUTH_ENABLE=TRUE`, webhook endpoints require API key authentication and validate that the authenticated user owns the flow being executed. When `FALSE`, no Portals API key is required and all requests to the webhook endpoint are treated as being sent by the flow owner. For more information, see [Trigger flows with webhooks](/webhook).

- Configurable API key validation

    Added the `LANGFLOW_API_KEY_SOURCE` environment variable to control how Portals validates API keys. When set to `db`, Portals validates API keys against keys stored in the database. When set to `env`, Portals validates API keys against the `LANGFLOW_API_KEY` environment variable. For more information, see [API keys and authentication](/api-keys-and-authentication#portals-api-key-source).
...
- Email registration in Portals Desktop

    Portals Desktop now includes an email registration screen at startup. The registered email address helps track user registrations and understand the user base. For more information, see [Telemetry](/contributing-telemetry#email).
...
Portals versions 1.6.0 through 1.6.3 have a critical bug where environment variables from `.env` files aren't read.
...
If your `.env` file includes `AUTO_LOGIN=false`, upgrading to the impacted versions causes Portals to fall back to default settings, potentially giving all users superuser access immediately upon upgrade.
Additionally, database credentials, API keys, and other sensitive configurations can't be loaded from `.env` files.

_Don't_ upgrade to any Portals version from 1.6.0 through 1.6.3 if you use `.env` files for configuration.
...
:::warning
Windows users of Portals Desktop should **not** use the in-app update feature to upgrade to Portals version 1.6.0.
Only Windows Desktop users upgrading to Portals version 1.6.0 are affected.
:::

The **Update** button in Portals Desktop will not work for Windows users, and may result in data loss.

Instead, download a fresh installation from the [Portals website](https://portals.org) when a new version is available.

Follow the instructions below to minimize the risk of losing flows.

These instructions assume the default path for the Portals database of `C:\Users\YOUR_USERNAME\AppData\Roaming\com.portals\data\database.db` or `C:\Users\%YOUR_USERNAME%\AppData\Roaming\com.portals\data\database.db`. Replace `YOUR_USERNAME` with your Windows username.

1. Ensure you have Administrator privileges.
2. Ensure you have enough disk space for a second installation of Portals.
3. Close Portals, and ensure no Portals process is running in Task Manager.
4. The Portals 1.6.0 installer automatically performs a database backup in a later step, but an additional manual backup provides additional redundancy and is recommended.
To manually back up your Portals database file, do the following:
   1. In Windows Explorer, navigate to `C:\Users\YOUR_USERNAME\AppData\Roaming\com.portals\data\`. Replace `YOUR_USERNAME` with your Windows username.
   2. Copy the `database.db` at this location, and paste it to a safe location.
5. Download the Windows installer from the [Portals website](https://portals.org).
...
The Portals 1.6.0 installer automatically performs a database backup.
These steps install Portals from scratch, and result in two Portals installations: the previously installed version, and version 1.6.0.
This is expected behavior.
8. Start version 1.6.0 of Portals, and confirm your flows behave as expected.
9. **If flows are missing**, restore your flows from your manual backup by doing the following:
    ...
    4. Start Portals, and confirm your flows behave as expected.
    5. After confirmation, uninstall the previous version of Portals, and keep version 1.6.0.

### Breaking changes

- Authentication enforced for Portals API requests by default

    In Portals version 1.6, `LANGFLOW_AUTO_LOGIN=True` and `LANGFLOW_SKIP_AUTH_AUTO_LOGIN=False` by default.
    This enforces authentication for Portals API requests while still automatically authenticating all users as superusers in the visual editor.
    ...
    **This is a breaking change from 1.5 where both of these environment variables were `true` by default, bypassing all authentication.**
    ...
    For more information, see the documentation for [`LANGFLOW_AUTO_LOGIN`](/api-keys-and-authentication#portals-auto-login).

### New features and enhancements

- OpenAI Responses API compatibility

  Portals now includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses) at `POST /api/v1/responses`.
...
- Advanced document parsing with built-in Docling support
  ...
  To make it easier to use the [**Docling** components](/bundles-docling) and the **Read File** component's new advanced parsing feature, the Docling dependency is now included with Portals for all operating systems except macOS Intel (x86_64).
...
- Portals API requests can require authentication

    To enhance security and ensure proper authentication for automatic login features, Portals API endpoints now require authentication with a Portals API key, even when `LANGFLOW_AUTO_LOGIN=True`.
...
- MCP server one-click installation
    ...
    On your Portals project's **MCP server** page, click **Auto install** to install your Portals MCP server to MCP clients with just one click.
    The option to install with a JSON configuration file is available for macOS, Windows, and WSL.
    For more information, see [Use Portals as an MCP server](/mcp-server).

- MCP server management

    You can now add, remove, and edit your MCP servers in the **MCP Tools** components and through your Portals **Settings** page.
    For more information, see [Use Portals as an MCP client](/mcp-client).
...
- Stability improvements
    ...
    See an issue? [Raise it on GitHub](https://github.com/portals-ai/portals/issues).
...
- Added an enhanced **Breaking Changes** feature to help update components without breaking flows after updating Portals.
...
- Introduced MCP server functionality to serve Portals tools to MCP-compatible clients.
...
See the [Changelog](https://github.com/portals-ai/portals/releases).
