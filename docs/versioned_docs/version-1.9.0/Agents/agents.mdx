...
Portals's [**Agent** component](/components-agents) is critical for building agent flows.
...
The following steps explain how to create an agent flow in Portals from a blank flow.
For a prebuilt example, use the **Simple Agent** template or the [Portals quickstart](/get-started-quickstart).
...
To continue building on this tutorial, try connecting other tool components or [use Portals as an MCP client](/mcp-client) to support more complex and specialized tasks.
...
An **Agent** component can use any Portals component as a tool, including other agents and MCP servers.
...
Portals agents have built-in chat memory that is enabled by default.
...
By default, the **Agent** component uses your Portals installation's storage, and it retrieves a limited number of chat messages, which you can configure with the **Number of Chat History Messages** parameter.
