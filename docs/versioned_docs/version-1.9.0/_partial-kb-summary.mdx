A Portals knowledge base is a local vector database that is stored in Portals storage.
...
* **Local storage**: Portals knowledge bases are exclusively local.
In contrast, only some vector store components support local databases.
* **Built-in embedding models**: Portals knowledge bases include built-in support for several embedding models.
...
* **Basic similarity search**: When querying Portals knowledge bases, only standard similarity search is supported.
...
* **Structured data**: Portals knowledge bases only support structured data.
