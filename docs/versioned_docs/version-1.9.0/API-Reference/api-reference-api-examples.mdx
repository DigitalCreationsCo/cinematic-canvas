---
title: Get started with the Portals API
slug: /api-reference-api-examples
---
import CodeSnippet from '@site/src/components/CodeSnippet';
import exampleApiReferenceApiExamplesFormPortalsApiRequests from '!!raw-loader!@site/docs/API-Reference/curl-examples/api-reference-api-examples/form-portals-api-requests.sh';
...
import examplePythonApiReferenceApiExamplesFormPortalsApiRequests from '!!raw-loader!@site/docs/API-Reference/python-examples/api-reference-api-examples/form-portals-api-requests.py';
import exampleJavascriptApiReferenceApiExamplesFormPortalsApiRequests from '!!raw-loader!@site/docs/API-Reference/javascript-examples/api-reference-api-examples/form-portals-api-requests.js';
...
You can use the Portals API for programmatic interactions with Portals, such as the following:

* Create and edit flows, including file management for flows.
* Develop applications that use your flows.
* Develop custom components.
* Build Portals as a dependency of a larger application, codebase, or service.
* Contribute to the overall Portals codebase.

To view and test all available endpoints, you can access the Portals API's OpenAPI specification at your Portals deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the Portals API in a script, see the [Portals quickstart](/get-started-quickstart).
...
## Form Portals API requests

While individual options vary by endpoint, all Portals API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a Portals API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

<Tabs>
<TabItem value="Python" label="Python" default>

<CodeSnippet source={examplePythonApiReferenceApiExamplesFormPortalsApiRequests} language="python" />

</TabItem>
<TabItem value="JavaScript" label="JavaScript">

<CodeSnippet source={exampleJavascriptApiReferenceApiExamplesFormPortalsApiRequests} language="javascript" />

</TabItem>
<TabItem value="curl" label="curl">

<CodeSnippet source={exampleApiReferenceApiExamplesFormPortalsApiRequests} language="bash" />
...
By default, local deployments serve the Portals API at `http://localhost:7860/api`.

Remotely hosted Portals deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:PORTALS_PORT/api`.

You can configure the Portals port number in the `PORTALS_PORT` [environment variable](/environment-variables).

* `https://UUID.ngrok.app/api`
* `http://IP_OR_DNS/api`
* `http://IP_OR_DNS:PORTALS_PORT/api`

### Authentication

In Portals versions 1.5 and later, most API endpoints require authentication with a Portals API key in either an `x-api-key` header or query parameter.
...
Portals API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The Portals API serves `/v1` and `/v2` endpoints.
...
Commonly used values in Portals API requests include your [Portals server URL](#base-url), [Portals API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some Portals API requests

Once you have your Portals server URL, try calling these endpoints that return Portals metadata.

### Health check

Returns the health status of the Portals database and chat services:
...
Portals provides an additional `GET /health` endpoint.
This endpoint is served by uvicorn before Portals is fully initialized, so it's not reliable for checking Portals service health.

### Get version

Returns the current Portals API version:
...
Returns configuration details for your Portals deployment.
Requires a [Portals API key](/api-keys-and-authentication).
...
Returns a dictionary of all Portals components.
Requires a [Portals API key](/api-keys-and-authentication).
...
Because you can run Portals as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the Portals database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the Portals codebase, you won't directly call most of the Portals endpoints.
...
The following endpoints are useful for developing applications with Portals and administering Portals deployments with one or more users.
...
* Deployment details:
  * GET `/v1/version`: Return Portals version. See [Get version](/api-reference-api-examples#get-version).
...
* Custom components: You might use these endpoints when developing custom Portals components for your own use or to share with the Portals community:
  * GET `/v1/all`: Return all available Portals component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are most often used when contributing to the Portals codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
* MCP servers: The following endpoints are for managing Portals MCP servers and MCP server connections.
They aren't typically called directly; instead, they are used to drive internal functionality in the Portals frontend and when running flows that call MCP servers.
Portals MCP servers support both streamable HTTP and SSE transport.
...
* Use the Portals API to [run a flow](/api-flows-run).
* Use the Portals API to [upload files](/api-files).
* Use the Portals API to [get flow logs](/api-logs).
* Explore all endpoints in the [Portals API specification](/api).
