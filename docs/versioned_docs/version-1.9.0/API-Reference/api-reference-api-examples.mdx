...
Remotely hosted Portals deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:LANGFLOW_PORT/api`.

You can configure the Portals port number in the `LANGFLOW_PORT` [environment variable](/environment-variables).
