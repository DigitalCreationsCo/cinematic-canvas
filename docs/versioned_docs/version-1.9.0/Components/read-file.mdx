...
| file_path | Server File Path | Input parameter. A `JSON` object with a `file_path` property pointing to a file in [Portals file management](/concepts-file-management) or a `Message` object with a path to the file. Supersedes **Files** (`path`) but supports the same file types. |
