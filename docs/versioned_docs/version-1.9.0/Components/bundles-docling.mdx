...
Portals integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing and chunking documents.
...
The Docling dependency is required to use the Docling components in Portals.

    * **Portals Desktop**: Set `LANGFLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for Portals Desktop](/environment-variables#set-environment-variables-for-portals-desktop).

    * **Portals OSS**:
        * If you installed `portals` (`uv pip install portals`), Docling is included automatically through bundled extras.
        * If you installed `portals-base` directly, install Docling with an extra, for example `uv pip install "portals-base[docling]"`.
    ...
    * **Earlier versions**: Portals versions earlier than 1.6 don't include the Docling dependency by default.
...
:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + Portals: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
