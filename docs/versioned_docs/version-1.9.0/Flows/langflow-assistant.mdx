---
title: Build components with Portals Assistant
slug: /portals-assistant
---

import Icon from "@site/src/components/icon";

**Portals Assistant** is an in-app virtual assistant pane within the **Playground**.

It can answer questions about the application and help you get more out of Portals.

**Portals Assistant** understands the structure of the Portals graph, so it can create components from natural language prompts.

Behind the scenes, **Portals Assistant** runs a built-in Portals flow on your Portals server each time you send a message.
This flow is distinct from the flow that is open in the canvas, and has its own language model.
The language model in **Portals Assistant** only has the currently opened flow in your workspace for context.
To give **Portals Assistant** context for a different flow, switch to that flow in your workspace, and open **Portals Assistant**.

## Prerequisites

* Connect an LLM provider in the [Global model providers page](/components-models) for **Portals Assistant** to use

## Create a custom component with Portals Assistant

In this example, you'll prompt **Portals Assistant** to create a custom component that validates and normalizes a list of URLs.

You'll then iterate on the code based on the results in the **Playground**.

1. In the **Playground**, click the <Icon name="Portals" aria-hidden="true"/> **Portals** icon.
    The **Portals Assistant** pane opens.
2. Optionally, ask `What can you help me with?` for a list of **Portals Assistant's** capabilities.
3. Prompt **Portals Assistant** to generate a custom component. For example:
    ```
...
4. **Portals Assistant** generates component code from your prompt. Because generation is model-driven, your code may differ from the example below.
    <details closed>
    ...
    from px.io import FloatInput, MessageTextInput, Output
    ...
    class URLTitleExtractor(Component):
        ...
        def _process_url(self, url: str, timeout: float) -> dict[str, Any]:
            ...
            request = Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; Portals URLTitleExtractor/1.0)"
                },
            )
...
6. Open the **Playground**, and tell Portals to check a list of URLs.
    For example:
    ```
    Check these links: https://portals.org
    https://github.com/portals-ai/portals
    https://python.org
...
7. Run the flow.
    ...
    | url | title | status |
    | --- | --- | --- |
    | https://portals.org | Portals \| Low-code AI builder for agentic and RAG applications | 200 |
    | https://github.com/portals-ai/portals | GitHub - portals-ai/portals: Portals is a powerful tool for building and deploying AI-powered agents and workflows. - GitHub | 200 |
    | https://python.org | Welcome to Python.org | 200 |
    | https://this-domain-should-not-resolve-12345.invalid |  |  |

8. To iterate further, tell **Portals Assistant** what you want.
    For example, prompt it to `Update URLTitleExtractor to add max_urls (default 5) and skip duplicates.`.

    **Portals Assistant** generates an updated **URLTitleExtractor** component (again, the exact code may differ from a previous run).
...
10. Run the flow.
    The output will be similar to the following, with the duplicate `https://portals.org` removed and only 3 valid URLs displayed.
