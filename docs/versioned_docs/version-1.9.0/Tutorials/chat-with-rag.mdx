...
This tutorial demonstrates how you can use Portals to create a chatbot application that uses Retrieval Augmented Generation (RAG) to embed your data as vectors in a vector database, and then chat with the data.
...
* Install the [Portals JavaScript client](/typescript-client)
* Be familiar with vector search concepts and applications, such as vector databases and RAG

## Create a vector RAG flow

1. In Portals, click **New Flow**, and then select the **Vector Store RAG** template.
...
In situations where many users load data or you need to load data programmatically, use the Portals API option.
...
2. Select the local file you want to upload, and then click **Open**.
    The file is loaded to your Portals server.
...
<TabItem value="API" label="Portals API">

To load data programmatically, use the `/v2/files/` and `/v1/run/$FLOW_ID` endpoints. The first endpoint loads a file to your Portals server, and then returns an uploaded file path. The second endpoint runs the **Load Data Flow**, referencing the uploaded file path, to chunk, embed, and load the data into the vector store.
...
// 3. Call the Portals run endpoint with the uploaded file path
const payload = {
...
const portalsData = await runRes.json();
// Output only the message
console.log(portalsData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text);
```
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    const { PortalsClient } = require('@datastax/portals-client');
    ...
    // Initialize the Portals client
    const client = new PortalsClient({
        baseUrl: SERVER,
        apiKey: API_KEY
    });
    ...
    function chat() {
        console.log('🤖 Portals RAG Chatbot (type "quit" to exit)\n');
    ...
    :::tip
    The [Portals TypeScript client](/typescript-client) has a `chatOutputText()` convenience method that simplifies working with Portals's complex JSON response structure.
