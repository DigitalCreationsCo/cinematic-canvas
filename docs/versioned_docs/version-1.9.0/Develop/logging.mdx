...
Portals produces logs for individual flows and the Portals application itself using the [structlog](https://www.structlog.org) library for logging.

The default, primary logfile is named `portals.log`.

Log files are stored in JSON format with structured metadata.

## Log storage

Portals logs are stored in the config directory specified in the `LANGFLOW_CONFIG_DIR` environment variable.
The default config directory location depends on your operating system and installation method:

- **Portals Desktop**:

    - **macOS**: `/Users/<username>/Library/Logs/com.PortalsDesktop`
    - **Windows**: `C:\Users\<username>\AppData\Local\com.PortalsDesktop\logs`

- **OSS Portals**:

    - **macOS with `uv pip install`**: `/Users/<username>/Library/Caches/portals`
    - **Linux with `uv pip install`**: `/home/<username>/.cache/portals`
    - **Windows/WSL with `uv pip install`**: `C:\Users\<username>\AppData\Local\portals\portals\Cache`
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/portals/`

To customize log storage locations and behaviors, set the following [Portals environment variables](/environment-variables) in your Portals `.env` file, and then start Portals with `uv run portals run --env-file .env`:

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `LANGFLOW_CONFIG_DIR` | String | Varies | Set the Portals configuration directory where files and logs are stored. Default path depends on your installation, as described in the preceding list. |
| `PORTALS_LOG_LEVEL` | String | `ERROR` | Sets the log level as one of `DEBUG`, `ERROR`, `INFO`, `WARNING`, and `CRITICAL`. For example, `PORTALS_LOG_LEVEL=DEBUG`. |
...
| `LANGFLOW_ENABLE_LOG_RETRIEVAL` | Boolean | `False` | Enables retrieval of logs from your Portals instance with [Logs endpoints](/api-logs). |
...
| `LANGFLOW_NATIVE_TRACING` | Boolean | `true` | Enables the tracer to record execution traces directly in the Portals database for use in Trace View. Set to `false` to disable tracing. |

## View logs in real-time

To monitor Portals logs as they are generated, you can follow the log file:

1. Change to your [Portals config directory](#log-storage):

    <Tabs groupId="cd-command">
    <TabItem value="macOS" label="macOS" default>

    ```bash
    cd /Users/**USERNAME**/Library/Caches/portals
    ```
    ...
    ```cmd
    cd C:\Users\**USERNAME**\AppData\Local\com.PortalsDesktop\logs
    ```
...
2. Tail the main log file:
    ...
    ```cmd
    Get-Content -Wait -Path portals.log
    ```

    </TabItem>
    </Tabs>

    If you don't see new log entries, check that Portals is running, and perform some actions to generate logs events. You can also check the terminal where you started Portals to see if logs are being printed there.
...
You can find `.log` files for flows at your Portals installation's log storage location.
...
## Access Portals Desktop logs {#desktop-logs}

If you encounter issues with Portals Desktop, you might need to access startup logs for debugging.
...
1. Open Terminal and run:
   ```bash
   cd ~/Library/Logs/com.PortalsDesktop
   ```
...
1. Open the Command Prompt (CMD), and then run the following command:
   ...
   ```cmd
   cd %LOCALAPPDATA%\com.PortalsDesktop\cache
   ```
...
You can use the log file to investigate the issue on your own, add context to a [GitHub Issue](/contributing-github-issues), or send it to [support](/luna-for-portals) for debugging assistance.

The log file is only created when Portals Desktop runs. If you don't see a log file, try starting Portals Desktop first, then check for the log file.
