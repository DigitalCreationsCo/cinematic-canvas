...
Portals’s **Traces** feature records detailed execution traces for your flows and components so that you can debug issues, measure latency, and track token usage without relying on external observability services.

Trace data is stored in the Portals database in the `trace` and `span` tables.
...
Portals traces can run alongside supported third-party tracing providers.
If both are enabled, Portals stores native traces in the `trace` and `span` database tables for the **Trace View**, and also sends tracing data to the configured external provider.

To disable Portals native tracing and use only an external tracing provider, set `LANGFLOW_NATIVE_TRACING=false`.
...
To view traces in the Portals UI, do the following:
