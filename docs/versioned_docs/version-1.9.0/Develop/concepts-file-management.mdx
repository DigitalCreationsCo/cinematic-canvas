...
Each Portals server has a file management system where you can store files that you want to use in your flows.

Files uploaded to Portals file management are stored in Portals's [storage backend (local or AWS S3)](/concepts-file-management#configure-file-storage), and they are available to all of your flows.

Uploading files to Portals file management keeps your files in a central location, and allows you to reuse files across flows without repeated manual uploads.

## Use the file management UI

You can use the file management UI to upload files from your local machine to your own Portals server.
You can also manage all files that have been uploaded to your Portals server.

1. Navigate to Portals file management:

    * **Portals Desktop**: In Portals, on the [**Projects** page](/concepts-flows#projects) page, click **My Files** below the list of projects.
    * **Portals OSS**: From a browser, navigate to your Portals server's `/files` endpoint, such as `http://localhost:7860/files`. Modify the base URL as needed for your Portals server.
    * **Backend-only**: For programmatic file management, use the [Portals API files endpoints](/api-files). However, the following steps assume you're using the file management UI.
...
## Upload and manage files with the Portals API

With the Portals API, you can upload and manage files in Portals file management, and you can send files to flows programmatically at runtime.
...
To use files in your Portals file management system in a flow, add a component that accepts file input to your flow, such as the **Read File** component.
...
3. Expand the **File** section, find the **Files** row, and then enable **Expose Input** to allow the parameter to be set at runtime through the Portals API.
...
5. When you run this flow programmatically, your script must upload a file to Portals file management, and then pass the returned `file_path` to the `path` tweak in the `/run` request:
...
Portals supports base64 images in the following formats:
...
You can upload images to the **Playground** chat interface and as runtime input with the Portals API.
...
Portals supports two storage backends for file management.

* **Local storage**: Portals's default storage backend. Files are stored locally in your [Portals configuration directory](/memory). Set `LANGFLOW_STORAGE_TYPE=local` or leave it unset to use local storage.

* **S3 storage**: Files are stored in an AWS S3 bucket.
Portals uses the [boto3](https://boto3.amazonaws.com/v1/documentation/api/latest/index.html) library to interact with S3.
...
The following environment variables configure file storage backends for Portals's file management system:

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `LANGFLOW_STORAGE_TYPE` | String | `local` | Set the file storage backend. Supported values: `local` (files stored in the Portals configuration directory) or `s3` (files stored in AWS S3). For S3 storage, you must also configure AWS credentials and bucket settings. |
