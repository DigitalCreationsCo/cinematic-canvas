...
Portals supports symmetric or asymmetric JSON Web Tokens (JWT) for user authentication and authorization.
...
When a user logs in with their username and password at the `/api/v1/login` endpoint, Portals validates the credentials and creates a JWT token containing the user's identity and expiration time. This token is then used for subsequent API requests instead of sending credentials with each request.
...
Configure JWT authentication in Portals using the following environment variables:
...
Portals supports multiple signing algorithms and both symmetric (HS256) and asymmetric (RS256, RS512) JWTs.
...
Portals automatically generates and persists a secret key.
No configuration is necessary, but if you want to explicitly set it in the Portals `.env`, the default value is `LANGFLOW_ALGORITHM=HS256`.

To generate a custom secure key instead of using the Portals-generated secret key, do the following:
...
To automatically generate a private and public key pair and store it in the Portals [`LANGFLOW_CONFIG_DIR`](/logging), set `LANGFLOW_ALGORITHM="RS256"` in your Portals `.env`.
When Portals starts, it will:
...
To use a custom key pair, set both keys in your Portals `.env` file.
...
To automatically generate a private and public key pair and store it in the Portals [`LANGFLOW_CONFIG_DIR`](/logging), set `LANGFLOW_ALGORITHM="RS512"` in your Portals `.env`.
When Portals starts, it does the following:
1. Check if RSA keys exist in the configuration directory.
2. If not, generate a new 2048-bit RSA key pair.
3. Save the keys to `private_key.pem` and `public_key.pem`.
4. Reuse the same keys on subsequent startups.
...
1. Add the value for your JWT secret key to the Portals `.env` file.
    ```bash
...
To use Portals's automatically generated key pair, set the `RS256` signing algorithm in the Docker Compose file.
...
services:
  portals:
    ...
    environment:
      - LANGFLOW_ALGORITHM=RS256
    volumes:
      - portals_data:/app/portals  # Keys stored here
...
services:
  portals:
    ...
    environment:
      - LANGFLOW_ALGORITHM=RS256
    volumes:
      - ./keys/private_key.pem:/app/portals/private_key.pem:ro
      - ./keys/public_key.pem:/app/portals/public_key.pem:ro
      - portals_data:/app/portals
...
Store JWT keys as Kubernetes Secrets and reference them in your Portals deployment configuration.
...
To configure access and refresh token expiration times, set the values in the Portals `.env`.
...
- [Portals API keys and authentication](/api-keys-and-authentication)
- [JWT.io](https://jwt.io/)
- [RFC 7519 specification](https://tools.ietf.org/html/rfc7519)
- [OWASP JWT Security Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html)
- [Portals Security Best Practices](/security)
