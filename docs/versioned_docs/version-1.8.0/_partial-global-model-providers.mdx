...
To edit Portals's global model provider configuration, do the following:
...
3. In the **API Key** field, add your provider's API key.
    ...
    You can only add one key for each provider. Make sure the key has access to _all_ models that you want to use in Portals.
4. Enable the specific models that you want to use in Portals.
...
After you enable a model in Portals's global model configuration, you can use that model in any model-driven component in your flows.