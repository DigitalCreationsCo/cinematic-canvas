...
The Docling dependency is required to use the Docling components in Portals.
    ...
    * **Portals version 1.6 and later**: The Docling dependency is included by default for all operating systems except macOS Intel (x86_64).
    ...
    * **Earlier versions**: Portals versions earlier than 1.6 don't include the Docling dependency.
    For Portals OSS, install the Docling extra with `uv pip install 'portals[docling]'`.
    For Portals Desktop, add the Docling dependency to Portals Desktop's `requirements.txt`.
