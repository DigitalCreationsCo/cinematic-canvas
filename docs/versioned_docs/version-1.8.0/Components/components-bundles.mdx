...
Bundles contain custom components that support specific third-party integrations with Portals.
You add them to your flows and configure them in the same way as Portals's core components.
...
Many bundled components are developed by third-party contributors to the Portals codebase.
...
To find documentation for a specific bundled component, browse the Portals docs and your provider's documentation.
...
The Portals documentation focuses on using bundles within flows.
For that reason, it focuses on the Portals-specific configuration steps for bundled components.
...
:::tip
The Portals documentation doesn't list all bundles or components in bundles.
For the most accurate and up-to-date list of bundles and components for your version of Portals, check <Icon name="Blocks" aria-hidden="true" /> **Bundles** in the visual editor.

If you can't find a component that you used in an earlier version of Portals, it may have been removed or marked as a [legacy component](#legacy-bundles).
:::

Portals offers generic <Icon name="Component" aria-hidden="true" /> **Core components** in addition to third-party, provider-specific bundles.
...
The **Local DB** component reads and writes to a persistent, in-memory Chroma DB instance intended for use with Portals.
It has separate modes for reads and writes, automatic collection management, and default persistence in your Portals cache directory.
...
<!-- Not documented but in Portals as of 1.5.11 -->
<!--
* AgentQL
* Confluence
* Firecrawl
