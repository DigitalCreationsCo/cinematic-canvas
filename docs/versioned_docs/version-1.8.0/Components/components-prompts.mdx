...
Variables in a **Prompt Template** component dynamically add fields to the **Prompt Template** component so that your flow can receive definitions for those values from other components, Portals global variables, or fixed input.
...
Portals supports variable replacement with double brackets, but does not support the full Mustache engine.
