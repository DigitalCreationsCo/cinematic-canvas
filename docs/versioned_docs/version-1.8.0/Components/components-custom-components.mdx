...
Create your own custom components to add any functionality you need to Portals, from API integrations to data processing.

In Portals's node-based environment, each node is a "component" that performs discrete functions.
Custom components in Portals are built upon:
...
Use the [Custom component quickstart](#quickstart) to add an example component to Portals, and then use the reference guide that follows for more advanced component customization.

## Custom component quickstart {#quickstart}

Create a custom `DataFrameProcessor` component by creating a Python file, saving it in the correct folder, including an `__init__.py` file, and loading it into Portals.
...
Save the custom component in the Portals directory where the UI will discover and load it.

By default, Portals looks for custom components in the `src/px/src/px/components` directory.
...
The category folder name determines where the component appears in the Portals <Icon name="Component" aria-hidden="true" /> **Core components** menu.
For example, to add the example `DataFrameProcessor` component to the **Data** category, place it in the `data` subfolder:

```
src/px/src/px/components/
...
Each category directory **must** contain an `__init__.py` file for Portals to properly recognize and load the components.
...
if TYPE_CHECKING:
    from px.components.data.dataframe_processor import DataFrameProcessor
...
For an additional example of lazy loading, see the [FAISS component](https://github.com/portals-ai/portals/blob/main/src/px/src/px/components/FAISS/__init__.py).
...
1. To rebuild the backend and frontend, run `make install_frontend && make build_frontend && make install_backend && uv run portals run --port 7860`.
...
When running Portals in Docker, mount your custom components directory and set the `LANGFLOW_COMPONENTS_PATH` environment variable in the `docker run` command to point to the custom components directory.
...
Inputs are defined in a class-level `inputs` list. When Portals loads the component, it uses this list to render component fields and [ports](/concepts-components#component-ports) in the visual editor. Users or other components provide values or connections to fill these inputs.

An input is usually an instance of a class from `px.io` (such as `StrInput`, `DataInput`, or `MessageTextInput`).
...
```python
from px.io import StrInput, BoolInput, DropdownInput
...
For a list of all available parameters, see the [BaseInputMixin definition](https://github.com/portals-ai/portals/blob/main/src/px/src/px/inputs/input_mixin.py) in the Portals codebase.

For a list of all available input types, see the [input type definitions](https://github.com/portals-ai/portals/blob/main/src/px/src/px/inputs/inputs.py) in the Portals codebase.

```python
from px.io import StrInput, DataInput, MultilineInput, IntInput, BoolInput, DropdownInput, FileInput, CodeInput, ModelInput, HandleInput, Output
```

### Outputs

Outputs are defined in a class-level `outputs` list. When Portals renders a component, each output becomes a connector point in the visual editor. When you connect something to an output, Portals automatically calls the corresponding method and passes the returned object to the next component.

An output is usually an instance of `Output` from `px.io`.
...
The `Output` creates a connector point in the visual editor labeled **DataFrame Output**. The `name="df_out"` parameter identifies this output, while `display_name="DataFrame Output"` shows the label in the UI. The `method="build_df"` parameter tells Portals to call the `build_df` method when this output is connected to another component.

The `build_df` method processes data and returns a `DataFrame`. The `-> DataFrame` type annotation helps Portals validate connections and provides color-coding in the visual editor. You can also set `self.status` to show progress messages in the UI.

For a complete list of all available parameters, see the [Output class definition](https://github.com/portals-ai/portals/blob/main/src/px/src/px/template/field/base.py) in the Portals codebase. Common parameters include:
...
By default, components in Portals that produce multiple outputs only allow one output selection in the visual editor.
...
In Portals, typed annotations allow Portals to visually guide users and maintain flow consistency.
...
When a user changes a field with `real_time_refresh=True`, Portals calls your `update_build_config` method.
...
You can raise standard Python exceptions such as `ValueError` or specialized exceptions like `ToolException` when validation fails. Portals automatically catches these and displays appropriate error messages in the visual editor, helping users quickly identify what went wrong.
...
Portals provides several tools to help you debug and manage component execution. You can use `self.status` to display short messages about execution results directly in the visual editor, making troubleshooting easier for users.
...
Component logs are distinct from Portals's main application logging system. `self.log()` creates component-specific logs that appear in the UI, while Portals's main logging system uses [structlog](https://www.structlog.org) for application-level logging  that outputs to `portals.log` files. For more information, see [Logs](/logging).
...
## Contribute custom components to Portals

To contribute your custom component to the Portals project, see [Contribute components](/contributing-components).