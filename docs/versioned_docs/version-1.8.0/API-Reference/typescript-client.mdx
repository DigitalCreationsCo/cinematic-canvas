---
title: Portals TypeScript client
...
The Portals TypeScript client allows your TypeScript applications to programmatically interact with the Portals API.

For the client code repository, see [portals-client-ts](https://github.com/datastax/portals-client-ts/).

For the npm package, see [@datastax/portals-client](https://www.npmjs.com/package/@datastax/portals-client).

## Install the Portals TypeScript package

To install the Portals typescript client package, use one of the following commands:

<Tabs>
<TabItem value="npm" label="npm" default>

```bash
npm install @datastax/portals-client
```
...
```bash
yarn add @datastax/portals-client
```
...
```bash
pnpm add @datastax/portals-client
```

</TabItem>
</Tabs>

## Initialize the Portals TypeScript client

1. Import the client into your code.

    ```tsx
    import { PortalsClient } from "@datastax/portals-client";
    ```

2. Initialize a `PortalsClient` object to interact with your server:

    ```tsx
    const baseUrl = "BASE_URL";
    const apiKey = "API_KEY";
    const client = new PortalsClient({ baseUrl, apiKey });
    ```

    Replace `BASE_URL` and `API_KEY` with values from your deployment.
    The default Portals base URL is `http://localhost:7860`.
    To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## Connect to your server and get responses

1. With your Portals client initialized, test the connection by calling your Portals server.
    ...
    const client = new PortalsClient({ baseUrl });
    ...
    * `baseUrl`: The URL of your Portals server.
...
2. Review the result to confirm that the client connected to your Portals server.

    The following example shows the response from a well-formed `runFlow` request that reached the Portals server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with Portals:
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments to get a streaming response:
    ...
    For more information on streaming Portals responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve Portals logs with the TypeScript client

To retrieve [Portals logs](/logging), you must enable log retrieval on your Portals server by including the following values in your Portals `.env` file:

```text
LANGFLOW_ENABLE_LOG_RETRIEVAL=True
LANGFLOW_LOG_RETRIEVER_BUFFER_SIZE=10000
PORTALS_LOG_LEVEL=DEBUG
```
...
async function runFlow(client: PortalsClient) {
    const input = "Is anyone there?";
...
async function main() {
    const client = new PortalsClient({ baseUrl: baseUrl });
...
Log: Log {
  ...
  message: `2025-05-30T07:49:18.094364-0400 DEBUG Vertex OpenAIModel-RtlZm, result: <portals.graph.utils.UnbuiltResult object at 0x364d24dd0>, object: {'text_output': "Hey there! I'm here and ready to help you build something awesome with AI. What are you thinking about creating today?"}\n`
}
```
