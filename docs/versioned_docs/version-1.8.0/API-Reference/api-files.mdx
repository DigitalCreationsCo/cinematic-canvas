...
Use the `/files` endpoints to move files between your local machine and Portals.

All `/files` endpoints (both `/v1/files` and `/v2/files`) require authentication with a Portals API key.
...
- `/v2` files are organized by `user_id` instead of `flow_id`.
  ...
  You can upload a file to Portals one time, and use it with multiple flows.
- `/v2` files are tracked in the Portals database.
...
Send image files to Portals to use them in flows.
...
1. Attach the image to a `POST /v1/files/upload/$FLOW_ID` request with `--form` (`-F`) and the file path:
    ...
    A successful request returns the `file_path` for the image in the Portals file management system in the format `FLOW_ID/TIMESTAMP_FILENAME.TYPE`.
    For example:
...
To create a Portals API key and export it as an environment variable, see [Get started with the Portals API](/api-reference-api-examples).
...
1. To retrieve your current `user_id`, call the `/whoami` endpoint:
    ...
    ```
    {"id":"07e5b864-e367-4f52-b647-a48035ae7e5e","username":"portals","profile_image":null,"store_api_key":null,"is_active":true,"is_superuser":true,"create_at":"2025-05-08T17:59:07.855965","updated_at":"2025-05-28T19:00:42.556460","last_login_at":"2025-05-28T19:00:42.554338","optins":{"github_starred":false,"dialog_dismissed":true,"discord_clicked":false,"mcp_dialog_dismissed":true}}
    ```
...
This endpoint uploads files to your Portals server's file management system.
...
This component loads files into flows from your local machine or Portals file management.
