...
Use the `/projects` endpoint to create, read, update, and delete [Portals projects](/concepts-flows#projects).

## Read projects

Get a list of Portals projects, including project IDs, names, and descriptions.
...
Import a project and its flows by uploading a Portals project zip file:
...
curl -X POST \
  ...
  -H "x-api-key: $LANGFLOW_API_KEY" \
  -F "file=@20241230_135006_portals_flows.zip;type=application/zip"
```