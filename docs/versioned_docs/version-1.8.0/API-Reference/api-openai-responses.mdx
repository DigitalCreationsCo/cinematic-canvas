...
Portals includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses).
...
To be compatible with Portals's OpenAI Responses API endpoint, your flow and request must adhere to the following requirements:
...
Portals uses `x-api-key` headers for authentication, while OpenAI uses `Authorization: Bearer` headers.
When sending requests to Portals with OpenAI client libraries, you must configure custom headers and include an `api_key` configuration.
...
|--------|----------|-------------|---------|
| `x-api-key` | Yes | Your Portals API key for authentication | `"sk-..."` |
...
The response contains fields that Portals sets dynamically and fields that use OpenAI-compatible defaults.
...
Fields set dynamically by Portals:
...
This example demonstrates passing an `OPENAI_API_KEY` variable, which is a variable Portals automatically detects from environment variables, with two custom variables for `USER_ID` and `ENVIRONMENT`. The variables don't have to be created in Portals's Global Variables section - you can pass any variable name in the `X-LANGFLOW-GLOBAL-VAR-{VARIABLE_NAME}` header format.
...
If your flow components reference variables that aren't provided in headers or your Portals database, the flow fails by default.
