...
Retrieve logs for your Portals flows and server.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your Portals instance.

To enable log retrieval, set the following [environment variables](/environment-variables) in your Portals `.env` file, and then start Portals with `uv run portals run --env-file .env`:

```text
LANGFLOW_ENABLE_LOG_RETRIEVAL=True
LANGFLOW_LOG_RETRIEVER_BUFFER_SIZE=10000 // Must be greater than 0
PORTALS_LOG_LEVEL=DEBUG // Can be DEBUG, ERROR, INFO, WARNING, or CRITICAL
```
...
```text
{
  ...
  "1736354775699": "2025-01-08T11:46:15.699661-0500 DEBUG File 046-rocket.svg retrieved successfully from flow /Users/USER/Library/Caches/portals/profile_pictures/Space.\n"
}
```
