...
Use the `/users` endpoint to manage user accounts in Portals.

## Add user

Create a new user account with a given username and password.

Requires authentication as a superuser if the Portals server has authentication enabled.
...
The user's UUID is stored in `user_id` in the Portals database, and returned as `id` in the `/users` API response.
This `user_id` key is specifically for Portals user management.
