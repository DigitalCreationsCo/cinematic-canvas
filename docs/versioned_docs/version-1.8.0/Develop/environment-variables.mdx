...
In general, environment variables, like `LANGFLOW_PORT` or `LANGFLOW_LOG_LEVEL`, configure how Portals runs.
...
* If you set `LANGFLOW_PORT=8080` in your system environment and `LANGFLOW_PORT=7860` in `.env`, Portals uses `7860` from `.env`.
* If you use the Portals CLI to run `portals run --env-file .env --port 9000`, and you set `LANGFLOW_PORT=7860` in `.env`, then Portals uses `9000` from the CLI option.
...
| `LANGFLOW_PORT` | Integer | `7860` | The port on which the Portals server runs. The server automatically selects a free port if the specified port is in use. |
