...
In general, environment variables, like `PORTALS_PORT` or `PORTALS_LOG_LEVEL`, configure how Portals runs.
These are broad settings that apply to your entire Portals deployment.

In contrast, global variables are user-defined values stored in Portals's database for use in flows, such as `OPENAI_API_KEY`.
Portals can also source global variables from environment variables.
For more information, see [Portals global variables](/configuration-global-variables).

## Configure environment variables for Portals OSS

Portals recognizes [supported environment variables](#supported-variables) from the following sources:

- Environment variables set in your terminal.
- Environment variables imported from a `.env` file when starting Portals.
- Environment variables set with the [Portals CLI](./configuration-cli), including the `--env-file` option and direct options, such as `--port`.
...
1. Portals CLI options override all other sources.
2. The `.env` file overrides system environment variables.
3. System environment variables are used only if not set elsewhere.
When running a Portals Docker image, the `-e` flag can be used to set additional system environment variables.

For example:

* If you set `PORTALS_PORT=8080` in your system environment and `PORTALS_PORT=7860` in `.env`, Portals uses `7860` from `.env`.
* If you use the Portals CLI to run `portals run --env-file .env --port 9000`, and you set `PORTALS_PORT=7860` in `.env`, then Portals uses `9000` from the CLI option.
...
docker run -it --rm \
    ...
    portalsai/portals:latest
```

</TabItem>
</Tabs>

When you start Portals, it looks for environment variables that you've set in your terminal.
...
1. If Portals is running, quit Portals.

2. Create a `.env` file, and then open it in your preferred editor.

3. Define [Portals environment variables](#supported-variables) in the `.env` file.
    ...
    For additional examples, see [`.env.example`](https://github.com/portals-ai/portals/blob/main/.env.example) in the Portals repository.

    </details>

4. Save and close `.env`.

5. Start Portals with your `.env` file:
...
On startup, Portals imports the environment variables from your `.env` file, as well as any others that you set in your terminal, and then adopts their specified values.

### Configure environment variables for development

The following examples show how to configure Portals using environment variables in different development scenarios.
...
Create or edit a `.env` file in the root directory of your application or Portals environment, and then add your configuration variables to the file:
...
Environment="LANGFLOW_DATABASE_URL=postgresql://user:password@localhost:5432/portals"
...
Environment="LANGFLOW_LOG_FILE=logs/portals.log"
Environment="PORTALS_LOG_LEVEL=error"
Environment="LANGFLOW_OPEN_BROWSER=false"
Environment="PORTALS_PORT=7860"
...
```json title=".vscode/tasks.json"
{
    ...
    "options": {
        "env": {
            ...
            "LANGFLOW_DATABASE_URL": "postgresql://postgres:password@localhost:5432/portals",
            "LANGFLOW_DEV": "false",
            "LANGFLOW_FALLBACK_TO_ENV_VAR": "false",
            "LANGFLOW_HEALTH_CHECK_MAX_RETRIES": "5",
            "LANGFLOW_HOST": "localhost",
    ...
    "tasks": [
        {
            "label": "portals backend",
            "type": "shell",
            "command": ". ./portalsnightly/Scripts/activate && portals run",
            "isBackground": true,
            "problemMatcher": []
        }
    ]
...
To run Portals using the above VSCode `tasks.json` file, in the VSCode command palette, select **Tasks: Run Task** > **portals backend**.

</TabItem>
</Tabs>

## Set environment variables for Portals Desktop

Environment variables set in your terminal aren't automatically available to GUI-based applications like Portals Desktop when you launch them from the Windows or macOS GUI.
...
To set environment variables for Portals Desktop, you need to use specific commands or files, depending on your OS.

<Tabs>
<TabItem value="macos" label="macOS" default>

Portals Desktop for macOS cannot automatically use variables set in your terminal, such as those in`.zshrc` or `.bash_profile`, when launched from the macOS GUI.
...
2. In the `LaunchAgents` directory, create a `.plist` file called `dev.portals.env`.

3. Add the following content to `dev.portals.env.plist`, and then add, change, or remove Portals environment variables as needed for your configuration.
       ...
       <array>
         ...
         <string>-c</string>
          <string>
            ...
            launchctl setenv PORTALS_PORT 7860 ;
...
4. Load the file with `launchctl`:
    ...
    ```bash
    launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/dev.portals.env.plist
    ```

</TabItem>
<TabItem value="windows1" label="Window System Properties">

Portals Desktop for Windows cannot automatically use variables set in your terminal, such as those defined with `set` in `cmd` or `$env:VAR=...` in PowerShell, when launched from the Windows GUI.

To make environment variables available to the Portals Desktop app, you must set them at the user or system level using the **System Properties** interface or the Terminal.
...
4. Enter the name of the Portals variable you want to set, such as `LANGFLOW_CONFIG_DIR`, and the desired value, such as `C:\Users\your_user\.portals_config`.
5. Click **OK** to save the variable.
6. Repeat until you have set all necessary Portals environment variables.
7. Launch or restart Portals Desktop to apply the environment variables.
...
1. Enter the name of the Portals variable you want to set, such as `LANGFLOW_CONFIG_DIR`, and the desired value, such as `C:\Users\your_user\.portals_config`.

    To set an environment variable for the current user:
    ```powershell
    [System.Environment]::SetEnvironmentVariable("LANGFLOW_CONFIG_DIR", "C:\Users\your_user\.portals_config", "User")
    ```

   To set an environment variable for all users (you must have Administrator priveleges):
   ```powershell
   [System.Environment]::SetEnvironmentVariable("LANGFLOW_CONFIG_DIR", "C:\Portals\Config", "Machine")
   ```

2. Repeat until you have set all necessary Portals environment variables.
3. Launch or restart Portals Desktop to apply the environment variables.
...
The following sections provide information about specific Portals environment variables.
...
For information about the relationship between Portals global variables and environment variables, as well as environment variables that control handling of global variables, see [Global variables](/configuration-global-variables).
...
See [Use Portals as an MCP server](/mcp-server).

### Monitoring and metrics

For environment variables for specific monitoring service providers, see the Portals monitoring integration guides, such as [Langfuse](/integrations-langfuse) and [Best practices for Portals on Kubernetes](/deployment-prod-best-practices).

### Server

The following environment variables set base Portals server configuration, such as where the server is hosted, required files for SSL encryption, and the deployment type (frontend and backend, backend-only, development mode).

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `LANGFLOW_HOST` | String | `localhost` | The host on which the Portals server will run. |
| `PORTALS_PORT` | Integer | `7860` | The port on which the Portals server runs. The server automatically selects a free port if the specified port is in use. |
| `LANGFLOW_BACKEND_ONLY` | Boolean | `False` | Run only the Portals backend service (no frontend). |
| `LANGFLOW_DEV` | Boolean | `False` | Whether to run Portals in development mode (may contain bugs). |
| `LANGFLOW_OPEN_BROWSER` | Boolean | `False` | Open the system web browser on startup. |
| `LANGFLOW_HEALTH_CHECK_MAX_RETRIES` | Integer | `5` | Set the maximum number of retries for Portals's server status health checks. |
| `LANGFLOW_WORKERS` | Integer | `1` | Number of worker processes. |
| `LANGFLOW_WORKER_TIMEOUT` | Integer | `300` | Worker timeout in seconds. |
| `LANGFLOW_SSL_CERT_FILE` | String | Not set | Path to the SSL certificate file for enabling HTTPS on the Portals web server. This is separate from [database SSL connections](/configuration-custom-database#connect-portals-to-a-local-postgresql-database). |
| `LANGFLOW_SSL_KEY_FILE` | String | Not set | Path to the SSL key file for enabling HTTPS on the Portals web server. This is separate from [database SSL connections](/configuration-custom-database#connect-portals-to-a-local-postgresql-database). |
...
For more information about deploying Portals servers, see [Portals deployment overview](/deployment-overview).
...
| `LANGFLOW_CREATE_STARTER_PROJECTS` | Boolean | `True` | Whether to create templates during initialization. If `false`, Portals doesn't create templates, and `LANGFLOW_UPDATE_STARTER_PROJECTS` is treated as `false`. |
...
| `LANGFLOW_LAZY_LOAD_COMPONENTS` | Boolean | `False` | If `true`, Portals only partially loads components at startup and fully loads them on demand. This significantly reduces startup time but can cause a slight delay when a component is first used. |
