...
Get started with Portals by loading a template flow, running it, and then serving it at the `/run` API endpoint.
...
- Create a [Portals API key](/api-keys-and-authentication)
    ...
    <details>
    <summary>Create a Portals API key</summary>

    A Portals API key is a user-specific token you can use with Portals.

    To create a Portals API key, do the following:

    1. In Portals, click your user icon, and then select **Settings**.
    2. Click **Portals API Keys**, and then click <Icon name="Plus" aria-hidden="true"/> **Add New**.
    3. Name your key, and then click **Create API Key**.
    4. Copy the API key and store it securely.
    5. To use your Portals API key in a request, set a `LANGFLOW_API_KEY` environment variable in your terminal, and then include an `x-api-key` header or query parameter with your request.
    For example:
...
3. In the **Agent** component, select your configured model from the **Language Model** dropdown.
    ...
    * Edit Portals's global <Icon name="BrainCog" aria-hidden="true" /> **Models** configuration. These providers and models are part of Portals's core functionality. Use the **Ollama** provider to connect to any model hosted on a local or remote Ollama instance.
...
Portals is an IDE, but it's also a runtime you can call through the [Portals API](/api-reference-api-examples) with Python, JavaScript, or HTTP.

When you start Portals locally, you can send requests to the local Portals server.
For production applications, you need to [deploy a stable Portals instance](/deployment-overview) to handle API calls.

For example, you can use the `/run` endpoint to run a flow and get the result.

Portals provides code snippets to help you get started with the Portals API.

1. When editing a flow, click **Share**, and then click **API access**.

    The default code in the API access pane constructs a request with the Portals server `url`, `headers`, and a `payload` of request data.
...
In a production application, you probably want to select parts of this response to return to the user, store in logs, and so on. The next steps demonstrate how you can extract data from a Portals API response to use in your application.
...
To assist with formatting, you can define tweaks in Portals's **Input Schema** pane before copying the code snippet.
...
Portals updates the `tweaks` object in the code snippets based on your input parameters, and includes default values to guide you.
...
* [Trigger flows with the Portals API](/concepts-publish)
* [Use Portals as a Model Context Protocol (MCP) server](/mcp-server)
