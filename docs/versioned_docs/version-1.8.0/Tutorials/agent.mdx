...
This tutorial shows you how to connect a JavaScript application to a [Portals agent](/agents).
...
In this example, the application sends a customer's email address to the Portals agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    async function runAgentFlow(): Promise<void> {
        try {
            ...
            console.log(`Connecting to Portals server at: ${LANGFLOW_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your Portals server is running and accessible at:', LANGFLOW_SERVER_ADDRESS);
                }
    ...
    // Run the function
    console.log('Starting Portals Agent...\n');
...
3.  Save and run the script to send the request and test the flow.
    ...
    ```
    Starting Portals Agent...

    Connecting to Portals server at: http://localhost:7860
