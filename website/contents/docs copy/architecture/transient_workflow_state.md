...
export async function getProjectWithStoryboard(projectId: string): Promise<PipelineFullState> {
  ...
  return {
    ...
    scenes: projectScenes.map(s => SceneEntity.parse(s)),
...
import { SceneEntity } from "./zod-db";
...
2. Implementation Reasoning
	...
	•	JSONB Immutability: Note that we treat currentAssets as a mutable object within the handler, but the SceneEntity ensures that when we save it, the structure strictly adheres to your AssetRegistrySchema.
...
Purpose: Establish the source of truth and the bridge between relational storage and AI-ready schemas.
	...
	3	[Task] Derived Types Export: * Instruction: Export TypeScript types from these schemas 
```typescript
export type ProjectEntity = z.infer<typeof DbProjectSchema>
```
  These will be the primary types for the application layer.
...
Type Safety: Use SceneEntity.parse() to validate data coming out of the DB.
