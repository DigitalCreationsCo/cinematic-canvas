...
describe("executeWithRetry", () => {
  ...
  it("should return the result on the first successful call", async () => {
    ...
    const result = await executeWithRetry(lmCall, "test-params", retryConfig);
  ...
  it("should throw on failure (no retry resolution)", async () => {
    ...
    await expect(executeWithRetry(lmCall, "test-params", retryConfig)).rejects.toThrow("failure");
    expect(lmCall).toHaveBeenCalledTimes(1);
  });

  it("should retry on 429 ApiError and eventually succeed", async () => {
    const err429 = new ApiError({ status: 429, message: "Rate limited" });
    const lmCall = vi.fn().mockRejectedValueOnce(err429).mockResolvedValue("success");
    const result = await executeWithRetry(lmCall, "test-params", retryConfig);
    expect(result).toBe("success");
  ...
  it("should throw after maxRetries when 429 persists", async () => {
    ...
    await expect(
      executeWithRetry(lmCall, "test-params", { attempt: 1, maxRetries: 2, initialDelay: 1, projectId: "1" }),
  ...
  it.skip("should use 10s default delay for retries", async () => {
    ...
    const promise = executeWithRetry(lmCall, "test-params", configWithDefault);
  ...
  it("should throw non-429 ApiError without retry", async () => {
    ...
    await expect(executeWithRetry(lmCall, "test-params", retryConfig)).rejects.toThrow(err403);
    expect(lmCall).toHaveBeenCalledTimes(1);
  });
});
