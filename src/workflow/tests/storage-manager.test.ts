...
describe('GCPStorageManager', () => {
  ...
  describe('scanCurrentAttempts', () => {
    it('should sync latest attempts from GCS', async () => {
      ...
      const path1 = storageManager.getObjectPath({ type: 'scene_video', sceneId: 1, attempt: 'latest' });
      expect(path1).toBe('test-bucket/test-video/scenes/scene_001_05.mp4');

      const path2 = storageManager.getObjectPath({ type: 'scene_video', sceneId: 2, attempt: 'latest' });
    ...
    it('should handle empty GCS gracefully', async () => {
      ...
      const attempts = await storageManager.scanCurrentAttempts();
      storageManager.initializeAttempts(attempts);
      const path = storageManager.getObjectPath({ type: 'scene_video', sceneId: 1, attempt: 'latest' });
  ...
  describe('getObjectPath', () => {
    it('should generate correct paths for all object types (including bucket)', () => {
      expect(storageManager.getObjectPath({ type: 'storyboard' })).toBe('test-bucket/test-video/scenes/storyboard.json');
      expect(storageManager.getObjectPath({ type: 'character_image', characterId: 'char1' })).toBe('test-bucket/test-video/images/characters/char1_reference.png');
      expect(storageManager.getObjectPath({ type: 'scene_end_frame', sceneId: 1, attempt: 3 })).toBe('test-bucket/test-video/images/frames/scene_001_frame_end_03.png');
      expect(storageManager.getObjectPath({ type: 'composite_frame', sceneId: 1, attempt: 2 })).toBe('test-bucket/test-video/images/frames/scene_001_composite_02.png');
      expect(storageManager.getObjectPath({ type: 'scene_video', sceneId: 1, attempt: 1 })).toBe('test-bucket/test-video/scenes/scene_001_01.mp4');
      expect(storageManager.getObjectPath({ type: 'scene_quality_evaluation', sceneId: 1, attempt: 5 })).toBe('test-bucket/test-video/scenes/scene_001_evaluation_05.json');
      expect(storageManager.getObjectPath({ type: 'frame_quality_evaluation', sceneId: 1, framePosition: "end", attempt: 5 })).toBe('test-bucket/test-video/images/frames/scene_001_frame_end_evaluation_05.json');
      expect(storageManager.getObjectPath({ type: 'render_video' })).toBe('test-bucket/test-video/final/movie.mp4');
      expect(storageManager.getObjectPath({ type: 'final_output' })).toBe('test-bucket/test-video/final/final_output.json');
    });

    it('should use default attempt (1) when attempt is "latest" and no history exists', () => {
      expect(storageManager.getObjectPath({ type: 'scene_video', sceneId: 1, attempt: 'latest' })).toBe('test-bucket/test-video/scenes/scene_001_01.mp4');
      expect(storageManager.getObjectPath({ type: 'scene_end_frame', sceneId: 2, attempt: 'latest' })).toBe('test-bucket/test-video/images/frames/scene_002_frame_end_01.png');
      expect(storageManager.getObjectPath({ type: 'composite_frame', sceneId: 3, attempt: 'latest' })).toBe('test-bucket/test-video/images/frames/scene_003_composite_01.png');
      expect(storageManager.getObjectPath({ type: 'scene_quality_evaluation', sceneId: 4, attempt: 'latest' })).toBe('test-bucket/test-video/scenes/scene_004_evaluation_01.json');
      expect(storageManager.getObjectPath({ type: 'frame_quality_evaluation', sceneId: 4, framePosition: "start", attempt: 'latest' })).toBe('test-bucket/test-video/images/frames/scene_004_frame_start_evaluation_01.json');
    });
    ...
    it('should use latest attempt when attempt is "latest"', () => {
      ...
      expect(storageManager.getObjectPath({ type: 'scene_video', sceneId: 1, attempt: 'latest' })).toBe('test-bucket/test-video/scenes/scene_001_05.mp4');
      expect(storageManager.getObjectPath({ type: 'scene_end_frame', sceneId: 2, attempt: 'latest' })).toBe('test-bucket/test-video/images/frames/scene_002_frame_end_03.png');
    });
  ...
  describe('updateLatestAttempt', () => {
    it('should set the latest attempt for a given object type and sceneId', () => {
      ...
      expect(storageManager.getObjectPath({ type: 'scene_video', sceneId: 1, attempt: 'latest' })).toBe('test-bucket/test-video/scenes/scene_001_03.mp4');
    });
