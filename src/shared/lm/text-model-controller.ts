...
import { buildProviderTools } from "#shared/lm/tools/tools-converter.js";
import {
  getProviderTextModelNames,
  getProviderImageModelNames,
  getProviderQualityCheckModelNames,
} from "#shared/lm/models.js";
...
export class TextModelController extends BaseChatModel<ProviderChatModelCallOptions> {
  ...
  constructor(params: ProviderTextModelParams = {}) {
    ...
    this.modeModelPriority =
      params.options?.modeModelPriority || process.env.MODEL_PRIORITY === "speed" ? "speed" : "quality";

    const testMode = getTestMode();
    console.info(
      { providerSelected, modeModelPriority: this.modeModelPriority, testMode },
      `Initializing text model provider`,
    );
  ...
  get currentModel() {
    return this.modelCurrentText;
  }
  get defaultModel() {
    return this.modelDefaultText;
  }
  ...
  get imageModel() {
    return this.modelCurrentImage;
  }
  get qualityCheckModel() {
    return this.modelCurrentQuality;
  }
   ...
   */
  override bindTools(tools: StructuredToolInterface[], kwargs?: Partial<ProviderChatModelCallOptions>): Runnable {
  ...
  async generateContent(
    params: { model?: string } & Omit<Parameters<ITextModelProvider["generateContent"]>[0], "model">,
  ): ReturnType<ITextModelProvider["generateContent"]> {
    await GlobalCooldown.wait();
    const timeStartMs = Date.now();
  ...
  async generateBatchContent(
    params: { model?: string } & Omit<Parameters<ITextModelProvider["generateBatchContent"]>[0], "model">,
  ...
  async generateBatchImages(
    params: { model?: string } & Omit<Parameters<ITextModelProvider["generateBatchImages"]>[0], "model">,
  ...
  private handleGenerationSuccess(typeModel: "text" | "image" | "quality"): void {
    console.trace(
      { shouldPublish: false },
      `[TextModelController] Generation successful for ${typeModel}. Resolving state based on priority: ${this.modeModelPriority}`,
    );
  ...
  private updateCurrentModel(typeModel: "text" | "image" | "quality"): void {
    switch (typeModel) {
      case "text":
        this.modelCurrentText = this.modelsFallback.text[this.indexCurrentModel.text];
        break;
      case "image":
        this.modelCurrentImage = this.modelsFallback.image[this.indexCurrentModel.image];
        break;
      case "quality":
        this.modelCurrentQuality = this.modelsFallback.quality[this.indexCurrentModel.quality];
        break;
    }
  }
  ...
  private handleGenerationError(typeModel: "text" | "image" | "quality", error: unknown): void {
    ...
    console.trace(
      { error, typeModel, model: this.getCurrentModelString(typeModel) } as any,
      `[TextModelController] Attempt ${this.countAttemptModel[typeModel]}/${attemptsMax} failed`,
    );

    if (this.countAttemptModel[typeModel] >= attemptsMax) {
      this.indexCurrentModel[typeModel] =
        (this.indexCurrentModel[typeModel] + 1) % this.modelsFallback[typeModel].length;
      this.countAttemptModel[typeModel] = 0;
      this.updateCurrentModel(typeModel);
      console.debug(
        `[TextModelController] Advancing ${typeModel} model (Round Robin enabled). New index: ${this.indexCurrentModel[typeModel]}`,
      );
    }

    console.warn(
      `[TextModelController] Model attempt failed. Next model targeting: ${this.getCurrentModelString(typeModel)}`,
    );
  }
  ...
  async countTokens(
    params: { model?: string } & Omit<Parameters<ITextModelProvider["countTokens"]>[0], "model">,
  ): ReturnType<ITextModelProvider["countTokens"]> {
