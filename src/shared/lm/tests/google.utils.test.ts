...
import { RawReferenceImage, MaskReferenceImage, StyleReferenceImage, SubjectReferenceImage } from "@google/genai";
...
describe("Google LM Utils", () => {
  describe("buildAPIReferenceImagesFromParams", () => {
    it("should convert ReferenceImageInputs to Google API objects", () => {
        ...
        style: [
          {
            ...
            config: { styleDescription: "Art" },
          },
        ],
        subject: [
          {
            referenceType: "subject",
            referenceImage: { gcsUri: "gs://b/subj.png" },
            config: { subjectDescription: "Person", subjectType: "SUBJECT_TYPE_PERSON" as any },
          },
        ],
      };
      ...
      expect((subjectRef as any)?.config).toEqual({ subjectDescription: "Person", subjectType: "SUBJECT_TYPE_PERSON" });
    });
    ...
    it("should assign sequential referenceIds", () => {
      const inputs: ReferenceImageInputs = {
        ...
        style: [
          { referenceType: "style", referenceImage: { gcsUri: "gs://b/2.png" }, config: { styleDescription: "s" } },
        ],
      };
      const result = buildAPIReferenceImagesFromParams(inputs as any);
  ...
  describe("toReferenceImagesFromContentsFileData", () => {
    ...
    it("should skip contents without fileUri", () => {
      const contents: any[] = [{ parts: [{ text: "text-only" }], referenceType: "base" }];
      const result = toReferenceImagesFromContentsFileData({ contents });
