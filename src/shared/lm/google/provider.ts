...
import { buildBatchParams, buildCountTokensParams, buildGenerateContentParams, buildGenerateImagesParams, buildGenerateVideosParams } from "./params.js";
...
export class GoogleProvider implements ITextModelProvider, IVideoModelProvider {
    ...
    async getBatchJob(
        params: Parameters<ITextModelProvider['getBatchJob']>[0]
    ): Promise<BatchJob> {
    ...
    private async executeNativeBatch(
        params: { model: string; requests: string } &
            Omit<Parameters<ITextModelProvider['generateBatchContent']>[0], 'requests'>
    ): Promise<BatchJob> {
        ...
        console.debug({ inputGcsUri, destGcsUri, projectId: params.projectId },
    ...
    private async executeSimulatedContentBatch(
        ...
        const results = await Promise.all(
            requests.map(async (reqCurrent, indexReq) => {
                await new Promise(resolve =>
                    setTimeout(resolve, indexReq * delayStaggerBaseMs)
                );
                try {
                ...
                try {
                    ...
                    const { imageContents, textContents } = reqToContents.contents.reduce(
                        (acc, content) => {
                            ...
                            const hasText =
                                content.parts?.some(p => p.text) && !hasImageConfig;
