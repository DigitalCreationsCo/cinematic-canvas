    ...
    if (executionMode === "BATCH") {
        ...
        try {
            ...
            return Promise.all(results.map(async res => {
                const item = params.characters.find(c => c.id === res.customId);
        ...
        return Promise.all(params.characters.map(async (char) => {
            try {
                const prompt = buildCharacterImagePrompt(char as any, params.generationRules);
                const [imageData] = extractGeneratedResponse("image", await executeWithRetry(
                    (p) => context.provider.generateImages({
                        prompt: p.prompt,
                        config: {
                            abortSignal: context.options?.signal,
                            numberOfImages: 1,
                ...
                const imagePath = context.storageManager.getObjectPath({ type: "character_image", projectId, characterId: char.id, version: char.version });
                const gcsUri = await context.storageManager.uploadBuffer(imageBuffer, imagePath, imageMimeType);
                return { success: true as const, id: char.id, output: gcsUri, metadata: { prompt, model: context.provider.imageModel } };
            } catch (error) {
        ...
        for (const char of params.characters) {
            try {
                const prompt = buildCharacterImagePrompt(char as any, params.generationRules);
                const [imageData] = extractGeneratedResponse("image", await executeWithRetry(
                    (p) => context.provider.generateImages({ prompt: p.prompt, config: { abortSignal: context.options?.signal, numberOfImages: 1, seed: Math.floor(Math.random() * 1000000), aspectRatio: aspectRatios.vertical.aspectRatio, outputMimeType: imageMimeType } }),
                    { prompt },
                    { attempt: char.version, maxRetries: context.safetyRetries + char.version, projectId },
                ...
                const imagePath = context.storageManager.getObjectPath({ type: "character_image", projectId, characterId: char.id, version: char.version });
                const gcsUri = await context.storageManager.uploadBuffer(imageBuffer, imagePath, imageMimeType);
                results.push({ success: true, id: char.id, output: gcsUri, metadata: { prompt, model: context.provider.imageModel } });
            } catch (error) {
...
class GenerateCharacterImagesTool extends StructuredTool<typeof GenerateCharacterImagesInput> {
    ...
    async _call(
        input: GenerateCharacterImagesInput,
