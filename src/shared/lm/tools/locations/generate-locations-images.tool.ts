...
import { GenerateBatchImagesParameters, StyleImage, UserMessage } from "#shared/lm/provider.js";
...
const LocationBasePartialWithIdAndVersion = LocationAttributes.extend({
  id: IdentityBase.shape.id,
  version: z.number(),
});
type LocationBasePartialWithIdAndVersion = z.infer<typeof LocationBasePartialWithIdAndVersion>;

const GenerateLocationImagesInput = z.object({
  locations: z.array(LocationBasePartialWithIdAndVersion),
...
async function persistImageAsset(
  ...
  await context.saveAssets(
    { locationIds: [loc.id], projectId: context.projectId },
    ["location_image"],
    "image",
    [gcsUri],
...
class GenerateLocationImagesTool extends StructuredTool<typeof GenerateLocationImagesInput> {
  ...
  async run(
    params: {
      locations: LocationBasePartialWithIdAndVersion[];
          ...
          try {
            ...
            const [imageData] = extractGeneratedResponse(
              ...
              await executeWithRetry(
                ...
                async (err, att, p) => {
                  this.deps.context.incrementAttempt(err.message, "BACKOFF_RETRY");
