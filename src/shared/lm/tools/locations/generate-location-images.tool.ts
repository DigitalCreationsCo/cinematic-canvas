...
export type GenerateLocationImagesResult = GenerateLocationImagesResultSuccess | { success: false; id: string; error: Error };
...
async function finaliseResults(
    ...
    const entityById = new Map(updatedEntities.map(({ entity }) => [(entity as LocationWithAssets).id, entity as LocationWithAssets]));
    ...
    if (context.publishPipelineEvent) {
        await context.publishPipelineEvent({
            ...
            payload: updatedEntities.map(({ entity, entityType }) => ({ id: (entity as LocationWithAssets).id, entityType, entity })),
        });
    }
...
async function run(
    params: { locations: { id: string; name: string; version: number }[]; generationRules: string[]; attempt: number; incrementAttempt: IncrementAttemptHook },
    context: GenerateLocationImagesContext,
): Promise<GenerateLocationImagesResult[]> {
    ...
    if (executionMode === "BATCH") {
        ...
        const batchRequests: GenerateBatchImagesParameters["requests"] = params.locations.map((loc) => {
            ...
            return {
                ...
                config: { responseModalities: [Modality.IMAGE], imageConfig: { ...aspectRatios.widescreen, outputMimeType: imageMimeType } },
            };
        });
        ...
        try {
            ...
            imageResults = await Promise.all(params.locations.map(async (loc): Promise<GenerateLocationImagesResult> => {
                const res = batchApiResults.find((r) => r.customId === loc.id);
                if (!res || res.status !== "SUCCESS") return { success: false, id: loc.id, error: res?.error || new Error("Batch failed") };
                ...
                const src = await context.storageManager.uploadBuffer(Buffer.from(res.imageBytes!, "base64"), outputPath, imageMimeType);
                await persistImageAsset(loc, src, ctx.prompt, context);
                return { success: true, id: loc.id, output: src, metadata: { prompt: ctx.prompt, model: context.provider.imageModel } };
            }));
        } catch (e) {
        ...
        imageResults = await Promise.all(params.locations.map(async (loc) => {
            try {
                const prompt = buildLocationImagePrompt(loc as any, params.generationRules);
                const [imageData] = extractGeneratedResponse("image", await executeWithRetry(
                    (p) => context.provider.generateImages({ prompt: p.prompt, config: { aspectRatio: aspectRatios.widescreen.aspectRatio, outputMimeType: imageMimeType } }),
                    { prompt }, { attempt: loc.version, maxRetries: context.safetyRetries, projectId },
                    async (err, att, p) => { params.incrementAttempt(err.message, "BACKOFF_RETRY"); return { attempt: att, params: p }; }
                ), "google");
                const imagePath = context.storageManager.getObjectPath({ type: "location_image", projectId, locationId: loc.id, version: loc.version });
                const gcsUri = await context.storageManager.uploadBuffer(Buffer.from(imageData, "base64"), imagePath, imageMimeType);
            ...
            } catch (e) { return { success: false as const, id: loc.id, error: e as Error }; }
        }));
    }
...
export class GenerateLocationImagesTool extends StructuredTool<typeof GenerateLocationImagesInput> {
    name = "generate_location_images";
    description = "Generates location reference images.";
    schema = GenerateLocationImagesInput;
    constructor(private readonly deps: { context: GenerateLocationImagesContext }, params?: ToolParams) { super(params); }
