...
const ParseLocationsInput = z.object({ input: z.string() });
...
type ToolResultItem = { success: true; locations: LocationAttributes[] } | { success: false; error: string };
...
async function run(text: string, context: ToolContext<TextModelController>): Promise<LocationAttributes[]> {
...
class ParseLocationsTool extends StructuredTool<typeof ParseLocationsInput> {
  ...
  async _call({ input }: ParseLocationsInput, _runManager?: CallbackManagerForToolRun): Promise<string> {
...
export function createParseLocationsTool(deps: ParseLocationsToolDeps, params?: ToolParams): ParseLocationsTool {
