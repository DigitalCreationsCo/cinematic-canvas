...
const ParseCharactersInput = z.object({ input: z.string() });
...
type ToolResultItem = { success: true; characters: CharacterAttributes[] } | { success: false; error: string };
...
async function run(text: string, context: ToolContext<TextModelController>): Promise<CharacterAttributes[]> {
...
class ParseCharactersTool extends StructuredTool<typeof ParseCharactersInput> {
  ...
  async _call({ input }: ParseCharactersInput, _runManager?: CallbackManagerForToolRun): Promise<string> {
...
export function createParseCharactersTool(deps: ParseCharactersToolDeps, params?: ToolParams): ParseCharactersTool {
