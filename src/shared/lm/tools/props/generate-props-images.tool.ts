...
import { PropAttributes, PropWithAssets } from "#shared/types/workflow.types.js";
...
const PropBasePartialWithIdAndVersion = PropAttributes.extend({
  id: IdentityBase.shape.id,
  version: z.number(),
});
type PropBasePartialWithIdAndVersion = z.infer<typeof PropBasePartialWithIdAndVersion>;

const GeneratePropImagesInput = z.object({
  props: z.array(PropBasePartialWithIdAndVersion),
...
async function persistImageAsset(
  ...
  await context.saveAssets(
    { propIds: [prop.id], projectId: context.projectId },
    ["prop_image"],
    "image",
    [gcsUri],
...
async function finaliseResults(
  ...
  _props: PropBasePartialWithIdAndVersion[],
  context: GeneratePropImagesContext,
  ...
  const enrichedResults: GeneratePropImagesResult[] = imageResults.map((r) => {
    ...
    const entity = entityById.get(r.id)! as PropWithAssets;
    return { ...r, entity };
  });
...
class GeneratePropImagesTool extends StructuredTool<typeof GeneratePropImagesInput> {
  ...
  async run(
    params: {
      props: PropBasePartialWithIdAndVersion[];
            ...
            const [imageData] = extractGeneratedResponse(
              ...
              await executeWithRetry(
                ...
                async (error, attempt, p) => {
                  context.incrementAttempt(error.message, "BACKOFF_RETRY");
