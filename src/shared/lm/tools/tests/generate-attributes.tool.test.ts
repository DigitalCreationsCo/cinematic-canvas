...
// ── Mock generateEntityAttributes once at the top level ───────────────────────
...
describe("GenerateCharacterAttributesTool", () => {
  ...
  it("calls generateEntityAttributes with entityDescription 'character profile'", async () => {
    ...
    vi.mocked(generateEntityAttributes).mockResolvedValue([
      { success: true, id, data: characterAttr, entityType: "character" },
    ]);

    const tool = createGenerateCharacterAttributesTool({ context });
    await tool.run({ characters: [{ id }] as any });
  ...
  it("returns success results from generateEntityAttributes unchanged", async () => {
    ...
    const tool = createGenerateCharacterAttributesTool({ context });
    const results = await tool.run({ characters: [{ id }] as any });

    expect(results).toHaveLength(1);
    expect(results[0].success).toBe(true);
    if (results[0].success) {
      expect(results[0].id).toBe(id);
      expect(results[0].output.name).toBe("Alice");
    }
  });
  ...
  it("returns failure results from generateEntityAttributes unchanged", async () => {
    const id = generateId();
    vi.mocked(generateEntityAttributes).mockResolvedValue([
      { success: false, id, data: {}, entityType: "character", error: new Error("LLM failed") },
    ]);
  ...
  it("handles empty input without calling any side effects", async () => {
    ...
    const tool = createGenerateCharacterAttributesTool({ context });
    const results = await tool.run({ characters: [] });
  ...
  it("handles partial failures: maps success and failure results independently", async () => {
    ...
    vi.mocked(generateEntityAttributes).mockResolvedValue([
      { success: true, id: id1, data: characterAttr, entityType: "character" },
      { success: false, id: id2, data: {}, entityType: "character", error: new Error("id2 failed") },
    ]);

    const tool = createGenerateCharacterAttributesTool({ context });
    const results = await tool.run({ characters: [{ id: id1 }, { id: id2 }] as any });
  ...
  it("serialises _call output in the expected JSON format", async () => {
    ...
    expect(parsed.results[0].character.name).toBe("Alice");
  });
});
...
describe("GenerateLocationAttributesTool", () => {
  ...
  it("calls generateEntityAttributes with entityDescription 'location profile'", async () => {
    const id = generateId();
    vi.mocked(generateEntityAttributes).mockResolvedValue([
      { success: true, id, data: locationAttr, entityType: "location" },
    ]);

    const tool = createGenerateLocationAttributesTool({ context });
    await tool.run({ locations: [{ id }] as any });
  ...
  it("returns success results from generateEntityAttributes unchanged", async () => {
    ...
    const tool = createGenerateLocationAttributesTool({ context });
    const results = await tool.run({ locations: [{ id }] as any });
  ...
  it("returns failure results from generateEntityAttributes unchanged", async () => {
    const id = generateId();
    vi.mocked(generateEntityAttributes).mockResolvedValue([
      { success: false, id, data: {}, entityType: "location", error: new Error("LLM failed") },
    ]);
  ...
  it("serialises _call output in the expected JSON format", async () => {
    ...
    expect(parsed.summary).toEqual({ total: 1, succeeded: 1, failed: 0 });
    expect(parsed.results[0].success).toBe(true);
    expect(parsed.results[0].location.name).toBe("Cyberpunk Bar");
  });
});
...
describe("GeneratePropAttributesTool", () => {
  ...
  it("calls generateEntityAttributes with entityDescription 'prop profile'", async () => {
    const id = generateId();
    vi.mocked(generateEntityAttributes).mockResolvedValue([{ success: true, id, data: propAttr, entityType: "prop" }]);

    const tool = createGeneratePropAttributesTool({ context });
    await tool.run({ props: [{ id }] as any });
  ...
  it("returns success results from generateEntityAttributes unchanged", async () => {
    ...
    const tool = createGeneratePropAttributesTool({ context });
    const results = await tool.run({ props: [{ id }] as any });
  ...
  it("returns failure results from generateEntityAttributes unchanged", async () => {
    const id = generateId();
    vi.mocked(generateEntityAttributes).mockResolvedValue([
      { success: false, id, data: {}, entityType: "prop", error: new Error("LLM failed") },
    ]);
  ...
  it("serialises _call output in the expected JSON format", async () => {
    ...
    expect(parsed.summary).toEqual({ total: 1, succeeded: 1, failed: 0 });
    expect(parsed.results[0].success).toBe(true);
    expect(parsed.results[0].prop.name).toBe("Plasma Sword");
  });
});
...
describe("GenerateSceneAttributesTool", () => {
  ...
  it("calls generateEntityAttributes with entityDescription 'scene specification'", async () => {
    const id = generateId();
    vi.mocked(generateEntityAttributes).mockResolvedValue([
      { success: true, id, data: sceneAttr, entityType: "scene" },
    ]);

    const tool = createGenerateSceneAttributesTool({ context });
    await tool.run({ scenes: [{ partial: { id }, images: [] }] as any });
  ...
  it("returns success results from generateEntityAttributes unchanged", async () => {
    ...
    const tool = createGenerateSceneAttributesTool({ context });
    const results = await tool.run({ scenes: [{ partial: { id }, images: [] }] as any });

    expect(results).toHaveLength(1);
    expect(results[0].success).toBe(true);
    if (results[0].success) {
      expect(results[0].id).toBe(id);
      expect(results[0].output.name).toBe("The Escape");
    }
  });
  ...
  it("returns failure results from generateEntityAttributes unchanged", async () => {
    const id = generateId();
    vi.mocked(generateEntityAttributes).mockResolvedValue([
      { success: false, id, data: {}, entityType: "scene", error: new Error("LLM failed") },
    ]);
  ...
  it("serialises _call output in the expected JSON format", async () => {
    ...
    const tool = createGenerateSceneAttributesTool({ context });
    const json = await tool._call({ scenes: [{ partial: { id }, images: [] }] as any });
    const parsed = JSON.parse(json);

    expect(parsed.summary).toEqual({ total: 1, succeeded: 1, failed: 0 });
    expect(parsed.results[0].success).toBe(true);
    expect(parsed.results[0].scene.name).toBe("The Escape");
  });
});
