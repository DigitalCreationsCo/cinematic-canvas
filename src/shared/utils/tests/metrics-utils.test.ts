...
describe("deriveGlobalMetrics", () => {
    ...
    it("totalAssets = totalScenes × assetKeys.length", () => {
        const result = deriveGlobalMetrics(threeSceneRegistries, [ "scene_video", "scene_prompt" ]);
    ...
    it("returns empty assetBreakdown for keys not present in any registry", () => {
        const result = deriveGlobalMetrics(threeSceneRegistries, [ "character_image" ]);
        // character_image isn't in any registry — completedCount should be 0
        expect(result.assetBreakdown.character_image!.completedCount).toBe(0);
    });
});
...
describe("getSceneAssetHistory", () => {
    ...
    it("returns empty array for missing asset key", () => {
        expect(getSceneAssetHistory(singleSceneRegistry, "character_image")).toHaveLength(0);
    });
