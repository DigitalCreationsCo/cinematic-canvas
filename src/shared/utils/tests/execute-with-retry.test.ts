...
describe("executeWithRetry", () => {
  ...
  it("should retry on 429 error and succeed", async () => {
    ...
    mockLmCall.mockRejectedValueOnce(error429).mockResolvedValueOnce("success");
  ...
  it("should preserve original error even if retry exhaustion throws", async () => {
    ...
    await expect(executeWithRetry(mockLmCall, { foo: "bar" }, { ...config, maxRetries: 1 })).rejects.toThrow(
      "Original Error",
    );
  });
});
