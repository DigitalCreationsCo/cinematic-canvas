...
describe('StorageManager Batch Processing (Vitest)', () => {
    ...
    it('should process image batch and save as PNG', async () => {
        ...
        const results = await manager.processImageBatchResults('gs://b/out.jsonl');
    ...
    it('should throw on invalid GCS URI format', async () => {
        await expect(manager.processImageBatchResults('invalid-uri')).rejects.toThrow('Invalid GCS URI');
    });

    it('should throw if the batch output file does not exist', async () => {
        mockFile.exists.mockResolvedValue([ false ]);
        await expect(manager.processImageBatchResults('gs://b/none.jsonl')).rejects.toThrow('Batch file not found');
    });
});