...
export class ChatService {
  ...
  async getConversationsForProject(projectId: string, limit = 50): Promise<Conversation[]> {
    const conversationsRows = await db.select().from(conversations)
  ...
  async addMessage(conversationId: string, role: MessageType, content: string, userId: string, metadata?: Record<string, unknown>): Promise<Message> {
