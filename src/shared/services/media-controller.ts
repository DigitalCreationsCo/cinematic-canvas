...
export class MediaController {
  ...
  async renderScenes(
    ...
    attempt: number,
    audioPath?: string,
  ): Promise<{ gcsUri: string; thumbnailGcsUri: string; duration: number }> {
  ...
  private async executeRenderVideo(videoPaths: string[], audioPath?: string): Promise<string> {
    ...
    try {
      ...
      await Promise.all(
        videoPaths.map(async (pathUrl, i) => {
          ...
          const command = this.ffmpeg(tempVideoPath)
            ...
            .on("error", (err: Error) => {
              ...
              const finalError = new Error(
                `ffmpeg failed to extract frame: ${err.message}\nFFMPEG stderr:\n${ffmpegError}`,
              );
              if (fs.existsSync(tempVideoPath)) fs.unlinkSync(tempVideoPath);
            ...
            .on("end", async () => {
              try {
                if (!fs.existsSync(tempFramePath)) {
                  const finalError = new Error(
                    `Frame extraction failed. File not found at ${tempFramePath}.\nFFMPEG stderr:\n${ffmpegError}`,
                  );
                  reject(finalError);
                  return;
                }
