...
import { FileData, ThinkingLevel } from "@google/genai";
import { cleanJsonOutput, getModelCompatibleSchema } from "#shared/utils/utils.js";
import { buildAudioProcessingInstruction } from "#shared/prompts/audio-analysis.prompt.js";
...
import { GenerativeResultProcessAudioToScenes, JobRenderVideo } from "#shared/types/job.types.js";
...
export class MediaProcessingAgent {
   ...
   */
  async processAudioToScenes(
    audioPath: string | undefined,
    enhancedPrompt: string,
  ): Promise<GenerativeResultProcessAudioToScenes> {
    if (!audioPath) {
  ...
  private async doAudioAnalysis(
    ...
    try {
      ...
      numTokens = audioCountToken.totalTokens;
    } catch (error) {
      console.warn({ audioPath, error }, "Token count failed. Proceeding audio analysis without mediaResolution.");
    }
        ...
        new UserMessage({
          content: [
            // Media first mitigates "lost-in-the-middle" effect
            {
              type: "audio_url",
              audio_url: audioFile,
              ...(numTokens !== undefined ? { mediaResolution: { numTokens } } : {}),
            },
