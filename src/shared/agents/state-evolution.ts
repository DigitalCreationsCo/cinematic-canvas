        ...
        if (desc.includes(keyword)) {
            ...
            if (!isDuplicate) {
                injuries.push({
                    ...
                    location,
                    severity,
                    acquiredInScene: scene.sceneIndex
                });
            }
        }
    }
...
function evolveAtmosphericEffects(
    ...
    const effects = current.map(effect => ({
        ...
        dissipating: effect.addedInScene < scene.sceneIndex - 2
    }));
    ...
    return effects.filter(effect => effect.addedInScene >= scene.sceneIndex - 5);
}
