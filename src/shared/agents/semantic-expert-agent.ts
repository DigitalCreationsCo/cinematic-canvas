...
export class SemanticExpertAgent {
  ...
  async generateRules(storyboard: LiveStoryboard): Promise<GenerativeResultSemanticAnalysis> {
    ...
    try {
      ...
      if (!response.text) {
        ...
        return {
          data: { dynamicRules: [] },
          metadata: { model: this.lm.qualityCheckModel, attempts: 1, acceptedAttempt: 1 },
        };
      }
