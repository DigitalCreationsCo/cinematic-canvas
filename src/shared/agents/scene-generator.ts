...
import { Character, Location, Scene, SceneGenerationResult } from "#shared/types/workflow.types.js";
import { QualityEvaluationResult } from "#shared/types/quality.types.js";
import { IncrementAttemptHook, SaveAssetsCallback, UpdateEntitiesCallback } from "#shared/types/pipeline.types.js";
import { RAIError } from "#shared/utils/errors.js";
import { formatTime, roundToValidDuration } from "#shared/utils/utils.js";
...
import { GenerativeResultEnvelope, GenerativeResultGenerateSceneVideo } from "#shared/types/job.types.js";
...
export class SceneGeneratorAgent {
  ...
  /**
   * Isolated sleep so tests can spy on it without touching real timers.
   * @internal — `protected` (not `private`) to allow spying in tests.
   */
  ...
  async generateSceneWithQualityCheck({
    ...
    console.log(
      { sceneId: scene.id, projectId: scene.projectId, duration: scene.duration },
    ...
    try {
      if (!this.qualityAgent.qualityConfig.enabled) {
        ...
        console.log(
          { sceneId: scene.id, projectId: scene.projectId, durationMs, model: this.videoModel.model },
          `Scene generation completed (no quality check).`,
        );
      ...
      console.log(
        { sceneId: scene.id, projectId: scene.projectId, durationMs, model: this.videoModel.model },
        `Scene generation completed (with quality check).`,
      );
  ...
  private async generateWithQualityRetry(
    ...
    console.log(
      { sceneId: scene.id, projectId: scene.projectId, duration: scene.duration },
      `Quality-controlled scene generation started...`,
    );
    ...
    for (
      let lastestAttempt = version + numAttempts;
      numAttempts <= this.qualityAgent.qualityConfig.maxRetries;
      numAttempts++
    ) {
      ...
      try {
        ...
        console.log(
          ...
          `Quality check attempt completed.`,
        );
        ...
        if (evaluation.score >= acceptanceThreshold) {
          ...
          console.log(
            ...
            `Quality-controlled scene generation completed successfully.`,
          );
    ...
    throw attemptError
      ? attemptError
      : new Error(`Failed to generate acceptable scene after ${totalAttempts} attempts`);
  }
  ...
  private async executeVideoGeneration({
    ...
    try {
      operation = await this.videoModel.generateVideos({
        ...
        config: {
          ...
          negativePrompt:
            "children, celebrity, famous person, photorealistic representation of real person, distorted face, watermark, text, bad quality",
        },
      });
    } catch (error) {
    ...
    while (!operation.done) {
      ...
      await this.sleep(SCENE_GEN_WAITTIME_MS);

      operation = await this.videoModel.getVideosOperation({
        operation,
