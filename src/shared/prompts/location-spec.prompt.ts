...
export const buildLocationFullSpec = (location: LocationWithAssets | LocationAttributes): string => {
  ...
  const lightingDesc = [
    lighting.direction.keyLightPosition && `${lighting.direction.keyLightPosition} light position`,
    lighting.atmosphere.haze === "None" ? "" : ` with ${lighting.atmosphere.haze}`,
    lighting.motivatedSources.primaryLight && `Primary light: ${lighting.motivatedSources.primaryLight}`,
    ...
    lighting.motivatedSources.lightBeams && `${lighting.motivatedSources.lightBeams === "None" ? "" : `${lighting.motivatedSources.lightBeams} light beams`}`,
  ...
  return [
    ...
    `Set during ${location.timeOfDay}${state.season !== "unspecified" ? ` in ${state.season}` : ""}, with ${location.weather || "clear"} weather${atmosphericParts.length > 0 ? ` and ${atmosphericParts.join(", ")}` : ""}.`,
