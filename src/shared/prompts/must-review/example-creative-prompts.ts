/**
 * @fileoverview Example Creative Prompts - Test/Reference Samples
 * 
 * Contains sample creative prompts used for testing, demonstration, and as
 * reference examples for the cinematic generation system.
 * 
 * @module shared/prompts/example-creative-prompts
 * 
 * @description
 * These prompts serve as:
 * - Default inputs when no user prompt is provided
 * - Reference examples showing optimal prompt structure
 * - Test cases for the generation pipeline
 * 
 * @usage
 * NOT actively used in production. These are example/reference prompts.
 * 
 * @example
 * import { defaultCreativePrompt, defaultCreativePrompt2 } from './example-creative-prompts';
 * // Use as fallback or test input
 */

export const defaultCreativePrompt = `
A surf contest between 5 contestants at a picturesque California beach. Each contestant is a different age with a unique personal style. Start with the 5 contestants running into the water.

Show each surfer in a sequence surfing a wave in their unique style.

One by one, the riders fall during the contest until the last rider is left standing. Show the surfers after the contest palling around.

Sunny day. Sense of peril, excitement, and triumph. Show occasional shots of the excited contest announcers and the crowd.
