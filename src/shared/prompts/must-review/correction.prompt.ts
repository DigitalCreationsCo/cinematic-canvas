/**
 * @fileoverview Prompt Refinement Instruction - Iterative Correction
 * 
 * Generates system prompts for refining and correcting prompts based on
 * specific quality feedback issues identified during evaluation.
 * 
 * @module shared/prompts/prompt-refinement-instruction
 * 
 * @description
 * This module provides the prompt correction framework used when generated
 * assets fail to meet quality standards. It instructs the AI on how to:
 * - Apply corrections additively (not reductively)
 * - Add specificity to eliminate ambiguity
 * - Include negative constraints for problematic terms
 * - Specify exact counts and identities
 * - Clarify spatial relationships and motion
 * - Preserve working context from original prompt
 * 
 * @usage
...
import { PromptCorrection, SceneWithAssets } from "../../types/index.js";
...
export const buildCorrectionPrompt = (originalPrompt: string, scene: SceneWithAssets, corrections: PromptCorrection[]) => `As a prompt refinement specialist, apply the following corrections to improve this video generation prompt.
