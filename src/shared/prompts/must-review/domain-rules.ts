/**
 * @fileoverview Domain Rules Presets - Constraint Libraries
 * 
 * Provides preset generation rules for specific domains (surfing, sports, medical,
 * urban, nature) and common quality issues. These rules constrain AI generation
 * to prevent semantic misinterpretation and enforce consistency.
 * 
 * @module shared/prompts/domain-rules-presets
 * 
 * @description
 * This module provides reusable constraint libraries that can be automatically
 * detected and injected into generation prompts based on scene content. Includes:
 * - DOMAIN_SPECIFIC_RULES: Domain-specific physics and terminology constraints
 * - QUALITY_ISSUE_RULES: Rules addressing common quality problems
 * - PROACTIVE_QUALITY_RULES: Pre-emptive rules for common failure patterns
 * 
 * Helper functions:
 * - detectRelevantDomainRules(): Auto-detect applicable rules from scene content
 * - getQualityRulesForIssues(): Get rules based on observed issues
 * - getProactiveRules(): Get baseline quality rules
 * 
 * @usage
 * Used by: semantic-constraints-instruction.ts, worker-service.ts
 */
...
export const PROACTIVE_QUALITY_RULES = [
  "[STRICT_CHARACTER_IDENTITY_LOCKS]: Character facial features, bone structure, and body proportions must strictly match reference embeddings. 28-year-old male = masculine features. Do not swap genders. Do not change character age. Reference images are absolute, not suggestions.",
  "[CRITICAL_COUNTING] QUANTITY ENFORCEMENT: Character counts are binary requirements. If '5 characters' are prompted, exactly 5 distinct entities must be visible. 4 is a CRITICAL FAILURE. 6 is a CRITICAL FAILURE.",
