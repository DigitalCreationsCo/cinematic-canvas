...
export const buildSafetyGuidelinesPrompt = (instructions?: string, originalPrompt?: string, errorMessage?: string): string => [
    instructions ? instructions : '',
    `Avoid violating AI usage guidelines: Do not depict any celebrity or real person, children, violence, or other sensitive content. Describe people using only generic physical attributes. Describe children as young adults. Keep the visual style, action, and lighting instructions intact.`,
    errorMessage ? `Error message: ${errorMessage}` : '',
    originalPrompt ? `original_prompt: ${originalPrompt}` : ''
].join('\n');

export const printSafetyErrorCodes = () => `Refer to this list of safety error codes for guidance: Safety Error Codes:
