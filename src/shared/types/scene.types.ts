...
export const ScriptSupervisorScene = z
  .object({
    continuityNotes: z.array(z.string()).describe("continuity requirements").default([]),
    characterReferenceIds: z
      .array(z.string())
      .default([])
      .describe(
        "Flattened list of character reference IDs present in scene (not uuid). Example: 'Hero, North_Villain'",
      ),
    characterIds: z
      .preprocess(
        (val) => {
      ...
      .describe("Flattened list of character IDs present in scene")
      .default([]),
    locationReferenceId: z
      .string()
      .describe("Reference ID where scene takes place (not uuid). Example: 'Dune_Beach, Dark_Warehouse'"),
...
export const SceneAttributes = z
  .object({
