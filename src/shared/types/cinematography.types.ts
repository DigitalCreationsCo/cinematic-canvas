...
export const Composition = z.object({
  ...
  "Leading Lines": z
    .string()
    .default("None")
    .describe("Deliberate edges, shapes, or trajectories directing a viewer's eyes toward subject"),
  Headroom: z.string().default("Standard").describe("Space above a subject's head. e.g., Tight, Standard, Generous"),
  "Look Room": z
    .string()
    .default("None")
    .describe(
      "negative intentional space between a subject's face and the edge of the frame in the direction they are looking",
    ),
});
  ...
  quality: z
    .object({
      hardness: z
        .string()
        .default("Soft")
        .describe("e.g. Soft (diffused, gentle shadows), Hard (sharp, defined shadows)"),
      colorTemperature: z
        .string()
        .default("Neutral")
        .describe("Warm (2700-3500K), Neutral (4000-5000K), Cool (5500-7000K)"),
      intensity: z
        .string()
        .default("Medium")
        .describe("e.g., Low (dim, moody), Medium (balanced), High (bright, energetic)"),
    })
    .default({
      hardness: "Soft",
  ...
  motivatedSources: z
    .object({
      primaryLight: z
        .string()
        .default("")
        .describe("e.g., Sun through window, street lamp, overhead ceiling, firelight, etc"),
  ...
  direction: z
    .object({
      keyLightPosition: z
        .string()
        .default("")
        .describe("e.g., Front - left, right 45°, Side 90°, Back 135 - 180°, Top-down, etc"),
      shadowDirection: z.string().default("").describe("e.g, Falling left, right, forward, behind subject, etc"),
      contrastRatio: z
        .string()
        .default("")
        .describe("e.g., Low(1: 2) flat, Medium(1: 4) standard, High(1: 8 +) dramatic, etc"),
    })
    .default({
      keyLightPosition: "",
      shadowDirection: "",
