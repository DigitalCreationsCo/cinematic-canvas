...
export class MockProvider implements ITextModelProvider, IVideoModelProvider {
  async generateContent(params: GenerateContentParameters): Promise<GenerateContentResponse> {
    ...
    return {
      candidates: [
        {
          content: {
            parts: [{ text: "This is a mock response from the test mode provider." }],
            role: "model",
          },
          finishReason: "STOP",
          avgLogprobs: 0,
  ...
  async generateVideos(params: GenerateVideosParameters): Promise<any> {
    // Return type any to avoid complex Google types
