...
import { fetchCanvasLayouts, upsertBatchCanvasLayouts, deleteCanvasLayout, confirmCanvasChanges, OCCConflictError, } from '../shared/services/canvasLayoutService.js';
import { ResolveMentionsRequestSchema, RegisterHandleInputSchema, SuggestMentionsRequestSchema, } from '../shared/types/mention.types.js';
...
import { AssetKey, CharacterAttributes, GuidanceLevel, LocationAttributes, PropAttributes, UploadResult } from '#shared/types/index.js';
...
function getJobsCache(projectId) {
    const cached = jobsCache.get(projectId);
    if (cached && cached.expires > Date.now())
        return cached.data;
    return null;
}
function setJobsCache(projectId, data) {
    jobsCache.set(projectId, { data, expires: Date.now() + JOBS_CACHE_TTL_MS });
}
function invalidateJobsCache(projectId) {
...
export function createAppRouter(deps) {
    ...
    async function publishPipelineEvent(eventPayload) {
        console.debug({ eventType: eventPayload.type, projectId: eventPayload.projectId }, '[Router] Publishing pipeline event.');
    ...
    return router({
            ...
            access: teamProcedure
                ...
                try {
                    ...
                    if (!grant) {
                        ...
                        return { role: 'owner', licenseType: 'base_ledger' };
                    }
        ...
        jobs: router({
            list: teamProcedure
                ...
                try {
                    ...
                    setJobsCache(input.projectId, activeJobs);
            ...
            cancel: teamProcedure
                ...
                try {
                    ...
                    const [cancelled] = await db
                        ...
                        .where(and(eq(schema.jobs.id, jobId), eq(schema.jobs.projectId, projectId), eq(schema.jobs.state, 'PENDING')))
                        .returning();
                    if (cancelled) {
        ...
        entities: router({
            ...
            create: teamProcedure
                ...
                try {
                    ...
                    throw new TRPCError({
                        ...
                        message: err?.message || 'Failed to create entities.',
                    });
                }
            }),
            patch: teamProcedure
            ...
            createSceneWithAutoFill: teamProcedure
                ...
                try {
                    ...
                    throw new TRPCError({
                        code: 'INTERNAL_SERVER_ERROR',
                        message: err?.message || 'Failed to queue scene creation.',
                    });
                }
            }),
            ...
            sceneFrameInput: teamProcedure
                ...
                try {
                    ...
                    const [history] = await assetVersionManager.createVersionedAssets({ projectId, sceneIds: [sceneId] }, ['scene_start_frame'], 'image', [sourceDataUri], []);
                    return {
                        result: {
                            sceneId,
                            sourceType,
        ...
        assets: router({
            get: protectedProcedure
                ...
                .query(async ({ input }) => {
                try {
                    return assetVersionManager.getAssetRegistryForEntity(input.entityId, input.entityType);
                }
                catch (err) {
                ...
                try {
                    ...
                    await publishPipelineEvent({
                        ...
                        payload: [{
                                ...
                                entity: {},
                                assets: await avm.getAssetRegistryForEntity(input.entityId, input.entityType),
                            }],
                        timestamp: new Date().toISOString(),
                    });
                    return { success: true };
            ...
            patch: teamProcedure
                ...
                try {
                    const avm = new AssetVersionManager(projectRepository);
                    const scope = { projectId: input.projectId, [`${input.entityType}Ids`]: [input.entityId] };
                    await avm.setBestVersion(scope, [input.assetKey], [input.version]);
                    await publishPipelineEvent({
                        type: 'ENTITY_UPDATED',
                        projectId: input.projectId,
                        teamId: ctx.teamId,
            ...
            generateCharacterImage: teamProcedure
                ...
                try {
                    ...
                    throw new TRPCError({
                        code: 'INTERNAL_SERVER_ERROR',
                        message: err?.message || 'Failed to create characters.',
                    });
                }
            }),
            ...
            generateLocationImage: teamProcedure
                ...
                try {
                    ...
                    throw new TRPCError({
                        code: 'INTERNAL_SERVER_ERROR',
                        message: err?.message || 'Failed to create locations.',
                    });
                }
            }),
        }),
        ...
        canvas: router({
            ...
            confirmChanges: protectedProcedure
                ...
                try {
                    ...
                    throw new TRPCError({
                        code: 'INTERNAL_SERVER_ERROR',
                        message: err?.message || 'Failed to commit batch changes atomically.',
                    });
                }
            }),
        }),
        ...
        sac: router({
            ...
            worldRepo: protectedProcedure
                ...
                try {
                    ...
                    await usersAndTeamsDbService.updateWorldSacRepo(input.worldId, result.repoId, result.repoUrl);
                    return result;
                }
                catch (err) {
