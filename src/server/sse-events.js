...
// ════════════════════════════════════════════════════════════════════════
export const createEventsRouter = ({ eventBus }) => router({
    project: teamProcedure
        ...
        const pipelineEventHandler = async (evt) => {
            if (abortHandler)
                return;
        ...
        const jobEventHandler = async (evt) => {
            if (abortHandler)
                return;
        ...
        try {
            await eventBus.subscribeToPipelineEvents(sseSubscriptionName, pipelineEventHandler, {
                temporary: true,
                ackDeadlineSeconds: 60,
                filter: `attributes.projectId = "${projectId}"`,
                expirationPolicy: { ttl: { seconds: 12 * 60 * 60 } },
            });
            await eventBus.subscribeToJobEvents(jobSseSubscriptionName, jobEventHandler, {
                temporary: true,
                ackDeadlineSeconds: 60,
            ...
            if (isRealtimeConfigured()) {
                try {
                    realtimeChannel = subscribeToLayoutChanges(projectId, (layoutPayload) => {
                    ...
                    console.debug(`[SSE] Supabase Realtime subscribed for project ${projectId}.`);
                }
                catch (errRealtime) {
