  ...
  describe('validateRequest middleware', () => {
    it('should pass valid request through', async () => {
      ...
      const response = await request(app)
        .post('/test')
        .send({ projectId: 'proj-123', initialPrompt: 'test prompt' });
    ...
    it('should reject invalid request body', async () => {
      ...
      const response = await request(app)
        .post('/test')
        .send({ initialPrompt: 'test prompt' }); // missing required projectId
  ...
  describe('Canvas batch endpoint validation', () => {
    ...
    it('should accept valid canvas batch request', async () => {
      ...
      const response = await request(app)
        .put('/batch')
        .send([
          { idEntityTarget: 'ent-1', nodeTypeTarget: 'scene', valPosXTarget: 100, valPosYTarget: 200 }
        ]);
