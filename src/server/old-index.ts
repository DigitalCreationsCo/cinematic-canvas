...
import { PubSubEventBus } from "../shared/messaging/pubsub-event-bus.js";
import { TOPIC_NAMES, SUBSCRIPTION_NAMES } from '../shared/config.js';
...
export async function initializeServer(options: { eventBus: IEventBus; port: number }) {
    try {
        const app = express();
        ...
        return new Promise<http.Server>((resolve) => {
            httpServer.listen(
                ...
                () => {
                    ...
                    console.log(`Bus: ${options.eventBus.constructor.name})`);
...
if (isEntryPoint) {
    ...
    const eventBus = new PubSubEventBus(process.env.GCP_PROJECT_ID!);
    console.log("[Server] Verifying PubSub resources...");

    const subNames = [
        `${SUBSCRIPTION_NAMES.SERVER_PIPELINE_EVENTS_SUBSCRIPTION}-${serverId}`
    ...
    console.log("[Server] Running in DISTRIBUTED mode...");
}
