...
class TestCSVAgentComponent:
    ...
    def test_get_local_path_with_local_file(self, component_class, model_value):
        ...
        with patch("px.components.langchain_utilities.csv_agent.get_settings_service") as mock_get_settings:
    ...
    def test_get_local_path_with_s3_file(self, component_class, model_value):
        ...
        with (
            patch("px.components.langchain_utilities.csv_agent.get_settings_service") as mock_get_settings,
            patch(
                "px.components.langchain_utilities.csv_agent.read_file_bytes", new_callable=AsyncMock
            ) as mock_read_bytes,
        ):
            mock_settings = MagicMock()
