...
class TestExecuteRealFlows:
    ...
    def test_run_json_flow_blocks_real_custom_component_fixture(
        self, webhook_custom_json
    ):
        ...
        with (
            ...
            patch.object(
                component_cache, "type_to_current_hash", _builtin_hashes_from_flow(flow)
            ),
        ):
            result = runner.invoke(
                app,
    ...
    def test_run_error_cases(self):
        ...
        if json_line:
            ...
            assert "does not exist" in error_output["exception_message"], (
                f"Got: {error_output}"
            )
        ...
        if json_line:
            ...
            assert error_output["success"] is False
            assert "exception_message" in error_output, f"Got: {error_output}"
            assert "Multiple input sources" in error_output["exception_message"], (
                f"Got: {error_output}"
            )
    ...
    def test_run_json_output_format(self, simple_chat_json):
        ...
        json_output = (
            result_verbose.stdout if result_verbose.stdout else result_verbose.output
        )
...
class TestAsyncFunctionality:
    ...
    def test_async_load_is_used(self, simple_chat_json, monkeypatch):
        """Test that aload_flow_from_json is being used - expects failure in px environment."""
        ...
        monkeypatch.setattr("px.load.aload_flow_from_json", mock_aload)
