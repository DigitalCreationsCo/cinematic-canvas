...
class MCPStdio(Component):
    ...
    description = "Connects to an MCP server over stdio and exposes it's tools as portals tools to be used by an Agent."
            ...
            tool_list.append(
                StructuredTool(
                    ...
                    coroutine=create_tool_coroutine(
                        tool.name, args_schema, self.client.session
                    ),
                )
            )
