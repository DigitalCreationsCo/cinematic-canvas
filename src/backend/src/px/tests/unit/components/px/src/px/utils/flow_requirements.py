"""Generate requirements.txt from a Portals flow JSON.

Analyzes a flow's component code and configuration to determine the minimal
set of PyPI packages needed to run that flow on a standalone PX runner.

Uses ``importlib.metadata`` to dynamically resolve import names to PyPI
distribution names and to compute the transitive dependency tree of ``px``,
...
* **System-level dependencies** — Native libraries required by Python packages
  ...
  ``requirements.txt``.
* **``--px-package`` and transitive filtering** — The ``px_package`` parameter
  controls only the output name (e.g. ``px-nightly``).  The "already provided
  by px" filter always resolves against the ``px`` distribution installed in
...
# Import names that are internal to the px/portals runtime and should
# never appear as separate requirements.
_INTERNAL_IMPORT_NAMES: frozenset[str] = frozenset({"px", "portals", "portals_base"})
...
@lru_cache(maxsize=1)
def _get_px_transitive_dists() -> frozenset[str]:
    """Compute the full transitive closure of distributions provided by px.
...
@lru_cache(maxsize=1)
def _get_px_provided_imports() -> frozenset[str]:
    """Build the set of import names transitively provided by px.
...
def _resolve_provider_packages(provider_name: str) -> set[str]:
    ...
    Parent classes (e.g. ``LCModelComponent``) are all part of px, so any
    imports they introduce are already in px's transitive dependency tree
    ...
    px_provided = _get_px_provided_imports()
...
def _resolve_embedding_provider_packages(provider_name: str) -> set[str]:
    ...
    the ``LanguageModelComponent``: its code field only imports from ``px``
    ...
    # Use install_hint if provided (handles internal module paths like px.base.models.*)
    if install_hint:
        return {install_hint}
        ...
        if source and isinstance(source, str):
            ...
            for imp in imports:
                ...
                if imp in STDLIB_MODULES:
                    continue
                # Skip px / portals internal imports - px provides these
                ...
                # even if the import itself is provided by px, because the
                # extras may not be.
                if imp in MODULE_EXTRA_DEPS:
                    for extra in MODULE_EXTRA_DEPS[imp]:
                        packages.add(extra)

                # Skip imports already provided by px
                if imp in px_provided:
                    continue
...
def generate_requirements_from_flow(
    ...
    *,
    ...
    """Generate a requirements list from a Portals flow JSON.

    Args:
        flow: Parsed Portals flow JSON (dict).
        px_package: Name of the PX package to include (e.g. ``"px"`` or
            ``"px-nightly"``).
        include_px: Whether to include the PX package itself.
...
def generate_requirements_txt(
    ...
    *,
    ...
    """Generate requirements.txt content from a Portals flow JSON.

    Args:
        flow: Parsed Portals flow JSON (dict).
        px_package: Name of the PX package to include.
        include_px: Whether to include the PX package itself.
    ...
    reqs = generate_requirements_from_flow(
        flow,
        px_package=px_package,
        include_px=include_px,
        pin_versions=pin_versions,
    )
    lines = [
        "# Auto-generated requirements for Portals flow",
...
def generate_requirements_from_file(
    ...
    *,
    ...
    Args:
        flow_path: Path to a Portals flow JSON file.
        px_package: Name of the PX package to include.
        include_px: Whether to include the PX package itself.
        pin_versions: If True, pin each package to the currently installed
            version.
