...
authentication.  The SDK helpers in :mod:`px.load.utils` were updated to
forward an optional ``api_key`` (or ``PORTALS_API_KEY`` env var) as the
...
import httpx
import pytest
from px.load.utils import UploadError, upload, upload_file
...
def test_upload_sends_api_key_header_when_passed_explicitly(tmp_path):
    ...
    with patch("px.load.utils.httpx.post", return_value=_ok_response()) as mock_post:
...
def test_upload_falls_back_to_env_var(tmp_path, monkeypatch):
    file_path = tmp_path / "x.txt"
    file_path.write_bytes(b"contents")
    monkeypatch.setenv("PORTALS_API_KEY", "sk-from-env")  # pragma: allowlist secret
...
def test_upload_file_forwards_api_key(tmp_path):
    ...
    with patch("px.load.utils.upload", return_value={"file_path": "flow/x.txt"}) as mock_upload:
        upload_file(
            str(file_path),
            host="http://host",
            flow_id="flow-1",
...
def test_upload_raises_upload_error_on_auth_failure(tmp_path, monkeypatch):
    ...
    with patch("px.load.utils.httpx.post", return_value=response), pytest.raises(UploadError):
        upload(str(file_path), "http://host", "flow-1")
