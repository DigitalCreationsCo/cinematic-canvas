...
from px.base.constants import STREAM_INFO_TEXT
from px.custom.custom_component.component import Component
from px.field_typing import LanguageModel
from px.inputs.inputs import BoolInput, InputTypes, MessageInput, MultilineInput
from px.log.logger import logger
from px.schema.message import Message
from px.schema.properties import Usage
from px.schema.token_usage import extract_usage_from_message
from px.template.field.base import Output
from px.utils.constants import MESSAGE_SENDER_AI
...
class LCModelComponent(Component):
    ...
    outputs = [
        Output(
            display_name="Model Response", name="text_output", method="text_response"
        ),
        Output(
    ...
    async def _get_chat_result(
        ...
        *,
        ...
        try:
            ...
            if stream:
                lf_message, result, message = await self._handle_stream(
                    runnable, inputs
                )
            else:
    ...
    async def _handle_stream(self, runnable, inputs):
        ...
        if self.is_connected_to_chat_output():
            ...
            if session_id and event_manager:
                ...
                model_message.properties.source = self._build_source(
                    self._id, self.display_name, self
                )
                ...
                component_label = getattr(self, "display_name", None) or getattr(
                    self, "_id", "<unknown>"
                )
                logger.warning(
    ...
    def get_llm(
        self,
        provider_name: str,
        model_info: dict[str, dict[str, str | list[InputTypes]]],
    ) -> LanguageModel:
        ...
        try:
            if provider_name not in [
                model.get("display_name") for model in model_info.values()
            ]:
            ...
            component_info, module_name = next(
                (
                    ...
                    if info.get("display_name") == provider_name
                ),
                (None, None),
            )
            ...
            with warnings.catch_warnings():
                ...
                models_module = importlib.import_module("px.components.models")
    ...
    def build_llm_model_from_inputs(
        ...
        input_data = {
            str(component_input.name): getattr(
                self, f"{prefix}{component_input.name}", None
            )
