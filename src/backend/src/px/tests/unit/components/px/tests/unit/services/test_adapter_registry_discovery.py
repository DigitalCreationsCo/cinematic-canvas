...
import pytest
from px.services.adapters import registry as adapter_registry_mod
...
def test_get_adapter_registry_raises_on_parameter_mismatch_for_same_adapter_type():
    ...
    with pytest.raises(adapter_registry_mod.AdapterRegistryConflictError):
        adapter_registry_mod.get_adapter_registry(
            ...
            entry_point_group="px.OTHER.group",
...
def test_get_adapter_registry_convention_defaults():
    ...
    assert registry.entry_point_group == "px.deployment.adapters"
...
def test_discovery_precedence_entrypoint_decorator_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda group: [DummyEntryPoint("local", DummyEntryPointAdapter)] if group == "px.deployment.adapters" else [],
    )

    adapter_registry_mod.register_adapter(AdapterType.DEPLOYMENT, "local")(DummyDecoratorAdapter)

    (tmp_path / "px.toml").write_text(
        f"""
[deployment.adapters]
...
def test_px_toml_takes_precedence_over_pyproject(tmp_path):
    registry = make_deployment_adapter_registry()
...
def test_entry_point_load_failure_does_not_abort_discovery(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "importlib.metadata.entry_points",
        lambda group: [BrokenEntryPoint()] if group == "px.deployment.adapters" else [],
    )
    (tmp_path / "px.toml").write_text(
        f"""
[deployment.adapters]
...
def test_get_nested_section_returns_none_for_non_dict_path():
    from px.services.config_discovery import get_nested_section

    nested = {"tool": {"px": {"deployment": "not-a-dict"}}}

    section = get_nested_section(nested, ("tool", "px", "deployment", "adapters"))
...
def test_multi_key_discovery_across_sources(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "importlib.metadata.entry_points",
        lambda group: (
            [DummyEntryPoint("ep_only", DummyEntryPointAdapter)] if group == "px.deployment.adapters" else []
        ),
    )
