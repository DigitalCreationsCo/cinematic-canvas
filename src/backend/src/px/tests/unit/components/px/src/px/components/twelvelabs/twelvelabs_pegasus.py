...
class TwelveLabsPegasus(Component):
    ...
    documentation = "https://github.com/twelvelabs-io/twelvelabs-developer-experience/blob/main/integrations/Portals/TWELVE_LABS_COMPONENTS_README.md"

    inputs = [
        DataInput(
            name="videodata", display_name="Video Data", info="Video Data", is_list=True
        ),
        SecretStrInput(
            name="api_key",
    ...
    def validate_video_file(self, filepath: str) -> tuple[bool, str]:
        ...
        if not isinstance(filepath, str) or any(
    ...
    def process_video(self) -> Message:
        ...
        if hasattr(self, "index_id") and self.index_id:
            self._index_id = (
                self.index_id.text if hasattr(self.index_id, "text") else self.index_id
            )

        if hasattr(self, "index_name") and self.index_name:
            self._index_name = (
                self.index_name.text
                if hasattr(self.index_name, "text")
                else self.index_name
            )

        if hasattr(self, "video_id") and self.video_id:
            self._video_id = (
                self.video_id.text if hasattr(self.video_id, "text") else self.video_id
            )

        if hasattr(self, "message") and self.message:
            self._message = (
                self.message.text if hasattr(self.message, "text") else self.message
            )
        ...
        try:
            ...
            if self._message and self._video_id and self._video_id != "":
                ...
                self.status = (
                    f"Processing query (w/ video ID): {self._video_id} {self._message}"
                )
                self.log(self.status)
            ...
            if (
                not self.videodata
                or not isinstance(self.videodata, list)
                or len(self.videodata) != 1
            ):
            ...
            success_msg = f"Video processed successfully. You can now ask questions about the video. Video ID: {self._video_id}"
