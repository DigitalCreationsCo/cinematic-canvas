...
from portals.helpers.flow import get_flow_by_id_or_name
from px.base.tools.constants import TOOL_OUTPUT_NAME
...
# TODO: switch to px
from px.helpers import get_flow_inputs, run_flow
...
from px.services.deps import get_shared_component_cache_service
...
class RunFlowBaseComponent(Component):
    ...
    async def get_graph(
        ...
        if flow_id_selected and (
            flow := self._flow_cache_call("get", flow_id=flow_id_selected)
        ):
            if self._is_cached_flow_up_to_date(flow, updated_at):
                return flow
            self._flow_cache_call(
                "delete", flow_id=flow_id_selected
            )  # stale, delete it
    ...
    def add_new_fields(
        self, build_config: dotdict, new_fields: list[dotdict]
    ) -> dotdict:
    ...
    async def get_required_data(self) -> tuple[str, list[dotdict]] | None:
        ...
        return (
            graph.description,
            [field for field in new_fields if field.get("tool_mode", False)],
        )
    ...
    async def update_outputs(
        ...
        if (field_name == "flow_name_selected" and field_value) or (
            ...
            selected_flow = frontend_node.get("template", {}).get(
                "flow_name_selected", {}
            )
    ...
    def _flow_cache_call(self, action: str, *args, **kwargs):
        ...
        try:
            ...
            if not key and args:
                key = args[0]
            logger.warning(
                "Cache %s failed for key %s: %s", action, key or "[missing key]", exc
            )
            return None
    ...
    def _get_cached_flow(self, *, flow_id: str | None = None) -> Graph | None:
        ...
        if not cache_entry:
            logger.warning(
                f"None or empty cache entry ({cache_entry}) for key {cache_key}"
            )
            return None
    ...
    def _build_graph_from_dict(self, *, cache_entry: dict[str, Any]) -> Graph | None:
        ...
        graph.description = cache_entry.get("description") or graph_dump.get(
            "description"
        )
        graph.updated_at = cache_entry.get("updated_at") or graph_dump.get("updated_at")
        return graph

    def _is_cached_flow_up_to_date(
        self, cached_flow: Graph, updated_at: str | None
    ) -> bool:
        if not updated_at or not (
            cached_ts := getattr(cached_flow, "updated_at", None)
        ):
    ...
    def _build_inputs(
        ...
        extracted_inputs = tweaks or self._extract_ioputs_from_keyed_values(
            self._attributes
        )
    ...
    def _build_flow_tweak_data(self) -> dict[str, dict[str, Any]]:
        ...
        combined_values = (
            self._attributes
        )  # allow mutation for now to avoid overhead of creating a copy
    ...
    def _pre_run_setup(self) -> None:  # Note: overrides the base pre_run_setup method
        ...
        if self._cached_flow_updated_at:
            self._attributes["flow_name_selected_updated_at"] = (
                self._cached_flow_updated_at
            )
    ...
    def _process_tweaks_on_graph(self, graph: Graph, tweaks: dict[str, dict[str, Any]]):
        # there is a bug with the px process_tweaks_on_graph function
