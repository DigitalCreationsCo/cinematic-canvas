...
def _get_lf_version() -> str | None:
    """Return the installed Portals version string, or *None* if not installed.
    ...
    for pkg in ("portals-base", "portals", "portals-base-nightly", "portals-nightly"):
        try:
            return version(pkg)
    ...
    *,
    ...
    if show_issues:
        for issue in result.issues:
            effective_severity = (
                "error" if (strict and issue.severity == "warning") else issue.severity
            )
            color = "red" if effective_severity == "error" else "yellow"
            loc = (
                f" [{issue.node_name or issue.node_id}]"
                if (issue.node_id or issue.node_name)
                else ""
            )
            console.print(
    ...
    *,
    ...
    for i, p in enumerate(paths, start=1):
        ...
        if output_format != "json":
            _render_result(
                result, index=i, total=len(paths), verbose=verbose, strict=strict
            )
