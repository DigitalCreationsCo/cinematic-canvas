"""Unit tests for px validate — structural, extended checks, directory scanning, strict mode.

All tests run entirely in-process (no running Portals instance or component
...
class TestVersionMismatch:
    ...
    def test_multiple_different_versions_produce_one_warning_each(self):
        ...
        flow = {
            **_MINIMAL_VALID,
            "data": {"nodes": [_NODE_WITH_VERSION, node_b], "edges": []},
        }
        result = self._make_result()
        with patch("px.cli.validation.core._get_lf_version", return_value="1.9.0"):
            _check_version_mismatch(flow, result)
    ...
    def test_portals_not_installed_skips_check(self):
        """If Portals is not installed, version check is skipped silently."""
        flow = {**_MINIMAL_VALID, "data": {"nodes": [_NODE_WITH_VERSION], "edges": []}}
        result = self._make_result()
        with patch("px.cli.validation.core._get_lf_version", return_value=None):
            _check_version_mismatch(flow, result)
        assert not result.warnings
    ...
    def test_version_warning_strict_mode_causes_exit_1(self, tmp_path):
        ...
        p = _write_flow(tmp_path, "flow.json", flow)
        with (
            patch("px.cli.validation.core._get_lf_version", return_value="1.9.0"),
