...
from px.inputs import BoolInput, DropdownInput, HandleInput, IntInput
from px.schema import Data
from px.template import Output
...
class SplitVideoComponent(Component):
    ...
    def get_video_duration(self, video_path: str) -> float:
        ...
        try:
            ...
            if not isinstance(video_path, str) or any(
                c in video_path for c in ";&|`$(){}[]<>*?!#~"
            ):
    ...
    def get_output_dir(self, video_path: str) -> str:
        ...
        output_dir = (
            Path(path_obj.parent) / f"clips_{base_name}_{timestamp}_{path_hash}"
        )
    ...
    def process_video(
        self, video_path: str, clip_duration: int, *, include_original: bool
    ) -> list[Data]:
        ...
        try:
            ...
            for i in range(int(num_clips)):  # Convert num_clips to int for range
                ...
                if (
                    i == int(num_clips) - 1 and duration < clip_duration
    ...
    def process(self) -> list[Data]:
        ...
        try:
            ...
            return self.process_video(
                video_path, self.clip_duration, include_original=self.include_original
            )
