...
from px.services.manager import ServiceManager
from px.services.schema import ServiceType

from .conftest import MockSessionService


class TestStandalonePX:
    """Test PX running standalone without portals."""
    ...
    def clean_manager(self):
        ...
        manager.register_service_class(
            ServiceType.SESSION_SERVICE, MockSessionService, override=True
        )
        yield manager
        # Cleanup
        import asyncio
    ...
    def test_minimal_storage_service_loads(self, clean_manager):
        ...
        clean_manager.register_service_class(
            ServiceType.STORAGE_SERVICE, LocalStorageService
        )
    ...
    def test_minimal_telemetry_service_loads(self, clean_manager):
        ...
        clean_manager.register_service_class(
            ServiceType.TELEMETRY_SERVICE, TelemetryService
        )
    ...
    def test_minimal_variable_service_loads(self, clean_manager):
        ...
        from px.services.variable.service import VariableService

        clean_manager.register_service_class(
            ServiceType.VARIABLE_SERVICE, VariableService
        )
...
class TestPXWithPortalsConfig:
    """Test PX with portals configuration."""

    @pytest.fixture
    def portals_config_dir(self, tmp_path):
        """Create a temporary portals-style config directory."""
        config_dir = tmp_path / "config"
        config_dir.mkdir()

        # Create px.toml with portals services
        config_file = config_dir / "px.toml"
        config_file.write_text(
            """
[services]
storage_service = "px.services.storage.local:LocalStorageService"
cache_service = "px.services.cache.service:ThreadingInMemoryCache"
"""
        )
    ...
    def test_config_overrides_defaults(self, clean_manager, portals_config_dir):
        """Test that config file overrides default services."""
        # Discover plugins from config
        clean_manager.discover_plugins(portals_config_dir)
        ...
        assert (
            clean_manager.service_classes[ServiceType.STORAGE_SERVICE]
            == LocalStorageService
        )

    def test_multiple_services_from_config(self, clean_manager, portals_config_dir):
...
"""
        ...
        clean_manager.register_service_class(
            ServiceType.STORAGE_SERVICE, CustomStorageService, override=True
        )
    ...
    def test_override_false_preserves_existing(self, clean_manager):
        ...
        # Register first
        clean_manager.register_service_class(
            ServiceType.STORAGE_SERVICE, FirstService, override=True
        )

        # Try to register second with override=False
        clean_manager.register_service_class(
            ServiceType.STORAGE_SERVICE, SecondService, override=False
        )

        # Should still be first
        assert (
            clean_manager.service_classes[ServiceType.STORAGE_SERVICE] == FirstService
        )
...
"""
    ...
    def test_service_without_name_attribute(self, clean_manager):
        ...
        clean_manager.register_service_class(
            ServiceType.STORAGE_SERVICE, InvalidService
        )
    ...
    def test_service_initialization_failure(self, clean_manager):
        ...
        clean_manager.register_service_class(
            ServiceType.STORAGE_SERVICE, FailingService
        )
...
class TestDependencyResolution:
    ...
    def test_service_with_settings_dependency(self, clean_manager):
        ...
        clean_manager.register_service_class(
            ServiceType.STORAGE_SERVICE, ServiceWithSettings
        )
    ...
    def test_service_with_unresolvable_dependency(self, clean_manager):
        ...
        clean_manager.register_service_class(
            ServiceType.STORAGE_SERVICE, ServiceWithUnknownDep
        )
...
"""
    ...
    def test_px_toml_takes_precedence(self, clean_manager, tmp_path):
        """Test that px.toml takes precedence over pyproject.toml."""
        config_dir = tmp_path / "config"
        config_dir.mkdir()

        # Create both files with different services
        (config_dir / "px.toml").write_text(
            """
[services]
storage_service = "px.services.storage.local:LocalStorageService"
"""
...
"""
        ...
        # Should only have storage from px.toml
        assert ServiceType.STORAGE_SERVICE in clean_manager.service_classes
