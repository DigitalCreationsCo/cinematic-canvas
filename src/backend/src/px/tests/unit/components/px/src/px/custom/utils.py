...
from px.custom import validate
...
from px.custom.dependency_analyzer import analyze_component_dependencies
from px.custom.directory_reader.utils import (
...
from px.custom.schema import MissingDefault
from px.field_typing.range_spec import RangeSpec
from px.helpers.custom import format_type
from px.log.logger import logger
from px.schema.dotdict import dotdict
from px.template.field.base import Input
from px.template.frontend_node.custom_components import (
    ComponentFrontendNode,
    CustomComponentFrontendNode,
)
from px.type_extraction.type_extraction import extract_inner_type
from px.utils.util import get_base_classes
...
def reorder_fields(
    frontend_node: CustomComponentFrontendNode, field_order: list[str]
) -> None:
...
def add_extra_fields(frontend_node, field_config, function_args) -> None:
    ...
    if "kwargs" in function_args_names and not all(
        key in function_args_names for key in field_config
    ):
        for field_name, config in field_config_.items():
            ...
            field_name_, field_type, field_value, field_required = get_field_properties(
                extra_field=config_
            )
            frontend_node = add_new_custom_field(
                frontend_node=frontend_node,
...
def get_component_instance(
    custom_component: CustomComponent | Component, user_id: str | UUID | None = None
):
...
def run_build_config(
    ...
    # we have imported it
    # If not, it means the component was loaded through PORTALS_COMPONENTS_PATH
    ...
    if custom_component._code is None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail={
                    "error": (
                        "Invalid type conversion. Please check your code and try again."
                    ),
                    "traceback": traceback.format_exc(),
                },
            ) from exc
        ...
        try:
            ...
            for field_name, field in build_config.copy().items():
                ...
                if "rangeSpec" in field_dict and isinstance(
                    field_dict["rangeSpec"], RangeSpec
                ):
...
def build_custom_component_template_from_inputs(
    ...
    if ctype_name in _COMPONENT_TYPE_NAMES:
        ...
        frontend_node = ComponentFrontendNode.from_inputs(
            **custom_component.template_config
        )
...
def build_component_metadata(
    ...
    try:
        ...
        logger.debug(
            f"Error generating code hash for {custom_component.__class__.__name__}",
            exc_info=exc,
        )
    ...
    try:
        ...
        logger.warning(
            f"Failed to analyze dependencies for component {ctype_name}: {exc}"
        )
...
def build_custom_component_template(
    ...
    try:
        ...
        add_base_classes(
            frontend_node, custom_component._get_function_entrypoint_return_type
        )
        add_output_types(
            frontend_node, custom_component._get_function_entrypoint_return_type
        )
        ...
        frontend_node = build_component_metadata(
            ...
            custom_component,
            module_name,
            custom_component.__class__.__name__,
        )
...
def sanitize_field_config(field_config: dict | Input):
    ...
    field_dict = (
        field_config.to_dict() if isinstance(field_config, Input) else field_config
    )
    for key in [
        "name",
        "field_type",
...
async def update_component_build_config(
    ...
    if inspect.iscoroutinefunction(component.update_build_config):
        return await component.update_build_config(
            build_config, field_value, field_name
        )
    return await asyncio.to_thread(
        component.update_build_config, build_config, field_value, field_name
    )
...
async def get_single_component_dict(
    ...
    for base_path in components_paths:
        ...
        if module_path.exists():
            ...
            module_name = f"px.components.{component_type}.{component_name}"
            try:
                ...
                await logger.aerror(
                    f"Import error loading component {module_path}: {e!s}"
                )
            except AttributeError as e:
                await logger.aerror(
                    f"Attribute error loading component {module_path}: {e!s}"
                )
            except ValueError as e:
                await logger.aerror(
                    f"Value error loading component {module_path}: {e!s}"
                )
            except (KeyError, IndexError) as e:
                await logger.aerror(
                    f"Data structure error loading component {module_path}: {e!s}"
                )
            except RuntimeError as e:
                await logger.aerror(
                    f"Runtime error loading component {module_path}: {e!s}"
                )
...
async def load_custom_component(component_name: str, components_paths: list[str]):
    ...
    """
    from px.interface.custom_component import get_custom_component_from_name
                ...
                for category_dir in base_path.iterdir():
                    if category_dir.is_dir():
                        ...
                        if component_file.exists():
                            # Try to import the module
                            module_name = (
                                f"px.components.{category_dir.name}.{component_name}"
                            )
                            try:
                                import importlib.util
                                ...
                                await logger.aerror(
                                    f"Import error loading component {component_file}: {e!s}"
                                )
                                await logger.adebug(
                                    "Import error traceback", exc_info=True
                                )
                            except AttributeError as e:
                                await logger.aerror(
                                    f"Attribute error loading component {component_file}: {e!s}"
                                )
                                await logger.adebug(
                                    "Attribute error traceback", exc_info=True
                                )
                            except (ValueError, TypeError) as e:
                                await logger.aerror(
                                    f"Value/Type error loading component {component_file}: {e!s}"
                                )
                                await logger.adebug(
                                    "Value/Type error traceback", exc_info=True
                                )
                            except (KeyError, IndexError) as e:
                                await logger.aerror(
                                    f"Data structure error loading component {component_file}: {e!s}"
                                )
                                await logger.adebug(
                                    "Data structure error traceback", exc_info=True
                                )
                            except RuntimeError as e:
                                await logger.aerror(
                                    f"Runtime error loading component {component_file}: {e!s}"
                                )
                                await logger.adebug(
                                    "Runtime error traceback", exc_info=True
                                )
                            except OSError as e:
                                await logger.aerror(
                                    f"OS error loading component {component_file}: {e!s}"
                                )
                                await logger.adebug("OS error traceback", exc_info=True)

    except ImportError as e:
        await logger.aerror(
            f"Import error loading custom component {component_name}: {e!s}"
        )
        return None
    except AttributeError as e:
        await logger.aerror(
            f"Attribute error loading custom component {component_name}: {e!s}"
        )
        return None
    except ValueError as e:
        await logger.aerror(
            f"Value error loading custom component {component_name}: {e!s}"
        )
        return None
    except (KeyError, IndexError) as e:
        await logger.aerror(
            f"Data structure error loading custom component {component_name}: {e!s}"
        )
        return None
    except RuntimeError as e:
        await logger.aerror(
            f"Runtime error loading custom component {component_name}: {e!s}"
        )
    ...
    await logger.awarning(
        f"Component {component_name} not found in any of the provided paths"
    )
    return None
