...
class TestFieldTypingLazyImports:
    ...
    def test_input_import(self):
        ...
        from px.field_typing import Input
        from px.template.field.base import Input as DirectInput

        assert Input is DirectInput

    def test_output_import(self):
        """Test that Output can be imported from field_typing."""
        from px.field_typing import Output
        from px.template.field.base import Output as DirectOutput

        assert Output is DirectOutput

    def test_range_spec_import(self):
        """Test that RangeSpec can be imported from field_typing."""
        from px.field_typing import RangeSpec
        from px.field_typing.range_spec import RangeSpec as DirectRangeSpec
    ...
    def test_repeated_access_returns_same_object(self):
        ...
        from px.field_typing import Data as Data1
        from px.field_typing import Data as Data2
    ...
    def test_constants_match_direct_imports(self):
        ...
        from px.field_typing import Data, Document, Object
        from px.field_typing.constants import Data as DirectData
        from px.field_typing.constants import Document as DirectDocument
        from px.field_typing.constants import Object as DirectObject
...
class TestFieldTypingNativeDependencyFailure:
    ...
    OSError raised by `torch._load_dll_libraries()` propagates upward through:
        ...
        px.field_typing.constants line 9 (`from langchain_classic.agents import AgentExecutor`)

    `px.field_typing.constants` already wraps its langchain imports in a
    ...
    def test_should_use_stub_classes_when_langchain_import_raises_oserror(
        self, monkeypatch
    ):
        """Bug: `portals --version` crashes on fresh Windows because px fails to import.

        Cause: `c10.dll` raises `OSError: [WinError 126]` and the existing fallback
        only catches `ImportError`. After fix: stub classes are used and px loads
        ...
        monkeypatch.delitem(sys.modules, "px.field_typing.constants", raising=False)
        monkeypatch.delitem(sys.modules, "px.field_typing", raising=False)

        # Act — must not raise (this is the regression guard)
        from px.field_typing import constants

        # Assert — the in-module stub class is what's exposed, not the real one
        assert constants.AgentExecutor is not None
        assert constants.AgentExecutor.__module__ == "px.field_typing.constants"
        assert "AgentExecutor" in constants.LANGCHAIN_BASE_TYPES
        assert (
            constants.LANGCHAIN_BASE_TYPES["AgentExecutor"] is constants.AgentExecutor
        )
...
class TestFieldTypingModuleStructure:
    ...
    def test_all_list_exists(self):
        ...
        assert hasattr(px.field_typing, "__all__")
        assert isinstance(px.field_typing.__all__, list)
        assert len(px.field_typing.__all__) > 0
    ...
    def test_getattr_function_exists(self):
        ...
        assert hasattr(px.field_typing, "Input") or "Input" in px.field_typing.__all__

    def test_constants_names_set_exists(self):
        """Test that _CONSTANTS_NAMES is defined."""
        import px.field_typing

        assert hasattr(px.field_typing, "_CONSTANTS_NAMES")
        assert isinstance(px.field_typing._CONSTANTS_NAMES, set)
