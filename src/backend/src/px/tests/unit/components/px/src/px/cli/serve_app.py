"""FastAPI application factory for serving **multiple** PX graphs at once.
...
:func:`px.cli.commands.verify_api_key`.
"""
...
from px.cli.common import execute_graph_with_capture, extract_result_data, get_api_key
...
# Security - use the same pattern as Portals main API
...
def _generate_dynamic_run_description(graph: Graph) -> str:
    ...
    description_parts = [
        f"Execute the deployed PX graph with {analysis['node_count']} components.",
        "",
        "**Graph Analysis**:",
...
class RunRequest(BaseModel):
    """Request model for executing a PX flow."""
...
class StreamRequest(BaseModel):
    """Request model for streaming execution of a PX flow."""
...
async def run_flow_generator_for_serve(
    ...
    try:
        ...
        # to integrate with the full PX streaming pipeline from endpoints.py
...
def create_multi_serve_app(
    *,
    ...
    """Create a FastAPI app exposing multiple PX flows.
    ...
    app = FastAPI(
        title=f"PX Multi-Flow Server ({len(graphs)})",
        description=(
            "This server hosts multiple PX graphs under the `/flows/{id}` prefix. "
    ...
    def create_flow_router(flow_id: str, graph: Graph, meta: FlowMeta) -> APIRouter:
        ...
        async def run_flow(
            ...
            try:
                ...
                # Debug logging
                logger.debug(
                    f"Flow {flow_id} execution completed: {len(results)} results, {len(logs)} log chars"
                )
                ...
                if not result_data.get("success", True):
                    ...
                    if not error_logs.strip():
                        error_logs = f"Flow execution completed but no valid result was produced.\nResult data: {result_data}"
        ...
        async def stream_flow(
            ...
            try:
                ...
                from px.events.event_manager import create_stream_tokens_event_manager
                ...
                async def on_disconnect() -> None:
                    logger.debug(
                        f"Client disconnected from flow {flow_id}, closing tasks"
                    )
                    main_task.cancel()
