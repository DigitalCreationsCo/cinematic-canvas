"""Unit tests for ``px create`` and the ``--example`` seeding in ``px init``.

All tests run entirely in-process; no running Portals instance required.
"""
...
from px.cli.init import init_command
...
class TestListTemplates:
    ...
    def test_returns_empty_when_dir_missing(self, tmp_path):
        ...
        with patch("px.cli.create._FLOWS_TEMPLATE_DIR", fake_dir):
...
class TestTemplateStructure:
    ...
    def test_has_required_top_level_keys(self, template_name):
        ...
        for key in ("id", "name", "data"):
            assert key in flow, (
                f"Template {template_name!r} missing top-level key '{key}'"
            )
    ...
    def test_every_node_has_type(self, template_name):
        ...
        for node in flow["data"]["nodes"]:
            assert "type" in node["data"], (
                f"Node {node['id']!r} in template {template_name!r} missing 'data.type'"
            )
    ...
    def test_edges_reference_existing_nodes(self, template_name):
        ...
        for edge in flow["data"]["edges"]:
            assert edge["source"] in node_ids, (
                f"Edge source {edge['source']!r} not in nodes for {template_name!r}"
            )
            assert edge["target"] in node_ids, (
                f"Edge target {edge['target']!r} not in nodes for {template_name!r}"
            )
...
class TestCreateCommand:
    ...
    def test_raises_exit1_when_no_templates_dir(self, tmp_path):
        ...
        with (
            patch("px.cli.create._FLOWS_TEMPLATE_DIR", fake_dir),
...
class TestPrintTemplates:
    ...
    def test_prints_nothing_when_no_templates(self, tmp_path):
        with patch("px.cli.create._FLOWS_TEMPLATE_DIR", tmp_path / "empty"):
            print_templates()


# ---------------------------------------------------------------------------
# CLI integration — px create
# ---------------------------------------------------------------------------
...
class TestCreateCLI:
    ...
    def test_no_overwrite_exits_nonzero_on_existing(self, tmp_path):
        runner.invoke(app, ["create", "my-flow", "--output-dir", str(tmp_path)])
        result = runner.invoke(
            app, ["create", "my-flow", "--output-dir", str(tmp_path)]
        )
        assert result.exit_code != 0
...
class TestInitExampleSeeding:
    ...
    def test_seeded_flow_has_unique_uuid(self, tmp_path):
        ...
        p2 = tmp_path / "proj2"
        init_command(
            project_dir=p1, github_actions=False, overwrite=False, example=True
        )
        init_command(
            project_dir=p2, github_actions=False, overwrite=False, example=True
        )
    ...
    def test_graceful_fallback_when_template_fails(self, tmp_path):
        ...
        assert (tmp_path / "proj" / ".px" / "environments.yaml").exists()
