...
from px.base.data.storage_utils import read_file_text
from px.custom.custom_component.component import Component
from px.io import FileInput, MessageTextInput, MultilineInput, Output
...
class JSONToDataComponent(Component):
    ...
    def convert_json_to_data(self) -> Data | list[Data]:
        if (
            sum(
                bool(field)
                for field in [self.json_file, self.json_path, self.json_string]
            )
            != 1
        ):
            ...
            if self.json_file:
                ...
                if not file_path.lower().endswith(".json"):
                    ...
                    json_data = run_until_complete(
                        read_file_text(
                            file_path, encoding="utf-8", resolve_path=self.resolve_path
                        )
                    )
