...
when the same class is re-exported from different modules (e.g., px.Message vs
portals.schema.Message) but Python's isinstance() checks fail due to different
module paths.
"""
...
class CrossModuleMeta(type(BaseModel)):  # type: ignore[misc]
    ...
    def __instancecheck__(cls, instance: Any) -> bool:
        ...
        cls_fields = (
            set(cls.model_fields.keys()) if hasattr(cls, "model_fields") else set()
        )
