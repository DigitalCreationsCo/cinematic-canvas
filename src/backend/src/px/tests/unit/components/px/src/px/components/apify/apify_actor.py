...
from px.io import MultilineInput, Output, SecretStrInput, StrInput
...
class ApifyActorsComponent(Component):
    ...
    documentation: str = "https://docs.portals.org/bundles-apify"
    icon = "Apify"
    name = "ApifyActors"
    ...
    outputs = [
        Output(
            display_name="Output", name="output", type_=list[Data], method="run_model"
        ),
    ...
    def run_model(self) -> list[Data]:
        ...
        fields = (
            ApifyActorsComponent.parse_dataset_fields(self.dataset_fields)
    ...
    def create_tool_class(
        ...
        class ApifyActorRun(BaseTool):
            ...
            name: str = (
                f"apify_actor_{ApifyActorsComponent.actor_id_to_tool_name(actor_id)}"
            )
            description: str = (
            ...
            def _run(self, run_input: str | dict) -> str:
                """Use the Apify Actor."""
                input_dict = (
                    json.loads(run_input) if isinstance(run_input, str) else run_input
                )
                ...
                return "\n\n".join(
                    [ApifyActorsComponent.dict_to_json_str(item) for item in res]
                )
    ...
    def _get_apify_client(self) -> ApifyClient:
        ...
        if self._apify_client is None or self._apify_client.token != self.apify_token:
            ...
            if httpx_client := self._apify_client.http_client.httpx_client:
                httpx_client.headers["user-agent"] += "; Origin/portals"
    ...
    def run_actor(
        self, actor_id: str, run_input: dict, fields: list[str] | None = None
    ) -> list[dict]:
        ...
        client = self._get_apify_client()
        if (
            details := client.actor(actor_id=actor_id).call(
                run_input=run_input, wait_secs=1
            )
        ) is None:
        ...
        loader = ApifyDatasetLoader(
            ...
            dataset_mapping_function=lambda item: (
                ...
                if not fields
                    ...
                    k.replace(".", "_"): ApifyActorsComponent.get_nested_value(item, k)
                    for k in fields
                }
            ),
        )
    ...
    def parse_dataset_fields(dataset_fields: str) -> list[str]:
        ...
        dataset_fields = (
            dataset_fields.replace("'", "").replace('"', "").replace("`", "")
        )
