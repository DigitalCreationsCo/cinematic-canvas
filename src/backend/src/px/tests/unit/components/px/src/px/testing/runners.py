...
from px.testing.result import FlowResult, _build_result, _build_result_from_sdk_response
...
def _apply_tweaks(
    flow_dict: dict[str, Any], tweaks: dict[str, dict[str, Any]]
) -> dict[str, Any]:
    ...
    if tweaks and flow_path.suffix.lower() == ".json":
        try:
            ...
            logging.getLogger(__name__).debug(
                "Failed to apply tweaks to %s; using unmodified flow",
                flow_path,
                exc_info=True,
            )
...
async def _run_async(
    *,
    ...
    from px.run.base import RunError, run_flow
...
def _import_remote_run_request():
    ...
    try:
        from portals_sdk.models import RunRequest  # type: ignore[import-untyped]
    except ImportError as exc:
        msg = "portals-sdk is required for remote flow testing. Install: pip install portals-sdk"
...
class LocalFlowRunner(_BaseLocalFlowRunner):
    ...
    * A ``dict`` (already-parsed flow JSON).
    ...
    Relative paths are resolved against ``--px-flow-dir`` (default: ``cwd``).
    """
...
# ---------------------------------------------------------------------------
# Remote runners (requires portals-sdk)
# ---------------------------------------------------------------------------


class RemoteFlowRunner(_BaseRemoteFlowRunner):
    """Sync callable that runs flows against a live Portals instance.

    Returned by :func:`flow_runner` when ``--portals-env`` or
    ``--portals-url`` is passed to pytest.  Call it like a function::
...
class AsyncRemoteFlowRunner(_BaseRemoteFlowRunner):
    """Async callable that runs flows against a live Portals instance.

    Returned by :func:`async_flow_runner` when ``--portals-env`` or
    ``--portals-url`` is passed to pytest.  Use with ``await``::
