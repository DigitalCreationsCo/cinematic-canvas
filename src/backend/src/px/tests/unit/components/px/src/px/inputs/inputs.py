...
from px.inputs.validators import CoalesceBool
    ...
    def validate_value(cls, v: Any, _info):
        ...
        if not isinstance(v, list):
            msg = (
                ...
                "- A Data object (Portals's internal data structure)\n"
            )
        ...
        for i, item in enumerate(v):
            if not isinstance(item, dict | Data):
                msg = (
                    ...
                    "- A Data object (Portals's internal data structure for passing data between components)\n"
...
class JSONInput(HandleInput, InputTraceMixin, ListableInputMixin, ToolModeMixin):
    ...
    This is the new standard input for Portals data structures.
...
class ModelInput(BaseInputMixin, ModelInputMixin, ListableInputMixin, InputTraceMixin, ToolModeMixin):
    ...
    def normalize_value(cls, v):
        ...
        if isinstance(v, str) or (isinstance(v, list) and all(isinstance(item, str) for item in v)):
            ...
            try:
                from px.base.models.unified_models import normalize_model_names_to_dicts
...
class MessageInput(StrInput, InputTraceMixin):
    ...
    def _validate_value(v: Any, _info):
        ...
        if isinstance(v, Message):
            # If it's from a different module (e.g., portals.schema.Message),
...
class MessageTextInput(StrInput, MetadataTraceMixin, InputTraceMixin, ToolModeMixin):
    """Represents a text input component for the Portals system.

    This component is used to handle text inputs in the Portals system.
...
from px.template.field.base import Input  # noqa: E402
