...
from px.services.adapters.deployment.exceptions import DeploymentNotConfiguredError
...
# testable.  Concrete adapters (e.g. in Portals) subclass BaseDeploymentService
# and register under meaningful keys like "local" or "remote".
class DeploymentService(BaseDeploymentService):
    """Null deployment service for PX.

    All operations raise :class:`DeploymentNotConfiguredError` because PX does
    not ship a deployment adapter.  Concrete adapters (e.g. in Portals) should
