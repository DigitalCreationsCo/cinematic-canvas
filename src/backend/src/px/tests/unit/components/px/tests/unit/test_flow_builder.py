"""Tests for px.graph.flow_builder — pure flow-building utilities.

All tests use synthetic registries (plain dicts) — no px component loading,
no I/O, no network.
"""
...
from px.graph.flow_builder.connect import _custom_stringify, _scaped_json_stringify
...
class TestComponent:
    ...
    def test_add_component_with_explicit_id(self):
        ...
        result = add_component(
            flow, "ChatInput", REGISTRY, component_id="ChatInput-fixed"
        )
...
class TestConnect:
    ...
    def test_add_connection_dedupes_against_ui_saved_edges(self):
        ...
        edges from older Portals versions use the `xy-edge__` id prefix
    ...
    def test_remove_connection_no_match(self):
        ...
        removed = remove_connection(
            flow, r1["id"], r2["id"], source_output="nonexistent"
        )
        assert removed == 0
        assert len(flow["data"]["edges"]) == 1
...
class TestLayout:
    ...
    def test_layout_disconnected_distinct(self):
        ...
        positions = [
            (n["position"]["x"], n["position"]["y"]) for n in flow["data"]["nodes"]
        ]
        assert positions[0] != positions[1]

    def test_layout_uses_spacing_constants(self):
        from px.graph.flow_builder.layout import LAYER_SPACING_X
    ...
    def test_layout_y_spacing_multiple_at_same_layer(self):
        ...
        from px.graph.flow_builder.layout import NODE_SPACING_Y
    ...
    def test_layout_idempotent(self):
        ...
        layout_flow(flow)
        positions_1 = [
            (n["data"]["id"], n["position"].copy()) for n in flow["data"]["nodes"]
        ]
    ...
    def test_assign_layers_directly(self):
        ...
        from px.graph.flow_builder.layout import _assign_layers
...
class TestConnectErrorCases:
    def test_connect_nonexistent_source_raises(self):
        ...
        with pytest.raises(ValueError, match="Component not found in flow"):
            add_connection(
                flow, "NoSuch-00001", "message", "Target-00001", "input_value"
            )
    ...
    def test_connect_nonexistent_target_raises(self):
        ...
        with pytest.raises(ValueError, match="Component not found in flow"):
            add_connection(
                flow, "Source-00001", "message", "NoSuch-00001", "input_value"
            )
...
class TestConnectExtended:
    ...
    def test_remove_connection_by_target_input(self):
        ...
        add_connection(flow, r1["id"], "message", r2["id"], "input_value")
        removed = remove_connection(
            flow, r1["id"], r2["id"], target_input="input_value"
        )
        assert removed == 1
...
class TestCustomStringifyExtended:
    ...
    def test_scaped_round_trips_handle_format(self):
        """A source handle dict should produce a string that matches Portals's format."""
        handle = {
            "dataType": "ChatInput",
            "id": "ChatInput-abc12",
            "name": "message",
...
"""
        ...
        assert result["config"] == {
            "A": {"input_value": "Hello world", "sender_name": "Bot"}
        }
