...
2. Config file (px.toml / pyproject.toml)
...
from px.services.config_discovery import (
    get_nested_section,
...
from px.utils.concurrency import KeyedMemoryLockManager

if TYPE_CHECKING:
    from px.services.base import Service
    from px.services.factory import ServiceFactory
...
class ServiceManager:
    ...
    def __init__(self) -> None:
        ...
        ] = {}  # New: direct service class registry
        ...
        from px.services.settings.factory import SettingsServiceFactory
    ...
    def register_service_class(
        ...
        *,
        ...
        if service_type in self.service_classes and not override:
            logger.warning(
                f"Service {service_type.value} already registered. Use override=True to replace it."
            )
            return
        ...
        logger.debug(
            f"Registered service class: {service_type.value} -> {service_class.__name__}"
        )
    ...
    def get(
        self, service_name: ServiceType, default: ServiceFactory | None = None
    ) -> Service:
    ...
    def _create_service(
        ...
        if not self._plugins_discovered:
            ...
            if ServiceType.SETTINGS_SERVICE in self.services:
                ...
                if (
                    hasattr(settings_service, "settings")
                    and settings_service.settings.config_dir
                ):
    ...
    def _create_service_from_class(self, service_name: ServiceType) -> None:
        ...
        logger.debug(
            f"Creating service from class: {service_name.value} -> {service_class.__name__}"
        )
        ...
        for param_name, param in init_signature.parameters.items():
            ...
            if param.annotation != inspect.Parameter.empty:
                dependency_type = self._resolve_service_type_from_annotation(
                    param.annotation
                )
    ...
    async def teardown(self) -> None:
        ...
        try:
            from px.services.adapters.registry import teardown_all_adapter_registries

            await teardown_all_adapter_registries()
        except Exception as exc:  # noqa: BLE001
            logger.error(
                f"Error during adapter registry teardown: {exc}", exc_info=True
            )
        ...
        for name in service_names:
            try:
                ...
                for _, obj in inspect.getmembers(module, inspect.isclass):
                    if (
                        isinstance(obj, type)
                        and issubclass(obj, ServiceFactory)
                        and obj is not ServiceFactory
                    ):
    ...
    def discover_plugins(self, config_dir: Path | None = None) -> None:
        ...
        with self._lock:
            ...
            config_dir = resolve_config_dir(
                config_dir, settings_service=settings_service
            )
            ...
            logger.debug(
                f"Plugin discovery complete. Registered services: {list(self.service_classes.keys())}"
            )
    ...
    def _discover_from_entry_points(self) -> None:
        ...
        try:
            eps = entry_points(group="px.services")
        except Exception:  # noqa: BLE001
            logger.exception(
                "Failed to enumerate entry points for group='px.services'. "
    ...
    def _discover_from_config(self, config_dir: Path) -> None:
        """Discover services from config files (px.toml / pyproject.toml)."""
        source = get_preferred_config_source(
            config_dir,
            px_root_path=("services",),
            pyproject_root_path=("tool", "px", "services"),
        )
        if source is None:
            return
        self._load_services_from_config(*source)

    def _load_services_from_config(
        self, config_path: Path, root_path: tuple[str, ...]
    ) -> None:
        ...
        if config_path.name == "px.toml" or services:
    ...
    def _register_service_from_path(self, service_key: str, service_path: str) -> None:
        ...
        Args:
            ...
            service_path: Import path (e.g., "portals.services.database.service:DatabaseService")
        """
        try:
            ...
            logger.warning(
                f"Invalid service key '{service_key}' - must match ServiceType enum value"
            )
            return
