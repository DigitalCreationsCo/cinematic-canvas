"""Unit tests for ``px init`` — init_command and helpers.
...
class TestInitCommandDirectoryCreation:
    ...
    def test_creates_px_directory(self, tmp_path: Path) -> None:
        _run_init(tmp_path / "proj")
        assert (tmp_path / "proj" / ".px").is_dir()
...
class TestInitCommandRequiredFiles:
    ...
    def test_environments_yaml_has_expected_content(self, tmp_path: Path) -> None:
        _run_init(tmp_path)
        content = (tmp_path / ".px" / "environments.yaml").read_text(encoding="utf-8")
    ...
    def test_gitignore_has_portals_entry(self, tmp_path: Path) -> None:
        _run_init(tmp_path)
        content = (tmp_path / ".gitignore").read_text(encoding="utf-8")
        assert "portals-environments.toml" in content
...
class TestInitCommandGitHubActions:
    ...
    def test_warns_when_gha_templates_dir_missing(self, tmp_path: Path) -> None:
        ...
        with patch("px.cli.init._TEMPLATES_DIR", missing_templates):
            # Should not raise
            init_command(
                project_dir=tmp_path / "proj",
                github_actions=True,
...
class TestInitCLI:
    ...
    def test_overwrite_flag_reinitialises_non_empty_dir(self, tmp_path: Path) -> None:
        ...
        result = runner.invoke(
            app,
            ["init", str(proj), "--no-github-actions", "--no-example", "--overwrite"],
        )
        assert result.exit_code == 0, result.output
        assert (proj / ".px" / "environments.yaml").exists()
