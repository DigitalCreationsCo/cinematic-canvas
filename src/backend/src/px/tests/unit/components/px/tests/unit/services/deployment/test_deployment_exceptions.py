...
import pytest
from px.services.adapters.deployment import (
...
from px.services.interfaces import DeploymentServiceProtocol
...
def test_exception_error_codes_are_set() -> None:
    assert (
        AuthorizationError("forbidden", error_code="authorization_error").error_code
        == "authorization_error"
    )
    ...
    assert (
        InvalidDeploymentOperationError().error_code == "invalid_deployment_operation"
    )
...
def test_deployment_type_exceptions_have_distinct_default_messages() -> None:
    assert (
        str(DeploymentSupportError())
        == "Deployment type is unsupported by this adapter"
    )
    assert (
        str(InvalidDeploymentTypeError()) == "Deployment type is malformed or unknown"
    )
...
async def test_deployment_service_stub_methods_raise(
    method_name: str, kwargs: dict
) -> None:
    svc = DeploymentService()
    with pytest.raises(
        DeploymentNotConfiguredError, match="requires a concrete deployment adapter"
    ):
...
def test_raise_as_deployment_error_maps_known_http_statuses() -> None:
    with pytest.raises(AuthenticationError):
        raise_as_deployment_error(
            status_code=401, detail="unauthorized", message_prefix="x"
        )
    with pytest.raises(AuthorizationError):
        raise_as_deployment_error(
            status_code=403, detail="forbidden", message_prefix="x"
        )
    with pytest.raises(ResourceNotFoundError):
        raise_as_deployment_error(status_code=404, detail="missing", message_prefix="x")
    with pytest.raises(ResourceConflictError):
        raise_as_deployment_error(
            status_code=409, detail="conflict", message_prefix="x"
        )
    with pytest.raises(InvalidContentError):
        raise_as_deployment_error(
            status_code=422, detail="unprocessable", message_prefix="x"
        )
    with pytest.raises(InvalidDeploymentOperationError):
        raise_as_deployment_error(
            status_code=400, detail="bad request", message_prefix="x"
        )
    with pytest.raises(InvalidDeploymentOperationError):
        raise_as_deployment_error(
            status_code=405, detail="method not allowed", message_prefix="x"
        )
    with pytest.raises(InvalidContentError):
        raise_as_deployment_error(
            status_code=413, detail="payload too large", message_prefix="x"
        )
    with pytest.raises(InvalidContentError):
        raise_as_deployment_error(
            status_code=415, detail="unsupported media type", message_prefix="x"
        )
    with pytest.raises(ResourceNotFoundError):
        raise_as_deployment_error(status_code=410, detail="gone", message_prefix="x")
    with pytest.raises(RateLimitError):
        raise_as_deployment_error(
            status_code=429, detail="too many requests", message_prefix="x"
        )
    with pytest.raises(DeploymentTimeoutError):
        raise_as_deployment_error(status_code=408, detail="timeout", message_prefix="x")
    with pytest.raises(DeploymentTimeoutError):
        raise_as_deployment_error(
            status_code=504, detail="gateway timeout", message_prefix="x"
        )
    with pytest.raises(ServiceUnavailableError):
        raise_as_deployment_error(
            status_code=502, detail="bad gateway", message_prefix="x"
        )
    with pytest.raises(ServiceUnavailableError):
        raise_as_deployment_error(
            status_code=503, detail="service unavailable", message_prefix="x"
        )
