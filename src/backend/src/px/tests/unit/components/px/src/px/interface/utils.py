...
from px.services.chat.config import ChatConfig
...
def set_langchain_cache(settings) -> None:
    ...
    from portals.interface.importing.utils import import_class

    if cache_type := os.getenv("PORTALS_LANGCHAIN_CACHE"):
        try:
