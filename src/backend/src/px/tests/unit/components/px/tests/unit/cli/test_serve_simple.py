"""Simple tests for PX serve command focusing on CLI functionality."""
...
# Tests that invoke `serve` with valid-looking input may hang while portals
...
def test_cli_imports():
    ...
    from px.__main__ import app, main
...
def test_serve_command_invalid_json():
    ...
    with patch.dict(
        ...
        result = runner.invoke(
            app, ["serve", "--flow-json", invalid_json], catch_exceptions=False
        )
...
def test_serve_command_nonexistent_file():
    ...
    with patch.dict(
        ...
        runner = CliRunner()
        result = runner.invoke(
            app, ["serve", "/path/to/nonexistent/file.json"], catch_exceptions=False
        )
...
def test_serve_command_missing_api_key():
    """Serve command must exit 1 and mention PORTALS_API_KEY when no key is set."""
    from px.__main__ import app
    ...
    try:
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert result.exit_code == 1
            assert "PORTALS_API_KEY" in str(result.output or result.exception or "")
    finally:
...
def test_serve_command_with_flow_json():
    ...
    env = {"PORTALS_API_KEY": "test-key"}  # pragma: allowlist secret
