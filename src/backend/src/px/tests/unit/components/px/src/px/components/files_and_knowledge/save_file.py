...
from px.io import BoolInput, DropdownInput, HandleInput, SecretStrInput, StrInput
from px.schema import Data, DataFrame, Message
from px.services.deps import get_settings_service, get_storage_service, session_scope
...
class SaveToFileComponent(Component):
    ...
    documentation: str = "https://docs.portals.org/write-file"
    icon = "file-text"
    name = "SaveToFile"
    ...
    inputs = [
        ...
        DropdownInput(
            ...
            options=list(
                dict.fromkeys(LOCAL_DATA_FORMAT_CHOICES + LOCAL_MESSAGE_FORMAT_CHOICES)
            ),
    ...
    async def _upload_file(self, file_path: Path) -> None:
        ...
        from portals.api.v2.files import upload_user_file
        ...
        with file_path.open("rb") as f:
            async with session_scope() as db:
                ...
                await upload_user_file(
                    file=UploadFile(
                        filename=file_path.name, file=f, size=file_path.stat().st_size
                    ),
                    session=db,
    ...
    def _save_dataframe(self, dataframe: DataFrame, path: Path, fmt: str) -> str:
        ...
        should_append = (
            append_mode and path.exists() and self._is_plain_text_format(fmt)
        )
    ...
    def _save_data(self, data: Data, path: Path, fmt: str) -> str:
        ...
        if fmt == "csv":
            ...
            if should_append:
                ...
                content = orjson.dumps(new_data, option=orjson.OPT_INDENT_2).decode(
                    "utf-8"
                )
                path.write_text(content, encoding="utf-8")
        elif fmt == "markdown":
    ...
    async def _save_to_aws(self) -> Message:
        ...
        if not bucket_name:
            msg = (
                ...
                "or set PORTALS_OBJECT_STORAGE_BUCKET_NAME environment variable."
            )
            raise ValueError(msg)
    ...
    async def _save_to_google_drive(self) -> Message:
        ...
        if file_format in ["slides", "docs"]:
            return await self._save_to_google_apps(
                drive_service, credentials, content, file_format
            )
        ...
        try:
            ...
            return Message(
                text=f"File successfully uploaded to Google Drive: {file_url}"
            )
        finally:
            # Clean up temp file
            if Path(temp_file_path).exists():
    ...
    async def _save_to_google_apps(
        self, drive_service, credentials, content: str, app_type: str
    ) -> Message:
        ...
        return Message(
            text=f"File successfully created in Google {app_type.title()}: {file_url}"
        )
