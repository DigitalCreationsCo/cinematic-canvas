...
def _sort_single_layer_by_dependency(
    ...
    # Track vertices currently being processed to detect intra-layer cycles.
    # This can occur when cycle vertices (identified by find_cycle_vertices earlier)
    # end up in the same layer after layered_topological_sort. This is expected
    # behavior for cyclic graphs, not a failure of earlier cycle detection.
    ...
    def max_dependency_index(vertex: str) -> int:
        ...
        # Intra-layer cycle detection: if we're already processing this vertex,
        # we have a cycle within this layer (e.g., A -> B -> A where both are in the same layer).
        # This happens when cycle vertices from the graph end up grouped in the same layer.
        # We break the cycle by returning the vertex's current index.
        if vertex in processing:
            logger.debug(f"Intra-layer cycle detected at vertex '{vertex}' during layer sorting, breaking cycle")
