...
from px.cli.serve_app import FlowMeta, StreamRequest, create_multi_serve_app
...
def mock_graphs():
    ...
    return {
        ...
        "flow2": MockGraph(
            ...
            edges=[
                MockEdge("text_input", "processor"),
                MockEdge("processor", "text_output"),
            ],
        ),
    }
...
def mock_metas():
    ...
    return {
        "flow1": FlowMeta(
            id="flow1",
            relative_path="flow1.json",
            title="Test Flow 1",
            description="A simple test flow for chat",
        ),
        "flow2": FlowMeta(
            id="flow2",
            relative_path="flow2.json",
            title="Test Flow 2",
            description="A test flow with text processing",
        ),
    }
...
def multi_serve_app(mock_graphs, mock_metas, monkeypatch):
    ...
    monkeypatch.setenv("PORTALS_API_KEY", "test-api-key")
    monkeypatch.setattr(
        get_settings_service().settings, "allow_custom_components", True
    )

    with patch("px.cli.serve_app.execute_graph_with_capture") as mock_execute:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def mock_verify_api_key(
                query_param: str | None = None, header_param: str | None = None
            ) -> str:  # noqa: ARG001
                return "test-api-key"

            # Import the original dependency
            from px.cli.serve_app import verify_api_key
...
def mock_api_key(monkeypatch):
    ...
    with patch("px.cli.serve_app.verify_api_key") as mock_verify:
...
class TestMultiServeStreaming:
    ...
    async def test_stream_endpoint_exists(self, multi_serve_app, mock_api_key):
        ...
        async with (
            LifespanManager(
                multi_serve_app, startup_timeout=None, shutdown_timeout=None
            ) as manager,
            AsyncClient(
    ...
    async def test_stream_basic_functionality(self, multi_serve_app, mock_api_key):
        ...
        with patch("px.cli.serve_app.run_flow_generator_for_serve") as mock_generator:
            ...
            async def mock_stream_generator(*args, **kwargs):  # noqa: ARG001
                ...
                if event_manager:
                    event_manager.on_end(
                        data={
                            "result": {"result": "Streamed response", "success": True}
                        }
                    )
    ...
    async def test_stream_authentication_required(self, multi_serve_app):
        ...
        try:
            async with (
                ...
                response = await client.post(
                    "/flows/flow1/stream", json={"input_value": "test"}
                )
    ...
    async def test_stream_error_handling(self, multi_serve_app, mock_api_key):
        ...
        with patch("px.cli.serve_app.run_flow_generator_for_serve") as mock_generator:
            ...
            async def mock_error_generator(
                graph, input_request, flow_id, event_manager, client_consumed_queue
            ):  # noqa: ARG001
                try:
    ...
    async def test_stream_multiple_flows(self, multi_serve_app, mock_api_key):
        ...
        async with (
            ...
            response2 = await client.post(
                "/flows/flow2/stream",
                json={"input_value": "test flow 2"},
                headers={"x-api-key": mock_api_key},
            )
            assert response2.status_code == 200

    @pytest.mark.asyncio
    async def test_regular_run_endpoint_still_works(
        self, multi_serve_app, mock_api_key
    ):
        """Test that regular run endpoints still work alongside streaming."""
        with patch("px.cli.serve_app.extract_result_data") as mock_extract:
            ...
            async with (
                ...
                response = await client.post(
                    "/flows/flow1/run",
                    json={"input_value": "test regular run"},
                    headers={"x-api-key": mock_api_key},
                )
    ...
    async def test_stream_blocks_custom_components_when_disabled(
        ...
        async with (
            ...
            with (
                ...
                patch(
                    "px.cli.serve_app.run_flow_generator_for_serve",
                    new=AsyncMock(),
                ) as mock_run_flow,
            ):
                response = await client.post(
