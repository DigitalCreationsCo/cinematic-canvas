...
class TestMetadataInTemplateBuilders:
    ...
    def test_build_from_inputs_adds_metadata_with_module(self, mock_frontend_class):
        ...
        from px.custom.utils import build_custom_component_template_from_inputs
        ...
        with patch("px.custom.utils.get_component_instance") as mock_get_instance:
            mock_instance = Mock()
            ...
            with (
                patch("px.custom.utils.add_code_field", return_value=mock_frontend),
    ...
    @patch("px.custom.utils.CustomComponentFrontendNode")
    def test_build_template_adds_metadata_with_module(self, mock_frontend_class):
        ...
        with patch("px.custom.utils.run_build_config") as mock_run_build:
            mock_instance = Mock()
            mock_instance._get_field_order = Mock(return_value=[])
            mock_run_build.return_value = ({}, mock_instance)

            with (
                patch("px.custom.utils.add_extra_fields"),
                patch("px.custom.utils.add_code_field", return_value=mock_frontend),
                patch("px.custom.utils.add_base_classes"),
                patch("px.custom.utils.add_output_types"),
                patch("px.custom.utils.reorder_fields"),
            ):
                # Call the function
...
class TestDependencyAnalyzer:
    ...
    def test_top_level_basic(self):
        ...
        from px.custom.dependency_analyzer import _top_level
    ...
    def test_is_relative(self):
        ...
        from px.custom.dependency_analyzer import _is_relative
    ...
    def test_analyze_dependencies_basic(self):
        ...
        from px.custom.dependency_analyzer import analyze_dependencies
...
"""
    ...
    def test_dependency_info_dataclass(self):
        ...
        from px.custom.dependency_analyzer import DependencyInfo
    ...
    def test_get_distribution_version_deterministic(self):
        ...
        """
        from px.custom.dependency_analyzer import _get_distribution_version
        ...
        for fake_map in (fake_map_asc, fake_map_desc):
            ...
            with (
                patch("px.custom.dependency_analyzer._get_packages_distributions", return_value=fake_map),
                patch("px.custom.dependency_analyzer.md.distribution", return_value=mock_dist) as mock_md,
            ):
...
"""
        ...
        assert "portals" in package_names  # portals should be detected as external
...
"""
    ...
    def test_build_component_metadata_with_real_component(self):
        ...
        from px.custom.custom_component.component import Component
        from px.custom.utils import build_component_metadata
...
from portals.base.models.model import LCModelComponent
from portals.field_typing import LanguageModel
...
"""
        ...
        # Call the function
        build_component_metadata(
            mock_frontend, test_component, "portals.components.lmstudio", "LMStudioModelComponent"
        )
        ...
        assert "portals" in package_names  # project dependency
