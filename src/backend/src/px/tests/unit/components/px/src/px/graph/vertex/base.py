...
from px.graph.schema import INPUT_COMPONENTS, OUTPUT_COMPONENTS, InterfaceComponentTypes, ResultData
from px.graph.utils import UnbuiltObject, UnbuiltResult, emit_build_start_event, log_transaction
from px.graph.vertex.param_handler import ParameterHandler
from px.interface import initialize
from px.interface.listing import lazy_load_dict
from px.log.logger import logger
from px.schema.artifact import ArtifactType
from px.schema.data import Data
from px.schema.message import Message
from px.schema.properties import Usage
from px.schema.schema import INPUT_FIELD_NAME, OutputValue, build_output_logs
from px.utils.schemas import ChatOutputResponse
from px.utils.util import sync_to_async
...
if TYPE_CHECKING:
    ...
    from px.graph.vertex.schema import NodeData
...
class Vertex:
    ...
    async def build(
        ...
        **kwargs,
        ...
        from px.interface.components import ensure_component_loaded
    ...
    def before_callback_event(self, *args, **kwargs) -> StepStartedEvent:  # noqa: ARG002
        ...
        return StepStartedEvent(step_name=self.display_name, raw_event={"portals": metrics})
    ...
    def after_callback_event(self, result, *args, **kwargs) -> StepFinishedEvent:  # noqa: ARG002
        ...
        if hasattr(self, "raw_event_metrics"):
            metrics = self.raw_event_metrics({"component_id": self.id})
        return StepFinishedEvent(step_name=self.display_name, raw_event={"portals": metrics})
