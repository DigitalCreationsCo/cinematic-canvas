"""Unit tests for px status -- status_command and helpers.
...
class _FakePortalsNotFoundError(Exception):
    """Stand-in for portals_sdk.exceptions.PortalsNotFoundError in unit tests."""
...
def _make_sdk_triple(
    ...
    return _identity_normalize, _json_flow_to_json, client_cls, _FakePortalsNotFoundError
...
class TestFlowHash:
    def test_returns_12_char_string(self):
        from px.cli.status import _flow_hash
...
class TestCollectFiles:
    def test_dir_path_returns_json_files(self, tmp_path):
        from px.cli.status import _collect_files
...
class TestRenderTable:
    ...
    def _capture_render(statuses, env_label):
        ...
        with patch("px.cli.status.console", fake_console):
    ...
    def test_no_id_status_contains_detail(self, tmp_path):
        ...
        statuses = [
            FlowStatus(
                ...
                detail="run px export first",
            )
        ]
        output = self._capture_render(statuses, "test-env")
        assert "OrphanFlow" in output
        assert "no id" in output.lower()
        assert "run px export first" in output
...
class TestStatusCommandSynced:
    ...
    def test_client_constructed_with_env_url_and_key(self, tmp_path):
        ...
        with (
            patch("px.cli.status._load_sdk", return_value=(normalize_fn, to_json_fn, client_cls, not_found_error)),
            patch("px.config.resolve_environment", return_value=cfg),
        ):
            from px.cli.status import status_command
    ...
    def test_client_is_closed_after_requests_finish(self, tmp_path):
        ...
        triple = _identity_normalize, _json_flow_to_json, MagicMock(return_value=client), _FakePortalsNotFoundError
