...
from px.base.data.utils import read_text_file
...
class TestReadTextFile:
    ...
    def test_chardet_returns_none(self, tmp_path):
        ...
        with patch("px.base.data.utils.chardet.detect", return_value={"encoding": None, "confidence": 0.0}):
            result = read_text_file(str(f))
        assert result == "hello"
    ...
    def test_chardet_returns_invalid_encoding(self, tmp_path):
        ...
        f = tmp_path / "test.txt"
        f.write_text("hello", encoding="utf-8")
        with patch("px.base.data.utils.chardet.detect", return_value={"encoding": "FAKE-ENCODING", "confidence": 1.0}):
            result = read_text_file(str(f))
        assert result == "hello"
