...
class TestExportCommandRemoteFlowId:
    def test_missing_env_and_target_raises_exit_1(self):
        ...
        with patch("px.cli.export.load_sdk", return_value=sdk), pytest.raises(typer.Exit) as exc_info:
            export_command(
                flow_paths=[],
                output=None,
                output_dir=None,
...
class TestExportCommandRemoteProject:
    ...
    def test_project_config_error_raises_exit_1(self, tmp_path):
        ...
        with (
            patch("px.cli.export.load_sdk", return_value=sdk),
            patch("px.config.resolve_environment", side_effect=ConfigError("cfg error")),
            pytest.raises(typer.Exit) as exc_info,
        ):
            export_command(
                flow_paths=[],
