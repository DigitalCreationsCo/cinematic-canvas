...
def _validate_graph_instance(graph_obj: Any) -> "Graph":
    ...
    if not isinstance(graph_obj, Graph):
        msg = f"Graph object is not a PX Graph instance: {type(graph_obj)}"
        raise TypeError(msg)
    ...
    if "Chat Input" not in display_names:
        msg = (
            f"Graph does not contain any ChatInput component. Vertices: {display_names}"
        )
    ...
    for result in results:
        if (
            ...
            try:
                ...
                return json.dumps(
                    json.loads(message.model_dump_json()), ensure_ascii=False
                )
        ...
        if (
            ...
            try:
                ...
                if isinstance(message, dict):
                    text_content = (
                        message.get("text") if message.get("text") else str(message)
                    )
                else:
    ...
    for result in results:
        if (
            ...
            try:
                result_message = (
                    message.text
                    if extract_text and hasattr(message, "text")
                    else message
                )
