...
from px.io import HandleInput, IntInput, SecretStrInput, StrInput
...
class CouchbaseVectorStoreComponent(LCVectorStoreComponent):
    ...
    inputs = [
        ...
        StrInput(
            name="couchbase_username", display_name="Couchbase username", required=True
        ),
        SecretStrInput(
            name="couchbase_password", display_name="Couchbase password", required=True
        ),
    ...
    def build_vector_store(self) -> CouchbaseVectorStore:
        try:
            ...
            msg = "Failed to import Couchbase dependencies. Install it using `uv pip install portals[couchbase] --pre`"
