"""Unit tests for px.testing — the flow_runner pytest plugin.

All tests mock ``_run_sync`` / ``_run_async`` so no real Portals instance is needed.
"""
...
class TestApplyTweaks:
    ...
    def test_no_tweaks_returns_identical_structure(self):
        ...
        assert (
            patched["data"]["nodes"][0]["data"]["node"]["template"]["k"]["value"] == "v"
        )
...
class TestResolveFlowArgs:
    ...
    def test_dict_flow_with_tweaks(self):
        flow = _make_flow_dict(node_type="OpenAI", fields={"model_name": "gpt-4"})
        _, flow_json = _resolve_flow_args(
            flow, {"OpenAI": {"model_name": "gpt-4o-mini"}}, Path("/base")
        )
    ...
    def test_json_file_with_tweaks_inlines(self, tmp_path):
        ...
        p = tmp_path / "flow.json"
        p.write_text(json.dumps(flow))
        script_path, flow_json = _resolve_flow_args(
            p, {"OpenAI": {"model_name": "gpt-4o-mini"}}, tmp_path
        )
        assert script_path is None
        parsed = json.loads(flow_json)  # type: ignore[arg-type]
        template = parsed["data"]["nodes"][0]["data"]["node"]["template"]
    ...
    def test_py_file_with_tweaks_uses_file_path(self, tmp_path):
        ...
        script_path, flow_json = _resolve_flow_args(
            p, {"SomeNode": {"field": "value"}}, tmp_path
        )
...
class TestLocalFlowRunner:
    def test_success_result(self):
        with patch(
            "px.testing.runners._run_sync", return_value=_SUCCESS_RAW
        ) as mock_run:
    ...
    def test_error_result(self):
        with patch("px.testing.runners._run_sync", return_value=_ERROR_RAW):
            runner = LocalFlowRunner()
    ...
    def test_env_file_loaded(self, tmp_path):
        ...
        with (
            patch("px.testing.runners._run_sync", return_value=_SUCCESS_RAW),
            patch("px.testing.runners._load_dotenv") as mock_load,
        ):
...
class TestAsyncLocalFlowRunner:
    async def test_success_result(self):
        with patch(
            "px.testing.runners._run_async",
            new_callable=AsyncMock,
            return_value=_SUCCESS_RAW,
        ):
            runner = AsyncLocalFlowRunner()
            result = await runner(
                {"data": {"nodes": [], "edges": []}}, input_value="hi"
            )
        assert result.ok
        assert result.text == "Hello, world!"
...
class TestGetMarkerArg:
    def test_returns_arg_when_marker_present(self):
        ...
        result = _get_marker_arg(request, "px_env_file")
...
class TestResolveRunnerConfig:
    def _make_request(
        ...
        *,
        ...
        def get_closest_marker(name: str):
            if name == "px_env_file" and env_file_marker is not None:
                m = MagicMock()
                m.args = [env_file_marker]
                return m
            if name == "px_timeout" and timeout_marker is not None:
                m = MagicMock()
        ...
        def getoption(name, default=None):
            mapping = {
                "px_env_file": cli_env_file,
                "px_timeout": cli_timeout,
                "px_flow_dir": cli_flow_dir,
            }
    ...
    def test_marker_takes_precedence_over_cli(self, monkeypatch):
        monkeypatch.delenv("PX_ENV_FILE", raising=False)
        monkeypatch.delenv("PX_TIMEOUT", raising=False)
        monkeypatch.delenv("PX_FLOW_DIR", raising=False)
    ...
    def test_env_var_used_as_fallback(self, monkeypatch):
        monkeypatch.setenv("PX_ENV_FILE", ".env.envvar")
        monkeypatch.setenv("PX_TIMEOUT", "42")
        monkeypatch.delenv("PX_FLOW_DIR", raising=False)
        req = self._make_request()
        env_file, timeout, _ = _resolve_runner_config(req)
    ...
    def test_flow_dir_env_var_fallback(self, monkeypatch, tmp_path):
        monkeypatch.delenv("PX_ENV_FILE", raising=False)
        monkeypatch.delenv("PX_TIMEOUT", raising=False)
        monkeypatch.setenv("PX_FLOW_DIR", str(tmp_path))
        req = self._make_request()
        _, _, base_dir = _resolve_runner_config(req)
        assert base_dir == tmp_path
    ...
    def test_invalid_timeout_env_var_ignored(self, monkeypatch):
        monkeypatch.delenv("PX_ENV_FILE", raising=False)
        monkeypatch.setenv("PX_TIMEOUT", "not-a-number")
        monkeypatch.delenv("PX_FLOW_DIR", raising=False)
        req = self._make_request()
...
class TestFixtures:
    ...
    def test_flow_runner_fixture_runs_flow(self, flow_runner):
        with patch("px.testing.runners._run_sync", return_value=_SUCCESS_RAW):
