...
from px.graph.edge.base import types_compatible

# Portals uses oe (U+0153) as a quote replacement in ReactFlow handle strings
_Q = "\u0153"


def _scaped_json_stringify(obj: Any) -> str:
    """Replicate Portals frontend's scapedJSONStringfy: sorted keys, compact, oe for quotes."""
    return _custom_stringify(obj).replace('"', _Q)


def _custom_stringify(obj: Any) -> str:
    """Replicate Portals frontend's customStringify for edge handle data.
...
def _resolve_input_types(
    flow: dict, component_id: str, input_name: str
) -> tuple[list[str], str]:
    ...
    for node in flow.get("data", {}).get("nodes", []):
        ...
        available = [
            ...
            if isinstance(v, dict) and v.get("input_types")
        ]
...
def add_connection(
    ...
    if target_types is None:
        target_types, target_field_type = _resolve_input_types(
            flow, target_id, target_input
        )
    else:
    ...
    edge_id = (
        f"reactflow__edge-{source_id}{source_handle_s}-{target_id}{target_handle_s}"
    )
    ...
    # of by edge id, since UI-saved edges from older Portals versions use a
    ...
    for existing in flow["data"]["edges"]:
        if (
            ...
            and (existing.get("data") or {}).get("sourceHandle", {}).get("name")
            == source_output
            and (existing.get("data") or {}).get("targetHandle", {}).get("fieldName")
            == target_input
        ):
            return existing
...
def remove_connection(
    ...
    def keep(e: dict) -> bool:
        ...
        if (
            source_output
            and e.get("data", {}).get("sourceHandle", {}).get("name") != source_output
        ):
            return True
        return bool(
            target_input
            and e.get("data", {}).get("targetHandle", {}).get("fieldName")
            != target_input
        )
