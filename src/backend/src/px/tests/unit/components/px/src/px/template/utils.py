...
def get_file_path_value(file_path):
    ...
    if not str(path).startswith(user_cache_dir("portals", "portals")):
        return ""
...
def update_template_field(new_template, key, previous_value_dict) -> None:
    ...
    if not template_field or template_field.get("type") != previous_value_dict.get(
        "type"
    ):
        return
    ...
    if "value" in previous_value_dict and previous_value_dict["value"] is not None:
        ...
        if template_field.get("value") != previous_value_dict["value"]:
            template_field["load_from_db"] = previous_value_dict.get(
                "load_from_db", False
            )
...
def update_frontend_node_with_template_values(frontend_node, raw_frontend_node):
    ...
    frontend_node["edited"] = raw_frontend_node.get("edited", False) or (
        old_code != new_code
    )
    ...
    if not frontend_node.get("edited", False):
        frontend_node["display_name"] = raw_frontend_node.get(
            "display_name", frontend_node.get("display_name", "")
        )
        frontend_node["description"] = raw_frontend_node.get(
            "description", frontend_node.get("description", "")
        )
...
def apply_json_filter(result, filter_) -> Data:  # type: ignore[return-value]
    ...
    if (
        isinstance(filter_, str)
        and filter_.strip().startswith("[")
        and filter_.strip().endswith("]")
    ):
        try:
    ...
    if isinstance(original_data, list) and all(
        isinstance(item, dict) for item in original_data
    ):
        if filter_ == "":
            return []
    ...
    try:
        ...
        if (
            filter_.strip()
            and not filter_.strip().startswith("[")
            and ".[" not in filter_
        ):
    ...
    normalized_query = (
        "." + filter_str if not filter_str.startswith(".") else filter_str
    )
