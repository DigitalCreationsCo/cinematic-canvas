"""px environment configuration — resolve Portals instance URL and API key.

Config file lookup order
------------------------
1.  Explicit path given via ``--environments-file`` / ``environments_file`` parameter.
2.  ``.px/environments.yaml`` in the current working directory, then each
    parent directory up to the first ``.git`` boundary (project root discovery).
3.  ``~/.px/environments.yaml`` (user-level config).
4.  ``portals-environments.toml`` in the current working directory
    (backward-compatible with the portals-sdk TOML format).
...
.. code-block:: yaml
    ...
    environments:
      local:
        ...
        api_key_env: PORTALS_LOCAL_API_KEY
...
TOML format is also accepted (``portals-environments.toml`` or any ``.toml``
...
class ConfigError(Exception):
    ...
    will simply be ``None`` on the returned :class:`PortalsEnvironment`.
...
@dataclass
class PortalsEnvironment:
    """A fully-resolved Portals target instance.

    Attributes:
        name:    Human-readable label (environment name or ``"__inline__"``).
        url:     Base URL of the Portals instance.
...
_TOML_FALLBACK = "portals-environments.toml"
_PX_DIR = ".px"
...
def _find_config_file(override: Path | None) -> Path | None:
    ...
    # Walk up from cwd looking for .px/environments.yaml
    cwd = Path.cwd()
    for directory in (cwd, *cwd.parents):
        for name in _YAML_NAMES:
            candidate = directory / _PX_DIR / name
    ...
    # User-level YAML
    for name in _YAML_NAMES:
        user_yaml = Path.home() / _PX_DIR / name
        if user_yaml.is_file():
            return user_yaml

    # Backward-compat: portals-environments.toml in cwd
...
def _parse_env_block(name: str, block: Any, config_path: Path) -> PortalsEnvironment:
    ...
    return PortalsEnvironment(name=name, url=url, api_key=api_key)


def _load_config(path: Path) -> tuple[dict[str, PortalsEnvironment], str | None]:
    ...
    if not isinstance(raw_envs, dict):
        msg = (
            f"'environments' in {path} must be a mapping, got {type(raw_envs).__name__}"
        )
        raise ConfigError(msg)

    envs: dict[str, PortalsEnvironment] = {}
    ...
    default_name: str | None = (
        defaults.get("environment") if isinstance(defaults, dict) else None
    )
...
def resolve_environment(
    ...
    *,
    ...
    """Resolve an environment name (or inline flags) to a :class:`PortalsEnvironment`.
    ...
    3. **Env-var fallback** — if no config file exists and no *env* was
       requested, fall back to ``PORTALS_URL`` / ``PORTALS_API_KEY`` (or
       ``PX_URL`` / ``PX_API_KEY``) env vars before raising.
    ...
    if target is not None:
        ...
        return PortalsEnvironment(name=name, url=target, api_key=api_key)
    ...
    if config_path is None:
        ...
        lf_url = os.environ.get("PORTALS_URL") or os.environ.get("PX_URL")
        if lf_url and env is None:
            lf_key = (
                api_key
                or os.environ.get("PORTALS_API_KEY")
                or os.environ.get("PX_API_KEY")
            )
            return PortalsEnvironment(name="__env__", url=lf_url, api_key=lf_key)

        if env is not None:
            msg = (
                f"Environment {env!r} requested but no config file was found.\n"
                f"  • Create .px/environments.yaml in your project root, or\n"
                f"  • Pass --target <url> [--api-key <key>] for inline configuration.\n"
                f"  • Run 'px init' to scaffold a project with a config template."
            )
            raise ConfigError(msg)
        ...
        msg = (
            ...
            "Options:\n"
            "  • px <cmd> --env <name>              (requires .px/environments.yaml)\n"
            "  • px <cmd> --target <url>             (inline, no config file needed)\n"
            "  • export PORTALS_URL=<url>            (env-var fallback)\n"
            "  • px init                             (scaffold a project with a template)"
        )
        raise ConfigError(msg)
    ...
    if api_key is not None:
        resolved = PortalsEnvironment(
            name=resolved.name, url=resolved.url, api_key=api_key
        )
