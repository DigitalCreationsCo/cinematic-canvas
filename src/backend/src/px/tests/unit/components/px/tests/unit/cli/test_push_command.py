"""Unit tests for px push -- push_command and helpers.
...
# ---------------------------------------------------------------------------
# Fake exception class — avoids importing portals_sdk in isolation mode
# ---------------------------------------------------------------------------
...
def _fake_project(
    name: str = "My Project", project_id: UUID = _PROJECT_ID
) -> MagicMock:
    """Return a MagicMock that looks like a portals_sdk.models.Project."""
    proj = MagicMock()
    proj.name = name
    proj.id = project_id
    return proj
...
def _fake_flow(flow_id: UUID = _FLOW_ID, name: str = "My Test Flow") -> MagicMock:
    """Return a MagicMock that looks like a portals_sdk.models.Flow."""
    flow = MagicMock()
    flow.id = flow_id
    flow.name = name
    return flow
...
def _make_sdk_mock(client_mock: MagicMock | None = None) -> MagicMock:
    ...
    the test file has zero imports from portals_sdk.
    """
...
def _run_push(
    ...
    *,
    ...
    with patch("px.cli.push.load_sdk", return_value=mock):
        push_command(
...
class TestPushResult:
    def test_created_is_ok(self, tmp_path):
        from px.cli.push import PushResult

        r = PushResult(
            path=tmp_path / "f.json", flow_id=_FLOW_ID, flow_name="F", status="created"
        )
        assert r.ok is True
    ...
    def test_updated_is_ok(self, tmp_path):
        ...
        r = PushResult(
            path=tmp_path / "f.json", flow_id=_FLOW_ID, flow_name="F", status="updated"
        )
        assert r.ok is True
    ...
    def test_dry_run_is_ok(self, tmp_path):
        ...
        r = PushResult(
            path=tmp_path / "f.json", flow_id=_FLOW_ID, flow_name="F", status="dry-run"
        )
        assert r.ok is True
...
class TestLoadFlowFile:
    def test_valid_json_returns_dict(self, tmp_path):
        from px.cli.push import _load_flow_file
...
class TestExtractFlowId:
    def test_valid_uuid_returned(self, tmp_path):
        from px.cli.push import _extract_flow_id
...
class TestCollectFlowFiles:
    def test_single_path_returned(self, tmp_path):
        from px.cli.push import _collect_flow_files
...
class TestUpsertSingle:
    ...
    def test_dry_run_returns_dry_run_status(self, tmp_path):
        from px.cli.push import _upsert_single
    ...
    def test_http_error_returns_error_status(self, tmp_path):
        ...
        client.upsert_flow.side_effect = _FakePortalsHTTPError(500, "server error")
        sdk = _make_sdk_mock(client)
        result = _upsert_single(
            client,
            sdk,
...
class TestFindOrCreateProject:
    def test_found_project_returns_id(self):
        from px.cli.push import _find_or_create_project
...
class TestPushCommand:
    ...
    def test_http_error_exits_with_code_1(self, tmp_path):
        ...
        p = _write_flow(tmp_path, "flow.json")
        client = _make_client_mock()
        client.upsert_flow.side_effect = _FakePortalsHTTPError(500, "oops")
        sdk = _make_sdk_mock(client)
    ...
    def test_partial_error_still_attempts_all_files(self, tmp_path):
        ...
        client.upsert_flow.side_effect = [
            (_fake_flow(), True),
            _FakePortalsHTTPError(500, "server down"),
        ]
        sdk = _make_sdk_mock(client)
        with pytest.raises(typer.Exit) as exc_info:
    ...
    def test_environments_file_resolves_url(self, tmp_path):
        ...
        p = _write_flow(tmp_path, "flow.json")
        env_file = tmp_path / "portals-environments.toml"
        env_file.write_text(
            f'[environments.ci]\nurl = "{_BASE_URL}"\n',
            encoding="utf-8",
        )
        ...
        client = _make_client_mock(create=True)
        sdk = _make_sdk_mock(client)
        with patch("px.cli.push.load_sdk", return_value=sdk):
            push_command(
                flow_paths=[str(p)],
                env="ci",
                dir_path=None,
    ...
    def test_missing_environments_file_exits(self, tmp_path):
        ...
        sdk = _make_sdk_mock()
        with (
            patch("px.cli.push.load_sdk", return_value=sdk),
