...
from px.io import IntInput, MessageTextInput, Output, SecretStrInput
...
class NeedleComponent(Component):
    ...
    outputs = [
        Output(display_name="Result", name="result", type_="Message", method="run")
    ]
    ...
    def run(self) -> Message:
        ...
        try:
            if isinstance(query_input, dict) and "top_k" in query_input:
                ...
                top_k = (
                    max(20, int(agent_top_k))
                    if agent_top_k is not None
                    else max(20, self.top_k)
                )
            else:
