...
# px.base.models.unified_models, which centralises provider-field show/hide
...
def update_provider_fields_visibility(
    ...
    """Deprecated. Use handle_model_input_update() from px.base.models.unified_models instead.
    ...
    warnings.warn(
        ...
        "Use handle_model_input_update() from px.base.models.unified_models instead.",
        DeprecationWarning,
        stacklevel=2,
    )
    current_model_value = (
        field_value
        if field_name == "model"
        else build_config.get("model", {}).get("value")
    )
