"""px init -- scaffold a new Flow DevOps project.
...
Examples::
    ...
    px init my-rag-project
    px init .                   # scaffold into the current directory
    px init my-project --no-github-actions
"""
...
_ENVIRONMENTS_YAML = """\
# .px/environments.yaml
#
# Configure your Portals instances here.
...
#
# Quick start:
#   1. Open Portals → Settings → API Keys → Create a new key
#   2. export PORTALS_LOCAL_API_KEY=<your key>
#   3. px export --env local --flow-id <uuid> --output-dir flows/
...
_TEST_FLOWS_PY = '''\
"""Integration tests for Portals flows.

Run against a local instance (started with ``px serve``):

    pytest tests/ --portals-url http://localhost:8000

Run against a named environment (staging, production, etc.):

    pytest tests/ --portals-env staging -m integration
...
def test_flow_output_quality(flow_runner):
    ...
    result = flow_runner("my-flow-endpoint", "What is Portals?")
...
_GITIGNORE = """\
# Portals credentials -- never commit API keys
# (portals-environments.toml may contain literal keys; .px/environments.yaml is safe to commit)
portals-environments.toml
"""
...
def _render_tree(target: Path, created: list[tuple[str, str]]) -> None:
    label = (
        ...
        if target != Path.cwd()
        else "[bold].[/bold] (current directory)"
    )
    tree = Tree(label)
...
def init_command(
    ...
    *,
    ...
    if example:
        from px.cli.create import create_command as _create
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning:[/yellow] Could not seed starter flow: {exc}"
            )
    else:
    ...
    _write(
        target / "tests" / "test_flows.py",
        _TEST_FLOWS_PY,
        "flow_runner example tests",
        **kw,
    )

    # .px/environments.yaml
    _write(
        target / ".px" / "environments.yaml",
        _ENVIRONMENTS_YAML,
        "edit with your instance URLs + API key env var names (safe to commit)",
        **kw,
    )

    # .gitignore — keep portals-environments.toml ignored for backward compat
    gitignore = target / ".gitignore"
    if gitignore.exists():
        existing_content = gitignore.read_text(encoding="utf-8")
        if "portals-environments.toml" not in existing_content:
    ...
    if github_actions:
        ...
        if gha_src.exists():
            ...
            console.print(
                "[yellow]Warning:[/yellow] GitHub Actions templates not found; skipping."
            )
    ...
    console.print(
        "  1. Edit [bold].px/environments.yaml[/bold] with your instance URL"
    )
    console.print(
        "  2. [bold]export PORTALS_LOCAL_API_KEY=<key>[/bold]   (Settings → API Keys)"
    )
    if example:
        console.print(
            "  3. [bold]px validate flows/hello-world.json[/bold]  (check the starter flow)"
        )
        console.print(
            "  4. [bold]px serve flows/hello-world.json[/bold]     (run it locally)"
        )
        console.print(
            "  5. [bold]px push --dir flows/ --env local[/bold]    (deploy to Portals)"
        )
    else:
        console.print("  3. [bold]px create my-flow --template hello-world[/bold]")
        console.print("  4. [bold]px push --dir flows/ --env local[/bold]")
    console.print(
        f"  {'6' if example else '5'}. [bold]pytest tests/ --portals-env local[/bold]"
    )
    console.print()
