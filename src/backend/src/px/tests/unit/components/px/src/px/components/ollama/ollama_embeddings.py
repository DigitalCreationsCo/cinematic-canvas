...
from px.io import DropdownInput, Output, SecretStrInput, StrInput
...
class OllamaEmbeddingsComponent(LCModelComponent):
    ...
    async def update_build_config(
        ...
        if field_name in {
            "base_url",
            "model_name",
        } and not await self.is_valid_ollama_url(self.base_url):
        ...
        if field_name in {"model_name", "base_url"}:
            ...
            base_url_to_check = (
                field_value if field_name == "base_url" else self.base_url
            )
            ...
            logger.warning(
                f"Fetching Ollama models from updated URL: {base_url_to_check}"
            )
            ...
            async with httpx.AsyncClient() as client:
                ...
                for model in models[self.JSON_MODELS_KEY]:
                    ...
                    show_response = await client.post(
                        url=show_url, json=payload, headers=headers
                    )
