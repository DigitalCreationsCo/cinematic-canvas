from px.base.flow_controls.loop_utils import (
    execute_loop_body,
    extract_loop_output,
...
from px.components.processing.converter import convert_to_data
from px.custom.custom_component.component import Component
from px.inputs.inputs import HandleInput
...
class LoopComponent(Component):
    ...
    documentation: str = "https://docs.portals.org/loop"
    icon = "infinity"
    ...
    outputs = [
        ...
        Output(
            display_name="Done", name="done", method="done_output", group_outputs=True
        ),
    ]
    ...
    async def execute_loop_body(
        self, data_list: list[Data], event_manager=None
    ) -> list[Data]:
    ...
    async def _iterate(self) -> list[Data]:
        ...
        try:
            ...
            if not data_list:
                self.update_ctx(
                    {f"{self._id}_aggregated": [], f"{self._id}_iterated": True}
                )
                self.log("No items to iterate, skipping loop body", name="Skipped")
                return []

            aggregated_results = await self.execute_loop_body(
                data_list, event_manager=self._event_manager
            )
            ...
            self.update_ctx(
                {f"{self._id}_iteration_error": exc, f"{self._id}_iterated": True}
            )
            raise
        ...
        self.update_ctx(
            {f"{self._id}_aggregated": aggregated_results, f"{self._id}_iterated": True}
        )
    ...
    async def item_output(self) -> Data:
        ...
        data_list = self.ctx.get(f"{self._id}_data", [])
        return Data(
            data={"count": len(data_list), "items": [d.data for d in data_list]}
        )
