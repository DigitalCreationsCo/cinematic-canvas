"""Core run functionality for executing Portals graphs."""
...
from px.cli.validation import validate_global_variables_for_env
from px.log.logger import logger
from px.run._defaults import apply_run_defaults, resolve_fallback_to_env_vars, validate_provided_id
...
async def run_flow(
    ...
    *,
    ...
    """Execute a Portals graph script or JSON flow and return the result.

    This function analyzes and executes either a Python script containing a Portals graph,
    ...
    from px.log.logger import configure
    ...
    try:
        ...
        if flow_dict is not None:
            ...
            from px.load import aload_flow_from_json
        ...
        # don't fail. See px.run._defaults.apply_run_defaults for the full rationale.
    ...
    try:
        ...
        # See px.run._defaults.resolve_fallback_to_env_vars for why this flag is
        # plumbed through (mirrors portals's API path so load_from_db variables
