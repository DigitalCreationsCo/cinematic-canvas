"""Unit tests for px login -- login_command and helpers.

All tests run entirely in-process; no real Portals instance or SDK required.
...
class _FakePortalsAuthError(Exception):
    """Stand-in for portals_sdk.PortalsAuthError in unit tests."""


class _FakePortalsConnectionError(Exception):
    """Stand-in for portals_sdk.PortalsConnectionError in unit tests."""


class _FakePortalsHTTPError(Exception):
    """Stand-in for portals_sdk.PortalsHTTPError in unit tests."""
...
def _make_client_mock(flows: list | None = None) -> MagicMock:
    ...
    client.list_flows.return_value = (
        flows if flows is not None else [MagicMock(), MagicMock()]
    )
    return client


def _make_sdk_mock(client_mock: MagicMock | None = None) -> MagicMock:
    """Return a mock portals_sdk module wired up for login tests."""
    if client_mock is None:
        client_mock = _make_client_mock()
    sdk = MagicMock()
    sdk.Client.return_value = client_mock
    sdk.PortalsAuthError = _FakePortalsAuthError
    sdk.PortalsConnectionError = _FakePortalsConnectionError
    sdk.PortalsHTTPError = _FakePortalsHTTPError
    return sdk
...
def _run_login(
    *,
    ...
    from px.cli.login import login_command

    mock = sdk_mock if sdk_mock is not None else _make_sdk_mock()
    resolved_env = env_cfg if env_cfg is not None else _make_env_cfg()

    with (
        patch("px.cli.login.load_sdk", return_value=mock),
        patch("px.config.resolve_environment", return_value=resolved_env),
    ):
        login_command(
            env=env,
...
class TestMaskKey:
    def test_long_key_shows_first_8_chars_plus_ellipsis(self):
        from px.cli.login import _mask_key
...
class TestProbeConnection:
    def test_successful_list_flows_returns_ok(self):
        from px.cli.login import _probe_connection
    ...
    def test_auth_error_returns_auth_message(self):
        ...
        client.list_flows.side_effect = _FakePortalsAuthError("unauthorized")
        sdk = _make_sdk_mock(client_mock=client)
        ok, msg, count = _probe_connection(client, sdk)
        assert ok is False
        assert msg == "auth"
    ...
    def test_connection_error_returns_connection_message(self):
        ...
        client = _make_client_mock()
        client.list_flows.side_effect = _FakePortalsConnectionError("refused")
        sdk = _make_sdk_mock(client_mock=client)
        ok, msg, count = _probe_connection(client, sdk)
        assert ok is False
    ...
    def test_http_error_returns_http_message(self):
        ...
        client = _make_client_mock()
        client.list_flows.side_effect = _FakePortalsHTTPError(
    ...
    def test_connection_error_message_contains_original_message(self):
        ...
        client = _make_client_mock()
        client.list_flows.side_effect = _FakePortalsConnectionError(original_msg)
        sdk = _make_sdk_mock(client_mock=client)
...
class TestLoginCommandAuthFailure:
    ...
    def test_auth_failure_with_api_key_includes_masked_key(self):
        ...
        client = _make_client_mock()
        client.list_flows.side_effect = _FakePortalsAuthError("forbidden")
        sdk = _make_sdk_mock(client_mock=client)
...
class TestLoginCommandConnectionFailure:
    def test_connection_error_raises_exit_1(self):
        client = _make_client_mock()
        client.list_flows.side_effect = _FakePortalsConnectionError("timeout")
        sdk = _make_sdk_mock(client_mock=client)
        env_cfg = _make_env_cfg()
        with pytest.raises(typer.Exit) as exc_info:
            _run_login(sdk_mock=sdk, env_cfg=env_cfg)
...
class TestLoginCommandHTTPError:
    ...
    def test_http_404_raises_exit_1(self):
        client = _make_client_mock()
        client.list_flows.side_effect = _FakePortalsHTTPError(404, "not found")
        sdk = _make_sdk_mock(client_mock=client)
        env_cfg = _make_env_cfg()
        with pytest.raises(typer.Exit) as exc_info:
            _run_login(sdk_mock=sdk, env_cfg=env_cfg)
...
class TestLoginCommandConfigError:
    def test_config_error_raises_exit_1(self):
        from px.cli.login import login_command
        from px.config import ConfigError

        sdk = _make_sdk_mock()
        with (
            patch("px.cli.login.load_sdk", return_value=sdk),
            patch(
                "px.config.resolve_environment", side_effect=ConfigError("no such env")
            ),
...
class TestLoginCommandSdkNotInstalled:
    def test_sdk_load_raises_bad_parameter(self):
        ...
        with (
            patch(
                "px.cli.login.load_sdk",
                side_effect=typer.BadParameter("portals-sdk is required for px login"),
            ),
    ...
    def test_sdk_load_bad_parameter_message(self):
        ...
        with (
            patch(
                "px.cli.login.load_sdk",
                side_effect=typer.BadParameter("portals-sdk is required"),
            ),
        ...
        assert "portals-sdk" in str(exc_info.value)
...
class TestLoginCommandInlineEnv:
    def test_inline_env_success_does_not_raise(self):
        ...
        _run_login(
            env=None, target=_BASE_URL, api_key=_API_KEY, sdk_mock=sdk, env_cfg=env_cfg
        )
