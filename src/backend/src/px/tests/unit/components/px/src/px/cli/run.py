...
def _check_langchain_version_compatibility(error_message: str) -> str | None:
    ...
    if (
        "langchain_core.memory" in error_message
        or "No module named 'langchain_core.memory'" in error_message
    ):
        try:
        ...
        return (
            ...
            "The 'langchain_core.memory' module was removed in langchain-core 1.x.\n"
            "px requires langchain-core < 1.0.0.\n\n"
...
async def run(
    ...
    input_value: str | None = typer.Argument(
        None, help="Input value to pass to the graph"
    ),
    ...
    *,
    ...
    This command analyzes and executes either a Python script containing a Portals graph,
    ...
    verbosity = (
        3 if verbose_full else (2 if verbose_detailed else (1 if verbose else 0))
    )
