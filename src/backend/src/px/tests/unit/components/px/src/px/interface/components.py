...
import orjson
from px.constants import BASE_COMPONENTS_PATH
from px.custom.utils import abuild_custom_components, create_component_template
from px.log.logger import logger
from px.utils.flow_validation import collect_component_hash_lookups
from px.utils.validate_cloud import (
...
MIN_MODULE_PARTS = 2
MIN_MODULE_PARTS_WITH_FILENAME = (
    4  # Minimum parts needed to have a module filename (px.components.type.filename)
)
EXPECTED_RESULT_LENGTH = (
    2  # Expected length of the tuple returned by _process_single_module
)
...
def _parse_dev_mode() -> tuple[bool, list[str] | None]:
    """Parse PX_DEV to determine dev mode and which modules to load.

    Development mode must be explicitly enabled via the PX_DEV environment variable.
    ...
    Supports two modes:
    - Boolean mode: PX_DEV=1/true/yes loads all modules dynamically
    - List mode: PX_DEV=mistral,openai,anthropic loads only specified modules
...
def _read_component_index(custom_path: str | None = None) -> dict | None:
    ...
    try:
        ...
        if custom_path:
            ...
            if custom_path.startswith(("http://", "https://")):
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to fetch component index from {custom_path}: {e}"
                    )
                    return None
                except orjson.JSONDecodeError as e:
                    logger.warning(
                        f"Component index from {custom_path} is corrupted or invalid JSON: {e}"
                    )
                    return None
            else:
                # Load from file path
                ...
                try:
                    ...
                    logger.warning(
                        f"Component index at {custom_path} is corrupted or invalid JSON: {e}"
                    )
                    return None
        else:
            # Use built-in index
            pkg_dir = Path(inspect.getfile(px)).parent
            ...
            try:
                ...
                logger.warning(
                    f"Built-in component index is corrupted or invalid JSON: {e}"
                )
                return None
        ...
        if not sha:
            logger.warning(
                "Component index missing SHA256 hash - index may be tampered"
            )
            return None

        # Use orjson for hash calculation to match build script
        calc = hashlib.sha256(
            orjson.dumps(tmp, option=orjson.OPT_SORT_KEYS)
        ).hexdigest()
        if sha != calc:
            logger.warning(
        ...
        # Version check: ensure index matches installed px version
        from importlib.metadata import PackageNotFoundError, version

        try:
            installed_version = version("px")
        except PackageNotFoundError:
            # In some deployment environments (e.g. Docker with workspace installs),
            # px may be importable but lack dist-info metadata. Skip version check.
            logger.debug(
                "Could not determine installed px version (no package metadata); skipping version check"
            )
        ...
        logger.warning(
            f"Unexpected error reading component index: {type(e).__name__}: {e}"
        )
        return None
    return blob
...
def _get_cache_path() -> Path:
    ...
    cache_dir = Path(user_cache_dir("px", "portals"))
...
def _save_generated_index(modules_dict: dict) -> None:
    ...
    try:
        ...
        entries = [
            [top_level, components] for top_level, components in modules_dict.items()
        ]
        ...
        portals_version = version("portals")

        # Build index structure
        index = {
            "version": portals_version,
            "metadata": {
        ...
        json_bytes = orjson.dumps(
            index, option=orjson.OPT_SORT_KEYS | orjson.OPT_INDENT_2
        )
...
async def _send_telemetry(
    ...
    try:
        ...
        from portals.services.telemetry.schema import ComponentIndexPayload
...
async def _load_from_index_or_cache(
    ...
    if index and "entries" in index:
        ...
        await logger.adebug(
            f"Loaded {len(modules_dict)} component categories from index"
        )
    ...
    try:
        ...
        if cache_path.exists():
            ...
            if index and "entries" in index:
                ...
                modules_dict = filter_disabled_components_from_dict(modules_dict)
                await logger.adebug(
                    f"Loaded {len(modules_dict)} component categories from cache"
                )
...
async def _load_components_dynamically(
    ...
    try:
        import px.components as components_pkg
    except ImportError as e:
        await logger.aerror(
            f"Failed to import portals.components package: {e}", exc_info=True
        )
        return modules_dict
    ...
    for _, modname, _ in pkgutil.walk_packages(
        ...
        if len(parts) > MIN_MODULE_PARTS:
            ...
            if len(parts) >= MIN_MODULE_PARTS_WITH_FILENAME:
                ...
                if is_component_disabled_in_astra_cloud(
                    component_type.lower(), module_filename
                ):
                    continue
    ...
    tasks = [
        asyncio.to_thread(_process_single_module, modname) for modname in module_names
    ]
    ...
    try:
        ...
        await logger.aerror(
            f"Error during parallel module processing: {e}", exc_info=True
        )
        return modules_dict
    ...
    for result in module_results:
        ...
        if (
            ...
            and len(result) == EXPECTED_RESULT_LENGTH
        ):
...
async def _load_full_dev_mode() -> tuple[dict[str, Any], str]:
    ...
    """
    await logger.adebug("PX_DEV full mode: loading all modules dynamically")
...
async def _load_selective_dev_mode(
    ...
    Returns:
        Tuple of (modules_dict, index_source)
    """
    await logger.adebug(f"PX_DEV selective mode: reloading {target_modules}")
    ...
    await logger.adebug(
        f"Reloaded {len(target_modules)} module(s), kept others from index"
    )
    return modules_dict, "dynamic"
...
async def import_portals_components(
    settings_service: Optional["SettingsService"] = None,
    telemetry_service: Any | None = None,
) -> dict[str, dict[str, Any]]:
    """Asynchronously discovers and loads all built-in Portals components.
    ...
    if dev_mode_enabled and not target_modules:
        ...
        modules_dict, index_source = await _load_selective_dev_mode(
            settings_service, target_modules
        )
    else:
...
def _process_single_module(modname: str) -> tuple[str, dict] | None:
    ...
    try:
        ...
        return None
    # Extract the top-level subpackage name after "px.components."
    # e.g., "px.components.Notion.add_content_to_page" -> "Notion"
...
async def _determine_loading_strategy(
    ...
    if settings_service.settings.lazy_load_components:
        ...
        component_cache.all_types_dict = await aget_component_metadata(
        ...
        custom_paths = [
            p
            for p in settings_service.settings.components_path
            if p != BASE_COMPONENTS_PATH
        ]
        if custom_paths:
    ...
    component_count = sum(
        len(comps) for comps in components_dict.get("components", {}).values()
    )
...
def _build_code_hash_lookups(cache: ComponentCache) -> None:
    ...
    logger.debug(
        f"Built code hash lookups: {len(type_to_hash)} types, {len(all_hashes)} unique hashes"
    )
...
async def get_and_cache_all_types_dict(
    ...
    Supports both full and partial (lazy) loading. If the cache is empty, loads built-in Portals
    ...
    if component_cache.all_types_dict is None:
        ...
        portals_components = await import_portals_components(
            settings_service, telemetry_service
        )
        ...
        custom_flat = (
            custom_components_dict.get("components", custom_components_dict) or {}
        )

        # Merge built-in and custom components (no wrapper at cache level)
        component_cache.all_types_dict = {
            **portals_components["components"],
            **custom_flat,
        }
        component_count = sum(
            len(comps) for comps in component_cache.all_types_dict.values()
        )
...
async def aget_component_metadata(components_paths: list[str]):
    ...
    await logger.adebug(
        f"Discovered {len(component_types)} component types: {', '.join(component_types)}"
    )
    ...
    for component_type in component_types:
        ...
        component_names = await discover_component_names(
            component_type, components_paths
        )
        await logger.adebug(
            f"Found {len(component_names)} components for type {component_type}"
        )

        # Create stub entries with just basic metadata
        for name in component_names:
            # Get minimal metadata for component
            metadata = await get_component_minimal_metadata(
                component_type, name, components_paths
            )

            if metadata:
                components_dict["components"][component_type][name] = metadata
                # Mark as needing full loading
                components_dict["components"][component_type][name]["lazy_loaded"] = (
                    True
                )
...
async def discover_component_names(
    component_type: str, components_paths: list[str]
) -> list[str]:
...
async def get_component_minimal_metadata(
    component_type: str, component_name: str, components_paths: list[str]
):
...
async def ensure_component_loaded(
    component_type: str, component_name: str, settings_service: "SettingsService"
):
    ...
    if (
        ...
        or component_name
        not in component_cache.all_types_dict["components"][component_type]
    ):
        return

    # Check if component is marked for lazy loading
    if component_cache.all_types_dict["components"][component_type][component_name].get(
        "lazy_loaded", False
    ):
        await logger.adebug(
            f"Fully loading component {component_type}:{component_name}"
        )
        ...
        if full_component:
            ...
            component_cache.all_types_dict["components"][component_type][
                component_name
            ] = full_component
            # Remove lazy_loaded flag if it exists
            if (
                "lazy_loaded"
                in component_cache.all_types_dict["components"][component_type][
                    component_name
                ]
            ):
                del component_cache.all_types_dict["components"][component_type][
                    component_name
                ]["lazy_loaded"]
            ...
            await logger.adebug(
                f"Component {component_type}:{component_name} fully loaded"
            )
        else:
            await logger.awarning(
                f"Failed to fully load component {component_type}:{component_name}"
            )


async def load_single_component(
    component_type: str, component_name: str, components_paths: list[str]
):
    """Load a single component fully."""
    from px.custom.utils import get_single_component_dict

    try:
        # Delegate to a more specific function that knows how to load
        # a single component of a specific type
        return await get_single_component_dict(
            component_type, component_name, components_paths
        )
    except (ImportError, ModuleNotFoundError) as e:
        # Handle issues with importing the component or its dependencies
        await logger.aerror(
            f"Import error loading component {component_type}:{component_name}: {e!s}"
        )
        return None
    except (AttributeError, TypeError) as e:
        # Handle issues with component structure or type errors
        await logger.aerror(
            f"Component structure error for {component_type}:{component_name}: {e!s}"
        )
        return None
    except FileNotFoundError as e:
        # Handle missing files
        await logger.aerror(
            f"File not found for component {component_type}:{component_name}: {e!s}"
        )
        return None
    except ValueError as e:
        # Handle invalid values or configurations
        await logger.aerror(
            f"Invalid configuration for component {component_type}:{component_name}: {e!s}"
        )
        return None
    except (KeyError, IndexError) as e:
        # Handle data structure access errors
        await logger.aerror(
            f"Data structure error for component {component_type}:{component_name}: {e!s}"
        )
        return None
    except RuntimeError as e:
        # Handle runtime errors
        await logger.aerror(
            f"Runtime error loading component {component_type}:{component_name}: {e!s}"
        )
        ...
        await logger.aerror(
            f"OS error loading component {component_type}:{component_name}: {e!s}"
        )
        return None


# Also add a utility function to load specific component types
async def get_type_dict(
    component_type: str, settings_service: Optional["SettingsService"] = None
):
    ...
    if (
        ...
        if settings_service.settings.lazy_load_components:
            for component_name in list(
                component_cache.all_types_dict["components"][component_type].keys()
            ):
                await ensure_component_loaded(
                    component_type, component_name, settings_service
                )
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from px.custom.utils import build_custom_components
