"""Minimal variable service for px package with in-memory storage and environment fallback."""
...
class VariableService(Service):
    ...
    This is a lightweight implementation for PX that maintains in-memory
    ...
    async def get_variable(self, name: str, **kwargs) -> str | None:  # noqa: ARG002
        ...
        (px env-fallback vs portals DB-backed).

        Args:
            name: Variable name
            **kwargs: Additional arguments (ignored; user_id/field/session
                from portals's call signature are absorbed and not used,
