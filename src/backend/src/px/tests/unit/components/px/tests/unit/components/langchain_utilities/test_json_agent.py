...
class TestJsonAgentComponent:
    ...
    def test_get_local_path_with_local_json_file(self, component_class):
        ...
        try:
            ...
            with patch(
                "px.components.langchain_utilities.json_agent.get_settings_service"
    ...
    def test_get_local_path_with_s3_json_file(self, component_class):
        ...
        with (
            ...
            patch(
                "px.components.langchain_utilities.json_agent.read_file_bytes",
    ...
    def test_build_agent_with_local_json_file(
        self, component_class, mock_langchain_community
    ):
        ...
        try:
            component.set_attributes(
                {"llm": MagicMock(), "path": json_file, "verbose": False}
            )
    ...
    def test_build_agent_with_local_yaml_file(
        self, component_class, mock_langchain_community
    ):
        ...
        try:
            component.set_attributes(
                {"llm": MagicMock(), "path": yaml_file, "verbose": True}
            )
            ...
            with (
                ...
                patch(
                    "px.components.langchain_utilities.json_agent.yaml.safe_load"
                ) as mock_yaml_load,
            ):
                mock_settings = MagicMock()
                mock_settings.settings.storage_type = "local"
    ...
    def test_build_agent_with_s3_json_file(
        self, component_class, mock_langchain_community
    ):
        """Test build_agent with S3 JSON file (downloads to temp)."""
        component = component_class()
        component.set_attributes(
            {"llm": MagicMock(), "path": "flow_456/data.json", "verbose": False}
        )
    ...
    def test_build_agent_cleans_up_on_error(
        self, component_class, mock_langchain_community
    ):
