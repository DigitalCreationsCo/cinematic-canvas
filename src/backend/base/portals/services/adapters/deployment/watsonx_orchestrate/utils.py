...
from px.services.adapters.deployment.schema import _normalize_and_validate_id
...
def build_agent_payload_from_values(
    *,
    ...
    return {
        ...
        "description": str(description).strip() or f"Portals deployment {deployment_name}",
        "tools": list(tool_ids),
        "style": "default",
        "llm": str(llm).strip(),
    }
