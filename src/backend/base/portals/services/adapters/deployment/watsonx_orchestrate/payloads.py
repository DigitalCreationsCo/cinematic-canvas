...
from px.services.adapters.deployment.payloads import DeploymentPayloadSchemas
...
from px.services.adapters.payload import AdapterPayload, PayloadSlot
...
class WatsonxFlowArtifactProviderData(BaseModel):
    ...
    project_id: NormalizedId = Field(
        description="Portals project id carried for watsonx snapshot creation."
    )
...
class WatsonxConnectionRawPayload(BaseModel):
    ...
    app_id: NormalizedId = Field(
        description=(
            "App id used for operation references. Newly created connections preserve this app_id."
        )
    )
    environment_variables: dict[EnvVarKey, EnvVarValueSpec] | None = Field(
        None, description="Environment variables."
    )
    provider_config: AdapterPayload | None = Field(
        None, description="Provider-specific connection configuration."
    )
...
class WatsonxUpdateTools(BaseModel):
    ...
    raw_payloads: list[BaseFlowArtifact[WatsonxFlowArtifactProviderData]] | None = (
        Field(
            default=None,
    ...
    def dedupe_raw_tool_names(self) -> WatsonxUpdateTools:
        ...
        if not raw_payloads:
            return self
        deduped_by_name: dict[
            str, BaseFlowArtifact[WatsonxFlowArtifactProviderData]
        ] = {}
...
class WatsonxUpdateConnections(BaseModel):
    ...
    raw_payloads: list[WatsonxConnectionRawPayload] | None = Field(
        default=None,
        description=(
            "Raw connection payloads keyed by app_id. Newly created connections preserve this app_id."
        ),
    )
    ...
    def validate_unique_raw_app_ids(self) -> WatsonxUpdateConnections:
        ...
        duplicates = sorted(
            app_id for app_id, count in app_id_counts.items() if count > 1
        )
        if duplicates:
...
def _validate_bind_operation_references(
    *,
    ...
    for operation in operations:
        if (
            operation.tool.name_of_raw is not None
            and operation.tool.name_of_raw not in raw_tool_names
        ):
...
def _validate_overlapping_existing_tool_operations(operations: list[Any]) -> None:
    ...
    for operation in operations:
        if isinstance(operation, WatsonxBindOperation):
            ...
            if ref is None:
                continue
            bind_app_ids_by_tool.setdefault(ref.tool_id, set()).update(
                operation.app_ids
            )
            continue
        ...
        if isinstance(operation, WatsonxUnbindOperation):
            unbind_app_ids_by_tool.setdefault(operation.tool.tool_id, set()).update(
                operation.app_ids
            )
            continue
    ...
    for tool_id in sorted(remove_tool_ids):
        if (
            tool_id in bind_tool_ids
            or tool_id in attach_tool_ids
            or tool_id in unbind_app_ids_by_tool
        ):
    ...
    for tool_id, bind_app_ids in bind_app_ids_by_tool.items():
        overlap_app_ids = sorted(
            bind_app_ids.intersection(unbind_app_ids_by_tool.get(tool_id, set()))
        )
        if overlap_app_ids:
...
class WatsonxUnbindOperation(BaseModel):
    ...
    app_ids: list[NormalizedId] = Field(
        min_length=1,
        description=(
            "Operation app ids to unbind. Must not reference connections.raw_payloads app_ids."
        ),
    )
...
class WatsonxRenameToolOperation(BaseModel):
    """Rename a Portals-managed tool on the provider."""
...
class WatsonxDeploymentUpdatePayload(BaseModel):
    ...
    connections: WatsonxUpdateConnections = Field(
        default_factory=WatsonxUpdateConnections
    )
    ...
    llm: NormalizedId | None = Field(
        default=None,
        description=(
            "Provider language model identifier to use for the deployment agent."
        ),
    )
    ...
    def has_tool_work(self) -> bool:
        ...
        """
        return bool(
            self.put_tools is not None or self.operations or self.tools.raw_payloads
        )
    ...
    def validate_operation_references(self) -> WatsonxDeploymentUpdatePayload:
        ...
        raw_app_ids = {
            payload.app_id for payload in (self.connections.raw_payloads or [])
        }
        bind_operations = [
            operation
...
class WatsonxResultToolRefBinding(WatsonxToolRefBinding):
    ...
    created: bool = Field(
        description="True when the tool was created during this operation, False for pre-existing."
    )
...
class WatsonxDeploymentUpdateResultData(BaseModel):
    ...
    created_snapshot_bindings: list[WatsonxResultToolRefBinding] = Field(
        default_factory=list
    )
    # Newly attached snapshot/tool refs (created + newly attached existing).
    added_snapshot_bindings: list[WatsonxResultToolRefBinding] = Field(
        default_factory=list
    )
    # Detached snapshot/tool refs.
    removed_snapshot_bindings: list[WatsonxResultToolRefBinding] = Field(
        default_factory=list
    )
    # Full operation correlation set (created + existing refs).
    referenced_snapshot_bindings: list[WatsonxResultToolRefBinding] = Field(
        default_factory=list
    )
...
class WatsonxAgentExecutionResultData(BaseModel):
    ...
    agent_id: NormalizedId | None = Field(
        default=None,
        description="WXO agent identifier (resource_key in Portals DB).",
    )
