...
def build_provider_update_plan(
    *,
    ...
    raw_tools_to_create = [
        RawToolCreatePlan(
            raw_name=raw_name,
            payload=raw_tool_pool[raw_name],
            app_ids=app_ids.to_list(),
        )
...
async def _apply_tool_renames(
    *,
    ...
    2. Tool must be a Portals-managed tool (has ``binding.portals``).
