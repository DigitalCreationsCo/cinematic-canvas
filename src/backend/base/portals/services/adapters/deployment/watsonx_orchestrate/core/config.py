...
from px.services.adapters.payload import AdapterPayloadMissingError, AdapterPayloadValidationError, PayloadSlot

from portals.services.adapters.deployment.watsonx_orchestrate.client import resolve_runtime_credentials
from portals.services.adapters.deployment.watsonx_orchestrate.constants import ErrorPrefix
from portals.services.adapters.deployment.watsonx_orchestrate.core.tools import extract_portals_connections_binding
...
async def validate_connection(connections_client: ConnectionsClient, *, app_id: str) -> GetConnectionResponse:
    ...
    if config.security_scheme != ConnectionSecurityScheme.KEY_VALUE:
        msg = f"Connection '{app_id}' must use key-value credentials for Portals flows."
        raise InvalidContentError(message=msg)
