...
def ensure_portals_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Ensure ``binding.portals.connections`` exists in *tool_payload* and return the mutable dict.
    ...
    portals = _ensure_dict(binding, "portals")
    return _ensure_dict(portals, "connections")


def verify_portals_owned(tool: dict[str, Any], *, tool_id: str) -> None:
    """Raise ``InvalidContentError`` if the tool lacks ``binding.portals``.

    Call before any mutating operation on an existing tool to ensure
    Portals created it.  Tools created manually in the wxO console or
    by other integrations will not have this marker.
    """
    binding = tool.get("binding")
    if not isinstance(binding, dict) or "portals" not in binding:
        msg = f"Cannot modify tool '{tool_id}': it does not have a Portals binding and may not be managed by Portals."
        raise InvalidContentError(message=msg)


def extract_portals_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Extract ``binding.portals.connections`` from a provider tool payload.
    ...
    if not isinstance(binding, dict):
        return {}
    portals = binding.get("portals")
    if not isinstance(portals, dict):
        return {}
    connections = portals.get("connections")
...
def build_portals_artifact_bytes(
    *,
    tool: PortalsTool,
    ...
    px_requirement = _resolve_px_requirement()
    requirements = generate_requirements_from_flow(
        flow_definition,
        include_px=False,
        pin_versions=True,
    )
    requirements = [px_requirement, *requirements]
    requirements = dedupe_list(requirements)
    requirements_content = "\n".join(requirements) + "\n"
    logger.debug(
        "build_portals_artifact_bytes: filename='%s', requirements=%s",
        filename,
        requirements,
    )
    ...
    *,
    ...
    if not flow_definition.get("last_tested_version"):
        ...
        if not detected_version:
            msg = "Unable to determine running Portals version for snapshot creation."
            raise InvalidContentError(message=msg)
        flow_definition["last_tested_version"] = detected_version

    tool: PortalsTool = create_portals_tool(
    ...
    logger.debug(
        ...
        project_id,
        tool_payload.get("binding", {}).get("portals"),
    )

    artifacts: bytes = build_portals_artifact_bytes(
        tool=tool,
...
async def create_and_upload_wxo_flow_tools_with_bindings(
    *,
    ...
    logger.debug(
        "create_and_upload_wxo_flow_tools_with_bindings: %d tool bindings",
        len(tool_bindings),
    )
    specs = [
...
async def upload_wxo_flow_tool(
    *,
    ...
    logger.debug(
        "upload_wxo_flow_tool: created tool_id='%s', uploading artifact (%d bytes)",
        tool_id,
        len(artifact_bytes),
    )
...
async def process_raw_flows_with_app_id(
    ...
    """Create portals tools in wxO and connect them to the given app_id."""
...
def _resolve_px_requirement() -> str:
    """Pin px to the installed version, falling back to a minimum spec.

    If the ``WXO_PX_REQUIREMENT_OVERRIDE`` environment variable is set, its
    value is used verbatim as the px requirement line (e.g.
    ``px-nightly==0.4.0.dev32``) instead of resolving from the installed
    package metadata.
    """
    override = os.environ.get("WXO_PX_REQUIREMENT_OVERRIDE", "").strip()
    if override:
        logger.debug("Using wxO px requirement override: %s", override)
        return override
    try:
        return _pin_requirement_name("px")
        ...
        message = "Could not determine installed px version. Failing deployment."
...
async def verify_tools_by_ids(
    ...
    from px.services.adapters.deployment.schema import SnapshotItem, SnapshotListResult
    ...
    for tool in tools or []:
        if not isinstance(tool, dict) or not tool.get("id"):
            continue
        connections = extract_portals_connections_binding(tool)
        ...
        if len(normalized_connections) < len(connections):
            logger.warning(
                "Tool %s returned malformed portals connection bindings; defaulting to empty mapping",
                tool["id"],
            )
