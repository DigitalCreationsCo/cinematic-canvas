...
# Minimum PostgreSQL major version required by Portals.
...
POSTGRESQL_VERSION_REQUIRED_MESSAGE = (
    f"Portals requires PostgreSQL {MIN_POSTGRESQL_MAJOR_VERSION} or higher when using PostgreSQL as the database. "
    "The current PostgreSQL version does not support the syntax used by Portals's schema. "
    f"Please upgrade your PostgreSQL instance to version {MIN_POSTGRESQL_MAJOR_VERSION} or higher. "
    "See: https://docs.portals.org/configuration-custom-database"
)
