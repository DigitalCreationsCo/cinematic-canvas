...
class Deployment(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "deployment_provider_account_id",
            "resource_key",
            name="uq_deployment_resource_key_in_provider",
        ),
    )
    ...
    deployment_provider_account_id: UUIDstr = Field(
        sa_column=Column(
            sa.Uuid(),
            ForeignKey("deployment_provider_account.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        )
    )
    ...
    # ``DeploymentType`` is imported from PX
    # (``px.services.adapters.deployment.schema``).  The DB-level
    # ``deployment_type_enum`` constraint is defined by a Portals alembic
    # migration, **not** by PX code.  This decoupling is intentional:
    #
    # PX adds a new member  -> INSERT/UPDATE with the new value is rejected
    #                           by the DB until a Portals migration adds it
    #                           to the enum type.  Portals explicitly opts
    #                           in to new deployment types.
    #
    # PX renames a value    -> Existing rows are unaffected (the DB stores
    ...
    #
    # PX removes a member   -> Same as rename: stored rows retain the
    ...
    #   1. Add the member to ``DeploymentType`` in PX (or confirm it exists).
    #   2. Create a Portals alembic migration that runs:
    ...
    #   3. Coordinate with PX — adapters and callers may still reference
    #      the old value.
    #
