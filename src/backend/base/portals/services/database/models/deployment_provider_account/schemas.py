...
class DeploymentProviderKey(str, Enum):
    """Deployment provider identifiers recognised by Portals.

    Each member value must match the adapter registry key used by
    ``get_deployment_adapter(adapter_key)`` in PX and the corresponding
    mapper registration in the Portals mapper registry.
    """
