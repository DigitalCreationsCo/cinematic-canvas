...
from portals.services.database.utils import (
...
class DeploymentProviderAccount(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    # by a Portals alembic migration.  The enum is Portals-owned (see
    # ``DeploymentProviderKey`` above); PX uses plain strings for adapter
    ...
    #   2. Register the corresponding adapter in PX and mapper in Portals.
    #   3. Create a Portals alembic migration that runs:
    #        op.execute(
