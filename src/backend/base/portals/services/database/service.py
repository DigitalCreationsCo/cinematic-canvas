...
from portals.services.database import models
from portals.services.database.constants import (
...
from portals.services.database.session import NoopSession
...
class DatabaseService(Service):
    ...
    def __init__(self, settings_service: SettingsService):
        ...
        # This file is in portals.services.database.manager.py
        # the ini is in portals
        portals_dir = Path(__file__).parent.parent.parent
        self.script_location = portals_dir / "alembic"
        self.alembic_cfg_path = portals_dir / "alembic.ini"
        ...
        if self.alembic_log_to_stdout:
            ...
            self.alembic_log_path = Path(portals_dir) / alembic_log_file
    ...
    async def ensure_postgresql_version(self) -> None:
        ...
        Portals's schema uses UNIQUE NULLS DISTINCT, which is only supported in PostgreSQL 15+.
