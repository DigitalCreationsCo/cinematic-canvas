...
def _get_langsmith_tracer():
    from portals.services.tracing.langsmith import LangSmithTracer

    return LangSmithTracer


def _get_langwatch_tracer():
    from portals.services.tracing.langwatch import LangWatchTracer

    return LangWatchTracer


def _get_langfuse_tracer():
    from portals.services.tracing.langfuse import LangFuseTracer

    return LangFuseTracer


def _get_arize_phoenix_tracer():
    from portals.services.tracing.arize_phoenix import ArizePhoenixTracer

    return ArizePhoenixTracer


def _get_opik_tracer():
    from portals.services.tracing.opik import OpikTracer

    return OpikTracer


def _get_traceloop_tracer():
    from portals.services.tracing.traceloop import TraceloopTracer
...
def _get_openlayer_tracer():
    from portals.services.tracing.openlayer import OpenlayerTracer
...
class TracingService(Service):
    ...
    def project_name(self):
        if self.deactivated:
            return os.getenv("LANGCHAIN_PROJECT", "Portals")
        trace_context = trace_context_var.get()
        if trace_context is None:
