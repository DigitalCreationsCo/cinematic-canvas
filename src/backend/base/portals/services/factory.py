...
def import_all_services_into_a_dict():
    # Services are all in portals.services.{service_name}.service
    ...
    for service_type in ServiceType:
        try:
            ...
            # Special handling for mcp_composer which is now in px module
            if service_name == "mcp_composer":
                module_name = f"px.services.{service_name}.service"
            else:
                module_name = f"portals.services.{service_name}.service"
    ...
    # Import settings and auth base from px (used in type hints but not portals Service subclasses)
