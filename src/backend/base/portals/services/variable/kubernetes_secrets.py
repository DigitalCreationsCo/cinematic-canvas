...
class KubernetesSecretManager:
    ...
    def __init__(self, namespace: str = "portals"):
    ...
    def upsert_secret(self, secret_name: str, data: dict):
        ...
        try:
            ...
            existing_data = {
                k: b64decode(v).decode() for k, v in existing_secret.data.items()
            }
            existing_data.update(data)

            # Encode all data to base64
            encoded_data = {
                k: b64encode(v.encode()).decode() for k, v in existing_data.items()
            }

            # Update the existing secret
            existing_secret.data = encoded_data
            return self.core_api.replace_namespaced_secret(
                secret_name, self.namespace, existing_secret
            )
    ...
    def delete_secret_key(self, name: str, key: str):
        ...
        if key in secret.data:
            ...
            raise ApiException(
                status=404, reason="Not Found", msg="Key not found in the secret"
            )
