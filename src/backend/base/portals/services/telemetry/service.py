...
from portals.services.telemetry.opentelemetry import OpenTelemetry
from portals.services.telemetry.schema import (
...
class TelemetryService(Service):
    ...
    def __init__(self, settings_service: SettingsService):
        ...
        self.ot = OpenTelemetry(
            prometheus_enabled=settings_service.settings.prometheus_enabled
        )
        ...
        self.common_telemetry_fields = {
            ...
            "platform": "desktop" if self._get_portals_desktop() else "python_package",
    ...
    async def send_telemetry_data(
        self, payload: BaseModel, path: str | None = None
    ) -> None:
        ...
        try:
            ...
            if response.status_code != httpx.codes.OK:
                await logger.aerror(
                    f"Failed to send telemetry data: {response.status_code} {response.text}"
                )
            else:
    ...
    async def log_package_deployment_provider(self, payload: DeploymentPayload) -> None:
        await self._queue_event(
            (self.send_telemetry_data, payload, "deployment_provider")
        )
    ...
    async def log_package_shutdown(self) -> None:
        payload = ShutdownPayload(
            time_running=(datetime.now(timezone.utc) - self._start_time).seconds
        )
    ...
    def _get_portals_desktop(self) -> bool:
        # Coerce to bool, could be 1, 0, True, False, "1", "0", "True", "False"
        return str(os.getenv("PORTALS_DESKTOP", "False")).lower() in {"1", "true"}

    def _get_client_type(self) -> str:
        return "desktop" if self._get_portals_desktop() else "oss"

    async def _send_email_telemetry(self) -> None:
        """Send the telemetry event for the registered email address."""
        from portals.utils.registered_email_util import get_email_model

        payload: EmailPayload | None = get_email_model()

        if not payload:
            await logger.adebug(
                "Aborted operation to send email telemetry event. No registered email address."
            )
            return
        ...
        try:
            ...
            await logger.aerror(
                f"Failed to send email telemetry event: {payload.email}: {err}"
            )
            return
    ...
    async def log_package_component_inputs(
        self, payload: ComponentInputsPayload
    ) -> None:
        ...
        for chunk in chunks:
            await self._queue_event(
                (self.send_telemetry_data, chunk, "component_inputs")
            )
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self.log_package_version_task = asyncio.create_task(
                self.log_package_version()
            )
            if self._get_portals_desktop():
                self.log_package_email_task = asyncio.create_task(
                    self._send_email_telemetry()
                )
        except Exception:  # noqa: BLE001
        ...
        try:
            ...
            if self.worker_task:
                await self._cancel_task(
                    self.worker_task, "Cancel telemetry worker task"
                )
