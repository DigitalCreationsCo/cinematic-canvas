...
from portals.exceptions.api import WorkflowResourceError, WorkflowServiceUnavailableError
from portals.services.base import Service
from portals.services.deps import get_queue_service
from portals.services.task.backends.anyio import AnyIOBackend
from portals.services.task.backends.celery import CeleryBackend

if TYPE_CHECKING:
    from px.services.settings.service import SettingsService

    from portals.services.task.backends.base import TaskBackend
...
class TaskService(Service):
    ...
    async def revoke_task(self, task_id: UUID | str) -> bool:
        ...
        try:
            ...
            if str(e) != "PORTALS_USER_CANCELLED":
                raise
            return True
        return True
