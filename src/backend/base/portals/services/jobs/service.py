...
from portals.services.database.models.jobs.crud import (
    get_job_by_job_id,
    get_jobs_by_flow_id,
    get_latest_jobs_by_asset_ids,
    update_job_status,
)
from portals.services.database.models.jobs.model import Job, JobStatus, JobType
...
class JobService(Service):
    ...
    async def get_jobs_by_flow_id(
        self, flow_id: UUID | str, page: int = 1, page_size: int = 10
    ) -> list[Job]:
        ...
        async with session_scope() as session:
            return await get_jobs_by_flow_id(
                session, flow_id, page=page, size=page_size
            )
    ...
    async def get_latest_jobs_by_asset_ids(
        self, asset_ids: Sequence[UUID | str]
    ) -> dict[UUID, Job]:
    ...
    async def execute_with_status(self, job_id: UUID, run_coro_func, *args, **kwargs):
        ...
        try:
            ...
            await self.update_job_status(
                job_id, JobStatus.FAILED, finished_timestamp=True
            )
            raise
            ...
            await self.update_job_status(
                job_id, JobStatus.TIMED_OUT, finished_timestamp=True
            )
            raise

        except asyncio.CancelledError as exc:
            # Check the message code to determine if this was user-initiated or system-initiated
            if exc.args and exc.args[0] == "PORTALS_USER_CANCELLED":
            ...
            await self.update_job_status(
                job_id, JobStatus.COMPLETED, finished_timestamp=True
            )
            return result
