...
async def test_openai_response_stream_error_handling(client):
    ...
    with (
        patch(
            "portals.api.v1.openai_responses.get_flow_by_id_or_endpoint_name"
        ) as mock_get_flow,
        patch("portals.api.v1.openai_responses.run_flow_generator") as _,
        patch("portals.api.v1.openai_responses.consume_and_yield") as mock_consume,
    ):
        # Setup mock flow
        mock_flow = MagicMock()
        mock_flow.data = {
            "nodes": [{"data": {"type": "ChatInput"}}, {"data": {"type": "ChatOutput"}}]
        }
        ...
        error_event = json.dumps(
            {"event": "error", "data": {"error": "Simulated streaming error"}}
        ).encode("utf-8")
