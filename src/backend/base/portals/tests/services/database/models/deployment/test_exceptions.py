...
def test_get_friendly_guard_detail_falls_back_to_generic_code_message() -> None:
    assert (
        get_friendly_guard_detail("SOME_UNKNOWN_CODE")
        == "Operation blocked by deployment guard (SOME_UNKNOWN_CODE)."
    )


def test_parse_deployment_guard_error_returns_none_when_absent() -> None:
    parsed = parse_deployment_guard_error(
        Exception("plain error DEPLOYMENT_GUARD:PROJECT_HAS_DEPLOYMENTS:oops")
    )
...
@pytest.mark.asyncio
async def test_araise_if_deployment_guard_error_or_skip_raises_and_logs_message(
    monkeypatch,
) -> None:
    guard_error = DeploymentGuardError(
        code="PROJECT_HAS_DEPLOYMENTS",
    ...
    monkeypatch.setattr(
        "portals.services.database.models.deployment.exceptions.logger.adebug",
        _fake_adebug,
    )
...
@pytest.mark.asyncio
async def test_araise_if_deployment_guard_error_or_skip_raises_without_logging_when_message_absent(
    monkeypatch,
) -> None:
    guard_error = DeploymentGuardError(
        code="PROJECT_HAS_DEPLOYMENTS",
