...
class TestValidateProviderUrl:
    ...
    def test_accepts_valid_https_url(self):
        assert (
            validate_provider_url("https://example.com/api", self._info())
            == "https://example.com/api"
        )

    def test_strips_whitespace(self):
        assert (
            validate_provider_url("  https://example.com  ", self._info())
            == "https://example.com/"
        )

    def test_normalizes_scheme_lowercase(self):
        assert (
            validate_provider_url("HTTPS://example.com/path", self._info())
    ...
    def test_preserves_port(self):
        assert (
            validate_provider_url("https://example.com:8443/api", self._info())
...
class TestCheckProviderUrlAllowed:
    ...
    def test_wxo_accepts_cloud_ibm_com(self):
        check_provider_url_allowed(
            "https://api.us-south.wxo.cloud.ibm.com/v1", "watsonx-orchestrate"
        )

    def test_wxo_accepts_ibm_com_subdomain(self):
        check_provider_url_allowed(
            "https://dl.watson.ibm.com/api", "watsonx-orchestrate"
        )
    ...
    def test_wxo_rejects_non_ibm_hostname(self):
        with pytest.raises(ValueError, match="not allowed for provider"):
            check_provider_url_allowed(
                "https://evil.example.com/api", "watsonx-orchestrate"
            )
    ...
    def test_wxo_rejects_ibm_in_path_only(self):
        with pytest.raises(ValueError, match="not allowed for provider"):
            check_provider_url_allowed(
                "https://evil.com/ibm.com", "watsonx-orchestrate"
            )

    def test_wxo_rejects_spoofed_subdomain(self):
        with pytest.raises(ValueError, match="not allowed for provider"):
            check_provider_url_allowed(
                "https://ibm.com.evil.com/api", "watsonx-orchestrate"
            )
    ...
    def test_unknown_provider_rejected(self):
        with pytest.raises(ValueError, match="is not a valid DeploymentProviderKey"):
            check_provider_url_allowed(
                "https://anything.example.com", "unknown-provider"
            )
...
class TestExtractTenantFromUrl:
    ...
    def test_wxo_extracts_tenant_from_instances_path(self):
        ...
        assert (
            extract_tenant_from_url(url, DeploymentProviderKey.WATSONX_ORCHESTRATE)
            == self._TENANT_UUID
        )
    ...
    def test_unknown_provider_raises(self):
        with pytest.raises(ValueError, match="is not a valid DeploymentProviderKey"):
            extract_tenant_from_url(
                "https://example.com/instances/abc", "unknown-provider"
            )
...
class TestValidateTenantUrlConsistency:
    ...
    def test_raises_when_tenant_contradicts_url(self):
        ...
        with pytest.raises(ValueError, match="does not match"):
            validate_tenant_url_consistency(
                url, "99000000-0000-0000-0000-000000000999", self.WXO
            )
