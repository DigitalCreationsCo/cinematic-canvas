...
async def check_portals_version(component: StoreComponentCreate) -> None:
    ...
    if portals_version != component.last_tested_version:
        await logger.awarning(
            ...
            f"Please update to the latest version ({portals_version}) and try again."
        )
...
def extract_global_variables_from_headers(headers) -> dict[str, str]:
    """Extract global variables from HTTP headers with prefix X-PORTALS-GLOBAL-VAR-*.
    ...
    Example:
        headers = {"X-PORTALS-GLOBAL-VAR-API-KEY": "secret", "Content-Type": "application/json"}
