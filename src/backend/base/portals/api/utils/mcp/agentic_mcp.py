"""Utilities for auto-configuring the Portals Agentic MCP server."""
...
async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    """Auto-configure the Portals Agentic MCP server for all users.

    This function adds the portals-agentic MCP server to each user's MCP
    ...
    try:
        ...
        server_name = "portals-agentic"
        python_executable = sys.executable
        server_config = {
            "command": python_executable,
            "args": ["-m", "portals.agentic.mcp"],
            "metadata": {
                "description": "Portals Agentic MCP server providing tools for flow/component operations, "
                "template search, and graph visualization",
                "auto_configured": True,
                "portals_internal": True,
            },
        }
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the Portals Agentic MCP server from all users.

    This function removes the portals-agentic MCP server from each user's MCP
    configuration. Used when agentic experience is disabled.

    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing Portals Agentic MCP server from all users...")
