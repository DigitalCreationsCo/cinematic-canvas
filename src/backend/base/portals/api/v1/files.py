...
from portals.api.v1.schemas import UploadFileResponse
...
def _get_allowed_profile_picture_folders(settings_service: SettingsService) -> set[str]:
    ...
    defaults {"People", "Space"} shipped with Portals.
    """
    allowed: set[str] = set()
    try:
    ...
    try:
        ...
        if not file_path.exists():
            ...
            if package_path.exists():
                ...
                raise HTTPException(
                    status_code=404,
                    detail=f"Profile picture {safe_folder}/{safe_file} not found",
                )
