...
- Portals API orchestration (`src/backend/base/portals/api/v1`)
- Mapper translation/reconciliation layer (`src/backend/base/portals/api/v1/mappers/deployments`)
- Adapter execution layer (`src/backend/base/portals/services/adapters/deployment/*`)
- Generic adapter schema/payload contracts (`src/px/src/px/services/adapters/deployment/*`)
...
The mapper translates API payloads to Adapters and the Portals DB:
...
- Reconciliation extraction needed by Portals persistence logic
...
- Depend on Portals flow-version concepts directly
- Encode Portals DB semantics
- Assume API route-level orchestration details

---

## 2) Portals vs Adapter Vocabulary Rules

### 2.1 No Portals-specific identity terms in adapter contracts
...
Portals can choose to populate `source_ref` with serialized flow-version IDs, but this is an implementation decision at the mapper/API boundary.
...
- Prefer **clean break** for changes internal to Portals/adapter implementation layers when public API contracts are unchanged.
...
Both create and update follow a **provider-first** strategy: the provider is called first, then the Portals DB is updated and committed. If the DB commit fails, the route issues a best-effort compensating call to the provider.
...
- **Rollback** compensates for a provider-side change that the Portals DB failed to record (write-path problem). Its goal is to undo the provider-side change so that both sides remain consistent.
- **Synchronization** detects provider-side deletions (and maybe mutations) that the Portals DB hasn't been notified about (read-path problem). Its goal is to remove (or update, respectively) stale DB rows that no longer correspond to (or accurately represent, respectively) provider resources.
...
### 14.1 Do not mix Portals-owned and provider-owned fields at the same level

API response schemas must keep a clear ownership boundary between Portals-managed data and provider-managed data.

- **Top-level fields** should be Portals-owned: identifiers that Portals persists and controls (e.g. `deployment_id`, `name`, `created_at`).
- **Provider-owned data** (execution identifiers, agent metadata, status, timestamps, errors) belongs inside the `provider_data` dict.

This prevents future collisions — for example, if Portals starts persisting its own execution records, a top-level `execution_id` would be ambiguous against the provider's opaque run identifier.

### 14.2 Classification of fields

**Portals-owned** — fields derived from the Portals database or assigned by Portals logic:

- `deployment_id` (DB UUID), `id`, `name`, `description`, `type`
- `created_at`, `updated_at` (DB timestamps)
- `resource_key` — provider-originated but stored and indexed by Portals, so treated as Portals-owned once persisted.

**Provider-owned** — values returned by the external provider that Portals passes through without persisting or interpreting:
...
1. Is Portals the source of truth for this value? → top level.
2. Does this value come from the provider and Portals just relays it? → inside `provider_data`.
3. Does the provider supply it but Portals persists and indexes it (like `resource_key`)? → top level is acceptable.

### 14.4 Hard placement rule for non-persisted provider data

If data is not persisted in the Portals DB and comes directly from the provider,
it must go into `provider_data`.

Rules:

- Top-level response fields are reserved for values that Portals persists and controls.
- Provider-originated data that Portals only relays must stay in `provider_data` without exception.
