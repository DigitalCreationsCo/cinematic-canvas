...
class WatsonxApiDeploymentCreatePayload(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_create_operation_requirements(
        self,
...
class WatsonxApiCreatedTool(BaseModel):
    """API response shape for a tool created from a Portals flow version."""
...
class _WatsonxApiAgentExecutionResultBase(BaseModel):
    ...
    ``provider_data``.  The enclosing response only carries Portals-owned
    fields (``deployment_id``).  ``deployment_id`` (Portals DB UUID) is
