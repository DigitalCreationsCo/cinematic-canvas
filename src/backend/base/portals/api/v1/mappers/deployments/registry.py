...
- Mappers reshape Portals API payloads/results into adapter-facing contracts.
...
class DeploymentMapperRegistry:
    ...
    def register(
        self, provider_key: str, mapper_class: type[BaseDeploymentMapper]
    ) -> None:
...
def register_mapper(adapter_type: AdapterType, provider_key: str):
    ...
    def decorator(
        mapper_class: type[BaseDeploymentMapper],
    ) -> type[BaseDeploymentMapper]:
...
def get_mapper(
    adapter_type: AdapterType, provider_key: str | None
