"""Portals knowledge bases module - forwards to px.components.files_and_knowledge.
...
from px.components.files_and_knowledge import __all__ as _px_all

__all__: list[str] = list(_px_all)

# Register redirected submodules in sys.modules for direct importlib.import_module() calls
# This allows imports like: import portals.components.knowledge_bases.ingestion
_redirected_submodules = {
    # "portals.components.knowledge_bases.ingestion": "px.components.files_and_knowledge.ingestion",
    "portals.components.knowledge_bases.retrieval": "px.components.files_and_knowledge.retrieval",
}
...
def __getattr__(attr_name: str) -> Any:
    """Forward attribute access to px.components.files_and_knowledge."""
    # Handle submodule access for backwards compatibility
    if attr_name == "retrieval":
        from importlib import import_module

        result = import_module("px.components.files_and_knowledge.retrieval")
        globals()[attr_name] = result
        return result

    from px.components import files_and_knowledge

    return getattr(files_and_knowledge, attr_name)


def __dir__() -> list[str]:
    """Forward dir() to px.components.files_and_knowledge."""
    return list(__all__)
