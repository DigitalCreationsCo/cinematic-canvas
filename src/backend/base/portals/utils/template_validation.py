"""Template validation utilities for Portals starter projects.
...
from px.custom.validate import validate_code
...
def validate_flow_can_build(template_data: dict[str, Any], filename: str) -> list[str]:
    ...
    try:
        ...
        graph = Graph.from_payload(
            template_data, flow_id, flow_name, user_id="test_user"
        )
...
async def validate_flow_execution(
    ...
    try:
        ...
        create_response = await client.post(
            "api/v1/flows/", json=template_data, headers=headers, timeout=10
        )

        if create_response.status_code != 201:  # noqa: PLR2004
            errors.append(
                f"{filename}: Failed to create flow: {create_response.status_code}"
            )
            return errors
        ...
        try:
            ...
            build_response = await client.post(
                f"api/v1/build/{flow_id}/flow", json={}, headers=headers, timeout=10
            )

            if build_response.status_code != 200:  # noqa: PLR2004
                errors.append(
                    f"{filename}: Failed to build flow: {build_response.status_code}"
                )
                return errors
            ...
            events_response = await client.get(
                f"api/v1/build/{job_id}/events", headers=events_headers, timeout=10
            )

            if events_response.status_code != 200:  # noqa: PLR2004
                errors.append(
                    f"{filename}: Failed to get build events: {events_response.status_code}"
                )
                return errors
            ...
            try:  # noqa: SIM105
                await client.delete(
                    f"api/v1/flows/{flow_id}", headers=headers, timeout=10
                )
...
async def _validate_event_stream(
    response, job_id: str, filename: str, errors: list[str]
) -> None:
            ...
            async for line in response.aiter_lines():
                ...
                if event_type == "vertices_sorted":
                    ...
                    if not parsed.get("data", {}).get("ids"):
                        errors.append(
                            f"{filename}: Missing vertex IDs in vertices_sorted event"
                        )
                    ...
                    if not parsed.get("data", {}).get("build_data"):
                        errors.append(
                            f"{filename}: Missing build_data in end_vertex event"
                        )
                    ...
                    if isinstance(error_data, dict):
                        ...
                        if error_msg != "False" and error_msg is not False:
                            errors.append(
                                f"{filename}: Flow execution error: {error_msg}"
                            )
                    else:
