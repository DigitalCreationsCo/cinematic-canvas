...
from portals.cli.progress import create_portals_progress
from portals.initial_setup.setup import get_or_create_default_folder
from portals.main import setup_app
from portals.services.auth.utils import get_current_user_from_access_token
from portals.services.database.models.api_key.crud import check_key
from portals.services.database.service import (
...
from portals.services.utils import initialize_services
from portals.utils.version import fetch_latest_version, get_version_info
from portals.utils.version import is_pre_release as portals_is_pre_release
...
try:
    ...
    # PX not available, skip adding the sub-app
    pass
...
def run(
    *,
    ...
    log_level = (log_level or os.environ.get("PORTALS_LOG_LEVEL") or "info").lower()
    os.environ["PORTALS_LOG_LEVEL"] = log_level
    ...
    progress = create_portals_progress(verbose=verbose)
    ...
    if platform.system() == "Windows":
        ...
        from portals.helpers.windows_postgres_helper import (
            PORTALS_DATABASE_URL,
            POSTGRESQL_PREFIXES,
        )

        db_url = os.environ.get(PORTALS_DATABASE_URL, "")
        loop_type = "asyncio"
        if (
        ...
        with progress.step(6):
            # Use Gunicorn with PortalsUvicornWorker for non-Windows systems
            from portals.server import PortalsApplication
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "portals")
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use portals.utils.version to get the version info
    ...
    info_text = (
        f"{github_icon} GitHub: Star for updates {arrow} https://github.com/portals-ai/portals\n"
    ...
    access_link = f"[bold]{status_icon} Open Portals {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
    ...
    try:
        ...
        fallback_message = (
            ...
            "* GitHub: https://github.com/portals-ai/portals\n"
            "# Discord: https://discord.com/invite/EqksyE2EX9\n\n"
            f"{telemetry_text}\n\n"
            f"[OK] Open Portals -> {protocol}://{access_host}:{port}"
        )
        try:
            ...
            logger.info("GitHub: https://github.com/portals-ai/portals")
            logger.info("Discord: https://discord.com/invite/EqksyE2EX9")
            logger.info(f"Open Portals: {protocol}://{access_host}:{port}")
...
def superuser(
    username: str = typer.Option(
        None,
        help="Username for the superuser. Defaults to 'portals' when AUTO_LOGIN is enabled.",
    ),
    ...
    auth_token: str = typer.Option(
        ...
        envvar="PORTALS_SUPERUSER_TOKEN",
    ),
) -> None:
    """Create a superuser.
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    if not settings_service.auth_settings.ENABLE_SUPERUSER_CLI:
        ...
        typer.echo("Set PORTALS_ENABLE_SUPERUSER_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    from portals.services.database.models.user.crud import get_all_superusers
...
# command to copy the portals database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'portals.db' and 'portals-pre.db' files from the cache directory to the current
    directory.
    ...
    cache_dir = Path(user_cache_dir("portals"))
    db_path = cache_dir / "portals.db"
    pre_db_path = cache_dir / "portals-pre.db"
...
def api_key(
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            from portals.services.database.models.api_key.model import (
                ApiKey,
                ApiKeyCreate,
            )
