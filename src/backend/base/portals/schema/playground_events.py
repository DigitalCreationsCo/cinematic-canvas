...
from px.schema.properties import Properties
from px.schema.validators import timestamp_to_str_validator
from px.utils.constants import MESSAGE_SENDER_USER
...
class PlaygroundEvent(BaseModel):
    ...
    format_type: Literal["default", "error", "warning", "info"] = Field(
        default="default"
    )
...
def create_error(
    ...
    if traceback:
        ...
        content_blocks += [
            ContentBlock(
                title=title, contents=[ErrorContent(type="error", traceback=traceback)]
            )
        ]
    return ErrorEvent(
        text=text,
