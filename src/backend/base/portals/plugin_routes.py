...
Plugins register via the ``portals.plugins`` entry-point group. They receive
a wrapper so they cannot overwrite or shadow existing Portals routes.
"""
...
def _get_route_keys(app: FastAPI) -> set[tuple[str, str]]:
    ...
    routes cannot overwrite or shadow existing Portals routes.
    """
...
def load_plugin_routes(app: FastAPI) -> None:
    ...
    Plugins register themselves via the ``portals.plugins`` entry-point group.
    ...
    eps = entry_points(group="portals.plugins")
