"""Portals Assistant API router.

This module provides the HTTP endpoints for the Portals Assistant.
...
from portals.agentic.api.schemas import AssistantRequest
from portals.agentic.services.assistant_service import (
    execute_flow_with_validation,
    execute_flow_with_validation_streaming,
)
from portals.agentic.services.flow_executor import execute_flow_file
from portals.agentic.services.flow_types import (
    MAX_VALIDATION_RETRIES,
    PORTALS_ASSISTANT_FLOW,
)
from portals.agentic.services.provider_service import (
    PREFERRED_PROVIDERS,
    get_default_model,
    get_enabled_providers_for_user,
)
from portals.api.utils.core import CurrentActiveUser, DbSession
...
async def execute_named_flow(
    flow_name: str, request: AssistantRequest, current_user: CurrentActiveUser
) -> dict:
...
async def check_assistant_config(
    ...
    """Check if the Portals Assistant is properly configured.
...
async def assist(
    ...
    """Chat with the Portals Assistant."""
    ctx = await _resolve_assistant_context(request, current_user.id, session)

    logger.info(
        f"Executing {PORTALS_ASSISTANT_FLOW} with {ctx.provider}/{ctx.model_name}"
    )

    return await execute_flow_with_validation(
        flow_filename=PORTALS_ASSISTANT_FLOW,
...
async def assist_stream(
    ...
    """Chat with the Portals Assistant with streaming progress updates."""
    ctx = await _resolve_assistant_context(request, current_user.id, session)
