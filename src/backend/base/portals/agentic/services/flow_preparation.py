...
# Relative path embedded in the bundled PortalsAssistant.json flow for the
# Directory component that scans built-in px components. It only resolves
# correctly when the process CWD is the monorepo root, which is never the
# case for a packaged install (Portals Desktop, `pip install portals`,
# Docker, etc.). inject_px_components_path rewrites it to an absolute path
# derived from the installed px package at runtime.
PX_COMPONENTS_PATH_SENTINEL = "./src/px/src/px/components/"
...
def inject_model_into_flow(
    ...
    metadata = {
        "api_key_param": param_mapping.get(
            "api_key_param", provider_config.get("api_key_param", "api_key")
        ),
        "context_length": 128000,
        "model_class": param_mapping.get(
            "model_class", provider_config.get("model_class", "ChatOpenAI")
        ),
        "model_name_param": param_mapping.get(
            "model_name_param", provider_config.get("model_name_param", "model")
        ),
    }
...
def inject_px_components_path(flow_data: dict) -> dict:
    """Rewrite Directory nodes targeting bundled px components to an absolute path.

    The bundled PortalsAssistant flow hardcodes a relative path that only
    ...
    be a directory.", causing the Portals Assistant to fail with
    ...
    `path` value equals PX_COMPONENTS_PATH_SENTINEL, replaces it with the
    absolute path derived from the installed px package.
    """
    absolute_path = str(Path(px.__file__).parent / "components")
    ...
    for node in flow_data.get("data", {}).get("nodes", []):
        ...
        if path_field and path_field.get("value") == PX_COMPONENTS_PATH_SENTINEL:
...
def load_and_prepare_flow(
    ...
    if provider and model_name:
        flow_data = inject_model_into_flow(
            flow_data, provider, model_name, api_key_var, provider_vars
        )

    flow_data = inject_px_components_path(flow_data)
