...
DANGEROUS_ATTR_CALLS: list[tuple[str, str, str]] = [
    ("os", "system", "os.system() is forbidden — use Portals's built-in integrations"),
...
class _SecurityChecker(ast.NodeVisitor):
        ...
        for alias in node.names:
            ...
            if module in DANGEROUS_IMPORTS:
                self.violations.append(
                    f"Import of '{alias.name}' is forbidden in components"
                )
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        ...
        if root_module in DANGEROUS_IMPORTS:
            self.violations.append(
                f"Import from '{node.module}' is forbidden in components"
            )
            ...
            for alias in node.names:
                if alias.name in restricted:
                    self.violations.append(
                        f"Import of '{root_module}.{alias.name}' is forbidden in components"
                    )
