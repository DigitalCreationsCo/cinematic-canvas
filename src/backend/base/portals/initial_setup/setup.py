...
from portals.services.database.models.folder.constants import (
...
async def get_or_create_assistant_folder(session, user_id: UUID):
    """Create or get the Portals Assistant folder for a specific user.
...
async def create_or_update_agentic_flows(session: AsyncSession, user_id: UUID) -> None:
    """Create or update agentic flows in the Portals Assistant folder for a user.

    This function is called on user login to ensure that all agentic flows
    are present and up-to-date in the user's Portals Assistant folder.
    ...
    try:
        # Get or create the Portals Assistant folder
...
async def load_flows_from_directory() -> None:
    """On portals startup, this loads all flows from the directory specified in the settings.
