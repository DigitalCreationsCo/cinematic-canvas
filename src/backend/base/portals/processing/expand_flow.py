...
(used by AI agents) into the full flow format expected by Portals.
"""
...
def _expand_edge(
    ...
    field_type = (
        target_field.get("type", "str") if isinstance(target_field, dict) else "str"
    )
...
def expand_compact_flow(
    ...
    Returns:
        Full flow data structure ready for Portals UI
