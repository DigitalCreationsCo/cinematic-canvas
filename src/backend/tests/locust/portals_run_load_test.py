#!/usr/bin/env python3
"""Portals Load Test Runner

This script provides an easy way to run Portals load tests.
For first-time setup, use setup_portals_test.py to create test credentials.

Usage:
    # First time setup (run once):
    python setup_portals_test.py --interactive
...
def check_portals_running(host):
    """Check if Portals is already running."""
    try:
        import httpx
...
def wait_for_portals(host, timeout=60):
    """Wait for Portals to be ready."""
    print(f"Waiting for Portals to be ready at {host}...")
    start_time = time.time()

    while time.time() - start_time < timeout:
        if check_portals_running(host):
            print("✅ Portals is ready!")
            return True
        time.sleep(2)

    print(f"❌ Portals did not start within {timeout} seconds")
    return False


def start_portals(host, port):
    """Start Portals server if not already running."""
    if check_portals_running(host):
        print(f"✅ Portals is already running at {host}")
        return None

    print(f"Starting Portals server on port {port}...")

    # Start Portals in the background
    cmd = [
        sys.executable,
        "-m",
        "portals",
    ...
    if wait_for_portals(host, timeout=60):
        return process
    process.terminate()
    return None
...
def run_locust_test(args):
    ...
    locust_file = Path(__file__).parent / "portals_locustfile.py"

    # Check for required environment variables
    if not os.getenv("API_KEY"):
        print("❌ API_KEY environment variable not found!")
        print("Run portals_setup_test.py first to create test credentials.")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Portals load tests with automatic setup",
...
Examples:
  ...
  # Run against existing Portals instance
  python run_load_test.py --host http://localhost:8000 --no-start-portals
  ...
  python run_load_test.py --headless --csv results --users 25 --duration 60
    ...
    parser.add_argument(
        ...
        help="Portals host URL (default: http://localhost:7860, use https:// for remote instances)",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=7860,
        help="Port to start Portals on (default: 7860)",
    )
    parser.add_argument(
        "--no-start-portals",
        action="store_true",
        help="Don't start Portals automatically (assume it's already running)",
    )
    ...
    parser.add_argument(
        "--users", type=int, default=50, help="Number of concurrent users (default: 20)"
    )
    parser.add_argument(
        "--spawn-rate",
        type=int,
    ...
    parser.add_argument(
        "--shape", choices=["ramp100", "stepramp"], help="Load test shape to use"
    )
    parser.add_argument("--csv", help="Save results to CSV files with this prefix")
    parser.add_argument(
        "--html", help="Generate HTML report with this filename (e.g., report.html)"
    )
    ...
    portals_process = None

    try:
        # Start Portals if needed
        if not args.no_start_portals:
            if args.host.startswith("https://") or not args.host.startswith(
                "http://localhost"
            ):
                print(f"⚠️  Remote host detected: {args.host}")
                print("   For remote instances, use --no-start-portals flag")
                print(
                    "   Example: --host https://your-remote-instance.com --no-start-portals"
                )
                sys.exit(1)

            portals_process = start_portals(args.host, args.port)
            if not portals_process:
                print("❌ Failed to start Portals")
                sys.exit(1)
        # Just check if it's running
        elif not check_portals_running(args.host):
            print(f"❌ Portals is not running at {args.host}")
            if args.host.startswith("https://"):
                print("   Make sure your remote Portals instance is accessible")
            else:
                print("Either start Portals manually or remove --no-start-portals flag")
            sys.exit(1)
        else:
            print(f"🔗 Using existing Portals instance at {args.host}")
            if args.host.startswith("https://"):
        ...
        # Clean up Portals process
        if portals_process:
            print("\nStopping Portals server...")
            portals_process.terminate()
            try:
                portals_process.wait(timeout=10)
            except subprocess.TimeoutExpired:
                portals_process.kill()
            print("✅ Portals server stopped")
