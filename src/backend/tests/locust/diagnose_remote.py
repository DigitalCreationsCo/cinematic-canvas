#!/usr/bin/env python3
"""Diagnostic tool for remote Portals instances.
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                if results["success"]:
                    print(
                        f"   ✅ Flow execution successful ({results['response_time_ms']}ms)"
                    )
                else:
                    print(
                        f"   ⚠️  Flow executed but no outputs ({results['response_time_ms']}ms)"
                    )
...
def run_load_simulation(
    host: str, api_key: str, flow_id: str, num_requests: int = 10
) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                results["errors"].append(
                    f"Request {i + 1}: HTTP {response.status_code}"
                )
                print(
                    f"   ❌ Request {i + 1}: HTTP {response.status_code} ({response_time}ms)"
                )
...
def main():
    parser = argparse.ArgumentParser(description="Diagnose remote Portals instance")
    parser.add_argument("--host", required=True, help="Portals host URL")
    parser.add_argument("--api-key", help="API key for flow execution")
    parser.add_argument("--flow-id", help="Flow ID for testing")
    parser.add_argument(
        "--load-test", type=int, default=0, help="Number of requests for mini load test"
    )
    parser.add_argument("--output", help="Save results to JSON file")

    args = parser.parse_args()

    print(f"🔍 Diagnosing Portals instance: {args.host}")
    print("=" * 60)
    ...
    if args.load_test > 0 and args.api_key and args.flow_id:
        load_results = run_load_simulation(
            args.host, args.api_key, args.flow_id, args.load_test
        )
    ...
    print(
        f"Connectivity: {'✅ OK' if connectivity_results['reachable'] else '❌ FAILED'}"
    )
    print(
        f"Health Check: {'✅ OK' if connectivity_results['health_check'] else '❌ FAILED'}"
    )
    ...
    if load_results:
        success_rate = (
            load_results["successful_requests"] / load_results["total_requests"]
        ) * 100
        print(
    ...
    if not connectivity_results["reachable"]:
        ...
        print("❌ Health check failed - Portals may not be running properly")
    elif flow_results and not flow_results["success"]:
        print(
            "❌ Flow execution failed - check API key, flow ID, and flow configuration"
        )
