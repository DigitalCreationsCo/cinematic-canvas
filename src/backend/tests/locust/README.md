# Portals Load Testing

This directory contains comprehensive load testing tools for both Portals and PX APIs using Locust.

## 🔧 **Two Testing Systems**

### **Portals API Testing** (Enhanced System)

- Files: `portals_*.py`
- Tests the main Portals application API
- Includes automatic setup, real starter projects, and comprehensive error logging

### **PX API Testing** (Complex Serve)

- Files: `px_*.py`
- Tests the PX complex serve API
...
- **Enhanced Error Logging**: Detailed connection error analysis and Portals log capture
- **Easy Setup**: One-command execution with automatic Portals startup
...
For testing against a remote Portals instance:

### Setup for Remote Testing

```bash
# Using Makefile (recommended)
make load_test_remote_setup PORTALS_HOST="https://your-remote-instance.com"
make load_test_remote_run PORTALS_HOST="https://your-remote-instance.com"

# Or using Python scripts directly
python portals_setup_test.py --host https://your-remote-instance.com --interactive
python portals_run_load_test.py --host https://your-remote-instance.com --no-start-portals --headless --users 10 --duration 120
...
- **Always use `--no-start-portals`** when testing remote instances
...
Choose and set up a real Portals starter project for testing:

```bash
# Interactive flow selection
python portals_setup_test.py --interactive

# Use specific flow
python portals_setup_test.py --flow "Memory Chatbot"

# List available flows
python portals_setup_test.py --list-flows
```

This will:

- Use default Portals credentials (portals/portals)
- Generate API keys
...
python portals_run_load_test.py

# Headless mode with 20 users for 2 minutes
python portals_run_load_test.py --headless --users 20 --duration 120

# Use predefined load shape
python portals_run_load_test.py --shape ramp100 --headless --users 100 --duration 180
```
...
python portals_setup_test.py --interactive --save-credentials my_test_creds.json

# Test against existing remote Portals instance
python portals_run_load_test.py --host https://your-remote-instance.com --no-start-portals

# Save results to CSV and HTML
python portals_run_load_test.py --headless --csv results --html report.html --users 50 --duration 300

# Direct Locust usage (after setup)
export API_KEY="your-api-key-from-setup"
export FLOW_ID="your-flow-id-from-setup"
locust -f portals_locustfile.py --host http://localhost:7860

# Distributed testing (master)
locust -f portals_locustfile.py --host http://localhost:7860 --master

# Distributed testing (worker)
locust -f portals_locustfile.py --host http://localhost:7860 --worker --master-host=localhost
```
...
- `PORTALS_HOST`: Base URL for Portals server (default: http://localhost:7860)
- `SHAPE`: Load test shape (ramp100, stepramp)
- `REQUEST_TIMEOUT`: Request timeout in seconds (default: 30.0)

## Architecture

### Setup Process (`portals_setup_test.py`)

1. **Health Check**: Verify Portals is running
2. **Flow Selection**: Choose from 40+ real starter project flows
3. **Authentication**: Login with default credentials (portals/portals)
...
Instead of simple test flows, the system uses real Portals starter projects:
...
1. **Setup Failed**: Ensure Portals is accessible and not in read-only mode
2. **Authentication Errors**: Verify default credentials (portals/portals) are enabled
...
1. Run Portals manually with `--log-level debug`
2. Check the Portals logs for detailed error information
...
1. Start Portals: `python -m portals run --auto-login`
...
1. Inherit from `BasePortalsUser`
...
```bash
python portals_run_load_test.py --headless --users 10 --duration 60
```

### Stress Test

```bash
python portals_run_load_test.py --shape ramp100 --headless --users 100 --duration 300
```

### Performance Profiling

```bash
python portals_run_load_test.py --shape stepramp --headless --csv profile_results
```

### Production Readiness Test

```bash
python portals_run_load_test.py --users 50 --duration 600 --csv production_test --html production_report.html
```
...
python portals_run_load_test.py --headless --users 25 --duration 120 --html detailed_report.html

# Combined CSV + HTML reporting
python portals_run_load_test.py --headless --users 100 --duration 300 --csv data --html analysis.html --shape ramp100

# Quick test with report
python portals_setup_test.py --flow "Memory Chatbot"
python portals_run_load_test.py --headless --users 10 --duration 60 --html quick_test.html
```
