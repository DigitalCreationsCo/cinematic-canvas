"""Portals API Locust Load Testing File.

Comprehensive load testing for Portals API with multiple user behaviors and performance analysis.
...
Environment Variables:
    - PORTALS_HOST: Base URL for the Portals server (default: http://localhost:7860)
...
ERROR_LOG_FILE = None
PORTALS_LOG_FILE = None
DETAILED_ERRORS = []


def setup_error_logging():
    """Set up detailed error logging for the load test."""
    global ERROR_LOG_FILE, PORTALS_LOG_FILE

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # Create detailed error log
    ERROR_LOG_FILE = f"portals_load_test_detailed_errors_{timestamp}.log"

    # Set up error logger
    error_logger = logging.getLogger("portals_load_test_errors")
    ...
    for log_path in portals_log_paths:
        if Path(log_path).exists():
            PORTALS_LOG_FILE = log_path
            break

    print("📝 Error logging setup:")
    print(f"   • Detailed errors: {ERROR_LOG_FILE}")
    if PORTALS_LOG_FILE:
        print(f"   • Portals logs: {PORTALS_LOG_FILE}")
    else:
        print("   • Portals logs: Not found (will monitor common locations)")
...
def save_error_summary():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    summary_file = f"portals_load_test_error_summary_{timestamp}.json"
...
def capture_portals_logs():
    """Capture recent Portals logs if available."""
    if not PORTALS_LOG_FILE or not Path(PORTALS_LOG_FILE).exists():
        return None

    try:
        # Read last 1000 lines of Portals log
        with open(PORTALS_LOG_FILE) as f:
            lines = f.readlines()
            recent_lines = lines[-1000:] if len(lines) > 1000 else lines

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        captured_log_file = f"portals_server_logs_during_test_{timestamp}.log"

        with open(captured_log_file, "w") as f:
            f.write("# Portals server logs captured during load test\n")
            f.write(f"# Original log file: {PORTALS_LOG_FILE}\n")
            f.write(f"# Capture time: {datetime.now().isoformat()}\n")
            f.write(f"# Lines captured: {len(recent_lines)}\n\n")
            f.writelines(recent_lines)

        print(f"📋 Portals logs captured: {captured_log_file}")
        return captured_log_file

    except Exception as e:
        print(f"⚠️  Could not capture Portals logs: {e}")
        return None
...
def on_test_stop(environment, **_kwargs):
    ...
    # Capture Portals logs
    capture_portals_logs()
...
class BasePortalsUser(FastHttpUser):
    """Base class for all Portals API load testing user types."""

    abstract = True
    REQUEST_TIMEOUT = float(os.getenv("REQUEST_TIMEOUT", "30.0"))

    # Use the host provided by environment variable or default
    host = os.getenv("PORTALS_HOST", "http://localhost:7860")
        ...
        try:
            with self.client.post(
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        # Portals API success check - look for outputs
                        if data.get("outputs"):
...
class NormalUser(BasePortalsUser):
...
class AggressiveUser(BasePortalsUser):
...
class SustainedLoadUser(BasePortalsUser):
...
class TailLatencyHunter(BasePortalsUser):
...
class ScalabilityTestUser(BasePortalsUser):
...
class BurstUser(BasePortalsUser):
