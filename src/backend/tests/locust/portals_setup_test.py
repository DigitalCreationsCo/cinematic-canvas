#!/usr/bin/env python3
"""Portals Load Test Setup CLI

This script sets up a complete Portals test environment by:
1. Starting Portals (optional)
...
Usage:
    python setup_portals_test.py --help
    python setup_portals_test.py --interactive
    python setup_portals_test.py --flow "Basic Prompting"
    python setup_portals_test.py --list-flows
"""
...
async def get_starter_projects_from_api(host: str, access_token: str) -> list[dict]:
    """Get starter projects from Portals API."""
    import httpx
...
async def list_available_flows(host: str, access_token: str) -> list[tuple[str, str, str]]:
    """List all available starter project flows from Portals API.

    Args:
        host: Portals host URL
...
async def setup_portals_environment(host: str, flow_name: str | None = None, interactive: bool = False) -> dict:
    """Set up complete Portals environment with real starter project flows."""
    try:
        import httpx
    except ImportError:
    ...
    # Configuration - use default Portals credentials
    username = "portals"
    password = "portals"
    ...
    async with httpx.AsyncClient(base_url=host, timeout=60.0) as client:
        ...
        try:
            ...
            # Remove the id to let Portals generate a new one
...
def print_setup_results(setup_state: dict):
    ...
    print(f"export PORTALS_HOST='{setup_state['host']}'")
    ...
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-portals --headless --users 25 --duration 120"
    )
    print()

    print("# Generate HTML report:")
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-portals --headless --users 50 --duration 180 --html report.html"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Set up Portals load test environment with real starter project flows",
...
Examples:
  ...
  # Use specific flow
  python setup_portals_test.py --flow "Memory Chatbot"

  # List available flows
  python setup_portals_test.py --list-flows

  # Setup with custom host
  python setup_portals_test.py --host http://localhost:8000 --interactive

  # Save credentials to file
  python setup_portals_test.py --interactive --save-credentials test_creds.json
        """,
    )
        ...
        async def list_flows_only():
            ...
            async with httpx.AsyncClient(base_url=args.host, timeout=30.0) as client:
                ...
                try:
                    health_response = await client.get("/health")
                    if health_response.status_code != 200:
                        raise Exception(f"Portals not available at {args.host}")
                except Exception as e:
                    print(f"❌ Cannot connect to Portals at {args.host}: {e}")
                    sys.exit(1)
                ...
                try:
                    ...
                    print(f"❌ Authentication failed: {e}")
                    print("Make sure Portals is running with default credentials (portals/portals)")
                    sys.exit(1)
    ...
    try:
        ...
        setup_state = asyncio.run(
            setup_portals_environment(host=args.host, flow_name=args.flow, interactive=args.interactive)
        )
