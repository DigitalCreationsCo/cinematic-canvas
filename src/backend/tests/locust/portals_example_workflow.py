#!/usr/bin/env python3
"""Example Portals Load Testing Workflow

This script demonstrates the complete workflow for setting up and running
Portals load tests with real starter project flows.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Example Portals Load Testing Workflow")
    parser.add_argument("--auto", action="store_true", help="Run automatically without user input prompts")
    args = parser.parse_args()

    print("🚀 Portals Load Testing Example Workflow")
    ...
    setup_script = script_dir / "portals_setup_test.py"
    runner_script = script_dir / "portals_run_load_test.py"
    ...
    try:
        ...
        try:
            ...
            env = os.environ.copy()
            env["PORTALS_HOST"] = creds["host"]
            env["API_KEY"] = creds["api_key"]
            env["FLOW_ID"] = creds["flow_id"]

            print("   🔧 Setting environment variables:")
            print(f"      PORTALS_HOST={creds['host']}")
        ...
        success = run_command_with_env(
            [
                ...
                "--no-start-portals",
                "--html",
                "portals_load_test_report.html",
                "--csv",
                "portals_load_test_results",
            ],
        ...
        print("   python setup_portals_test.py --interactive")
        print()
        ...
        print("   - portals_load_test_report.html (detailed HTML report)")
        print("   - portals_load_test_results_*.csv (CSV data files)")
        print("   - portals_load_test_detailed_errors_*.log (detailed error logs)")
        print("   - portals_load_test_error_summary_*.json (error analysis)")
        print("   - portals_server_logs_during_test_*.log (Portals server logs)")
        print()
        print("🧹 Clean up:")
        print("   - Remove test flows from Portals UI")
        ...
        if args.auto:
            ...
            print("   - portals_load_test_report.html")
            print("   - portals_load_test_results_*.csv")
        else:
            ...
            if cleanup_response == "y":
                files_to_clean = [
                    "example_test_creds.json",
                    "portals_load_test_report.html",
                    "portals_load_test_results_failures.csv",
                    "portals_load_test_results_stats.csv",
                    "portals_load_test_results_stats_history.csv",
                    "portals_load_test_results_exceptions.csv",
                ]

                # Also clean up error logs (they have timestamps, so use glob pattern)
                import glob

                error_files = glob.glob("portals_load_test_detailed_errors_*.log")
                error_files.extend(glob.glob("portals_load_test_error_summary_*.json"))
                error_files.extend(glob.glob("portals_server_logs_during_test_*.log"))
        ...
        print("You're now ready to use Portals load testing for your own projects.")
        print()
        print("📊 View your load test results:")
        print("   • Open portals_load_test_report.html in your browser for detailed analysis")
        print("   • Check portals_load_test_results_*.csv for raw data")
        print("   • Review portals_load_test_detailed_errors_*.log for comprehensive error details")
        print("   • Analyze portals_load_test_error_summary_*.json for error patterns")
        print("   • Examine portals_server_logs_during_test_*.log for server-side issues")
