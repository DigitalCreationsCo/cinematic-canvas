...
import pytest
from portals.services.storage.s3 import S3StorageService
...
def mock_settings_service():
    ...
    - PORTALS_OBJECT_STORAGE_BUCKET_NAME: S3 bucket name (default: portals-ci)
    - PORTALS_OBJECT_STORAGE_PREFIX: S3 prefix (default: test-files-1)
    - PORTALS_OBJECT_STORAGE_TAGS: S3 tags as JSON string (default: {"env": "test-1"})

    Note: All settings use PORTALS_OBJECT_STORAGE_* names to test that
    the S3StorageService properly respects these settings.
    """
    settings_service = Mock()
    settings_service.settings.config_dir = str(
        Path(tempfile.gettempdir()) / "portals_test"
    )
    ...
    # Prefix from env - using standard PORTALS env var name
    settings_service.settings.object_storage_prefix = os.environ.get(
        "PORTALS_OBJECT_STORAGE_PREFIX", "test-files-1"
    )

    # Tags from env - using standard PORTALS env var name
    default_tags = {"env": "test-1"}
    tags_str = os.environ.get("PORTALS_OBJECT_STORAGE_TAGS")
    if tags_str:
        try:
...
@pytest.fixture
async def s3_storage_service(
    mock_session_service, mock_settings_service, _aws_credentials
):
...
class TestS3StorageServiceInitialization:
    ...
    async def test_initialization(self, s3_storage_service):
        ...
        # This tests that S3StorageService respects PORTALS_OBJECT_STORAGE_PREFIX
        ...
        # This tests that S3StorageService respects PORTALS_OBJECT_STORAGE_TAGS
        default_tags = {"env": "test-1"}
        tags_str = os.environ.get("PORTALS_OBJECT_STORAGE_TAGS")
...
class TestS3StorageServiceFileOperations:
    ...
    async def test_save_file_overwrites_existing(
        self, s3_storage_service, test_flow_id
    ):
...
class TestS3StorageServiceStreamOperations:
    ...
    async def test_get_file_stream(self, s3_storage_service, test_flow_id):
        ...
        try:
            ...
            chunks = [
                chunk
                async for chunk in s3_storage_service.get_file_stream(
                    test_flow_id, file_name, chunk_size=1024
                )
            ]
    ...
    async def test_get_file_stream_not_found(self, s3_storage_service, test_flow_id):
        ...
        with pytest.raises(FileNotFoundError):
            async for _ in s3_storage_service.get_file_stream(
                test_flow_id, "no_file.txt"
            ):
                pass

    async def test_get_file_stream_context_manager_lifecycle(
        self, s3_storage_service, test_flow_id
    ):
        ...
        try:
            ...
            assert streamed_data == data
            assert len(chunks) > 1, (
                "Should have multiple chunks to test context lifecycle"
            )
    ...
    async def test_get_file_stream_early_termination(
        self, s3_storage_service, test_flow_id
    ):
        ...
        try:
            ...
            # Create generator
            gen = s3_storage_service.get_file_stream(
                test_flow_id, file_name, chunk_size=1024
            )
            ...
            assert len(chunks_received) == 3, (
                "Should have received exactly 3 chunks before termination"
            )
            ...
            assert full_data == data, (
                "Should be able to stream full file after early termination"
            )
    ...
    async def test_get_file_stream_multiple_concurrent_streams(
        self, s3_storage_service, test_flow_id
    ):
        ...
        try:
            ...
            gen1 = s3_storage_service.get_file_stream(
                test_flow_id, file_name, chunk_size=1024
            )
            gen2 = s3_storage_service.get_file_stream(
                test_flow_id, file_name, chunk_size=1024
            )
...
class TestS3StorageServiceListOperations:
    ...
    async def test_list_files_excludes_other_flows(
        self, s3_storage_service, test_flow_id
    ):
...
class TestS3StorageServiceEdgeCases:
    ...
    async def test_save_file_with_unicode_content(
        self, s3_storage_service, test_flow_id
    ):
    ...
    async def test_file_name_with_special_characters(
        self, s3_storage_service, test_flow_id
    ):
