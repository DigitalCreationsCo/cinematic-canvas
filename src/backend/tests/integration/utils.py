...
from px.field_typing import Embeddings
from px.graph import Graph
from px.processing.process import run_graph_internal
...
def download_flow_from_github(name: str, version: str) -> JSONFlow:
    response = requests.get(
        f"https://raw.githubusercontent.com/portals-ai/portals/v{version}/src/backend/base/portals/initial_setup/starter_projects/{name}.json",
        timeout=10,
    )
...
def download_component_from_github(
    module: str, file_name: str, version: str
) -> Component:
    version_string = f"v{version}" if version != "main" else version
    response = requests.get(
        f"https://raw.githubusercontent.com/portals-ai/portals/{version_string}/src/backend/base/portals/components/{module}/{file_name}.py",
        timeout=10,
    )
    response.raise_for_status()
...
async def run_flow(
    graph: Graph, run_input: Any | None = None, session_id: str | None = None
) -> dict[str, Any]:
    graph.prepare()
    graph_run_inputs = (
        [InputValueRequest(input_value=run_input, type="chat")] if run_input else []
    )

    flow_id = str(uuid.uuid4())

    results, _ = await run_graph_internal(
        graph, flow_id, session_id=session_id, inputs=graph_run_inputs
    )
    outputs = {}
    for r in results:
...
async def run_single_component(
    ...
    graph.prepare()
    graph_run_inputs = (
        [InputValueRequest(input_value=run_input, type=input_type)] if run_input else []
    )
...
def build_component_instance_for_tests(
    version: str, module: str, file_name: str, **kwargs
):
