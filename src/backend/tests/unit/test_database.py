...
from portals.api.v1.schemas import FlowListCreate, ResultDataResponse
from portals.initial_setup.setup import load_starter_projects
from portals.services.database.models.base import orjson_dumps
from portals.services.database.models.flow import Flow, FlowCreate, FlowUpdate
from portals.services.database.models.folder.model import FolderCreate
from portals.services.database.utils import session_getter
from portals.services.deps import get_db_service
from px.graph.utils import log_transaction, log_vertex_build
...
async def test_read_flows_pagination_with_params(
...
async def test_read_flows_invalid_page(client: AsyncClient, logged_in_headers):
    ...
    )  # Assuming 422 is the status code for invalid input
...
async def test_read_flows_no_pagination_params(client: AsyncClient, logged_in_headers):
    ...
    response = await client.get(
        "api/v1/flows/", headers=logged_in_headers, params={"get_all": False}
    )
    assert response.status_code == 200
...
async def test_read_flows_components_only_paginated(
    client: AsyncClient, logged_in_headers
):
    number_of_flows = 10
    flows = [
        FlowCreate(
            name=f"Flow {i}", description="description", data={}, is_component=True
        )
        for i in range(number_of_flows)
    ]
...
async def test_read_flows_components_only(client: AsyncClient, logged_in_headers):
    ...
    for flow in flows:
        ...
        assert response.status_code == 201
    response = await client.get(
        "api/v1/flows/", headers=logged_in_headers, params={"components_only": True}
    )
    assert response.status_code == 200
    response_json = response.json()
...
async def test_delete_flows(client: AsyncClient, logged_in_headers):
    ...
    response = await client.request(
        "DELETE", "api/v1/flows/", headers=logged_in_headers, json=flow_ids
    )
...
async def test_delete_folder_with_flows_with_transaction_and_build(
    ...
    response = await client.post(
        "api/v1/projects/", json=project.model_dump(), headers=logged_in_headers
    )
    assert response.status_code == 201, (
    ...
    response = await client.request(
        "DELETE", f"api/v1/projects/{folder_id}", headers=logged_in_headers
    )
...
async def test_get_flows_from_folder_pagination_with_params(
    client: AsyncClient, logged_in_headers
):
    # Create a new project
    folder_name = f"Test Project {uuid4()}"
...
async def test_download_file(
    ...
    async with session_getter(db_manager) as _session:
        ...
        flow_ids = [
            str(db_flow.id) for db_flow in saved_flows
...
@pytest.mark.usefixtures("session")
async def test_download_single_flow_returns_normalized_json(
    client: AsyncClient, logged_in_headers
):
    ...
    create_response = await client.post(
        "api/v1/flows/", json=flow_payload.model_dump(), headers=logged_in_headers
    )
    ...
    assert downloaded["data"]["nodes"][0]["data"]["node"]["template"]["code"][
        "value"
    ] == [
        "print('hello')",
        "print('world')",
    ...
    assert (
        downloaded["data"]["nodes"][0]["data"]["node"]["template"]["api_key"]["value"]
        is None
    )


@pytest.mark.usefixtures("session")
async def test_upload_zip_file_to_flows(
...
@pytest.mark.usefixtures("session")
async def test_upload_zip_file_to_projects(
    client: AsyncClient, json_flow: str, logged_in_headers
):
    ...
    project_response = await client.get(
        f"api/v1/projects/{folder_id}", headers=logged_in_headers
    )
...
@pytest.mark.usefixtures("session")
async def test_download_then_upload_roundtrip(
    client: AsyncClient, json_flow: str, active_user, logged_in_headers
):
...
@pytest.mark.usefixtures("session")
async def test_upload_zip_with_invalid_json(
    client: AsyncClient, json_flow: str, logged_in_headers
):
...
async def test_upload_zip_exceeding_max_entries(
    ...
    with zipfile.ZipFile(zip_buffer, "w") as zf:
        for i in range(5):
            zf.writestr(
                f"flow_{i}.json", json.dumps({"name": f"flow_{i}", "data": data})
            )
    zip_buffer.seek(0)
...
@pytest.mark.usefixtures("session")
async def test_upload_zip_with_oversized_entry(
    client: AsyncClient, json_flow: str, logged_in_headers, monkeypatch
):
...
@pytest.mark.usefixtures("session")
async def test_upload_zip_with_mixed_valid_invalid(
    client: AsyncClient, json_flow: str, logged_in_headers, monkeypatch
):
...
@pytest.mark.usefixtures("session")
async def test_upload_zip_to_projects_filename_none(
    client: AsyncClient, json_flow: str, logged_in_headers
):
...
@pytest.mark.usefixtures("session")
async def test_upload_json_file_to_projects_rejoins_code_lines(
    client: AsyncClient, logged_in_headers
):
    ...
    assert (
        response_data[0]["data"]["nodes"][0]["data"]["node"]["template"]["code"][
            "value"
        ]
        == "print('alpha')\nprint('beta')"
    )
    ...
    project_response = await client.get(
        f"api/v1/projects/{response_data[0]['folder_id']}", headers=logged_in_headers
    )
    assert project_response.status_code == 200
...
@pytest.mark.usefixtures("session")
async def test_download_project_zip_sanitizes_flow_names(
    client: AsyncClient, json_flow: str, logged_in_headers
):
...
@pytest.mark.usefixtures("session")
async def test_upload_no_file_to_flows_returns_400(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("session")
async def test_upload_no_file_to_projects_returns_400(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("session")
async def test_upload_empty_file_to_flows_returns_400(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("session")
async def test_upload_empty_file_to_projects_returns_400(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("session")
async def test_upload_invalid_json_to_flows_returns_400(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("session")
async def test_upload_invalid_json_to_projects_returns_400(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("session")
async def test_upload_zip_to_projects_batch_name_dedup(
    client: AsyncClient, json_flow: str, logged_in_headers
):
    ...
    with zipfile.ZipFile(zip_buffer, "w") as zf:
        for i in range(3):
            zf.writestr(
                f"flow_{i}.json", json.dumps({"name": "duplicate_name", "data": data})
            )
    zip_buffer.seek(0)
...
@pytest.mark.usefixtures("active_user")
async def test_update_flow_idempotency(
    client: AsyncClient, json_flow: str, logged_in_headers
):
    flow_data = orjson.loads(json_flow)
    data = flow_data["data"]
...
@pytest.mark.usefixtures("active_user")
async def test_update_nonexistent_flow(
    client: AsyncClient, json_flow: str, logged_in_headers
):
    flow_data = orjson.loads(json_flow)
    data = flow_data["data"]
...
async def test_read_only_starter_projects(client: AsyncClient, logged_in_headers):
    response = await client.get(
        "api/v1/flows/basic_examples/", headers=logged_in_headers
    )
...
@pytest.mark.usefixtures("active_user")
async def test_read_folder_with_flows(
    client: AsyncClient, json_flow: str, logged_in_headers
):
    # Create a new project
    folder_name = f"Test Project {uuid4()}"
...
async def test_read_nonexistent_folder(client: AsyncClient, logged_in_headers):
    ...
    response = await client.get(
        f"api/v1/projects/{nonexistent_id}", headers=logged_in_headers
    )
    assert response.status_code == 404
...
@pytest.mark.usefixtures("active_user")
async def test_read_folder_with_search(
    client: AsyncClient, json_flow: str, logged_in_headers
):
    # Create a new project
    folder_name = f"Test Project {uuid4()}"
    ...
    await client.post(
        "api/v1/flows/", json=flow1.model_dump(), headers=logged_in_headers
    )
    await client.post(
        "api/v1/flows/", json=flow2.model_dump(), headers=logged_in_headers
    )
...
@pytest.mark.usefixtures("active_user")
async def test_read_folder_with_component_filter(
    client: AsyncClient, json_flow: str, logged_in_headers
):
    # Create a new project
    folder_name = f"Test Project {uuid4()}"
    ...
    await client.post(
        "api/v1/flows/", json=component_flow.model_dump(), headers=logged_in_headers
    )
