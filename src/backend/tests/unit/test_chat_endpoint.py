...
from portals.services.database.models.flow import FlowUpdate
from portals.services.job_queue.service import JobQueueService
from px.log.logger import logger
from px.memory import aget_messages
...
def allow_custom_components_by_default(monkeypatch):
    monkeypatch.setenv("PORTALS_ALLOW_CUSTOM_COMPONENTS", "true")
...
@pytest.mark.benchmark
async def test_build_flow_from_request_data(
    ...
    build_response = await build_flow(
        client, flow_id, logged_in_headers, json={"data": flow_data["data"]}
    )
    job_id = build_response["job_id"]
...
async def test_build_flow_validates_request_data_instead_of_stale_db_flow(
    ...
    saved_flow_validation_message = (
        "saved flow should not be validated when request data is provided"
    )
    ...
    monkeypatch.setattr(
        "portals.api.v1.chat.validate_flow_for_current_settings",
...
async def test_build_flow_with_frozen_path(
    client, json_memory_chatbot_no_llm, logged_in_headers
):
    ...
    response = await client.patch(
        ...
        json=FlowUpdate(
            name="Flow", description="description", data=flow_data["data"]
        ).model_dump(),
        headers=logged_in_headers,
    )
...
async def test_build_flow_invalid_flow_id(client, logged_in_headers):
    ...
    response = await client.post(
        f"api/v1/build/{invalid_flow_id}/flow", json={}, headers=logged_in_headers
    )
    assert response.status_code == codes.NOT_FOUND


@pytest.mark.benchmark
async def test_build_flow_start_only(
    client, json_memory_chatbot_no_llm, logged_in_headers
):
...
@pytest.mark.benchmark
async def test_build_flow_start_with_inputs(
    client, json_memory_chatbot_no_llm, logged_in_headers
):
    ...
    build_response = await build_flow(
        client, flow_id, logged_in_headers, json=test_inputs
    )
...
@pytest.mark.benchmark
async def test_build_flow_polling(
    client, json_memory_chatbot_no_llm, logged_in_headers
):
    ...
    build_response = await build_flow(client, flow_id, logged_in_headers)
    assert "job_id" in build_response, (
        f"Expected job_id in build_response, got {build_response}"
    )
    job_id = build_response["job_id"]
    assert job_id is not None
            ...
            try:
                ...
                while (
                    ...
                    try:
                        ...
                        logger.debug(
                            f"WARNING: Polling request timed out after {self.poll_timeout}s"
                        )
                        empty_polls += 1
                        continue
...
@pytest.mark.benchmark
async def test_cancel_build_unexpected_error(
    client, json_memory_chatbot_no_llm, logged_in_headers, monkeypatch
):
    ...
    import portals.api.v1.chat

    original_cancel_flow_build = portals.api.v1.chat.cancel_flow_build
    ...
    monkeypatch.setattr(
        portals.api.v1.chat, "cancel_flow_build", mock_cancel_flow_build_with_error
    )

    try:
        # Try to cancel the build - should return 500 Internal Server Error
        cancel_response = await client.post(
            f"api/v1/build/{job_id}/cancel", headers=logged_in_headers
        )
        ...
        monkeypatch.setattr(
            portals.api.v1.chat, "cancel_flow_build", original_cancel_flow_build
        )
...
async def test_cancel_build_success(
    ...
    monkeypatch.setattr(
        portals.api.v1.chat, "cancel_flow_build", mock_successful_cancel_flow_build
    )
...
async def test_cancel_nonexistent_build(client, logged_in_headers):
    ...
    response = await client.post(
        f"api/v1/build/{invalid_job_id}/cancel", headers=logged_in_headers
    )
    assert response.status_code == codes.NOT_FOUND
    assert "Job not found" in response.json()["detail"]
...
@pytest.mark.benchmark
async def test_cancel_build_failure(
    client, json_memory_chatbot_no_llm, logged_in_headers, monkeypatch
):
    ...
    monkeypatch.setattr(
        portals.api.v1.chat, "cancel_flow_build", mock_cancel_flow_build
    )
...
@pytest.mark.benchmark
async def test_cancel_build_with_cancelled_error(
    client, json_memory_chatbot_no_llm, logged_in_headers, monkeypatch
):
...
@pytest.mark.benchmark
async def test_build_public_tmp_ignores_data_parameter(
    client, json_memory_chatbot_no_llm, logged_in_headers
):
    ...
    malicious_data = {
        "nodes": [{"id": "malicious", "data": {"type": "CustomComponent"}}],
        "edges": [],
    }
...
async def test_build_public_tmp_checks_public_access_before_validation(
    ...
    public_access_validation_message = (
        "validation should not run before public access checks"
    )
...
async def test_build_flow_cross_user_blocked(
    ...
    """
    victim_flow_id = await create_flow(
        client, json_memory_chatbot_no_llm, logged_in_headers
    )
    ...
    response = await client.post(
        f"api/v1/build/{victim_flow_id}/flow", json={}, headers=attacker_headers
    )
...
@pytest.mark.benchmark
@pytest.mark.security
async def test_build_flow_unauthenticated_blocked(
    client, json_memory_chatbot_no_llm, logged_in_headers
):
...
async def test_build_flow_nonexistent_flow_returns_404(client, logged_in_headers):
    ...
    response = await client.post(
        f"api/v1/build/{nonexistent_id}/flow", json={}, headers=logged_in_headers
    )
    assert response.status_code == 404
...
@pytest.mark.benchmark
@pytest.mark.security
async def test_build_events_cross_user_blocked(
    client, json_memory_chatbot_no_llm, logged_in_headers, user_two
):
...
async def test_build_flow_public_flow_accessible_by_other_user(
    ...
    response = await client.post(
        f"api/v1/build/{flow_id}/flow", json={}, headers=other_headers
    )
    assert response.status_code == 200
...
@pytest.mark.benchmark
@pytest.mark.security
async def test_cancel_build_cross_user_blocked(
    client, json_memory_chatbot_no_llm, logged_in_headers, user_two
):
    ...
    response = await client.post(
        f"api/v1/build/{job_id}/cancel", headers=attacker_headers
    )
    assert response.status_code == 404


@pytest.mark.benchmark
async def test_build_public_tmp_without_data_parameter(
    client, json_memory_chatbot_no_llm, logged_in_headers
):
...
async def test_get_build_events_public_tmp_job_accessible_by_any_auth_user(
    ...
    monkeypatch.setattr(
        portals.api.v1.chat, "get_flow_events_response", mock_get_flow_events_response
    )
...
async def test_cancel_build_public_tmp_job_accessible_by_any_auth_user(
    ...
    cancel_response = await client.post(
        f"api/v1/build/{job_id}/cancel", headers=other_headers
    )
    assert cancel_response.status_code == codes.OK
...
def _stub_start_flow_build(monkeypatch, captured: dict) -> None:
    ...
    import portals.api.v1.chat as chat_module
...
async def test_build_public_tmp_namespacing_blocks_memory_query_collision(
    ...
    """
    from px.memory import aadd_messages, aget_messages
...
def test_scope_session_to_namespace_helper():
    from portals.api.utils import scope_session_to_namespace
    ...
    assert (
        scope_session_to_namespace("victim-session", "namespace-B")
        == "namespace-B:victim-session"
    )
    # A foreign-namespace prefix is treated as out-of-namespace and gets re-wrapped.
    assert (
        scope_session_to_namespace("namespace-B:victim", "namespace-A")
        == "namespace-A:namespace-B:victim"
    )
