...
def _make_app(options=None, env_args=None, monkeypatch=None):
    """Create a PortalsApplication with a dummy WSGI app.

    Args:
        options: Programmatic options passed to PortalsApplication.
    ...
    return PortalsApplication(dummy_app, options=options)
...
class TestGunicornEnvArgs:
    def test_env_args_applied(self, monkeypatch):
        ...
        app = _make_app(
            env_args="--max-requests 100 --max-requests-jitter 20",
    ...
    def test_env_does_not_override_worker_class(self, monkeypatch):
        ...
        assert (
            app.cfg.settings["worker_class"].get()
            == "portals.server.PortalsUvicornWorker"
        )
