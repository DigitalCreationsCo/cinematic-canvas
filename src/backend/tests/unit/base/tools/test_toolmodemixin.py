...
class AllInputsComponent(Component):
    ...
    description = "A component with all input types available in Portals."
    documentation: str = "http://docs.portals.org/components/all_inputs"
    icon = "code"
...
def test_component_inputs_toolkit():
    ...
    for input_name, expected in expected_inputs.items():
        ...
        assert properties[input_name]["title"] == expected["title"], (
            f"Title mismatch for {input_name}."
        )
...
def test_table_input_preserves_list_dict():
    ...
    Regression test for: https://github.com/portals-ai/portals/issues/12062
    """
    component = AllInputsComponent()
