...
import pytest
from px.base.tools.run_flow import RunFlowBaseComponent
...
def mock_shared_cache():
    ...
    with patch("px.base.tools.run_flow.get_shared_component_cache_service") as mock_get_cache:
...
class TestRunFlowBaseComponentFlowRetrieval:
    ...
    async def test_get_flow_with_id(self):
        ...
        with patch("px.base.tools.run_flow.get_flow_by_id_or_name", new_callable=AsyncMock) as mock_get:
    ...
    async def test_get_graph_fetches_and_caches_when_not_cached(self):
        ...
        with (
            ...
            patch("px.base.tools.run_flow.Graph.from_payload") as mock_from_payload,
        ):
