...
import pytest
from px.base.mcp import util
from px.base.mcp.util import (
    MCPSessionManager,
    MCPSseClient,
    MCPStdioClient,
...
class TestGlobalVariableResolution:
    ...
    def test_resolve_global_variables_basic(self):
        ...
        from px.base.mcp.util import _resolve_global_variables_in_headers
    ...
    def test_resolve_global_variables_special_characters(self):
        ...
        request_variables = {
            "MY_TOKEN": "Bearer token-with-special!@#$%^&*()_+-=[]{}|;:,.<>?"
        }
...
class TestUpdateToolsStdioHeaders:
    ...
    async def test_stdio_headers_injected_with_existing_headers_flag(self):
        ...
        assert (
            "--headers authorization 'Bearer token123' --headers x-api-key sk-existing"
            in full_command
        )
    ...
    async def test_stdio_headers_injected_without_existing_headers_flag(self):
        ...
        assert (
            "--headers x-api-key my-key http://localhost:7860/streamable"
            in full_command
        )
    ...
    async def test_stdio_no_headers_leaves_args_unchanged(self):
        ...
        assert (
            full_command
            == "uvx mcp-proxy --transport streamablehttp http://localhost/streamable"
        )
    ...
    async def test_stdio_headers_appended_when_all_args_are_flags(self):
        ...
        assert (
            full_command
            == "some-tool --verbose --debug --headers authorization 'Bearer tok'"
        )
    ...
    async def test_stdio_headers_appended_when_last_token_is_flag_value(self):
        ...
        assert (
            full_command == "some-tool --port 8080 --headers authorization 'Bearer tok'"
        )
    ...
    async def test_stdio_headers_inserted_before_positional_with_flag_value_pairs(self):
        ...
        assert (
            full_command
            == "uvx mcp-proxy --port 8080 --headers x-api-key my-key http://localhost/streamable"
        )

    @pytest.mark.asyncio
    async def test_stdio_headers_inserted_before_last_positional_with_multiple_positionals(
        self,
    ):
...
class TestUpdateToolsPerToolResilience:
    ...
    async def test_one_bad_tool_does_not_drop_the_other_tools_issue_11229(self):
        ...
        from px.schema.json_schema import (
            create_input_schema_from_json_schema as real_converter,
        )
        ...
        with (
            patch(
                "px.base.mcp.util.create_input_schema_from_json_schema",
                side_effect=selective_converter,
            ),
            patch("px.base.mcp.util.logger") as mock_logger,
        ):
        ...
        assert "bad_linear_like" in joined, (
            f"failing tool name must appear in log; got: {joined!r}"
        )
    ...
    async def test_resilience_under_many_mixed_tools(self):
        ...
        assert loaded == expected_good, (
            f"expected exactly the 10 healthy tools, got {loaded}"
        )
...
class TestFieldNameConversion:
    ...
    def test_convert_field_names_no_match(self):
        ...
        # Create test schema
        test_schema = create_model(
            "TestSchema", expected_field=(str, Field(..., description="Expected field"))
        )
    ...
    def test_convert_field_names_empty_input(self):
        ...
        test_schema = create_model(
            "TestSchema", test_field=(str, Field(..., description="Test field"))
        )
    ...
    def test_json_schema_alias_functionality(self):
        ...
        from px.schema.json_schema import create_input_schema_from_json_schema
    ...
    async def test_tool_empty_arguments_error_handling(self):
        ...
        tool_coroutine = util.create_tool_coroutine(
            "test_tool", input_schema, mock_client
        )

        # Test that calling with no arguments gives a helpful error message
        with pytest.raises(
            ValueError, match="requires arguments but none were provided"
        ) as exc_info:
...
class TestToolExecutionWithFieldConversion:
    ...
    def test_create_tool_coroutine_with_camel_case_fields(self):
        ...
        tool_coroutine = util.create_tool_coroutine(
            "test_tool", test_schema, mock_client
        )
    ...
    def test_create_tool_func_with_camel_case_fields(self):
        ...
        with patch(
            "px.base.mcp.util.run_until_complete", return_value="tool_result"
    ...
    async def test_tool_coroutine_field_conversion_end_to_end(self):
        ...
        mock_client.run_tool.assert_called_once_with(
            "test_tool", arguments={"weather_main": "Snow", "top_n": 6}
        )
    ...
    def test_tool_func_field_conversion_sync(self):
        ...
        # Mock run_until_complete from async_helpers
        with patch(
            "px.base.mcp.util.run_until_complete", return_value="sync_result"
        ) as mock_run_until_complete:
...
class TestMCPUtilityFunctions:
    ...
    def test_nested_dict_preservation_issue_9881(self):
        ...
        deep_input = {
            "msg": {"level1": {"level2": {"level3": "value"}}},
            "msg_type": "test",
            "topic": "/test",
        }
    ...
    def test_object_with_additional_properties_false_still_drops_extras(self):
        ...
        dumped = strict_model.model_validate(
            {"filter": {"id": "abc", "rogue": "x"}}
        ).model_dump(
            exclude_none=True,
        )
    ...
    def test_top_level_strict_mode_when_root_forbids_additional_properties(self):
        ...
        model = util.create_input_schema_from_json_schema(schema)
        dumped = model.model_validate({"declared": "ok", "rogue": "x"}).model_dump(
            exclude_none=True
        )
    ...
    def test_deeply_nested_dict_round_trip_preserves_all_levels(self):
        ...
        deep = {
            "envelope": {
                "level1": {
                    "level2": {"level3": {"level4": {"leaf": "value", "n": 42}}}
                },
            },
        }
        out = model.model_validate(deep).model_dump(exclude_none=True)
        assert (
            out["envelope"]["level1"]["level2"]["level3"]["level4"]["leaf"] == "value"
        )
    ...
    async def test_list_all_tools(self, stdio_client):
        ...
        try:
            ...
            for expected_tool in expected_tools:
                assert any(tool.name == expected_tool for tool in tools), (
...
class TestMCPStructuredTool:
    ...
    def mcp_tool(self, test_schema, mock_client):
        ...
        from px.base.mcp.util import create_tool_coroutine, create_tool_func
        ...
        class MCPStructuredTool(StructuredTool):
            ...
            def _run(self, *args, config: RunnableConfig, run_manager=None, **kwargs):
                from px.base.mcp.util import _convert_mcp_result
            ...
            async def _arun(
                self, *args, config: RunnableConfig, run_manager=None, **kwargs
            ):
                from px.base.mcp.util import _convert_mcp_result
            ...
            def _convert_parameters(self, input_dict):
                ...
                from px.base.agents.utils import maybe_unflatten_dict
                from px.base.mcp.util import _camel_to_snake
    ...
    def test_run_passes_config_and_kwargs(self, mcp_tool, mock_client):
        ...
        # Mock run_until_complete from async_helpers
        with patch("px.base.mcp.util.run_until_complete", return_value="tool_result"):
...
class TestNormalizeArgumentsForMcp:
    ...
    def test_str_to_dict_when_dict_expected(self):
        ...
        result = util._normalize_arguments_for_mcp(
            {"params": '{"x": 1}'}, Schema, "test_tool"
        )
    ...
    def test_already_correct_types_unchanged(self):
        ...
        result = util._normalize_arguments_for_mcp(
            {"count": 42, "params": {"search": "x"}}, Schema, "test_tool"
        )
    ...
    def test_optional_field_none_unchanged(self):
        ...
        result = util._normalize_arguments_for_mcp(
            {"count": 1, "optional": None}, Schema, "test_tool"
        )
    ...
    def test_extra_keys_preserved_for_pydantic_validation(self):
        ...
        result = util._normalize_arguments_for_mcp(
            {"valid_field": 1, "typo_field": 2}, Schema, "test_tool"
        )
    ...
    def test_invalid_json_for_dict_raises_clear_error(self):
        ...
        with pytest.raises(ValueError, match=r"expects object \(dict\)") as exc_info:
            util._normalize_arguments_for_mcp(
                {"params": "not valid json"}, Schema, "my_tool"
            )
    ...
    def test_invalid_json_for_list_raises_clear_error(self):
        ...
        with pytest.raises(ValueError, match=r"expects array \(list\)") as exc_info:
            util._normalize_arguments_for_mcp(
                {"items": "{invalid"}, Schema, "list_tool"
            )
    ...
    def test_str_to_dict_when_nested_model_expected(self):
        ...
        result = util._normalize_arguments_for_mcp(
            {"params": json_str}, Schema, "test_tool"
        )
    ...
    def test_nested_model_already_dict_unchanged(self):
        ...
        result = util._normalize_arguments_for_mcp(
            {"params": {"search": "test"}}, Schema, "test_tool"
        )
...
class TestSnakeToCamelConversion:
    ...
    def test_snake_to_camel_basic(self):
        ...
        from px.schema.json_schema import _snake_to_camel
...
class TestStripNoneRecursive:
    ...
    def test_should_remove_none_from_flat_dict(self):
        ...
        from px.base.mcp.util import _strip_none_recursive
...
class TestConvertMcpResult:
    ...
    def _make_resource_block(
        self, uri: str = "file:///data.csv", mime: str = "text/csv"
    ):
        block = MagicMock()
        block.type = "resource"
...
class TestMCPStructuredToolToolCallId:
    ...
    async def _build_tool_via_update_tools(self, raw_result):
        ...
        _, tools, _ = await update_tools(
            "test-server", server_config, mcp_stdio_client=mock_client
        )
    ...
    async def test_tool_call_id_is_preserved_for_callbacks(self):
        ...
        class RecordingHandler(AsyncCallbackHandler):
            ...
            async def on_tool_end(
                self, output, *, run_id, parent_run_id=None, **kwargs
            ):
