...
class TestAllModulesImportable:
    ...
    def test_backward_compatibility_imports(self):
        ...
        traditional_imports = [
            ("portals.components.openai", "OpenAIModelComponent"),
            ("portals.components.anthropic", "AnthropicModelComponent"),
            ("portals.components.data", "APIRequestComponent"),
            ("portals.components.models_and_agents", "AgentComponent"),
            ("portals.components.helpers", "CalculatorComponent"),
        ]
...
class TestDirectModuleImports:
    ...
    @pytest.mark.asyncio
    async def test_all_px_component_modules_directly_importable(self):
        """Test that all px component modules can be directly imported.
        ...
        try:
            ...
            pytest.skip("px.components not available")
    ...
    def test_no_deprecated_langchain_imports(self):
        ...
        try:
            ...
            px_path = Path(px.__file__).parent
        except ImportError:
            pytest.skip("px package not found")

        components_path = px_path / "components"
        if not components_path.exists():
            pytest.skip("px.components directory not found")
                    ...
                    if isinstance(node, ast.ImportFrom):
                        ...
                        for deprecated, replacement in deprecated_patterns:
                            if module.startswith(deprecated):
                                relative_path = py_file.relative_to(px_path)
    ...
    async def test_vector_store_components_directly_importable(self):
        ...
        vector_store_components = [
            ("px.components.chroma", "ChromaVectorStoreComponent"),
            ("px.components.pinecone", "PineconeVectorStoreComponent"),
            ("px.components.qdrant", "QdrantVectorStoreComponent"),
            ("px.components.weaviate", "WeaviateVectorStoreComponent"),
            ("px.components.vectorstores", "LocalDBComponent"),
        ]
    ...
    def test_qdrant_component_directly_importable(self):
        ...
        """
        try:
            from px.components.qdrant import QdrantVectorStoreComponent
