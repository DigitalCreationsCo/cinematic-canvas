...
import pytest
from px.base.knowledge_bases.knowledge_base_utils import get_knowledge_bases
...
class TestKnowledgeBaseComponent(ComponentTestBaseWithClient):
    ...
    def mock_knowledge_base_path(self, tmp_path):
        ...
        with patch("portals.components.knowledge_bases.retrieval._KNOWLEDGE_BASES_ROOT_PATH", tmp_path):
            yield
    ...
    def test_get_kb_metadata_success(self, component_class, default_kwargs, active_user):
        ...
        with patch("portals.components.knowledge_bases.retrieval.decrypt_api_key") as mock_decrypt:
    ...
    async def test_retrieve_data_method_exists(self, component_class, default_kwargs, active_user):
        ...
        with (
            ...
            patch("px.components.files_and_knowledge.retrieval.session_scope") as mock_session_scope,
            patch("px.components.files_and_knowledge.retrieval.get_user_by_id", return_value=mock_user),
            patch(
                "px.components.files_and_knowledge.retrieval._get_knowledge_bases_root_path",
                return_value=Path(default_kwargs["kb_root_path"]),
            ),
            patch("chromadb.api.client.SharedSystemClient.clear_system_cache"),
            patch("px.components.files_and_knowledge.retrieval.Chroma", return_value=mock_chroma_instance),
        ):
    ...
    async def test_resolve_provider_variables_empty_provider_vars(self, component_class, default_kwargs):
        ...
        with patch(
            "px.components.files_and_knowledge.retrieval.get_provider_all_variables",
            return_value=[],
        ):
    ...
    async def test_resolve_provider_variables_variable_service_returns_none(self, component_class, default_kwargs):
        ...
        with (
            ...
            patch("px.components.files_and_knowledge.retrieval.session_scope") as mock_session_scope,
            patch(
                "px.components.files_and_knowledge.retrieval.get_variable_service",
                return_value=None,
            ),
        ):
            mock_session = AsyncMock()
    ...
    async def test_resolve_api_key_unknown_provider(self, component_class, default_kwargs):
        ...
        with patch(
            "px.components.files_and_knowledge.retrieval.get_model_provider_variable_mapping",
            return_value={},
        ):
    ...
    async def test_retrieve_data_user_not_found(self, component_class, default_kwargs):
        ...
        with (
            patch("px.components.files_and_knowledge.retrieval.session_scope") as mock_session_scope,
            patch(
                "px.components.files_and_knowledge.retrieval.get_user_by_id",
                return_value=None,
            ),
        ):
            mock_session = AsyncMock()
