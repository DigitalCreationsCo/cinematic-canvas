...
class TestDirectoryComponent(ComponentTestBaseWithoutClient):
    ...
    @patch("px.components.data.directory.parallel_load_data")
    @patch("px.components.data.directory.retrieve_file_paths")
    @patch("px.components.data.DirectoryComponent.resolve_path")
    def test_directory_component_build_with_multithreading(
        ...
        mock_retrieve_file_paths.return_value = [
            str(p) for p in path.iterdir() if p.suffix == ".py"
        ]
    ...
    def test_directory_without_mocks(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert all(result.text in values for result in results), [
                (len(result.text), len(val))
                for result, val in zip(results, values, strict=True)
            ]
    ...
    def test_directory_as_dataframe(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(data_frame) == 3, (
                f"Expected DataFrame with 3 rows, got {len(data_frame)}"
            )
            ...
            missing_content = [
                content for content in expected_content.values() if content not in texts
            ]
            assert not missing_content, (
                f"Missing expected content in DataFrame: {missing_content}"
            )

            # Verify file paths are correct
            file_paths = data_frame["file_path"].tolist()
            expected_paths = [
                str(Path(temp_dir) / filename) for filename in files_content
            ]
            missing_paths = [
                path
                for path in expected_paths
                if not any(path in fp for fp in file_paths)
            ]
            assert not missing_paths, (
                f"Missing expected file paths in DataFrame: {missing_paths}"
            )
    ...
    def test_directory_with_depth(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (base_dir / "level1" / "level2" / "level2.txt").write_text(
                "level2", encoding="utf-8"
            )
            ...
            assert results_root[0].text == "root", (
                f"Expected root file content 'root', got '{results_root[0].text}'"
            )
            ...
            assert texts == expected_texts, (
                f"Expected texts {expected_texts}, got {texts}"
            )
    ...
    def test_directory_with_multithreading(self, mock_parallel_load):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert actual_texts == expected_texts, (
                f"Expected texts {expected_texts}, got {actual_texts}"
            )
