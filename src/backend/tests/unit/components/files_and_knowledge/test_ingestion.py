...
from portals.schema.dataframe import DataFrame
from portals.schema.message import Message
from px.base.knowledge_bases import get_knowledge_bases
from px.components.files_and_knowledge import KnowledgeIngestionComponent
...
class TestKnowledgeIngestionComponent(ComponentTestBaseWithClient):
    ...
    def mock_knowledge_base_path(self, tmp_path):
        ...
        with patch("px.components.files_and_knowledge.ingestion._KNOWLEDGE_BASES_ROOT_PATH", tmp_path):
            yield
    ...
    @patch("px.components.files_and_knowledge.ingestion.get_settings_service")
    @patch("px.components.files_and_knowledge.ingestion.encrypt_api_key")
    ...
    async def test_convert_df_to_data_objects(self, component_class, default_kwargs):
        ...
        with patch("px.components.files_and_knowledge.ingestion.Chroma") as mock_chroma:
    ...
    async def test_convert_df_to_data_objects_no_duplicates(self, component_class, default_kwargs):
        ...
        with patch("px.components.files_and_knowledge.ingestion.Chroma") as mock_chroma:
            ...
            with patch("px.components.files_and_knowledge.ingestion.hashlib.sha256") as mock_hash:
    ...
    @patch("px.components.files_and_knowledge.ingestion.get_embeddings")
    ...
    @patch("px.components.files_and_knowledge.ingestion.get_embedding_model_options")
    @patch("px.components.files_and_knowledge.ingestion.get_embeddings")
