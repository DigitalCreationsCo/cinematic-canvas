...
class TestSaveToFileComponent(ComponentTestBaseWithoutClient):
    ...
    async def test_save_dataframe_to_csv(self, component_class):
        ...
        with (
            ...
            patch("px.services.deps.session_scope") as mock_session,
            patch(
    ...
    async def test_cleanup_on_error(self, component_class):
        ...
        with (
            patch(
                "portals.api.v2.files.upload_user_file",
    ...
    async def test_append_mode_txt_file(self, component_class):
        ...
        try:
            ...
            with (
                patch("px.components.files_and_knowledge.save_file.Path") as mock_path_class,
                patch("portals.api.v2.files.upload_user_file", new_callable=AsyncMock) as mock_upload,
                patch("px.services.deps.session_scope") as mock_session,
                patch(
                    "portals.services.database.models.user.crud.get_user_by_id", new_callable=AsyncMock
