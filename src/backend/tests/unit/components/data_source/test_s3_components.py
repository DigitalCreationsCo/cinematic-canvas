...
from px.components.langchain_utilities.csv_agent import CSVAgentComponent
from px.components.langchain_utilities.json_agent import JsonAgentComponent
...
def mock_s3_environment(settings, storage_service):
    ...
    """
    patches = [
        patch("px.services.deps.get_settings_service", return_value=settings),
        patch("px.base.data.base_file.get_settings_service", return_value=settings),
        patch("px.base.data.storage_utils.get_settings_service", return_value=settings),
        patch("px.base.data.storage_utils.get_storage_service", return_value=storage_service),
        patch("px.base.data.utils.get_settings_service", return_value=settings),
        patch("px.components.files_and_knowledge.file.get_settings_service", return_value=settings),
        patch("px.components.files_and_knowledge.file.get_storage_service", return_value=storage_service),
        patch("px.components.langchain_utilities.csv_agent.get_settings_service", return_value=settings),
        patch("px.components.langchain_utilities.json_agent.get_settings_service", return_value=settings),
    ]
...
class TestS3CompatibleComponents:
    ...
    async def test_file_component_get_local_file_for_docling_s3(self, s3_settings, mock_storage_service):
        ...
        with (
            ...
            patch(
                "px.components.files_and_knowledge.file.parse_storage_path", return_value=("user_123", "document.pdf")
            ) as mock_parse,
            patch("px.components.files_and_knowledge.file.NamedTemporaryFile") as mock_temp,
        ):
    ...
    async def test_file_component_get_local_file_for_docling_local(self, local_settings):
        ...
        with patch("px.services.deps.get_settings_service", return_value=local_settings):
            component = FileComponent()
    ...
    async def test_save_file_component_s3_upload(self, s3_settings):
        ...
        with (
            patch("px.services.deps.get_settings_service", return_value=s3_settings),
            ...
            with (
                patch("px.services.deps.session_scope"),
                patch(
                    "portals.services.database.models.user.crud.get_user_by_id", new_callable=AsyncMock
                ) as mock_get_user,
                patch("portals.api.v2.files.upload_user_file", new_callable=AsyncMock) as mock_upload,
            ):
                mock_get_user.return_value = MagicMock()
                mock_upload.return_value = "s3_file.txt"

                # Test with DataFrame
                from portals.schema import Data, DataFrame
    ...
    async def test_csv_agent_s3_file_handling(self, s3_settings):
        ...
        with patch("px.services.deps.get_settings_service", return_value=s3_settings):
            component = CSVAgentComponent()
            component.set_attributes({"llm": MagicMock(), "path": "user_123/data.csv", "verbose": False})

            # Mock storage utils
            with patch(
                "px.base.data.storage_utils.read_file_bytes", new_callable=AsyncMock, return_value=b"name,age\nJohn,30"
            ):
    ...
    async def test_json_agent_s3_file_handling(self, s3_settings):
        ...
        with patch("px.services.deps.get_settings_service", return_value=s3_settings):
            ...
            # Mock storage utils
            with patch(
                "px.base.data.storage_utils.read_file_bytes", new_callable=AsyncMock, return_value=b'{"key": "value"}'
            ):
                local_path = component._get_local_path()
    ...
    async def test_components_work_in_local_mode(self, local_settings):
        ...
        with patch("portals.services.deps.get_settings_service", return_value=local_settings):
            component = FileComponent()
    ...
    async def test_csv_to_data_fileinput_local_only(self, s3_settings, local_settings):
        ...
        from px.components.data_source.csv_to_data import CSVToDataComponent
    ...
    async def test_csv_to_data_path_local(self, local_settings):
        ...
        with (
            patch("px.services.deps.get_settings_service", return_value=local_settings),
            patch("px.base.data.storage_utils.get_settings_service", return_value=local_settings),
            patch("px.base.data.storage_utils.read_file_text", new_callable=AsyncMock) as mock_read_file,
            patch(
                "px.components.data_source.csv_to_data.read_file_text", new_callable=AsyncMock
    ...
    async def test_json_to_data_fileinput_local_only(self, s3_settings, local_settings):
        ...
        from px.components.data_source.json_to_data import JSONToDataComponent
    ...
    async def test_json_to_data_path_local(self, local_settings):
        ...
        with (
            ...
            patch("px.base.data.storage_utils.read_file_text", new_callable=AsyncMock) as mock_read_file,
            patch(
                "px.components.data_source.json_to_data.read_file_text", new_callable=AsyncMock
            ) as mock_read_file_component,
        ):
