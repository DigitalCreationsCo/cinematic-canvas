...
from px.components.data_source.api_request import APIRequestComponent
...
class TestAPIRequestComponent(ComponentTestBaseWithoutClient):
    ...
    async def test_parse_curl(self, component):
        ...
        curl_cmd = "curl -X GET https://example.com/api/test -H 'Content-Type: application/json' -d '{\"key\": \"value\"}'"
    ...
    async def test_make_request_with_metadata(self, component):
        ...
        response_data = {"key": "value"}
        respx.get(url).mock(
            return_value=Response(200, json=response_data, headers=headers)
        )
    ...
    async def test_make_request_binary_response(self, component):
        ...
        respx.get(url).mock(
            return_value=Response(200, content=binary_content, headers=headers)
        )
    ...
    async def test_make_request_with_redirects(self, component):
        ...
        respx.get(url).mock(
            return_value=Response(303, headers={"Location": redirect_url})
        )
    ...
    async def test_process_body(self, component):
        ...
        assert component._process_body(nested_data_body) == {
    ...
    async def test_response_info(self, component):
        ...
        is_binary, file_path = await component._response_info(
            response, with_file_path=True
        )
        ...
        is_binary, file_path = await component._response_info(
            binary_response, with_file_path=True
        )
...
class TestAPIRequestSSRFProtection:
    ...
    async def test_ssrf_protection_disabled_allows_all_urls(self, component):
        ...
        with (
            patch.dict(os.environ, {"PORTALS_SSRF_PROTECTION_ENABLED": "false"}),
            respx.mock,
        ):
            respx.get("http://127.0.0.1:8080").mock(
    ...
    async def test_ssrf_protection_blocks_localhost_127_0_0_1(self, component):
        ...
        with (
            patch.dict(os.environ, {"PORTALS_SSRF_PROTECTION_ENABLED": "true"}),
    ...
    async def test_ssrf_protection_allows_public_urls(self, component):
        ...
        public_urls = [
            ...
            "https://api.github.com/repos/portals-ai/portals",
            "https://www.google.com",
        ]

        with patch.dict(os.environ, {"PORTALS_SSRF_PROTECTION_ENABLED": "true"}):
    ...
    async def test_ssrf_allowlist_hostname(self, component):
        ...
        with (
            patch.dict(
                os.environ,
                {
                    "PORTALS_SSRF_PROTECTION_ENABLED": "true",
                    "PORTALS_SSRF_ALLOWED_HOSTS": "internal.company.local",
                },
            ),
            respx.mock,
        ):
            respx.get("http://internal.company.local/api").mock(
                return_value=Response(200, json={"status": "ok"})
            )
    ...
    async def test_ssrf_allowlist_ip_address(self, component):
        ...
        with (
            patch.dict(
                ...
                {
                    "PORTALS_SSRF_PROTECTION_ENABLED": "true",
                    "PORTALS_SSRF_ALLOWED_HOSTS": "192.168.1.100",
                },
            ),
            respx.mock,
        ):
            respx.get("http://192.168.1.100/api").mock(
                return_value=Response(200, json={"status": "ok"})
            )
    ...
    async def test_ssrf_allowlist_cidr_range(self, component):
        ...
        with (
            patch.dict(
                ...
                {
                    "PORTALS_SSRF_PROTECTION_ENABLED": "true",
                    "PORTALS_SSRF_ALLOWED_HOSTS": "192.168.1.0/24",
                },
            ),
            respx.mock,
        ):
            respx.get("http://192.168.1.5/api").mock(
                return_value=Response(200, json={"status": "ok"})
            )
    ...
    async def test_ssrf_allowlist_multiple_entries(self, component):
        ...
        with (
            patch.dict(
                ...
                {
                    "PORTALS_SSRF_PROTECTION_ENABLED": "true",
                    "PORTALS_SSRF_ALLOWED_HOSTS": "localhost,192.168.1.0/24,internal.local",
                },
            ),
            respx.mock,
        ):
    ...
    async def test_dns_pinning_is_used_when_protection_enabled(self, component):
        ...
        with (
            patch.dict(os.environ, {"PORTALS_SSRF_PROTECTION_ENABLED": "true"}),
            patch(
                "px.components.data_source.api_request.create_ssrf_protected_client"
    ...
    async def test_follow_redirects_security_warning(self, component):
        ...
        with (
            ...
            security_warning_found = any(
                "Security Warning" in msg and "SSRF bypass" in msg
                for msg in all_log_messages
            )
            assert security_warning_found, (
                f"Security warning not found in: {all_log_messages}"
            )
