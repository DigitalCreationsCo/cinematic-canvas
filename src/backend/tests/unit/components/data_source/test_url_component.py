...
from px.schema import DataFrame
from px.utils.ssrf_protection import SSRFProtectionError
...
class TestURLComponent(ComponentTestBaseWithoutClient):
    ...
    def mock_recursive_loader(self):
        ...
        with patch(
            "langchain_community.document_loaders.RecursiveUrlLoader.load"
        ) as mock:
            yield mock
...
class TestURLComponentSSRFProtection:
    ...
    def test_ssrf_validation_called_on_ensure_url(self):
        ...
        with patch(
            "px.components.data_source.url.validate_url_for_ssrf"
        ) as mock_validate:
            component.ensure_url("https://example.com")
            mock_validate.assert_called_once_with(
                "https://example.com", warn_only=False
            )
    ...
    def test_ssrf_blocks_localhost(self):
        ...
        with patch(
            ...
            mock_validate.side_effect = SSRFProtectionError(
                "Access to IP address 127.0.0.1 is blocked"
            )
    ...
    def test_ssrf_blocks_private_ip(self):
        ...
        with patch(
            ...
            mock_validate.side_effect = SSRFProtectionError(
                "Access to IP address 192.168.1.1 is blocked"
            )
    ...
    def test_ssrf_blocks_metadata_endpoint(self):
        ...
        with patch(
            ...
            mock_validate.side_effect = SSRFProtectionError(
                "Access to IP address 169.254.169.254 is blocked"
            )
