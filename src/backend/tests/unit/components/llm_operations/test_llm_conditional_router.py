...
import pytest
from px.components.llm_operations.llm_conditional_router import SmartRouterComponent
...
class TestSmartRouterComponent(ComponentTestBaseWithoutClient):
    ...
    def _create_component_with_mock_categorization(
        self, categorization_result, *, enable_else=False
    ):
    ...
    def test_positive_output(self):
        ...
        component.stop.assert_any_call(
            "category_2_result"
        )  # Negative should be stopped
    ...
    def test_negative_output(self):
        ...
        component.stop.assert_any_call(
            "category_1_result"
        )  # Positive should be stopped

    def test_neutral_output_no_match(self):
        """Test when input doesn't match any category (no else output)."""
        component = self._create_component_with_mock_categorization(
            "NONE", enable_else=False
        )
    ...
    def test_categorization_caching(self):
        ...
        with patch(
            "px.components.llm_operations.llm_conditional_router.get_llm",
