...
import pytest
from px.components.llm_operations.lambda_filter import LambdaFilterComponent
...
class TestGetInputTypeName(TestLambdaFilterComponent):
    ...
    def test_should_return_message_when_input_is_list_of_messages(
...
class TestExtractMessageText(TestLambdaFilterComponent):
    ...
    def test_should_return_empty_string_when_message_text_is_none(
        self, component_class
    ):
        # Arrange
        component = component_class()
...
class TestConvertResultToDataframe(TestLambdaFilterComponent):
    ...
    def test_should_create_dataframe_when_result_is_list_of_dicts(
        self, component_class
    ):
        # Arrange
        component = component_class()
...
class TestProcessAsDataIntegration(TestLambdaFilterComponent):
    ...
    async def test_should_return_filtered_data_when_lambda_is_valid(
        ...
        mock_llm.ainvoke.return_value.content = (
            "lambda x: [item for item in x['items'] if item['value'] > 15]"
        )
...
class TestLargeDataset(TestLambdaFilterComponent):
    ...
    async def test_should_filter_large_dataset_when_data_exceeds_max_size(
        ...
        default_kwargs["filter_instruction"] = (
            "Filter items with value greater than 1500"
        )
        component = await self.component_setup(component_class, default_kwargs)
        mock_llm.ainvoke.return_value.content = (
            "lambda x: [item for item in x['items'] if item['value'] > 1500]"
        )
