...
import httpx
import pytest
from px.components.litellm.litellm_proxy import LiteLLMProxyComponent
from px.inputs.inputs import IntInput, SecretStrInput, SliderInput, StrInput
...
class TestLiteLLMProxyComponent(ComponentTestBaseWithoutClient):
    ...
    def test_build_model(self, component_class, default_kwargs, mocker):
        ...
        mocker.patch(
            "px.components.litellm.litellm_proxy.httpx.get",
            return_value=_mock_models_response(),
        )
        mock_chat_openai = mocker.patch(
            "px.components.litellm.litellm_proxy.ChatOpenAI",
