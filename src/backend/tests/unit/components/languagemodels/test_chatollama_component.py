...
from px.base.models.ollama_constants import DEFAULT_OLLAMA_API_URL
from px.components.ollama.ollama import ChatOllamaComponent
from px.schema import Data, DataFrame
...
class TestChatOllamaComponent(ComponentTestBaseWithoutClient):
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    async def test_build_model_missing_base_url(
        ...
        with pytest.raises(
            ValueError, match=re.escape("Unable to connect to the Ollama API.")
        ):
            component.build_model()

    @patch("px.components.ollama.ollama.ChatOllama")
    async def test_build_model_with_mirostat_enabled(
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    async def test_build_model_with_mirostat_2_enabled(
        self, mock_chat_ollama, component_class
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.httpx.AsyncClient.post")
    @patch("px.components.ollama.ollama.httpx.AsyncClient.get")
    ...
    @patch("px.components.ollama.ollama.httpx.AsyncClient.post")
    @patch("px.components.ollama.ollama.httpx.AsyncClient.get")
    async def test_get_models_missing_capabilities_without_tool_model(
    ...
    @patch("px.components.ollama.ollama.httpx.AsyncClient.post")
    @patch("px.components.ollama.ollama.httpx.AsyncClient.get")
    async def test_get_models_missing_capabilities_with_tool_model(
        self, mock_get, mock_post
    ):
    ...
    async def test_get_models_mixed_capabilities_response(self, mock_get, mock_post):
        ...
        mock_post_response.json.side_effect = [
            {
                component.JSON_CAPABILITIES_KEY: [component.DESIRED_CAPABILITY]
            },  # new-model has completion
            {},  # old-model has no capabilities field
            {
                component.JSON_CAPABILITIES_KEY: ["embedding"]
            },  # embedding-model only has embedding
        ]
        mock_post.return_value = mock_post_response
    ...
    async def test_get_models_failure(self, mock_get):
        ...
        base_url = "http://localhost:11434"
        with pytest.raises(
            ValueError, match=re.escape("Could not get model names from Ollama.")
        ):
    ...
    def test_build_model_transforms_localhost_in_docker_container(
        ...
        mock_getaddrinfo.return_value = [
            ("AF_INET", "SOCK_STREAM", 6, "", ("192.168.65.2", 0))
        ]
    ...
    @patch("px.utils.util.Path")
    @patch("px.components.ollama.ollama.ChatOllama")
    def test_build_model_no_transform_outside_container(
        self, mock_chat_ollama, mock_path_class
    ):
    ...
    @patch("px.utils.util.Path")
    @patch("px.components.ollama.ollama.ChatOllama")
    def test_build_model_transforms_127_0_0_1_in_container(
        self, mock_chat_ollama, mock_path_class, mock_getaddrinfo
    ):
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    @patch("px.components.ollama.ollama.logger")
    def test_build_model_strips_v1_suffix_and_logs_warning(
        self, mock_logger, mock_chat_ollama
    ):
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    def test_build_model_with_json_format_string(
        self, mock_chat_ollama, component_class, default_kwargs
    ):
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    def test_build_model_with_json_schema_dict(
        self, mock_chat_ollama, component_class, default_kwargs
    ):
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    def test_build_model_with_complex_json_schema(
        self, mock_chat_ollama, component_class, default_kwargs
    ):
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    def test_build_model_with_pydantic_model_json_schema(
        self, mock_chat_ollama, component_class, default_kwargs
    ):
        ...
        https://github.com/portals-ai/portals/issues/7122
        """
        ...
        assert (
            call_args["format"]["properties"]["name"]["description"]
            == "The person's full name"
        )
        assert model == mock_instance

    @pytest.mark.asyncio
    async def test_parse_json_response_valid_dict(
    ...
    @pytest.mark.asyncio
    async def test_parse_json_response_valid_list(
        self, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    async def test_parse_json_response_empty_response(
        self, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.ChatOllamaComponent._parse_json_response")
    async def test_build_data_output_with_dict(
        self, mock_parse_json, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.ChatOllamaComponent._parse_json_response")
    async def test_build_data_output_with_list_single_item(
        self, mock_parse_json, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.ChatOllamaComponent._parse_json_response")
    async def test_build_data_output_with_list_multiple_items(
        self, mock_parse_json, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.ChatOllamaComponent._parse_json_response")
    async def test_build_data_output_with_primitive(
        self, mock_parse_json, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.ChatOllamaComponent._parse_json_response")
    async def test_build_dataframe_output_with_list_of_dicts(
        self, mock_parse_json, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.ChatOllamaComponent._parse_json_response")
    async def test_build_dataframe_output_with_empty_list(
        self, mock_parse_json, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.ChatOllamaComponent._parse_json_response")
    async def test_build_dataframe_output_with_single_dict(
        self, mock_parse_json, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.ChatOllamaComponent._parse_json_response")
    async def test_build_dataframe_output_with_primitive(
        self, mock_parse_json, component_class, default_kwargs
    ):
    ...
    @pytest.mark.asyncio
    @patch("px.components.ollama.ollama.ChatOllamaComponent._parse_json_response")
    async def test_build_dataframe_output_with_invalid_list(
        self, mock_parse_json, component_class, default_kwargs
    ):
    ...
    def test_build_model_with_cloud_api_and_headers(self, mock_chat_ollama):
        ...
        assert (
            call_args["client_kwargs"]["headers"]["Authorization"]
            == "Bearer test-cloud-api-key"
        )
    ...
    async def test_get_models_with_cloud_and_headers(self, mock_get, mock_post):
        ...
        assert (
            get_call_kwargs["headers"]["Authorization"] == "Bearer test-cloud-api-key"
        )
        ...
        assert (
            post_call_kwargs["headers"]["Authorization"] == "Bearer test-cloud-api-key"
        )
    ...
    def test_get_base_url_cloud_no_transform(self):
        ...
        from px.utils.util import transform_localhost_url
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    def test_build_model_with_structured_output_disabled(
        self, mock_chat_ollama, component_class, default_kwargs
    ):
        ...
        assert "format" not in call_args, (
            "format should not be passed when enable_structured_output is False"
        )
        assert model == mock_instance
    ...
    def test_build_model_with_structured_output_enabled_string_format(
        ...
        assert "format" in call_args, (
            "format should be passed when enable_structured_output is True"
        )
        assert call_args["format"] == "json"
        assert model == mock_instance
    ...
    @patch("px.components.ollama.ollama.ChatOllama")
    def test_build_model_structured_output_toggle_behavior(
        self, mock_chat_ollama, component_class, default_kwargs
    ):
