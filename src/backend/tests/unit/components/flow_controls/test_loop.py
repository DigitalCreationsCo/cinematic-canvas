...
from portals.memory import aget_messages
from portals.services.database.models.flow import FlowCreate
from px.components.data_source.url import URLComponent
from px.components.flow_controls import LoopComponent
from px.components.input_output import ChatOutput
from px.components.llm_operations import StructuredOutputComponent
...
from px.components.processing import ParserComponent, SplitTextComponent
...
class TestLoopComponentWithAPI(ComponentTestBaseWithClient):
    ...
    async def _create_flow(self, client, json_loop_test, logged_in_headers):
        ...
        vector_store = FlowCreate(
            name="Flow", description="description", data=data, endpoint_name="f"
        )
    ...
    async def test_run_flow_loop(
        self, client: AsyncClient, created_api_key, json_loop_test, logged_in_headers
    ):
        flow_id = await self._create_flow(client, json_loop_test, logged_in_headers)
        ...
        response = await client.post(
            f"/api/v1/run/{flow_id}", json=payload, headers=headers
        )
        data = response.json()
...
@pytest.mark.skipif(
    not has_api_key("OPENAI_API_KEY"), reason="OPENAI_API_KEY is not set"
)
def loop_flow():
    ...
    url_component.set(urls=["https://docs.portals.org/"])
...
class TestLoopComponentSubgraphExecution:
    ...
    async def test_event_manager_passed_to_subgraph(self):
        ...
        with patch.object(loop, "get_loop_body_vertices", return_value=set()):
            result = await loop.execute_loop_body(
                data_list, event_manager=mock_event_manager
            )
        ...
        mock_subgraph.get_vertex = MagicMock(
            return_value=MagicMock(update_raw_params=MagicMock())
        )
    ...
    async def test_done_output_passes_event_manager(self):
        ...
        mock_ctx.get = MagicMock(
            side_effect=lambda key, default=None: (
                data_list if key == f"{loop._id}_data" else default
            )
        )
        ...
        with (
            ...
            patch.object(
                type(loop), "ctx", new_callable=PropertyMock, return_value=mock_ctx
            ),
        ):
    ...
    def test_get_loop_body_vertices_simple_loop(self):
        ...
        with (
            patch.object(
                loop, "get_incoming_edge_by_target_param", return_value="loop_end"
            ),
            patch.object(
                type(loop), "graph", new_callable=PropertyMock, return_value=mock_graph
            ),
        ):
            result = loop.get_loop_body_vertices()
