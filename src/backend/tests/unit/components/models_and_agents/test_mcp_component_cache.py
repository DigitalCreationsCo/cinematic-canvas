...
import pytest
from px.base.agents.utils import safe_cache_get, safe_cache_set
from px.components.models_and_agents.mcp_component import MCPToolsComponent
...
class TestMCPComponentCache(ComponentTestBaseWithoutClient):
    ...
    async def test_cache_enabled_by_default(self, component_class, default_kwargs):
        ...
        assert (
            getattr(component, "use_cache", False) is False
    ...
    async def test_cache_stores_tools_when_enabled(
        ...
        current_servers_cache = safe_cache_get(
            component._shared_component_cache, "servers", {}
        )
        current_servers_cache[server_name] = cache_data
        safe_cache_set(
            component._shared_component_cache, "servers", current_servers_cache
        )
    ...
    async def test_cache_reuses_cached_tools(
        ...
        safe_cache_set(
            component._shared_component_cache, "servers", {server_name: cache_data}
        )

        with patch("px.base.mcp.util.update_tools") as mock_update_tools:
    ...
    @pytest.mark.asyncio
    async def test_cache_disabled_doesnt_store_tools(
    ...
    async def test_switching_from_cache_enabled_to_disabled(
        ...
        servers_cache = safe_cache_get(component._shared_component_cache, "servers", {})
        cached = (
            servers_cache.get(server_name) if isinstance(servers_cache, dict) else None
        )
    ...
    @pytest.mark.asyncio
    async def test_cache_with_corrupted_cache_data(
        self, component_class, default_kwargs
    ):
        ...
        safe_cache_set(
            component._shared_component_cache, "servers", {server_name: "invalid_data"}
        )
    ...
    async def test_cache_persistence_across_instances(
        ...
        safe_cache_set(
            component1._shared_component_cache, "servers", {server_name: cache_data}
        )
    ...
    @pytest.mark.asyncio
    async def test_build_config_respects_cache_setting(
        self, component_class, default_kwargs, mock_tools_list
    ):
        ...
        updated_config = await component.update_build_config(
            build_config, {"name": server_name}, "mcp_server"
        )
    ...
    @pytest.mark.asyncio
    async def test_build_output_with_cached_tools(
        self, component_class, default_kwargs, mock_tools_list
    ):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_with_cache_disabled(
        self, component_class, default_kwargs
    ):
        ...
        with (
            patch("portals.api.v2.mcp.get_server") as mock_get_server,
            patch("px.services.deps.session_scope"),
            patch(
                "portals.services.database.models.user.crud.get_user_by_id"
            ) as mock_get_user,
        ):
