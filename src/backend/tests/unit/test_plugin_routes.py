...
Ensures that plugins loaded via the portals.plugins entry-point group
cannot overwrite or shadow existing Portals routes.
"""
...
from portals.plugin_routes import (
    _get_route_keys,
    _PluginAppWrapper,
    load_plugin_routes,
)
...
class TestPluginAppWrapper:
    ...
    def test_raises_on_conflicting_get(self):
        ...
        with pytest.raises(
            ValueError,
            match="Plugin route conflicts with existing route: /api/login \\[GET\\]",
        ):
    ...
    def test_raises_on_conflicting_include_router(self):
        ...
        with pytest.raises(
            ValueError, match="Plugin route conflicts with existing route"
        ):
    ...
    def test_second_plugin_cannot_overwrite_first_plugin_route(self):
        ...
        with pytest.raises(
            ValueError, match=r"conflicts with existing route.*/api/v1/sso/login"
        ):
...
class TestLoadPluginRoutes:
    ...
    def test_no_crash_when_no_plugins(self):
        ...
        with patch("portals.plugin_routes.entry_points", return_value=[]):
    ...
    def test_plugin_that_registers_route_is_loaded(self):
        ...
        with patch("portals.plugin_routes.entry_points", return_value=[ep]):
            load_plugin_routes(app)
