...
class TestCORSConfiguration:
    ...
    def test_default_cors_settings_current_behavior(self):
        ...
        with (
            ...
            patch.dict(os.environ, {"PORTALS_CONFIG_DIR": temp_dir}),
        ):
            settings = Settings()
            ...
            )  # Currently defaults to True (insecure)
            ...
            warnings.warn(
                ...
                "to your Portals instance and potentially steal user credentials. "
                "This will be changed to more secure defaults in v1.7. "
                "Please configure PORTALS_CORS_ORIGINS with specific domains for production use.",
                UserWarning,
                stacklevel=2,
            )
    ...
    def test_default_cors_settings_secure_future(self):
        ...
        # with tempfile.TemporaryDirectory() as temp_dir, patch.dict(os.environ, {"PORTALS_CONFIG_DIR": temp_dir}):
        #     settings = Settings()
    ...
    def test_cors_origins_string_to_list_conversion(self):
        ...
        with (
            ...
            patch.dict(
                os.environ,
                {
                    "PORTALS_CONFIG_DIR": temp_dir,
                    "PORTALS_CORS_ORIGINS": "https://app1.example.com,https://app2.example.com",
                },
            ),
        ):
            settings = Settings()
    ...
    def test_single_origin_converted_to_list(self):
        ...
        with (
            ...
            patch.dict(
                ...
                {
                    "PORTALS_CONFIG_DIR": temp_dir,
                    "PORTALS_CORS_ORIGINS": "https://app.example.com",
                },
            ),
        ):
            settings = Settings()
    ...
    def test_wildcard_with_credentials_allowed_current_behavior(self):
        ...
        with (
            ...
            patch.dict(
                ...
                {
                    "PORTALS_CONFIG_DIR": temp_dir,
                    "PORTALS_CORS_ORIGINS": "*",
                    "PORTALS_CORS_ALLOW_CREDENTIALS": "true",
                },
            ),
        ):
            settings = Settings()
    ...
    @patch("portals.main.setup_sentry")  # Mock Sentry setup
    @patch("portals.main.get_settings_service")
    ...
    def test_cors_wildcard_credentials_runtime_check_current_behavior(
        ...
        mock_settings.settings.cors_allow_credentials = (
            True  # Gets disabled for security
        )
        ...
        assert (
            cors_middleware.kwargs["allow_credentials"] is True
        )  # Current behavior: NOT disabled (insecure!)
...
class TestRefreshTokenSecurity:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="Token type validation not implemented - security enhancement for future"
    )
    async def test_refresh_token_type_validation(self):
        ...
        with patch("portals.services.auth.utils.jwt.decode") as mock_decode:
            ...
            with patch(
                "portals.services.auth.utils.get_settings_service"
            ) as mock_settings:
                ...
                mock_settings.return_value.auth_settings.ACCESS_TOKEN_EXPIRE_SECONDS = (
                    3600
                )
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="User activity check not implemented yet - security enhancement for future"
    )
    async def test_refresh_token_user_active_check(self):
        ...
        with patch("portals.services.auth.utils.jwt.decode") as mock_decode:
            ...
            with patch(
                ...
                with patch(
                    "portals.services.auth.utils.get_user_by_id"
                ) as mock_get_user:
                    mock_get_user.return_value = mock_user

                    # This SHOULD raise an exception for inactive users, but currently doesn't
                    with pytest.raises(HTTPException) as exc_info:
                        await get_auth_service().create_refresh_token(
                            "fake-token", mock_db
                        )
    ...
    async def test_refresh_token_valid_flow(self):
        ...
        from portals.services.auth.utils import create_refresh_token
        ...
        # Create a portals AuthService instance (not px) with mocked settings
        mock_settings_service = MagicMock()
        auth_service = AuthService(mock_settings_service)

        with (
            patch(
                "portals.services.auth.utils._auth_service", return_value=auth_service
            ),
            patch("portals.services.auth.service.jwt.decode") as mock_decode,
        ):
            mock_decode.return_value = {
                "sub": str(user_id),
                "type": "refresh",
            ...
            with patch(
                "portals.services.auth.utils.get_jwt_verification_key"
            ) as mock_verification_key:
                mock_verification_key.return_value = "secret"

                with patch(
                    "portals.services.auth.service.get_user_by_id",
                    new_callable=AsyncMock,
                ) as mock_get_user:
                    mock_get_user.return_value = mock_user

                    with patch.object(
                        auth_service, "create_user_tokens", new_callable=AsyncMock
    ...
    def test_refresh_token_samesite_setting_current_behavior(self):
        ...
        from px.services.settings.auth import AuthSettings
        ...
        with (
            ...
            assert (
                auth_settings.REFRESH_SAME_SITE == "none"
            )  # Current: allows cross-site (less secure)
            assert (
                auth_settings.ACCESS_SAME_SITE == "lax"
            )  # Access token is already lax (good)
    ...
    @pytest.mark.skip(
        reason="Uncomment in v1.7 - represents future secure SameSite behavior"
    )
    def test_refresh_token_samesite_setting_future_secure(self):
        ...
        # from portals.services.settings.auth import AuthSettings
        # with tempfile.TemporaryDirectory() as temp_dir, patch.dict(os.environ, {"PORTALS_CONFIG_DIR": temp_dir}):
        #     auth_settings = AuthSettings(CONFIG_DIR=temp_dir)
...
class TestCORSIntegration:
    ...
    async def test_cors_headers_in_response_current_behavior(self, mock_setup_sentry):
        ...
        with patch("portals.main.get_settings_service") as mock_get_settings:
            mock_settings = MagicMock()
            mock_settings.settings.cors_origins = ["https://app.example.com"]
            mock_settings.settings.cors_allow_credentials = True
            mock_settings.settings.cors_allow_methods = "*"
    ...
    @pytest.mark.skip(
        reason="Uncomment in v1.7 - represents future secure CORS blocking behavior"
    )
    ...
    @pytest.mark.asyncio
    @patch("portals.main.setup_sentry")  # Mock Sentry setup
    async def test_cors_blocks_unauthorized_origin_current_behavior(
        self, mock_setup_sentry
    ):
...
class TestFutureSecureCORSBehavior:
    ...
    @pytest.mark.skip(
        reason="Uncomment in v1.7 - represents future secure default CORS configuration"
    )
    ...
    @pytest.mark.skip(
        reason="Uncomment in v1.7 - represents future secure wildcard rejection"
    )
    ...
    @pytest.mark.skip(
        reason="Uncomment in v1.7 - represents future secure middleware configuration"
    )
