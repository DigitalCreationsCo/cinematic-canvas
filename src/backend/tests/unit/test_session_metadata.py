...
import pytest
from portals.memory import aadd_messages, aget_messages, astore_message
...
def sample_session_metadata():
    ...
    return {
        ...
        "custom_fields": {"department": "engineering", "project": "portals"},
    }
...
@pytest.mark.usefixtures("client")
async def test_add_messages_with_session_metadata(
    sample_session_metadata, minimal_session_metadata
):
...
async def test_messageread_with_session_metadata(sample_session_metadata):
    ...
    async with session_scope() as session:
        ...
        message_table = MessageTable.model_validate(
            message_create, from_attributes=True
        )
