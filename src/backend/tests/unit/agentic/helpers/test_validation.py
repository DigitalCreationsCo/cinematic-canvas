...
import pytest
from portals.agentic.helpers.validation import (
...
MODULE = "portals.agentic.helpers.validation"
...
class TestValidateComponentCode:
    ...
    def test_should_not_execute_code_during_validation(self):
        ...
        """
        env_key = "_PORTALS_SECURITY_VALIDATION_TEST"
...
class TestValidateComponentRuntimeExecution:
    ...
    @pytest.mark.asyncio
    async def test_should_return_error_when_output_method_builds_data_with_list_payload(
        self,
    ):
        ...
        lowered = error.lower()
        assert "valid" in lowered, (
            f"Expected a pydantic validation error message, got: {error!r}"
        )
        assert "dict" in lowered, (
            f"Expected the pydantic message to mention 'dict', got: {error!r}"
        )

    @pytest.mark.asyncio
    async def test_should_return_error_when_output_method_builds_message_with_wrong_type(
        self,
    ):
