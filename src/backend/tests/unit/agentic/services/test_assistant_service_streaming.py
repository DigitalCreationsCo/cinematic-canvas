...
from portals.agentic.services.flow_types import IntentResult

MODULE = "portals.agentic.services.assistant_service"
...
class TestQAResponse:
    ...
    async def test_should_return_plain_text_for_qa_with_component_code(self):
        ...
        with (
            ...
            validation_events = [
                e for e in events if "extracting_code" in e or '"validating"' in e
            ]
    ...
    @pytest.mark.asyncio
    async def test_should_return_plain_text_when_question_response_contains_example_code(
        self,
    ):
...
class TestComponentGeneration:
    ...
    async def test_should_emit_progress_events_on_successful_validation(self):
        ...
        with (
            ...
            patch(
                f"{MODULE}.validate_component_runtime",
                new_callable=AsyncMock,
                return_value=None,
            ),
    ...
    async def test_should_retry_on_validation_failure(self):
        ...
        with (
            ...
            patch(
                ...
                side_effect=lambda **_kw: mock_streaming(),
            ),
            patch(f"{MODULE}.extract_component_code", return_value=component_code),
            patch(
                f"{MODULE}.validate_component_code",
            ...
            retry_events = [
                e
                for e in events
                if "retry" in e.lower() or "validation_failed" in e.lower()
            ]
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_should_emit_complete_event_with_validated_false_when_all_attempts_raise(
        self,
    ):
        ...
        with (
            ...
            patch(
                f"{MODULE}.execute_flow_file_streaming", side_effect=streaming_factory
            ),
            patch("asyncio.sleep", new_callable=AsyncMock),
        ):
            gen = execute_flow_with_validation_streaming(
            ...
            assert complete_events, (
                f"Expected a complete event after exhausted retries. Events: {events}"
            )
    ...
    async def test_should_emit_error_when_no_result(self):
        ...
        with (
            ...
            error_events = [
                e for e in events if "error" in e.lower() or "no result" in e.lower()
            ]
            assert len(error_events) >= 1
