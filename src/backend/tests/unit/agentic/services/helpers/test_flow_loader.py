...
class TestLoadGraphForExecution:
    ...
    async def test_should_use_python_loader_for_python_type(self):
        ...
        with patch(
            "portals.agentic.services.helpers.flow_loader._load_graph_from_python",
    ...
    async def test_should_use_json_loader_for_json_type(self):
        ...
        with (
            patch(
                "portals.agentic.services.helpers.flow_loader.load_and_prepare_flow",
                return_value='{"data": {"nodes": []}}',
            ) as mock_prepare,
            patch(
                "portals.agentic.services.helpers.flow_loader.aload_flow_from_json",
                new_callable=AsyncMock,
                return_value=mock_graph,
            ) as mock_load_json,
        ):
