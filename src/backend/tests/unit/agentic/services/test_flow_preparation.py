...
from portals.agentic.services.flow_preparation import (
    PX_COMPONENTS_PATH_SENTINEL,
    inject_model_into_flow,
    inject_px_components_path,
    load_and_prepare_flow,
)

import px

MODULE = "portals.agentic.services.flow_preparation"
...
class TestInjectPxComponentsPath:
    ...
    flow embedded a relative path './src/px/src/px/components/' which only
    ...
    def test_should_rewrite_directory_path_to_absolute_px_components_when_path_matches_sentinel(
        self,
    ):
        flow_data = _make_directory_flow(PX_COMPONENTS_PATH_SENTINEL)
        ...
        # The rewritten path must actually exist in the installed px package.
    ...
    def test_should_rewrite_sentinel_path_when_loading_assistant_flow(self, tmp_path):
        ...
        load_and_prepare_flow must apply the px path injection so that
        Desktop (and any non-monorepo CWD) can execute the assistant flow.
        """
        flow_data = _make_directory_flow(PX_COMPONENTS_PATH_SENTINEL)
        flow_file = tmp_path / "PortalsAssistant.json"
        flow_file.write_text(json.dumps(flow_data))
