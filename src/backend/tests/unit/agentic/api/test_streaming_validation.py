...
import pytest
from portals.agentic.helpers.code_extraction import extract_python_code
from portals.agentic.helpers.sse import (
    format_complete_event,
    format_error_event,
    format_progress_event,
)
from portals.agentic.helpers.validation import validate_component_code
from portals.agentic.services.assistant_service import (
    execute_flow_with_validation,
    execute_flow_with_validation_streaming,
)
from portals.agentic.services.flow_types import (
    VALIDATION_RETRY_TEMPLATE,
    IntentResult,
)

VALID_COMPONENT_CODE = """from portals.custom import Component
from portals.io import MessageTextInput, Output
...
INVALID_COMPONENT_CODE = """from portals.custom import Component
from portals.io import MessageTextInput, Output
...
class TestStreamingValidationFlow:
    ...
    async def test_valid_code_first_try_returns_validated(self):
        ...
        with (
            patch(
                "portals.agentic.services.assistant_service.classify_intent",
                side_effect=_mock_intent_classification("generate_component"),
            ),
            patch(
                "portals.agentic.services.assistant_service.execute_flow_file_streaming",
        ...
        generating_events = [
            ...
            if e.get("event") == "progress" and e.get("step") == "generating_component"
        ]
        ...
        validating_events = [
            e
            for e in parsed_events
            if e.get("event") == "progress" and e.get("step") == "validating"
        ]
    ...
    async def test_invalid_code_retries_until_success(self):
        ...
        assert complete_events[0]["data"]["validated"] is True
        assert (
            complete_events[0]["data"]["validation_attempts"] == 2
    ...
    async def test_all_retries_fail_returns_validation_error(self):
        ...
        assert (
            complete_events[0]["data"]["validation_attempts"] == 3
        )  # 3 total attempts
    ...
    async def test_no_code_in_response_returns_as_is(self):
        ...
        text_only_response = {
            "result": "Portals is a visual flow builder for LLM applications."
        }
        ...
        generating_events = [
            e
            for e in parsed_events
            if e.get("event") == "progress" and e.get("step") == "generating"
        ]
        assert len(generating_events) == 1
    ...
    @pytest.mark.asyncio
    async def test_flow_execution_error_on_component_generation_retries_then_emits_complete_event(
        self,
    ):
        ...
        assert len(error_events) == 0, (
            f"Expected 0 error events for component generation, got: {error_events}"
        )
    ...
    @pytest.mark.asyncio
    async def test_flow_execution_error_on_qa_intent_emits_error_event_without_retry(
        self,
    ):
...
class TestValidationRetryBehavior:
    ...
    async def test_retry_includes_previous_error_in_prompt(self):
        ...
        assert (
            INVALID_COMPONENT_CODE.strip() in retry_input
            or "BrokenComponent" in retry_input
        )
...
class TestNonStreamingValidation:
    ...
    async def test_non_streaming_valid_code_returns_validated(self):
        ...
        with patch(
            "portals.agentic.services.assistant_service.execute_flow_file",
...
{CUTOFF_COMPONENT_CODE}"""
        ...
        assert len(validating_events) == 1, (
            "Code should be extracted and validation attempted"
        )
        ...
        complete_data = complete_events[0]["data"]
        assert complete_data["validated"] is False, (
            "Incomplete code should fail validation"
        )
        assert complete_data.get("validation_error") is not None, (
...
{CUTOFF_COMPONENT_CODE}"""
        ...
        assert complete_data["validated"] is False
        assert (
            complete_data["validation_attempts"] == 3
