...
MODULE = "portals.agentic.utils.flow_graph"
...
async def test_get_flow_graph_summary_blocks_custom_components(monkeypatch):
    ...
    monkeypatch.setattr(
        "portals.agentic.utils.flow_graph.get_flow_by_id_or_endpoint_name", _get_flow
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        component_cache, "type_to_current_hash", {"ChatInput": "known-hash"}
    )
...
class TestGetFlowGraphSummary:
    ...
    async def test_should_return_metadata(self):
        ...
        flow = _make_flow()
        graph = _make_graph(
            vertex_ids=["a", "b", "c"], edge_pairs=[("a", "b"), ("b", "c")]
        )
