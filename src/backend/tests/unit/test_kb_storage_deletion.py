...
class TestRemoveSqliteLockFiles:
    ...
    def test_should_remove_all_lock_files(self, kb_dir):
        from portals.api.utils.kb_helpers import _remove_sqlite_lock_files
...
class TestTruncateSqliteFiles:
    ...
    def test_should_truncate_sqlite_files_to_zero(self, kb_dir):
        from portals.api.utils.kb_helpers import _truncate_sqlite_files
...
class TestReleaseChromaResources:
    ...
    def test_should_clear_registry_entry_for_path(self, kb_dir):
        ...
        from portals.api.utils.kb_helpers import KBStorageHelper
    ...
    @patch("portals.api.utils.kb_helpers.gc.collect")
...
class TestDeleteStorage:
    ...
    @patch(
        "portals.api.utils.kb_helpers.KBStorageHelper.get_fresh_chroma_client",
        new=MagicMock(),
    )
    @patch("portals.api.utils.kb_helpers.Chroma", new=MagicMock())
    @patch("portals.api.utils.kb_helpers.time.sleep", new=MagicMock())
    ...
    def test_should_retry_and_succeed_on_second_attempt(self, kb_dir):
        ...
        with patch(
            "portals.api.utils.kb_helpers.shutil.rmtree", side_effect=rmtree_fails_once
        ):
            result = KBStorageHelper.delete_storage(kb_dir, "test_kb")
    ...
    def test_should_rename_as_fallback_when_all_retries_fail(self, kb_dir):
        ...
        with patch(
            "portals.api.utils.kb_helpers.shutil.rmtree",
        ...
        assert not kb_dir.exists()
        renamed_dirs = [
            p for p in kb_dir.parent.iterdir() if p.name.startswith(".deleted_test_kb_")
        ]
    ...
    @patch("portals.api.utils.kb_helpers.Chroma", new=MagicMock())
    @patch("portals.api.utils.kb_helpers.time.sleep")
    def test_should_use_exponential_backoff_on_retries(self, mock_sleep, kb_dir):
        ...
        with patch(
            "portals.api.utils.kb_helpers.shutil.rmtree",
            side_effect=rmtree_fails_three_times,
        ):
            result = KBStorageHelper.delete_storage(kb_dir, "test_kb")
    ...
    @patch("portals.api.utils.kb_helpers.time.sleep", new=MagicMock())
    def test_should_teardown_collection_before_deletion(
        self, mock_chroma_cls, mock_client_cls, kb_dir
    ):
        from portals.api.utils.kb_helpers import KBStorageHelper
...
class TestDeleteEndpoint:
    ...
    async def test_should_delete_kb_successfully(
        ...
        response = await client.delete(
            "api/v1/knowledge_bases/My_KB", headers=logged_in_headers
        )

        assert response.status_code == 200

    @patch(
        "portals.api.utils.kb_helpers.KBStorageHelper.delete_storage",
        return_value=False,
    )
    @patch("portals.api.v1.knowledge_bases.KBStorageHelper.get_root_path")
    ...
    async def test_should_return_404_when_kb_not_found(self, client, logged_in_headers):
        response = await client.delete(
            "api/v1/knowledge_bases/NonExistent_KB", headers=logged_in_headers
        )
...
class TestBulkDeleteEndpoint:
    ...
    @patch("portals.api.utils.kb_helpers.time.sleep", new=MagicMock())
    @patch("portals.api.v1.knowledge_bases.KBStorageHelper.get_root_path")
    async def test_should_delete_multiple_kbs(
        self, mock_root, client, logged_in_headers, tmp_path
    ):
        mock_root.return_value = tmp_path
    ...
    @patch("portals.api.utils.kb_helpers.KBStorageHelper.delete_storage")
    @patch("portals.api.v1.knowledge_bases.KBStorageHelper.get_root_path")
    async def test_should_handle_partial_failure(
        self, mock_root, mock_delete, client, logged_in_headers, tmp_path
    ):
        mock_root.return_value = tmp_path
        kb_user_path = tmp_path / "activeuser"
    ...
    @patch("portals.api.v1.knowledge_bases.KBStorageHelper.get_root_path")
    async def test_should_report_not_found_kbs(
        self, mock_root, client, logged_in_headers, tmp_path
    ):
        mock_root.return_value = tmp_path
        kb_user_path = tmp_path / "activeuser"
