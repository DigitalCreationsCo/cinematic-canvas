...
from px.custom import Component, CustomComponent
from px.custom.code_parser.code_parser import CodeParser, CodeSyntaxError
from px.custom.custom_component.base_component import BaseComponent, ComponentCodeNullError
...
from px.field_typing import BaseLanguageModel
...
def test_code_parser_parse_classes():
    ...
    parser = CodeParser("from portals.custom import Component\n\nclass Test(Component): pass")
    tree = parser.get_tree()
    for node in ast.walk(tree):
