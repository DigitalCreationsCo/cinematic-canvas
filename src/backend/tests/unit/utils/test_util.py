...
from portals.utils.util import (
...
class TestBuildTemplateFromFunction:
    ...
    @patch("px.utils.util.parse")
    @patch("px.utils.util.get_default_factory")
    @patch("px.utils.util.get_base_classes")
    @patch("px.utils.util.format_dict")
    def test_successful_template_build(
        ...
        field_mock.__repr_args__ = Mock(
            return_value=[("default_factory", "test_factory")]
        )
    ...
    def test_add_function_base_class(self):
        ...
        with (
            patch("px.utils.util.parse") as mock_parse,
            patch("px.utils.util.get_base_classes") as mock_get_base_classes,
            patch("px.utils.util.format_dict") as mock_format_dict,
        ):
            mock_parse.return_value = Mock(short_description="Test", params={})
            mock_get_base_classes.return_value = ["BaseClass"]
            mock_format_dict.return_value = {}

            result = build_template_from_function(
                "TestClass", type_dict, add_function=True
            )
...
class TestBuildTemplateFromMethod:
    ...
    def test_method_not_found(self):
        ...
        with (
            ...
            pytest.raises(
                ValueError, match="Method test_method not found in class TestClass"
            ),
        ):
            build_template_from_method("TestClass", "test_method", type_dict)
    ...
    @patch("px.utils.util.get_base_classes")
    @patch("px.utils.util.format_dict")
    def test_successful_method_template_build(
        self, mock_format_dict, mock_get_base_classes, mock_parse
    ):
...
class TestFormatDict:
    ...
    def test_format_dict_basic(self):
        ...
        with (
            patch("px.utils.util.get_type") as mock_get_type,
            patch("px.utils.util.should_show_field") as mock_show,
            patch("px.utils.util.is_password_field") as mock_password,
            patch("px.utils.util.is_multiline_field") as mock_multiline,
        ):
    ...
    def test_format_dict_skips_basemodel(self):
        ...
        with patch("portals.utils.util.get_type", return_value="SomeBaseModel"):
            result = format_dict(test_dict)
...
class TestUtilityFunctions:
    ...
    def test_add_options_to_field(self):
        ...
        with patch("px.utils.util.constants") as mock_constants:
...
class TestUpdateSettings:
    ...
    @pytest.mark.asyncio
    @patch("px.utils.util.get_settings_service")
