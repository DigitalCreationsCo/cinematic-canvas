...
import pytest
from portals.utils.image import (
...
def test_image_content_dict_no_legacy_fields(sample_image):
    ...
    for field in legacy_fields:
        assert field not in content_dict, (
            f"Legacy field '{field}' should not be present"
        )
        assert field not in content_dict.get("image_url", {}), (
            f"Legacy field '{field}' should not be in image_url"
        )
