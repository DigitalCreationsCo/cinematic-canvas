...
import pytest
from px.utils.concurrency import KeyedMemoryLockManager, KeyedWorkerLockManager
...
class TestKeyedWorkerLockManager:
    ...
    def test_initialization(self):
        ...
        with (
            patch("px.utils.concurrency.user_cache_dir") as mock_cache_dir,
            patch("px.utils.concurrency.Path") as mock_path,
        ):
            ...
            mock_cache_dir.assert_called_once_with("portals", ensure_exists=True)
    ...
    def test_lock_with_valid_key(self):
        ...
        with patch("px.utils.concurrency.FileLock") as mock_filelock:
    ...
    def test_lock_file_path_construction(self):
        ...
        with (
            patch("px.utils.concurrency.user_cache_dir") as mock_cache_dir,
            patch("px.utils.concurrency.Path") as mock_path,
            patch("px.utils.concurrency.FileLock") as mock_filelock,
        ):
    ...
    @patch("px.utils.concurrency.user_cache_dir")
    def test_locks_directory_creation(self, mock_cache_dir):
        ...
        with patch("px.utils.concurrency.Path") as mock_path:
            mock_path_instance = MagicMock()
            mock_path.return_value = mock_path_instance
