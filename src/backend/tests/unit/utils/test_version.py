...
import httpx
import pytest
from portals.utils.version import (
...
class TestGetVersionInfo:
    ...
    @patch("portals.utils.version.metadata")
    def test_get_version_info_portals_package(self, mock_metadata):
        """Test getting version info for portals package."""
        ...
        assert result["package"] == "Portals"

    @patch("portals.utils.version.metadata")
    def test_get_version_info_portals_base_package(self, mock_metadata):
        """Test getting version info for portals-base package."""
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Portals Base"
    ...
    def test_get_version_info_nightly_package(self, mock_metadata):
        ...
        def mock_version(pkg_name):
            if pkg_name in ["portals", "portals-base"]:
                raise mock_metadata.PackageNotFoundError
            if pkg_name == "portals-nightly":
                return "1.0.0.dev456"
            raise mock_metadata.PackageNotFoundError
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Portals Nightly"
    ...
    def test_get_version_info_base_nightly_package(self, mock_metadata):
        ...
        def mock_version(pkg_name):
            if pkg_name in ["portals", "portals-base", "portals-nightly"]:
                raise mock_metadata.PackageNotFoundError
            if pkg_name == "portals-base-nightly":
                return "1.0.0.a1"
            raise mock_metadata.PackageNotFoundError
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Portals Base Nightly"
...
class TestFetchLatestVersion:
    ...
    def test_fetch_latest_version_with_prerelease(self, mock_httpx):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {
            "releases": {"1.0.0": [], "1.1.0": [], "2.0.0a1": [], "2.0.0b1": []}
        }
        mock_httpx.get.return_value = mock_response
    ...
    def test_fetch_latest_version_no_stable_versions(self, mock_httpx):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {
            "releases": {"1.0.0a1": [], "1.0.0b1": [], "1.0.0rc1": []}
        }
        mock_httpx.get.return_value = mock_response
    ...
    def test_fetch_latest_version_package_name_normalization(self, mock_httpx):
        ...
        mock_httpx.get.assert_called_once_with(
            "https://pypi.org/pypi/test-package-name/json"
        )
...
class TestGetVersionInfoFunction:
    ...
    @patch("portals.utils.version.VERSION_INFO")
    def test_get_version_info_returns_version_info(self, mock_version_info):
        ...
        with patch("portals.utils.version.VERSION_INFO", mock_version_info):
