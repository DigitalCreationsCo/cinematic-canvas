...
import pytest
from portals.utils.schemas import ChatOutputResponse, ContainsEnumMeta, DataOutputResponse, File
...
class TestChatOutputResponse:
    ...
    def test_chat_response_with_uuid_session_id_converts_to_string(self):
        """Test UUID session_id conversion to string."""
        session_id = uuid4()
        response = ChatOutputResponse(message="Test message", session_id=session_id, type="text")
    ...
    def test_validate_files_missing_name_and_type(self):
        ...
        with (
            patch("portals.utils.schemas.TEXT_FILE_TYPES", ["pdf", "txt"]),
            patch("portals.utils.schemas.IMG_FILE_TYPES", ["jpg", "png"]),
        ):
            response = ChatOutputResponse(message="Test", files=files, type="text")
    ...
    def test_validate_files_unknown_type_raises_error(self):
        ...
        with (
            patch("portals.utils.schemas.TEXT_FILE_TYPES", ["txt"]),
            patch("portals.utils.schemas.IMG_FILE_TYPES", ["jpg"]),
