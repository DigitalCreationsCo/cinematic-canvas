...
from px.custom.validate import (
    _create_portals_execution_context,
    add_type_ignores,
...
"""
    ...
    @patch("px.custom.validate.logger")
...
class TestCreatePortalsExecutionContext:
    """Test cases for _create_portals_execution_context function."""

    def test_creates_context_with_portals_imports(self):
        """Test that context includes portals imports."""
        # The function imports modules inside try/except blocks
        # We don't need to patch anything, just test it works
        context = _create_portals_execution_context()
    ...
    def test_does_not_include_pandas(self):
        """Test that pandas is not included in the portals execution context."""
        context = _create_portals_execution_context()
...
"""
        ...
        with patch("px.custom.validate.prepare_global_scope") as mock_prepare:
            mock_prepare.return_value = {"CustomComponent": type("CustomComponent", (), {})}
            with patch("px.custom.validate.extract_class_code") as mock_extract:
                mock_extract.return_value = Mock()
                with patch("px.custom.validate.compile_class_code") as mock_compile:
                    mock_compile.return_value = compile("pass", "<string>", "exec")
                    with patch("px.custom.validate.build_class_constructor") as mock_build:
...
"""
        ...
        with (
            patch("px.custom.validate.prepare_global_scope", side_effect=validation_error),
...
class TestGetDefaultImports:
    ...
    @patch("px.field_typing.constants.CUSTOM_COMPONENT_SUPPORTED_TYPES", {"TestType": Mock()})
    ...
    def test_includes_portals_imports(self):
        """Test that portals imports are included when found in code."""
        # Use an actual type from CUSTOM_COMPONENT_SUPPORTED_TYPES
        code = "Chain is used here"

        with patch("px.custom.validate.importlib") as mock_importlib:
            mock_module = Mock()
