...
class TestSettingsInitializationOrder:
    ...
    def test_is_settings_service_initialized_returns_false_initially(self):
        ...
        from portals.services.deps import is_settings_service_initialized
    ...
    def test_is_settings_service_initialized_returns_true_after_init(self):
        ...
        from portals.services.deps import get_settings_service, is_settings_service_initialized
        from px.services.manager import get_service_manager
    ...
    def test_dotenv_loading_before_settings_init(self, tmp_path):
        ...
        env_file.write_text("PORTALS_SAVE_DB_IN_CONFIG_DIR=true\n")
        ...
        assert os.environ.get("PORTALS_SAVE_DB_IN_CONFIG_DIR") == "true"
        ...
        # Clean up
        if "PORTALS_SAVE_DB_IN_CONFIG_DIR" in os.environ:
            del os.environ["PORTALS_SAVE_DB_IN_CONFIG_DIR"]
    ...
    def test_cli_check_pattern_success_case(self, tmp_path):
        ...
        env_file.write_text("PORTALS_DATABASE_URL=sqlite:///./test.db\n")
        ...
        if env_file:
            ...
            if is_settings_service_initialized():
                ...
                load_dotenv(env_file, override=True)
                assert os.environ.get("PORTALS_DATABASE_URL") == "sqlite:///./test.db"

        # Clean up
        if "PORTALS_DATABASE_URL" in os.environ:
            del os.environ["PORTALS_DATABASE_URL"]
...
class TestCLISubprocessIntegration:
    ...
    def test_cli_env_file_values_actually_used(self, tmp_path):
        ...
        env_file.write_text(
            f"""
PORTALS_DATABASE_URL=sqlite:///{db_path}
PORTALS_AUTO_SAVING=false
PORTALS_AUTO_LOGIN=false
PORTALS_LOG_LEVEL=ERROR
        """.strip()
        )

        # Create a test script that starts portals and checks if the database was created
...
# Start portals run with --env-file in background
...
proc = subprocess.Popen(
    [
        sys.executable, "-m", "portals", "run", "--env-file", str(env_file),
