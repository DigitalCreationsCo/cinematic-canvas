...
# Import portals validation utilities
from portals.utils.template_validation import (
...
def get_starter_projects_path() -> Path:
    ...
    return Path("src/backend/base/portals/initial_setup/starter_projects")
...
class TestStarterProjects:
    ...
    @pytest.mark.parametrize(
        "template_file", get_template_files(), ids=lambda x: x.name
    )
    ...
    def test_template_structure(self, template_file):
        ...
        if errors:
            error_msg = "\n".join(errors)
            pytest.fail(
                f"Template structure errors in {template_file.name}:\n{error_msg}"
            )
    ...
    async def test_template_validate_endpoint(
        self, template_file, client, logged_in_headers
    ):
        """Test template using the validate endpoint."""
        with template_file.open(encoding="utf-8") as f:
            template_data = json.load(f)

        errors = await validate_flow_execution(
            client, template_data, template_file.name, logged_in_headers
        )
        if errors:
            error_msg = "\n".join(errors)
            pytest.fail(
                f"Endpoint validation errors in {template_file.name}:\n{error_msg}"
            )
    ...
    async def test_template_flow_execution(
        self, template_file, client, logged_in_headers
    ):
        ...
        try:
            ...
            if errors:
                error_msg = "\n".join(errors)
                pytest.fail(
                    f"Template execution errors in {template_file.name}:\n{error_msg}"
                )
            ...
            pytest.fail(
                f"{template_file.name}: Unexpected error during validation: {e!s}"
            )
    ...
    def test_template_field_order_matches_component(self, template_file):
        ...
        for node in template_data.get("data", {}).get("nodes", []):
            ...
            # Parse module path: "px.components.foo.bar.ClassName"
            ...
            if json_field_order != component_field_order:
                ...
                missing = [
                    f for f in component_field_order if f not in json_field_order
                ]
    ...
    def test_template_nodes_no_overlap(self, template_file):
        ...
        for node in nodes:
            ...
            if x is None or y is None or w is None or h is None:
                continue
            display = node.get("data", {}).get("display_name") or node.get(
                "data", {}
            ).get("type", node.get("id", "?"))
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "template_file", get_basic_template_files(), ids=lambda x: x.name
    )
    async def test_basic_template_flow_execution(
        self, template_file, client, logged_in_headers
    ):
        ...
        try:
            ...
            if errors:
                error_msg = "\n".join(errors)
                pytest.fail(
                    f"Basic template execution errors in {template_file.name}:\n{error_msg}"
                )
