...
from portals.api.v1.flows_helpers import _new_flow, _save_flow_to_fs
...
async def test_new_flow_with_validate_folder_rejects_unknown_folder(async_session, current_user, storage_service):
    ...
    with (
        patch("portals.api.v1.flows_helpers.get_default_folder_id", new=AsyncMock()) as mock_default_folder_id,
...
async def test_save_flow_to_fs_returns_500_on_os_error(current_user, storage_service):
    ...
    with (
        patch("portals.api.v1.flows_helpers.aiofiles.open", side_effect=OSError("disk full")),
        pytest.raises(HTTPException) as exc_info,
    ):
