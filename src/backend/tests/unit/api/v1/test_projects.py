...
from portals.initial_setup.constants import STARTER_FOLDER_NAME
...
from portals.services.database.models.flow_version_deployment_attachment.model import (
...
async def test_create_project(client: AsyncClient, logged_in_headers, basic_case):
    response = await client.post(
        "api/v1/projects/", json=basic_case, headers=logged_in_headers
    )
    ...
    assert "description" in result, (
        "The dictionary must contain a key called 'description'"
    )
...
async def test_create_project_duplicate_name_escapes_like_wildcards(
    ...
    response = await client.post(
        "api/v1/projects/", json=unrelated, headers=logged_in_headers
    )
    assert response.status_code == status.HTTP_201_CREATED

    response = await client.post(
        "api/v1/projects/", json=wildcard, headers=logged_in_headers
    )
    assert response.status_code == status.HTTP_201_CREATED
...
async def test_read_project(client: AsyncClient, logged_in_headers, basic_case):
    ...
    if isinstance(result, dict) and "folder" in result:
        ...
        assert "description" in folder_data, (
            "The dictionary must contain a key called 'description'"
        )
...
async def test_update_project(client: AsyncClient, logged_in_headers, basic_case):
    ...
    response = await client.patch(
        f"api/v1/projects/{id_}", json=update_case, headers=logged_in_headers
    )
    result = response.json()
...
async def test_create_project_validation_error(
    ...
    response = await client.post(
        "api/v1/projects/", json=invalid_case, headers=logged_in_headers
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


async def test_delete_project_then_404(
    client: AsyncClient, logged_in_headers, basic_case
):
    ...
    del_resp = await client.delete(
        f"api/v1/projects/{proj_id}", headers=logged_in_headers
    )
...
async def test_read_projects_pagination(client: AsyncClient, logged_in_headers):
    response = await client.get(
        "api/v1/projects/?limit=1&offset=0", headers=logged_in_headers
    )
    assert response.status_code == status.HTTP_200_OK
    result = response.json()
...
async def test_create_and_read_project_cyrillic(client: AsyncClient, logged_in_headers):
    ...
    create_resp = await client.post(
        "api/v1/projects/", json=payload, headers=logged_in_headers
    )
...
async def test_update_project_preserves_flows(client: AsyncClient, logged_in_headers):
    ...
    create_resp = await client.post(
        "api/v1/projects/", json=project_payload, headers=logged_in_headers
    )
    assert create_resp.status_code == status.HTTP_201_CREATED
    ...
    flow1_resp = await client.post(
        "api/v1/flows/", json=flow1_payload, headers=logged_in_headers
    )
    flow2_resp = await client.post(
        "api/v1/flows/", json=flow2_payload, headers=logged_in_headers
    )
    ...
    get_resp = await client.get(
        f"api/v1/projects/{project_id}", headers=logged_in_headers
    )
    assert get_resp.status_code == status.HTTP_200_OK
    ...
    update_resp = await client.patch(
        f"api/v1/projects/{project_id}", json=update_payload, headers=logged_in_headers
    )
    ...
    get_after_resp = await client.get(
        f"api/v1/projects/{project_id}", headers=logged_in_headers
    )
    ...
    assert len(actual_flows_after) == 2, (
        f"Expected 2 flows after rename, got {len(actual_flows_after)}. Flows lost!"
    )
    ...
    flow1_get_resp = await client.get(
        f"api/v1/flows/{flow1_id}", headers=logged_in_headers
    )
    flow2_get_resp = await client.get(
        f"api/v1/flows/{flow2_id}", headers=logged_in_headers
    )
...
async def test_update_project_preserves_components(
    client: AsyncClient, logged_in_headers
):
    ...
    comp1_resp = await client.post(
        "api/v1/flows/", json=comp1_payload, headers=logged_in_headers
    )
    comp2_resp = await client.post(
        "api/v1/flows/", json=comp2_payload, headers=logged_in_headers
    )
    ...
    assert str(comp1_id) in component_ids_after, (
        "Component 1 was lost after project rename!"
    )
    assert str(comp2_id) in component_ids_after, (
        "Component 2 was lost after project rename!"
    )


async def test_update_project_preserves_mixed_flows_and_components(
    client: AsyncClient, logged_in_headers
):
    ...
    flow_resp = await client.post(
        "api/v1/flows/", json=flow_payload, headers=logged_in_headers
    )
    comp_resp = await client.post(
        "api/v1/flows/", json=component_payload, headers=logged_in_headers
    )
...
class TestProjectMCPIntegration:
    ...
    def mock_mcp_settings_enabled(self):
        ...
        with patch(
            "portals.api.v1.projects.get_settings_service"
    ...
    async def test_create_project_with_mcp_auto_add_enabled_success(
        ...
        with (
            patch("portals.api.v1.projects.get_settings_service") as mock_get_settings,
            patch(
                "portals.api.v1.projects_mcp_helpers.get_project_streamable_http_url"
            ) as mock_streamable_url,
            patch(
                "portals.api.v1.projects_mcp_helpers.validate_mcp_server_for_project"
            ) as mock_validate,
            patch(
                "portals.api.v1.projects_mcp_helpers.update_server"
            ) as mock_update_server,
            patch(
                "portals.api.v1.projects_mcp_helpers.create_api_key"
            ) as mock_create_api_key,
            patch(
                "portals.api.v1.projects_mcp_helpers.get_storage_service"
            ) as mock_storage,
        ):
            # Setup mocks
            mock_streamable_url.return_value = (
                "http://localhost:7860/api/v1/mcp/project/test-id/streamable"
            )
            ...
            mock_api_key_response.api_key = (
                "test-api-key-123"  # pragma: allowlist secret
            )
    ...
    async def test_create_project_with_mcp_auto_add_enabled_success_legacy_sse(
        ...
        with (
            patch("portals.api.v1.projects.get_settings_service") as mock_get_settings,
            patch("portals.api.v1.mcp_projects.get_project_sse_url") as mock_sse_url,
            patch(
                "portals.api.v1.projects_mcp_helpers.get_project_streamable_http_url"
            ) as mock_streamable_url,
            patch(
            ...
            # Setup mocks
            mock_sse_url.return_value = (
                "http://localhost:7860/api/v1/mcp/project/test-id/sse"
            )
            mock_streamable_url.return_value = (
                "http://localhost:7860/api/v1/mcp/project/test-id/streamable"
            )
    ...
    async def test_create_project_oauth_not_implemented(
        ...
        with (
            ...
            response = await client.post(
                "api/v1/projects/", json=oauth_case, headers=logged_in_headers
            )
    ...
    async def test_update_project_name_with_mcp_server_update(
        ...
        with (
            ...
            patch(
                "portals.api.v1.projects_mcp_helpers.validate_mcp_server_for_project"
            ) as mock_validate_create,
            patch("portals.api.v1.projects_mcp_helpers.update_server"),
            patch("portals.api.v1.projects_mcp_helpers.create_api_key"),
            patch("portals.api.v1.projects_mcp_helpers.get_storage_service"),
        ):
            # Mock settings to enable MCP auto-add
            mock_settings = MagicMock()
            mock_settings.settings.add_projects_to_mcp_servers = True
    ...
    async def test_update_project_name_with_mcp_conflict(
        ...
        with (
            ...
            mock_new_validation.conflict_message = (
                "Server name conflict with different project"
            )
    ...
    async def test_create_project_auto_login_disabled_adds_api_key_auth(
        ...
        with patch(
            ...
            mock_service = MagicMock()
            mock_service.settings.add_projects_to_mcp_servers = (
                False  # Disable MCP to focus on auth
            )
            mock_service.auth_settings.AUTO_LOGIN = False
            mock_get_settings.return_value = mock_service

            with patch(
                "portals.api.v1.projects.encrypt_auth_settings"
            ) as mock_encrypt:
...
class TestReadProjectBugFix:
    ...
    async def test_read_project_without_pagination_params(
        self, client: AsyncClient, logged_in_headers, basic_case
    ):
    ...
    async def test_read_project_with_pagination_params(
        self, client: AsyncClient, logged_in_headers, basic_case
    ):
        ...
        response = await client.get(
            f"api/v1/projects/{project_id}?page=1&size=10", headers=logged_in_headers
        )
    ...
    async def test_read_project_with_partial_pagination_params(
        ...
        response = await client.get(
            f"api/v1/projects/{project_id}?page=1", headers=logged_in_headers
        )
        assert response.status_code == status.HTTP_200_OK
        result = response.json()
        ...
        response = await client.get(
            f"api/v1/projects/{project_id}?size=10", headers=logged_in_headers
        )
        assert response.status_code == status.HTTP_200_OK
        result = response.json()
    ...
    async def test_read_project_with_filtering_params(
        self, client: AsyncClient, logged_in_headers, basic_case
    ):
        ...
        response = await client.get(
            f"api/v1/projects/{project_id}?is_flow=true", headers=logged_in_headers
        )
        assert response.status_code == status.HTTP_200_OK
        result = response.json()
    ...
    async def test_read_project_consistent_response_structure(
        self, client: AsyncClient, logged_in_headers, basic_case
    ):
        ...
        for test_case in test_cases:
            ...
            assert response.status_code == status.HTTP_200_OK, (
                f"Failed for params: {test_case['params']}"
            )

            result = response.json()
            assert isinstance(result, dict), (
                f"Result should be dict for params: {test_case['params']}"
            )

            if test_case["expect_paginated"]:
                # Paginated response structure
                assert "folder" in result, (
                    f"Paginated response missing 'folder' for params: {test_case['params']}"
                )
                assert "flows" in result, (
                    f"Paginated response missing 'flows' for params: {test_case['params']}"
                )
                assert "items" in result["flows"], (
                    f"Paginated flows missing 'items' for params: {test_case['params']}"
                )
                assert "total" in result["flows"], (
                    f"Paginated flows missing 'total' for params: {test_case['params']}"
                )
            else:
                # Non-paginated response structure
                assert "name" in result, (
                    f"Non-paginated response missing 'name' for params: {test_case['params']}"
                )
                assert "flows" in result, (
                    f"Non-paginated response missing 'flows' for params: {test_case['params']}"
                )
    ...
    async def test_read_project_error_handling_consistency(
        ...
        for params in test_cases:
            response = await client.get(
                f"api/v1/projects/{non_existent_id}{params}", headers=logged_in_headers
            )
            assert response.status_code == status.HTTP_404_NOT_FOUND, (
                f"Should return 404 for params: {params}"
            )

            result = response.json()
            assert "detail" in result, (
                f"Error response should have 'detail' for params: {params}"
            )
...
async def test_download_file_starter_project(
    client: AsyncClient, logged_in_headers, active_user, json_flow
):
    ...
    project_payload = {
        ...
        "description": "Starter projects to help you get started in Portals.",
        "flows_list": [],
        "components_list": [],
    }
    create_response = await client.post(
    ...
    with zipfile.ZipFile(io.BytesIO(zip_content), "r") as zip_file:
        ...
        assert len(file_names) == 3, (
            f"Expected 3 files in zip, got {len(file_names)}: {file_names}"
        )

        # Verify each basic flow file exists and contains valid JSON
        for i in range(2):
            expected_filename = f"Starter Flow {i + 1}.json"
            assert expected_filename in file_names, (
                f"Expected {expected_filename} in zip file"
            )
        ...
        assert note_flow_filename in file_names, (
            f"Expected {note_flow_filename} in zip file"
        )
        ...
        assert "backgroundColor" in note_template, (
            "Note backgroundColor should be preserved"
        )
        ...
        assert api_template["api_key"]["value"] is None, (
            "API key value should be removed/null"
        )
        assert "regular_field" in api_template, "Regular field should be preserved"
        assert api_template["regular_field"]["value"] == "keep-this", (
            "Regular field value should be kept"
        )

    # Clean up: delete the project (which will cascade delete flows)
    delete_response = await client.delete(
        f"api/v1/projects/{starter_project_id}", headers=logged_in_headers
    )
    assert delete_response.status_code == status.HTTP_204_NO_CONTENT


async def test_download_project_missing_returns_404(
    client: AsyncClient, logged_in_headers
):
    response = await client.get(
        f"api/v1/projects/download/{uuid4()}", headers=logged_in_headers
    )
...
async def test_download_project_with_no_flows_returns_404(
    client: AsyncClient, logged_in_headers, basic_case
):
    response = await client.post(
        "api/v1/projects/", json=basic_case, headers=logged_in_headers
    ...
    download_response = await client.get(
        f"api/v1/projects/download/{project_id}", headers=logged_in_headers
    )
...
async def _attach_deployment_to_flow(
    *, user_id: UUID, flow_id: UUID, project_id: UUID
) -> None:
...
async def test_create_project_does_not_reassign_other_users_flows(
    ...
    proj_detail = await client.get(
        f"api/v1/projects/{other_project_id}", headers=other_user_headers
    )
...
async def test_delete_project_with_deployments_returns_409_project_guard(
    ...
    assert (
        "flow cannot be deleted because it has deployed versions" not in detail.lower()
    )
