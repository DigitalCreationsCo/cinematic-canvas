...
from portals.api.v1.mcp_projects import (
    ProjectMCPServer,
...
from portals.services.auth.utils import create_user_longterm_token, get_password_hash
from portals.services.database.models.flow import Flow
from portals.services.database.models.folder import Folder
...
@pytest.mark.parametrize(
    ("args", "urls", "expected"),
    [
        (None, ["https://portals.local/sse"], False),
        ([], ["https://portals.local/sse"], False),
        ([123, {"url": "foo"}], ["https://portals.local/sse"], False),
        (["https://portals.local/sse", 42], ["https://portals.local/sse"], True),
        (["alpha", "beta"], [], False),
    ],
)
...
def mock_project_mcp_server():
    with patch("portals.api.v1.mcp_projects.ProjectMCPServer") as mock:
...
def mock_sse_transport():
    with patch("portals.api.v1.mcp_projects.SseServerTransport") as mock:
...
def mock_current_user_ctx(active_user):
    with patch("portals.api.v1.mcp_projects.current_user_ctx") as mock:
...
def mock_current_project_ctx(mock_project):
    with patch("portals.api.v1.mcp_projects.current_project_ctx") as mock:
...
def disable_mcp_composer_by_default():
    ...
    with patch("portals.api.v1.mcp_projects.get_settings_service") as mock_get_settings:
        from portals.services.deps import get_settings_service
...
async def test_update_project_auth_settings_encryption(
    ...
    from portals.services.auth.mcp_encryption import decrypt_auth_settings
...
async def test_project_sse_creation(user_test_project):
    ...
    assert mcp_server.server.name == f"portals-mcp-project-{project_id}"
...
async def test_project_session_manager_lifespan_handles_cleanup(user_test_project, monkeypatch):
    ...
    monkeypatch.setattr(
        "portals.api.v1.mcp_projects.StreamableHTTPSessionManager.run",
...
async def test_init_mcp_servers_error_handling_streamable():
    ...
    # Apply the patch
    with patch(
        "portals.api.v1.mcp_projects.get_project_mcp_server",
