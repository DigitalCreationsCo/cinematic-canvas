...
Covers the fix for https://github.com/portals-ai/portals/issues/9797 where MCP
SSE transport breaks when Portals sits behind a reverse proxy that adds a URL
prefix (basePath).
"""

import pytest
from mcp.server.sse import SseServerTransport
from portals.api.v1.mcp_projects import get_project_sse, project_sse_transports
...
class TestForwardedPrefixMiddleware:
    ...
    async def _captured_downstream_root_path(
        self, *, configured_root_path, headers=None
    ):
        # Import the middleware logic inline to test it in isolation
        from portals.main import get_settings_service
    ...
    async def test_middleware_sets_root_path_when_enabled(self):
        ...
        root_path = await self._captured_downstream_root_path(
            configured_root_path="/enabled",
            headers={"X-Forwarded-Prefix": "/portals"},
        )

        assert root_path == "/portals"
...
class TestRootPathSetting:
    ...
    def test_root_path_can_be_set(self, monkeypatch):
        ...
        monkeypatch.setenv("PORTALS_ROOT_PATH", "/basePath")
        s = Settings()
