...
async def test_create_folder(client: AsyncClient, logged_in_headers):
    ...
    assert "total_count" in result, (
        "The dictionary must contain a key called 'total_count'"
    )


async def test_create_api_key_route(
    ...
    response = await client.post(
        "api/v1/api_key/", json=basic_case, headers=logged_in_headers
    )
    result = response.json()
    ...
    assert "last_used_at" in result, (
        "The dictionary must contain a key called 'last_used_at'"
    )
    assert "name" in result, "The dictionary must contain a key called 'name'"
    assert "total_uses" in result, (
        "The dictionary must contain a key called 'total_uses'"
    )
    assert "user_id" in result, "The dictionary must contain a key called 'user_id'"


async def test_delete_api_key_route(
    client: AsyncClient, logged_in_headers, active_user
):
    basic_case = {
        "name": "string",
...
async def test_save_store_api_key(client: AsyncClient, logged_in_headers):
    ...
    response = await client.post(
        "api/v1/api_key/store", json=basic_case, headers=logged_in_headers
    )
    result = response.json()
...
async def test_delete_api_key_route_unauthorized(
    client: AsyncClient, logged_in_headers, active_user
):
    ...
    response = await client.delete(
        f"api/v1/api_key/{user1_api_key_id}", headers=user2_headers
    )
    ...
    response = await client.delete(
        f"api/v1/api_key/{user1_api_key_id}", headers=logged_in_headers
    )
    assert response.status_code == status.HTTP_200_OK
