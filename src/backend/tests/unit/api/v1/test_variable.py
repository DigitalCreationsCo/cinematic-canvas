...
async def test_create_variable(
    ...
    response = await client.post(
        "api/v1/variables/", json=generic_variable, headers=logged_in_headers
    )
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__variable_name_already_exists(
    client: AsyncClient, generic_variable, logged_in_headers
):
    await client.post(
        "api/v1/variables/", json=generic_variable, headers=logged_in_headers
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__httpexception(
    client: AsyncClient, credential_variable, logged_in_headers
):
    status_code = 418
    generic_message = "I'm a teapot"

    with mock.patch("portals.services.auth.utils.encrypt_api_key") as m:
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__exception(
    client: AsyncClient, credential_variable, logged_in_headers
):
...
async def test_read_variables(
    client: AsyncClient, generic_variable, credential_variable, logged_in_headers
):
...
async def test_read_variables__empty(client: AsyncClient, logged_in_headers):
    ...
    for variable in all_variables:
        await client.delete(
            f"api/v1/variables/{variable.get('id')}", headers=logged_in_headers
        )
...
async def test_read_variables__(client: AsyncClient, logged_in_headers):
    ...
    with mock.patch(
        "portals.services.variable.service.DatabaseVariableService.get_all",
...
@pytest.mark.usefixtures("active_user")
async def test_update_variable__exception(
    client: AsyncClient, generic_variable, logged_in_headers
):
    wrong_id = uuid4()
    ...
    response = await client.patch(
        f"api/v1/variables/{wrong_id}", json=generic_variable, headers=logged_in_headers
    )
    result = response.json()
...
async def test_delete_variable(
    ...
    response = await client.delete(
        f"api/v1/variables/{saved.get('id')}", headers=logged_in_headers
    )
...
async def test_delete_variable__exception(client: AsyncClient, logged_in_headers):
    ...
    response = await client.delete(
        f"api/v1/variables/{wrong_id}", headers=logged_in_headers
    )
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__openai_api_key_validation_success(
    ...
    for var in all_vars.json():
        if var.get("name") == "OPENAI_API_KEY":
            await client.delete(
                f"api/v1/variables/{var['id']}", headers=logged_in_headers
            )
    ...
    with mock.patch("langchain_openai.ChatOpenAI.invoke") as mock_invoke:
        ...
        response = await client.post(
            "api/v1/variables/", json=openai_variable, headers=logged_in_headers
        )
        result = response.json()
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__openai_api_key_validation_failure(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__anthropic_api_key_validation_success(
    client: AsyncClient, logged_in_headers
):
    ...
    with mock.patch("langchain_anthropic.ChatAnthropic.invoke") as mock_invoke:
        mock_invoke.return_value = "test response"
        response = await client.post(
            "api/v1/variables/", json=anthropic_variable, headers=logged_in_headers
        )
        result = response.json()
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__anthropic_api_key_validation_failure(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__google_api_key_validation_success(
    client: AsyncClient, logged_in_headers
):
    ...
    with mock.patch(
        ...
        mock_invoke.return_value = "test response"
        response = await client.post(
            "api/v1/variables/", json=google_variable, headers=logged_in_headers
        )
        result = response.json()
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__ollama_base_url_validation_success(
    client: AsyncClient, logged_in_headers
):
    ...
    with mock.patch("requests.get") as mock_get:
        ...
        response = await client.post(
            "api/v1/variables/", json=ollama_variable, headers=logged_in_headers
        )
        result = response.json()
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__ollama_base_url_validation_failure(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("active_user")
async def test_create_variable__model_provider_network_error_allows_creation(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("active_user")
async def test_delete_provider_credential_cleans_up_disabled_models(
    client: AsyncClient, logged_in_headers
):
    ...
    with mock.patch("px.base.models.unified_models.validate_model_provider_key"):
...
@pytest.mark.usefixtures("active_user")
async def test_delete_provider_credential_cleans_up_enabled_models(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("active_user")
async def test_delete_non_provider_credential_does_not_cleanup_models(
    client: AsyncClient, logged_in_headers
):
...
@pytest.mark.usefixtures("active_user")
async def test_detect_env_vars_endpoint__returns_detected_names(
    client: AsyncClient, logged_in_headers
):
    ...
    variable_service = SimpleNamespace(
        list_variables=mock.AsyncMock(return_value=["MY_OPENAI_KEY"])
    )

    with (
        mock.patch(
            "portals.api.v1.variable.get_flow_version_entries_by_ids",
            new_callable=mock.AsyncMock,
            return_value={flow_version_id: flow_version},
        ),
        mock.patch(
            "portals.api.v1.variable.get_variable_service",
...
@pytest.mark.usefixtures("active_user")
async def test_detect_env_vars_endpoint__rejects_missing_nodes(
    client: AsyncClient, logged_in_headers
):
    flow_version_id = uuid4()
    flow_version = SimpleNamespace(data={})
    variable_service = SimpleNamespace(
        list_variables=mock.AsyncMock(return_value=["ANY_KEY"])
    )
