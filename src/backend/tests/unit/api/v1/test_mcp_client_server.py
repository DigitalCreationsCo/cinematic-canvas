"""Integration tests for px.mcp.server MCP tools.

Uses the client_fixture (real Portals app via ASGITransport) — no mocking.
Tests the full roundtrip: MCP tool -> PortalsClient -> Portals API -> DB.
"""

import pytest
from httpx import AsyncClient
from px.mcp import server as mcp_server_module
from px.mcp.client import PortalsClient


@pytest.fixture
async def mcp_client(client: AsyncClient, logged_in_headers):
    """Wire up a PortalsClient that uses the test's AsyncClient transport."""
    ...
    lf_client = PortalsClient(server_url="http://testserver", access_token=access_token)
...
class TestRunFlow:
    ...
    async def test_stream_post_yields_events(self, mcp_client, created_api_key):
        """Verify stream_post yields SSE events from Portals's streaming endpoint."""
        mcp_client.api_key = created_api_key.api_key
