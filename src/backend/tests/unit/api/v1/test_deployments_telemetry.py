...
def mock_telemetry_service():
    with patch("portals.api.v1.deployments.get_telemetry_service") as mock_get:
        mock_ts = AsyncMock()
...
@pytest.fixture
def mock_adapter():
    with patch("portals.api.v1.deployments.resolve_deployment_adapter") as mock_resolve:
        mock_ad = AsyncMock()
...
@pytest.fixture
def mock_mapper():
    with patch("portals.api.v1.deployments.get_deployment_mapper") as mock_get:
        mock_map = MagicMock()
        ...
        mock_map.shape_execution_create_result.return_value = {
            "id": "run-1",
...
def mock_db_crud(mock_mapper):
    with ExitStack() as stack:
        mock_create = stack.enter_context(
            patch("portals.api.v1.deployments.create_provider_account_row")
        )
        mock_get_owned = stack.enter_context(
            patch("portals.api.v1.deployments.get_owned_provider_account_or_404")
        )
        _mock_del_prov = stack.enter_context(
            patch("portals.api.v1.deployments.delete_provider_account_row")
        )
        _mock_upd_prov = stack.enter_context(
            patch("portals.api.v1.deployments.update_provider_account_row")
        )
        mock_name_exists = stack.enter_context(
            patch("portals.api.v1.deployments.deployment_name_exists")
        )
        mock_proj_id = stack.enter_context(
            patch("portals.api.v1.deployments.resolve_project_id_for_deployment_create")
        )
        mock_create_dep = stack.enter_context(
            patch("portals.api.v1.deployments.create_deployment_db")
        )
        _mock_attach = stack.enter_context(
            patch("portals.api.v1.deployments.attach_flow_versions")
        )
        mock_res_am = stack.enter_context(
            patch("portals.api.v1.deployments.resolve_adapter_mapper_from_deployment")
        )
        mock_res_patch = stack.enter_context(
            patch("portals.api.v1.deployments.resolve_flow_version_patch_for_update")
        )
        _mock_val_proj = stack.enter_context(
            patch("portals.api.v1.deployments.validate_project_scoped_flow_version_ids")
        )
        mock_list_att = stack.enter_context(
            patch(
                "portals.api.v1.deployments.list_deployment_attachments_for_flow_version_ids"
            )
        )
        _mock_apply_patch = stack.enter_context(
            patch("portals.api.v1.deployments.apply_flow_version_patch_attachments")
        )
        mock_upd_dep = stack.enter_context(
            patch("portals.api.v1.deployments.update_deployment_db")
        )
        mock_res_ad = stack.enter_context(
            patch("portals.api.v1.deployments.resolve_adapter_from_deployment")
        )
        _mock_del_dep = stack.enter_context(
            patch(
                "portals.api.v1.deployments._delete_local_deployment_row_with_commit_retry"
            )
        )
        mock_get_att = stack.enter_context(
            patch("portals.api.v1.deployments.get_attachment_by_provider_snapshot_id")
        )
        mock_get_dep_row = stack.enter_context(
            patch("portals.services.database.models.deployment.crud.get_deployment")
        )
        ...
        mock_upd_fv = stack.enter_context(
            patch(
                "portals.api.v1.deployments.update_flow_version_by_provider_snapshot_id"
            )
        )
        mock_count_deps = stack.enter_context(
            patch(
                "portals.api.v1.deployments._count_provider_deployments_after_reconciliation"
            )
        )
        ...
        mock_create.return_value = AsyncMock(
            id=uuid4(),
            provider_key="watsonx-orchestrate",
            provider_tenant_id="tenant-test",
        )
        ...
        mock_res_ad.return_value = (
            AsyncMock(
                id=uuid4(), resource_key="res-1", deployment_provider_account_id=uuid4()
            ),
            AsyncMock(),
            "watsonx-orchestrate",
            "tenant-test",
        )
        mock_get_att.return_value = AsyncMock(
            deployment_id=uuid4(), flow_version_id=uuid4()
        )
...
async def test_delete_provider_account_telemetry(
    ...
    response = await client.delete(
        f"api/v1/deployments/providers/{uuid4()}", headers=logged_in_headers
    )
...
async def test_delete_deployment_telemetry(
    ...
    response = await client.delete(
        f"api/v1/deployments/{uuid4()}", headers=logged_in_headers
    )
    assert response.status_code == status.HTTP_204_NO_CONTENT
    mock_telemetry_service.log_package_deployment.assert_awaited_once()
    payload = mock_telemetry_service.log_package_deployment.call_args[0][0]
...
async def test_update_snapshot_telemetry(
    ...
    response = await client.patch(
        "api/v1/deployments/snapshots/snap-1",
        json={"flow_version_id": str(uuid4())},
        headers=logged_in_headers,
    )
    assert response.status_code == status.HTTP_200_OK
    mock_telemetry_service.log_package_deployment.assert_awaited_once()
...
async def test_cross_route_smoke_exception_after_provider_set(
    ...
    assert payload.deployment_action == "deployment.create"
    assert (
        payload.deployment_provider == "watsonx-orchestrate"
    )  # Provider should be captured!
    assert payload.deployment_success is False
