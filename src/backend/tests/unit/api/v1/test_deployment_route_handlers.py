...
from portals.api.v1.deployments import DeploymentTelemetryCtx
from portals.api.v1.mappers.deployments.contracts import ProviderSnapshotBinding
from portals.api.v1.schemas.deployments import (
...
ROUTES_MODULE = "portals.api.v1.deployments"
HELPERS_MODULE = "portals.api.v1.mappers.deployments.helpers"
...
def _fake_deployment_row(**overrides) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        deployment_provider_account_id=overrides.get(
...
class TestQueryParameterValidators:
    def test_dedupe_snapshot_names_preserves_order(self):
        from portals.api.v1.deployments import _dedupe_snapshot_names

        assert _dedupe_snapshot_names(
            ["my_tool", "other_tool", "my_tool", "third_tool"]
        ) == [
            "my_tool",
            "other_tool",
            "third_tool",
...
class TestCreateDeploymentRollback:
    ...
    @patch(
        f"{ROUTES_MODULE}.validate_project_scoped_flow_version_ids",
        new_callable=AsyncMock,
    )
    @patch(
        f"{ROUTES_MODULE}.resolve_project_id_for_deployment_create",
        new_callable=AsyncMock,
    )
    @patch(f"{ROUTES_MODULE}.resolve_deployment_adapter")
    @patch(f"{ROUTES_MODULE}.get_deployment_mapper")
    @patch(
        f"{ROUTES_MODULE}.deployment_name_exists",
        new_callable=AsyncMock,
        return_value=False,
    )
    ...
    async def test_rollback_called_on_commit_failure(
        ...
        from portals.api.v1.deployments import create_deployment
        ...
        assert (
            mock_rollback.call_args.kwargs["provider_result"]
            == create_result.provider_result
        )
    ...
    @patch(f"{ROUTES_MODULE}.get_deployment_mapper")
    @patch(
        f"{ROUTES_MODULE}.get_deployment_by_resource_key",
        new_callable=AsyncMock,
        return_value=None,
    )
    @patch(
    ...
    async def test_existing_agent_mutation_commit_failure_uses_non_delete_rollback(
        ...
        adapter.update.return_value = DeploymentUpdateResult(
            id="existing-agent-1", provider_result={"ok": True}
        )
        mock_resolve_adapter.return_value = adapter
        mapper = MagicMock()
        mapper.util_existing_deployment_resource_key_for_create.return_value = (
            "existing-agent-1"
        )
        ...
        mapper.util_create_result_from_existing_update.return_value = (
            DeploymentCreateResult(
                id="existing-agent-1",
...
class TestCreateDeploymentExistingAgent:
    ...
    async def test_existing_agent_without_mutation_skips_provider_calls(
        ...
        mapper.util_create_flow_version_ids.return_value = []
        mapper.util_create_result_from_existing_resource.return_value = (
            DeploymentCreateResult(id="existing-agent-1")
        )
        mock_get_mapper.return_value = mapper
        mock_resolve_project.return_value = uuid4()
    ...
    async def test_existing_agent_with_mutation_uses_mapper_create_result_normalizer(
        ...
        mapped_create_result = DeploymentCreateResult(
            id="existing-agent-1", provider_result={"ok": True}
        )
        mapper.util_create_result_from_existing_update.return_value = (
    ...
    async def test_existing_agent_conflict_when_resource_key_already_persisted(
        ...
        pa = _fake_provider_account()
        mock_get_pa.return_value = pa
        mock_get_by_resource_key.return_value = _fake_deployment_row(
            resource_key="existing-agent-1"
        )
        adapter = AsyncMock()
        mock_resolve_adapter.return_value = adapter
...
class TestListDeploymentsLoadFromProvider:
    ...
    async def test_load_from_provider_bypasses_db_view(
        ...
        from portals.api.v1.deployments import list_deployments
        ...
        mapper.resolve_load_from_provider_deployment_list_params.return_value = {
            "environment": "draft"
        }
        mock_get_mapper.return_value = mapper
...
class TestListDeploymentsFlowIdsFilter:
    ...
    @pytest.mark.asyncio
    @patch(
        f"{ROUTES_MODULE}.flow_version_ids_for_flows",
        new_callable=AsyncMock,
        return_value=[],
    )
...
class TestConfigAndSnapshotListRoutes:
    ...
    async def test_list_configs_global_scope_uses_provider_id(
        ...
        from portals.api.v1.deployments import list_deployment_configs
        ...
        mapper.resolve_config_list_adapter_params = AsyncMock(
            return_value=adapter_params
        )
    ...
    async def test_list_snapshots_deployment_scope_resolves_provider_from_deployment(
        ...
        from portals.api.v1.deployments import list_deployment_snapshots
        ...
        adapter_params = MagicMock()
        expected_response = MagicMock()
        mapper.resolve_snapshot_list_adapter_params = AsyncMock(
            return_value=adapter_params
        )
    ...
    async def test_list_snapshots_rejects_deployment_id_with_names(
        ...
        assert (
            ...
            == "filtering by both deployment_id and names is not supported."
        )
...
class TestUpdateSnapshotRoute:
    @pytest.mark.asyncio
    @patch(
        "portals.services.database.models.flow_version.crud.get_flow_version_entry",
        new_callable=AsyncMock,
    )
    @patch(
        "portals.services.database.models.deployment.crud.get_deployment",
        new_callable=AsyncMock,
    )
    @patch(
        f"{ROUTES_MODULE}.update_flow_version_by_provider_snapshot_id",
        new_callable=AsyncMock,
    )
    @patch(f"{ROUTES_MODULE}.resolve_deployment_adapter")
    @patch(f"{ROUTES_MODULE}.get_deployment_mapper")
    @patch(f"{ROUTES_MODULE}.get_owned_provider_account_or_404", new_callable=AsyncMock)
    @patch(
        f"{ROUTES_MODULE}.get_attachment_by_provider_snapshot_id",
        new_callable=AsyncMock,
    )
    async def test_updates_all_attachment_rows_for_snapshot(
        ...
        from portals.api.v1.deployments import update_snapshot
        ...
        mock_get_flow_version.return_value = SimpleNamespace(
            id=target_flow_version_id, flow_id=flow_id, data={}
        )
    ...
    async def test_commit_failure_attempts_provider_compensation(
        ...
        target_version = SimpleNamespace(
            id=target_flow_version_id, flow_id=flow_id, data={"nodes": []}
        )
        previous_version = SimpleNamespace(
            id=previous_flow_version_id, flow_id=flow_id, data={"nodes": []}
        )
        ...
        mock_resolve_adapter.return_value = adapter
        mapper = MagicMock()
        mapper.resolve_snapshot_update_artifact.side_effect = [
            {"artifact": "new"},
            {"artifact": "previous"},
        ]
        mock_get_mapper.return_value = mapper
...
class TestListDeploymentFlowVersionsRoute:
    @pytest.mark.asyncio
    @patch(
        f"{ROUTES_MODULE}.list_deployment_flow_versions_synced", new_callable=AsyncMock
    )
    @patch(
        f"{ROUTES_MODULE}.resolve_adapter_mapper_from_deployment",
        new_callable=AsyncMock,
    )
    async def test_returns_paginated_flow_versions(
        ...
        from portals.api.v1.deployments import list_deployment_flow_versions
        ...
        rows = [
            (
                SimpleNamespace(provider_snapshot_id="tool-1", created_at=None),
                SimpleNamespace(),
            )
        ]
...
class TestProviderAccountRoutes:
    ...
    async def test_update_provider_account_name_only_skips_credential_verification(
        ...
        from portals.api.v1.deployments import update_provider_account
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_provider_account(
                ...
                payload=DeploymentProviderAccountUpdateRequest(
                    provider_data={"api_key": "new-api-key"}
                ),
                current_user=_fake_user(),
                telemetry=_fake_telemetry(),
            )
    ...
    async def test_create_provider_account_conflict_returns_409(
        ...
        from portals.api.v1.deployments import create_provider_account
        ...
        adapter = AsyncMock()
        mock_resolve_adapter.return_value = adapter
        mock_create_provider_account.side_effect = ValueError(
    ...
    async def test_update_provider_account_raw_guard_exception_propagates(
        ...
        mapper = MagicMock()
        mapper.resolve_verify_credentials_for_update.return_value = None
        mapper.resolve_provider_account_update.return_value = {
            "provider_data": {"tenant_id": "tenant-renamed"}
        }
        mock_get_mapper.return_value = mapper
        mock_resolve_adapter.return_value = AsyncMock()
    ...
    @patch(
        f"{ROUTES_MODULE}.list_deployments_synced",
        new_callable=AsyncMock,
        return_value=([], 1),
    )
    @patch(
        f"{ROUTES_MODULE}.count_deployments_by_provider",
        new_callable=AsyncMock,
        return_value=1,
    )
    @patch(f"{ROUTES_MODULE}.resolve_deployment_adapter")
    ...
    async def test_delete_provider_account_rejects_when_deployments_exist(
        ...
        from portals.api.v1.deployments import delete_provider_account
    ...
    async def test_delete_provider_account_sets_deployment_provider_scope_for_reconciliation(
        ...
        """
        from portals.api.v1.deployments import delete_provider_account
        from portals.services.adapters.deployment.context import (
        ...
        async def capture_scope(*_args, **_kwargs):
            ...
            observed_provider_ids.append(
                None if current is None else current.provider_id
            )
            return ([], 0)
...
class TestUpdateDeploymentRollback:
    ...
    @patch(
        f"{ROUTES_MODULE}.apply_flow_version_patch_attachments", new_callable=AsyncMock
    )
    @patch(
        f"{ROUTES_MODULE}.resolve_added_snapshot_bindings_for_update", return_value=[]
    )
    @patch(
        f"{ROUTES_MODULE}.validate_project_scoped_flow_version_ids",
        new_callable=AsyncMock,
    ...
    @patch(
        f"{ROUTES_MODULE}.resolve_flow_version_patch_for_update", return_value=([], [])
    )
    @patch(
        f"{ROUTES_MODULE}.resolve_adapter_mapper_from_deployment",
        new_callable=AsyncMock,
    ...
    async def test_rollback_called_on_commit_failure(
        ...
        from portals.api.v1.deployments import update_deployment
        ...
        assert (
            mock_rollback.call_args.kwargs["deployment_resource_key"]
            == dep_row.resource_key
        )
        assert (
            mock_rollback.call_args.kwargs["deployment_provider_account_id"]
            == dep_row.deployment_provider_account_id
        )
...
class TestUpdateDeploymentAlreadyAttachedFiltering:
    ...
    @patch(
        f"{ROUTES_MODULE}.list_deployment_attachments_for_flow_version_ids",
        new_callable=AsyncMock,
    )
    @patch(
        f"{ROUTES_MODULE}.validate_project_scoped_flow_version_ids",
    ...
    async def test_already_attached_flow_versions_excluded_from_snapshot_resolution(
        ...
        mock_list_attachments.return_value = [
            SimpleNamespace(
                flow_version_id=reused_fv_id, provider_snapshot_id="existing-tool-1"
            ),
        ]
...
class TestGetDeploymentSync:
    @pytest.mark.asyncio
    @patch(
        f"{ROUTES_MODULE}.list_deployment_attachments",
        new_callable=AsyncMock,
        return_value=[],
    )
    ...
    async def test_stale_row_deleted_when_provider_returns_not_found(
        ...
        from portals.api.v1.deployments import get_deployment
        from portals.api.v1.mappers.deployments.base import BaseDeploymentMapper
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_deployment(
                deployment_id=dep_row.id, session=session, current_user=user
            )

        assert exc_info.value.status_code == 404
        mock_delete_row.assert_awaited_once_with(
            session, user_id=user.id, deployment_id=dep_row.id
        )
        session.commit.assert_awaited_once()
    ...
    async def test_non_404_adapter_error_does_not_delete_row(
        ...
        dep_row = _fake_deployment_row()
        adapter = AsyncMock()
        adapter.get.side_effect = AuthenticationError(
            message="bad creds", error_code="authentication_error"
        )
        mock_resolve.return_value = (
            dep_row,
            adapter,
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_deployment(
                deployment_id=dep_row.id, session=session, current_user=_fake_user()
            )
    ...
    async def test_snapshot_sync_corrects_attached_count(
        ...
        mapper.extract_snapshot_bindings_for_get.return_value = [
            ProviderSnapshotBinding(
                resource_key=dep_row.resource_key, snapshot_id="snap-1"
            )
        ]
    ...
    async def test_unsupported_provider_falls_back_to_unverified_count(
        ...
        provider_deployment.model_dump.return_value = {}
        adapter.get.return_value = provider_deployment
        mapper.extract_snapshot_bindings_for_get.side_effect = NotImplementedError(
            "not supported"
        )
        mapper.shape_deployment_get_data.return_value = None
        mock_resolve.return_value = (
...
class TestDeleteDeployment:
    ...
    async def test_provider_not_found_still_deletes_local_row(
        ...
        from portals.api.v1.deployments import delete_deployment
    ...
    async def test_non_404_provider_error_does_not_delete_local_row(
        ...
        dep_row = _fake_deployment_row()
        adapter = AsyncMock()
        adapter.delete.side_effect = AuthenticationError(
            message="bad creds", error_code="authentication_error"
        )
        mock_resolve.return_value = (
            dep_row,
...
class TestCreateDeploymentProjectValidation:
    ...
    async def test_empty_flow_version_ids_skips_validation(
        ...
        with (
            patch(
                f"{ROUTES_MODULE}.create_deployment_db",
                new_callable=AsyncMock,
                return_value=_fake_deployment_row(),
            ),
            patch(f"{ROUTES_MODULE}.attach_flow_versions", new_callable=AsyncMock),
        ):
            mapper.shape_deployment_create_result.return_value = MagicMock()
...
class TestUpdateDeploymentProjectValidation:
    ...
    async def test_validation_called_before_adapter(
        ...
        mock_validate_fv.side_effect = HTTPException(
            status_code=404, detail="out of project"
        )
...
class TestListDeploymentLlms:
    ...
    async def test_list_llms_returns_shaped_response(
        ...
        from portals.api.v1.deployments import list_deployment_llms
...
class TestResolveDeploymentAdapter:
    def test_empty_provider_key_raises_400(self):
        from portals.api.v1.mappers.deployments.helpers import (
            resolve_deployment_adapter,
        )
    ...
    @patch(
        f"{HELPERS_MODULE}.get_deployment_adapter",
...
class TestHandleAdapterErrors:
    def test_maps_authentication_error_to_401(self):
        from portals.api.v1.mappers.deployments.helpers import handle_adapter_errors
    ...
    def test_maps_conflict_passes_structured_resource_to_mapper_formatter(self):
        ...
        with (
            ...
            raise ResourceConflictError(
                message="raw provider conflict", resource="tool"
            )
