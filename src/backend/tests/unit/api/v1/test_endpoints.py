...
from portals.api.v1.schemas import CustomComponentRequest, UpdateCustomComponentRequest
from px.components.models_and_agents.agent import AgentComponent
...
async def test_get_version(client: AsyncClient):
    ...
    assert "main_version" in result, (
        "The dictionary must contain a key called 'main_version'"
    )
...
async def test_get_config_basic(client: AsyncClient, logged_in_headers: dict):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert isinstance(result, dict), "The result must be a dictionary"
    assert "frontend_timeout" in result, (
        "The dictionary must contain a key called 'frontend_timeout'"
    )
    assert "auto_saving" in result, (
        "The dictionary must contain a key called 'auto_saving'"
    )
    assert "health_check_max_retries" in result, (
        "The dictionary must contain a 'health_check_max_retries' key"
    )
    assert "max_file_size_upload" in result, (
        "The dictionary must contain a key called 'max_file_size_upload'"
    )
...
async def test_update_component_model_name_options(
    ...
    assert "model" in template, (
        f"model field not found. Available fields: {list(template.keys())}"
    )

    # load the code from the file at px.components.models_and_agents.agent.py asynchronously
    # we are at str/backend/tests/unit/api/v1/test_endpoints.py
    # find the file by using the class AgentComponent
    agent_component_file = await asyncio.to_thread(
        inspect.getsourcefile, AgentComponent
    )
...
async def test_custom_component_endpoint_returns_metadata(
    client: AsyncClient, logged_in_headers: dict
):
    """Test that the /custom_component endpoint returns metadata with module and code_hash."""
    component_code = """
from px.custom import Component
from px.inputs import MessageTextInput
...
"""
    ...
    response = await client.post(
        "api/v1/custom_component", json=request.model_dump(), headers=logged_in_headers
    )
    result = response.json()
...
async def test_custom_component_endpoint_metadata_consistency(
    client: AsyncClient, logged_in_headers: dict
):
...
async def test_get_config_without_authentication_returns_public_config(
    client: AsyncClient,
):
...
async def test_get_config_unauthenticated_returns_expected_fields(client: AsyncClient):
    ...
    assert "max_file_size_upload" in result, (
        "Response must contain 'max_file_size_upload'"
    )
    ...
    assert "voice_mode_available" in result, (
        "Response must contain 'voice_mode_available'"
    )
    ...
    assert result["type"] == "public", (
        "Unauthenticated response must have type='public'"
    )


async def test_get_config_unauthenticated_does_not_expose_sensitive_fields(
    client: AsyncClient,
):
    ...
    for field in sensitive_fields:
        assert field not in result, (
            f"Sensitive field '{field}' should not be exposed in unauthenticated config"
        )


async def test_get_config_unauthenticated_returns_correct_field_types(
    client: AsyncClient,
):
    ...
    # Verify field types
    assert isinstance(result["max_file_size_upload"], int), (
        "max_file_size_upload must be an integer"
    )
    assert isinstance(result["frontend_timeout"], int), (
        "frontend_timeout must be an integer"
    )
    assert isinstance(result["voice_mode_available"], bool), (
        "voice_mode_available must be a boolean"
    )
    assert isinstance(result["feature_flags"], dict), "feature_flags must be an object"
    assert result["feature_flags"].get("wxo_deployments") is False, (
        "wxo_deployments flag should default to false"
    )
...
async def test_get_config_returns_500_on_settings_error(
    ...
    monkeypatch.setattr(
        "portals.api.v1.endpoints.get_settings_service", raise_settings_error
    )
...
async def test_get_config_authenticated_returns_full_config(
    client: AsyncClient, logged_in_headers: dict
):
    ...
    assert "auto_saving_interval" in result, (
        "Authenticated response must contain 'auto_saving_interval'"
    )
    assert "health_check_max_retries" in result, (
        "Authenticated response must contain 'health_check_max_retries'"
    )
    assert "feature_flags" in result, (
        "Authenticated response must contain 'feature_flags'"
    )


async def test_get_config_returns_mcp_base_url(
    client: AsyncClient, logged_in_headers: dict
):
    ...
    result = response.json()
    assert response.status_code == status.HTTP_200_OK
    assert "mcp_base_url" in result, (
        "Authenticated response must contain 'mcp_base_url'"
    )
...
async def test_get_config_mcp_base_url_defaults_to_empty(
    client: AsyncClient, logged_in_headers: dict
):
    """Test that mcp_base_url defaults to empty string when PORTALS_MCP_BASE_URL is not set."""
    response = await client.get("api/v1/config", headers=logged_in_headers)
    result = response.json()
    assert response.status_code == status.HTTP_200_OK
...
async def test_get_config_mcp_base_url_from_settings(
    ...
    monkeypatch.setattr(
        settings_service.settings, "mcp_base_url", "https://portals.example.com"
    )

    response = await client.get("api/v1/config", headers=logged_in_headers)
    result = response.json()
    assert response.status_code == status.HTTP_200_OK
    assert result["mcp_base_url"] == "https://portals.example.com"
...
async def test_deprecated_upload_authenticated_succeeds(
    client: AsyncClient, logged_in_headers: dict, flow
):
