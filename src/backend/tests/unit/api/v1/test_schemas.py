import pytest
from portals.api.v1.schemas import VertexBuildResponse
from portals.serialization.constants import MAX_ITEMS_LENGTH
...
def assert_vertex_response_structure(result):
    ...
    assert set(result["data"]["outputs"]["dataframe"].keys()).issuperset(
        expected_keys_outputs
    )
...
def test_vertex_response_structure_when_truncate_applies():
    ...
    assert_vertex_response_structure(result)
    assert (
        len(result["data"]["outputs"]["dataframe"]["message"]) == MAX_ITEMS_LENGTH + 1
    )
