...
from portals.api.v2.files import (
    delete_all_files,
    delete_file,
    delete_files_batch,
    is_permanent_storage_failure,
)
from portals.api.v2.mcp import get_mcp_file
...
def max_file_size_upload_fixture(monkeypatch):
    monkeypatch.setenv("PORTALS_MAX_FILE_SIZE_UPLOAD", "1")
    yield
    monkeypatch.undo()
...
def max_file_size_upload_10mb_fixture(monkeypatch):
    monkeypatch.setenv("PORTALS_MAX_FILE_SIZE_UPLOAD", "10")
    yield
    monkeypatch.undo()
...
async def s3_files_client_fixture(
    ...
    if "noclient" in request.keywords:
        ...
        def init_app():
            ...
            monkeypatch.setenv("PORTALS_STORAGE_TYPE", "s3")
            monkeypatch.setenv(
                "PORTALS_OBJECT_STORAGE_BUCKET_NAME",
                os.environ.get("PORTALS_OBJECT_STORAGE_BUCKET_NAME", "portals-ci"),
            )
            # Use unique prefix per test run to avoid conflicts
            test_prefix = f"test-files-api-{uuid.uuid4().hex[:8]}"
            monkeypatch.setenv("PORTALS_OBJECT_STORAGE_PREFIX", test_prefix)
            tags_json = json.dumps({"env": "test-api", "type": "file-upload"})
            monkeypatch.setenv("PORTALS_OBJECT_STORAGE_TAGS", tags_json)

            from portals.services.manager import service_manager
...
class TestS3FileOperations:
    ...
    async def test_s3_delete_verifies_s3_cleanup(self, s3_files_client, s3_files_created_api_key):
        ...
        test_prefix = os.environ.get("PORTALS_OBJECT_STORAGE_PREFIX")
