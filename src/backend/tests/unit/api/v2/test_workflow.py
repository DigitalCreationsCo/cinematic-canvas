...
from portals.exceptions.api import WorkflowValidationError
from portals.services.database.models.flow.model import Flow
from portals.services.database.models.jobs.model import Job, JobType
from px.schema.workflow import JobStatus
...
class TestWorkflowDeveloperAPIProtection:
    ...
    def mock_settings_dev_api_disabled(self):
        ...
        with patch(
            "portals.api.v2.workflow.get_settings_service"
...
class TestWorkflowErrorHandling:
    ...
    async def test_flow_not_found_returns_404_with_error_code(
        ...
        headers = {"x-api-key": created_api_key.api_key}
        response = await client.post(
            "api/v2/workflows", json=request_data, headers=headers
        )
    ...
    async def test_database_error_returns_503(
        ...
        with patch(
            "portals.api.v2.workflow.get_flow_by_id_or_endpoint_name"
        ) as mock_get_flow:
    ...
    async def test_graph_build_failure_returns_500(
        ...
        try:
            ...
            assert (
                "invalid data structure" in error_msg or "must have nodes" in error_msg
            )
            assert result["detail"]["flow_id"] == str(flow_id)
    ...
    async def test_execution_timeout_with_real_delay(
        ...
        try:
            ...
            with (
                patch(
                    "portals.api.v2.workflow.execute_sync_workflow",
                    side_effect=slow_execution,
                ),
                patch(
                    "portals.api.v2.workflow.EXECUTION_TIMEOUT", 0.5
                ),  # 0.5 second timeout
            ):
                headers = {"x-api-key": created_api_key.api_key}
                response = await client.post(
    ...
    async def test_streaming_mode_returns_501(
        ...
        try:
            ...
            assert (
                "Streaming execution not yet implemented" in result["detail"]["message"]
            )
    ...
    async def test_workflow_validation_error_propagation(
        ...
        try:
            ...
            with patch("portals.api.v2.workflow.execute_sync_workflow") as mock_execute:
                mock_execute.side_effect = WorkflowValidationError(
...
class TestWorkflowSyncExecution:
    ...
    async def test_sync_execution_component_error_returns_200_with_error_in_body(
        ...
        try:
            ...
            with patch("portals.api.v2.workflow.run_graph_internal") as mock_run:
                mock_run.side_effect = Exception(
                    "Component execution failed: LLM API key not configured"
                )
    ...
    async def test_sync_execution_with_multiple_terminal_nodes(
        ...
        try:
            ...
            mock_chat_output.outputs = {
                "message": {"message": "Chat response", "type": "text"}
            }

            mock_file_output = MagicMock()
            mock_file_output.component_id = "SaveToFile-bbb"
            mock_file_output.outputs = {
                "message": {"message": "File saved successfully", "type": "text"}
            }
...
class TestWorkflowBackgroundQueueing:
    ...
    async def test_background_execution_flow(
        ...
        try:
            ...
            with (
                patch(
                    "portals.api.v2.workflow.get_task_service"
                ) as mock_get_task_service,
                patch("portals.api.v2.workflow.uuid4", return_value=UUID(mock_job_id)),
            ):
                ...
                mock_task_service.fire_and_forget_task.return_value.set_result(
                    mock_job_id
                )
    ...
    async def test_background_execution_queue_exception(
        ...
        try:
            ...
            with patch(
                ...
                mock_task_service = MagicMock()
                mock_task_service.fire_and_forget_task.side_effect = Exception(
                    "Queueing failed"
                )
                mock_get_task_service.return_value = mock_task_service
...
class TestWorkflowStatus:
    ...
    async def test_get_status_queued(
        ...
        with patch("portals.api.v2.workflow.get_job_service") as mock_get_job_service:
            mock_service = MagicMock()
            ...
            headers = {"x-api-key": created_api_key.api_key}
            response = await client.get(
                f"api/v2/workflows?job_id={job_id}", headers=headers
            )
    ...
    async def test_get_status_completed_reconstruction(
        ...
        with (
            patch("portals.api.v2.workflow.get_job_service") as mock_get_job_service,
            patch(
                "portals.api.v2.workflow.get_flow_by_id_or_endpoint_name"
            ) as mock_get_flow,
            patch(
                "portals.api.v2.workflow.reconstruct_workflow_response_from_job_id"
            ) as mock_reconstruct,
        ):
            mock_service = MagicMock()
            mock_service.get_job_by_job_id = AsyncMock(return_value=mock_job)
...
class TestWorkflowStop:
    ...
    async def test_stop_workflow_success(
        ...
        with (
            patch("portals.api.v2.workflow.get_job_service") as mock_get_job_service,
            patch("portals.api.v2.workflow.get_task_service") as mock_get_task_service,
        ):
            ...
            headers = {"x-api-key": created_api_key.api_key}
            response = await client.post(
                "api/v2/workflows/stop", json={"job_id": job_id}, headers=headers
            )
            ...
            mock_job_service.update_job_status.assert_called_once_with(
                UUID(job_id), JobStatus.CANCELLED
            )
...
class TestWorkflowIDORProtection:
    ...
    @pytest.fixture
    def mock_settings_dev_api_enabled(self):
        with patch("portals.api.v2.workflow.get_settings_service") as mock_get_settings:
            mock_service = MagicMock()
            mock_settings = MagicMock()
            mock_settings.developer_api_enabled = True
            mock_service.settings = mock_settings
    ...
    async def test_background_job_stores_user_id_and_blocks_cross_user_access(
        ...
        try:
            ...
            with patch("portals.api.v2.workflow.get_task_service") as mock_task_svc:
                mock_task_service = MagicMock()
