...
class TestListFlows:
    ...
    async def test_list_flows_queries_database(self):
        ...
        with patch("portals.helpers.flow.session_scope") as mock_session_scope:
            mock_session = MagicMock()
            mock_result = MagicMock()
            mock_result.all = MagicMock(return_value=[mock_flow1, mock_flow2])
            mock_session.exec = AsyncMock(return_value=mock_result)
            mock_session_scope.return_value.__aenter__ = AsyncMock(
    ...
    async def test_list_flows_by_flow_folder_respects_order_params(self):
        ...
        with patch("portals.helpers.flow.session_scope") as mock_session_scope:
            ...
            result = await list_flows_by_flow_folder(
                user_id=user_id, flow_id=flow_id, order_params=order_params
            )
...
class TestGetFlowByIdOrName:
    ...
    async def test_get_flow_by_id_or_name_prefers_id_over_name(self):
        ...
        mock_flow = MagicMock(spec=Flow)
        mock_flow.to_data = MagicMock(
            return_value=Data(data={"id": flow_id, "name": flow_name})
        )
        ...
        with patch("portals.helpers.flow.session_scope") as mock_session_scope:
            ...
            result = await get_flow_by_id_or_name(
                user_id=user_id, flow_id=flow_id, flow_name=flow_name
            )
...
class TestGetFlowByIdOrEndpointName:
    ...
    def _patch_session(mock_session):
        ...
        patcher = patch("portals.helpers.flow.session_scope")
    ...
    async def test_endpoint_name_branch_without_user_id_not_scoped(self):
        ...
        try:
            with patch("portals.helpers.flow.FlowRead") as mock_flow_read:
                mock_flow_read.model_validate = MagicMock(return_value="validated_flow")
                result = await get_flow_by_id_or_endpoint_name(
                    "webhook-ep", user_id=None
                )
                assert result == "validated_flow"
                stmt = mock_session.exec.await_args.args[0]
                where_sql = (
                    str(stmt.whereclause) if stmt.whereclause is not None else ""
                )
