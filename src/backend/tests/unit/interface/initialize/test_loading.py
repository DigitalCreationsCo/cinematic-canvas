...
import pytest
from px.interface.initialize.loading import (
...
async def test_update_params_fallback_to_env_when_variable_not_found():
    ...
    with patch("px.interface.initialize.loading.session_scope") as mock_session_scope:
...
async def test_update_params_handles_multiple_fields():
    ...
    with (
        patch("px.interface.initialize.loading.session_scope") as mock_session_scope,
        patch("px.services.deps.get_settings_service") as mock_get_settings,
    ):
...
async def test_update_params_with_table_fields():
    ...
    # Call the main function (px version with table support)
    with patch("px.interface.initialize.loading.session_scope") as mock_session_scope:
        mock_session_scope.return_value.__aenter__.return_value = MagicMock()
