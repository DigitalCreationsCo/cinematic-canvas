...
from portals.services.auth.utils import create_super_user
from portals.services.database.models import Flow
from portals.services.database.models.folder.model import Folder
from portals.services.deps import get_settings_service, session_scope
...
async def test_get_project_data():
    ...
    for _, project in projects:
        ...
        assert isinstance(project_description, str), (
            f"Project {project_name} has no description"
        )
        assert isinstance(project_is_component, bool), (
            f"Project {project_name} has no is_component"
        )
        assert isinstance(updated_at_datetime, datetime), (
            f"Project {project_name} has no updated_at_datetime"
        )
        ...
        assert isinstance(project_icon, str) or project_icon is None, (
            f"Project {project_name} has no icon"
        )
        assert (
...
async def test_create_or_update_starter_projects():
    async with session_scope() as session:
        ...
        stmt = (
            ...
            .where(Folder.name == STARTER_FOLDER_NAME)
        )
...
async def test_refresh_starter_projects():
    ...
    new_change = update_projects_components_with_latest_component_versions(
        graph_data, all_types
    )
    assert (
        graph_data["nodes"][1]["data"]["node"]["template"]["code"]["value"]
        == "changed !"
    )
    assert (
        new_change["nodes"][1]["data"]["node"]["template"]["code"]["value"]
        != "changed !"
    )

    assert (
        "should_store_message" not in graph_data["nodes"][1]["data"]["node"]["template"]
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "https://github.com/portals-ai/portals-bundles",
            "https://github.com/portals-ai/portals-bundles/archive/refs/heads/main.zip",
        ),
        (
            "https://github.com/portals-ai/portals-bundles/",
            "https://github.com/portals-ai/portals-bundles/archive/refs/heads/main.zip",
        ...
        (
            "https://github.com/portals-ai/portals-bundles.git",
            "https://github.com/portals-ai/portals-bundles/archive/refs/heads/main.zip",
        ),
        (
            "https://github.com/portals-ai/portals-bundles/tree/some.branch-0_1",
            "https://github.com/portals-ai/portals-bundles/archive/refs/heads/some.branch-0_1.zip",
        ),
        (
            "https://github.com/portals-ai/portals-bundles/tree/some/branch",
            "https://github.com/portals-ai/portals-bundles/archive/refs/heads/some/branch.zip",
        ),
        (
            "https://github.com/portals-ai/portals-bundles/tree/some/branch/",
            "https://github.com/portals-ai/portals-bundles/archive/refs/heads/some/branch.zip",
        ...
        (
            "https://github.com/portals-ai/portals-bundles/releases/tag/v1.0.0-0_1",
            "https://github.com/portals-ai/portals-bundles/archive/refs/tags/v1.0.0-0_1.zip",
        ),
        (
            "https://github.com/portals-ai/portals-bundles/releases/tag/foo/v1.0.0",
            "https://github.com/portals-ai/portals-bundles/archive/refs/tags/foo/v1.0.0.zip",
        ),
        (
            "https://github.com/portals-ai/portals-bundles/releases/tag/foo/v1.0.0/",
            "https://github.com/portals-ai/portals-bundles/archive/refs/tags/foo/v1.0.0.zip",
        ...
        (
            "https://github.com/portals-ai/portals-bundles/commit/68428ce16729a385fe1bcc0f1ec91fd5f5f420b9",
            "https://github.com/portals-ai/portals-bundles/archive/68428ce16729a385fe1bcc0f1ec91fd5f5f420b9.zip",
        ),
        (
            "https://github.com/portals-ai/portals-bundles/commit/68428ce16729a385fe1bcc0f1ec91fd5f5f420b9/",
            "https://github.com/portals-ai/portals-bundles/archive/68428ce16729a385fe1bcc0f1ec91fd5f5f420b9.zip",
...
async def test_detect_github_url(url, expected):
    ...
    with patch("httpx.AsyncClient.get", return_value=mock_response) as mock_get:
        ...
        parsed = urlparse(url)
        if parsed.hostname == "github.com" and not any(
            x in url for x in ["/tree/", "/releases/", "/commit/"]
        ):
...
async def test_load_bundles_from_urls():
    ...
    try:
        ...
        assert (
            "portals-bundles-68428ce16729a385fe1bcc0f1ec91fd5f5f420b9/components"
            in components_paths[0]
        )

        content = await (
            Path(components_paths[0]) / "embeddings" / "openai2.py"
        ...
        async with session_scope() as session:
            stmt = select(Flow).where(
                Flow.id == uuid.UUID("c54f9130-f2fa-4a3e-b22a-3856d946351b")
            )
...
def set_fs_flows_polling_interval():
    os.environ["PORTALS_FS_FLOWS_POLLING_INTERVAL"] = "100"
    yield
    os.unsetenv("PORTALS_FS_FLOWS_POLLING_INTERVAL")
...
async def test_sync_flows_from_fs(client: AsyncClient, logged_in_headers):
    ...
    try:
        ...
        response = await client.post(
            "api/v1/flows/", json=basic_case, headers=logged_in_headers
        )
        ...
        from portals.services.deps import get_storage_service
        ...
        for i in range(10):
            response = await client.get(
                f"api/v1/flows/{flow_id}", headers=logged_in_headers
            )
...
def profile_pictures_temp_config(monkeypatch):
    ...
    monkeypatch.setenv("PORTALS_CONFIG_DIR", str(config_path))
...
async def test_copy_profile_pictures_creates_directories():
    ...
    assert people_dir.exists(), (
        "People directory should exist after copy_profile_pictures"
    )
    assert space_dir.exists(), (
        "Space directory should exist after copy_profile_pictures"
    )
...
async def test_copy_profile_pictures_specific_files_exist():
    ...
    assert b"<svg" in content or b"<?xml" in content, (
        "Profile picture should be a valid SVG"
    )
...
async def test_copy_profile_pictures_is_idempotent():
    ...
    assert final_count == initial_count, (
        "Calling copy_profile_pictures again should not create duplicates"
    )


async def test_copy_profile_pictures_source_exists():
    """Test that the source profile pictures directory exists in the package."""
    from portals.initial_setup import setup

    source_path = Path(setup.__file__).parent / "profile_pictures"
    assert await source_path.exists(), (
        "Source profile_pictures directory should exist in package"
    )
...
@pytest.mark.usefixtures("client")
async def test_profile_pictures_available_via_api(
    client: AsyncClient, logged_in_headers
):
    """Test that profile pictures are available via the API after app startup."""
    response = await client.get(
        "api/v1/files/profile_pictures/list", headers=logged_in_headers
    )
    ...
    assert "Space/046-rocket.svg" in files, (
        "Default rocket profile picture should be available"
    )


@pytest.mark.usefixtures("client")
async def test_profile_picture_can_be_downloaded(
    client: AsyncClient, logged_in_headers
):
...
async def test_copy_profile_pictures_handles_missing_config_dir():
    ...
    with patch("portals.initial_setup.setup.get_storage_service") as mock_storage:
...
def test_update_projects_handles_components_without_metadata():
    ...
    assert (
        updated_project["nodes"][0]["data"]["node"]["template"]["code"]["value"]
        == "test code"
    )
...
def test_update_projects_resolves_prompt_via_component_type_alias():
    ...
    updated_code = updated_project["nodes"][0]["data"]["node"]["template"]["code"][
        "value"
    ]
...
async def test_update_project_file_success():
    ...
    from portals.initial_setup.setup import update_project_file
...
async def test_update_project_file_permission_denied():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("portals.initial_setup.setup.aiofiles.open") as mock_open:
...
async def test_update_project_file_logs_debug_on_oserror():
    ...
    with patch("portals.initial_setup.setup.logger") as mock_logger:
