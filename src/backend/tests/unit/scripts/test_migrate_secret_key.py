...
from portals.services.variable.constants import CREDENTIAL_TYPE
...
def migrate_module():
    ...
    repo_root = test_file.parents[5]  # Goes to portals repo root
...
class TestEnsureValidKey:
    ...
    def test_different_short_keys_produce_different_results(
        self, migrate_module, short_old_key, short_new_key
    ):
...
class TestMigrateAuthSettings:
    ...
    def test_migrate_oauth_client_secret(self, migrate_module, old_key, new_key):
        ...
        migrated, failed_fields = migrate_module.migrate_auth_settings(
            auth_settings, old_key, new_key
        )
        ...
        decrypted = migrate_module.decrypt_with_key(
            migrated["oauth_client_secret"], new_key
        )
    ...
    def test_migrate_preserves_non_sensitive_fields(
        self, migrate_module, old_key, new_key
    ):
    ...
    def test_migrate_returns_failed_fields_for_invalid_encryption(
        self, migrate_module, old_key, new_key
    ):
        ...
        auth_settings = {
            ...
            "oauth_client_secret": migrate_module.encrypt_with_key(
                "valid-secret", old_key
            ),
        }
...
class TestDatabaseMigrationUnit:
    ...
    def test_migrate_user_store_api_key(
        self, migrate_module, sqlite_db, old_key, new_key
    ):
        """Test migrating user.store_api_key column."""
        user_id = str(uuid4())
        original_value = "portals-store-api-key"
        ...
        with sqlite_db.connect() as conn:
            ...
            users = conn.execute(
                text(
                    'SELECT id, store_api_key FROM "user" WHERE store_api_key IS NOT NULL'
                )
            ).fetchall()

            for uid, encrypted_key in users:
                new_encrypted = migrate_module.migrate_value(
                    encrypted_key, old_key, new_key
                )
                assert new_encrypted is not None
                conn.execute(
    ...
    def test_migrate_variable_values(self, migrate_module, sqlite_db, old_key, new_key):
        ...
        with sqlite_db.connect() as conn:
            conn.execute(
                text(
                    "INSERT INTO variable (id, name, value, type) VALUES (:id, :name, :value, :type)"
                ),
                {
                    "id": var_id,
                    "name": "OPENAI_API_KEY",
            ...
            variables = conn.execute(
                text("SELECT id, name, value FROM variable")
            ).fetchall()

            for vid, _, encrypted_val in variables:
                if encrypted_val:
                    new_encrypted = migrate_module.migrate_value(
                        encrypted_val, old_key, new_key
                    )
                    assert new_encrypted is not None
                    conn.execute(
    ...
    def test_migrate_folder_auth_settings(
        self, migrate_module, sqlite_db, old_key, new_key
    ):
        ...
        with sqlite_db.connect() as conn:
            conn.execute(
                text(
                    "INSERT INTO folder (id, name, auth_settings) VALUES (:id, :name, :settings)"
                ),
                {
                    "id": folder_id,
                    "name": "My Project",
                    "settings": json.dumps(auth_settings),
                },
            )
            conn.commit()
            ...
            folders = conn.execute(
                text(
                    "SELECT id, name, auth_settings FROM folder WHERE auth_settings IS NOT NULL"
                )
            ).fetchall()

            for fid, _, settings_json in folders:
                settings_dict = json.loads(settings_json)
                new_settings, failed_fields = migrate_module.migrate_auth_settings(
                    settings_dict, old_key, new_key
                )
                assert failed_fields == []
                conn.execute(
            ...
            result = conn.execute(
                text("SELECT auth_settings FROM folder WHERE id = :id"),
                {"id": folder_id},
            ).fetchone()
            migrated_settings = json.loads(result[0])
            decrypted_secret = migrate_module.decrypt_with_key(
                migrated_settings["oauth_client_secret"], new_key
            )
...
class TestKeyFileManagement:
    ...
    def test_write_secret_key_custom_filename(self, migrate_module):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            migrate_module.write_secret_key_to_file(
                config_dir, test_key, "secret_key.backup"
            )
...
class TestMigrationWithRealDatabase:
    """Integration tests using real Portals database fixtures."""
    ...
    async def test_credential_variable_stored_encrypted(
        ...
        response = await client.post(
            "api/v1/variables/", json=credential_variable, headers=logged_in_headers
        )
        ...
        assert (
            our_var["value"] is None or our_var["value"] != credential_variable["value"]
        )

        # Cleanup
        await client.delete(
            f"api/v1/variables/{created_var['id']}", headers=logged_in_headers
        )
    ...
    async def test_create_folder_via_api(
        ...
        response = await client.post(
            "api/v1/folders/", json=project_data, headers=logged_in_headers
        )
        assert response.status_code == 201
        ...
        # Cleanup
        await client.delete(
            f"api/v1/folders/{created_folder['id']}", headers=logged_in_headers
        )


@pytest.mark.usefixtures("client")
class TestMigrationCompatibility:
    """Test that migration script is compatible with Portals's encryption."""

    def test_script_encryption_matches_portals(self, migrate_module):
        """Verify migration script produces same results as Portals's auth utils."""
        ...
        # Encrypt with Portals
        portals_encrypted = auth_utils.encrypt_api_key(plaintext, settings_service)

        # Decrypt with migration script
        script_decrypted = migrate_module.decrypt_with_key(
            portals_encrypted, secret_key
        )
        ...
        # Decrypt with Portals
        portals_decrypted = auth_utils.decrypt_api_key(
            script_encrypted, settings_service
        )
        assert portals_decrypted == plaintext
...
class TestTransactionAtomicity:
    ...
    def test_transaction_rollback_on_error(
        ...
        try:
            with sqlite_db.begin() as conn:
                ...
                new_encrypted = migrate_module.migrate_value(
                    encrypted_value, old_key, new_key
                )
                conn.execute(
                    text('UPDATE "user" SET store_api_key = :val WHERE id = :id'),
    ...
    def test_partial_migration_does_not_persist(
        self, migrate_module, sqlite_db, old_key, new_key
    ):
        ...
        with sqlite_db.connect() as conn:
            conn.execute(
                ...
                {
                    "id": user_id,
                    "key": migrate_module.encrypt_with_key(user_value, old_key),
                },
            )
        ...
        try:
            with sqlite_db.begin() as conn:
                ...
                new_encrypted = migrate_module.migrate_value(
                    original_user_key, old_key, new_key
                )
                conn.execute(
                    text('UPDATE "user" SET store_api_key = :val WHERE id = :id'),
                    {"val": new_encrypted, "id": user_id},
...
class TestErrorHandling:
    ...
    def test_migration_handles_invalid_encrypted_data(
        self, migrate_module, sqlite_db, old_key, new_key
    ):
        ...
        with sqlite_db.connect() as conn:
            ...
            conn.execute(
                ...
                {
                    "id": valid_id,
                    "key": migrate_module.encrypt_with_key(valid_value, old_key),
                },
            )
        ...
        with sqlite_db.connect() as conn:
            users = conn.execute(
                text('SELECT id, store_api_key FROM "user"')
            ).fetchall()
    ...
    def test_migration_handles_null_values(
        ...
        with sqlite_db.connect() as conn:
            result = conn.execute(
                text('SELECT id, store_api_key FROM "user" WHERE id = :id'),
                {"id": user_id},
            ).fetchone()
            assert result[1] is None

    def test_migration_handles_empty_auth_settings(
        self, migrate_module, old_key, new_key
    ):
        """Test that migration handles empty auth_settings dict."""
        empty_settings = {}
        result, failed_fields = migrate_module.migrate_auth_settings(
            empty_settings, old_key, new_key
        )
        assert result == {}
        assert failed_fields == []
...
class TestVerifyMigration:
    ...
    def test_verify_migration_failure(
        self, migrate_module, sqlite_db, old_key, new_key
    ):
