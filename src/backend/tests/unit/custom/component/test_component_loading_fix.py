...
from px.services.settings.base import BASE_COMPONENTS_PATH
...
class TestComponentLoadingFix:
    ...
    @pytest.fixture
    def mock_portals_components(self):
        """Create mock portals components response."""
        return {
            "components": {
                "category1": {
    ...
    async def test_base_components_path_filtering(
        self, mock_settings_service, mock_portals_components, mock_custom_components
    ):
        ...
        with (
            patch(
                "px.interface.components.import_portals_components",
                return_value=mock_portals_components,
            ),
            patch(
                "px.interface.components.aget_all_types_dict"
            ...
            mock_aget_all_types_dict.assert_called_once_with(
                ["/custom/path1", "/custom/path2"]
            )

            # Verify result contains both portals and custom components
    ...
    @pytest.mark.asyncio
    async def test_only_base_components_path_in_list(
        self, mock_settings_service, mock_portals_components
    ):
        ...
        with (
            ...
            # Verify result contains only portals components
            assert "category1" in result
            assert "category2" in result
            assert "Component1" in result["category1"]
    ...
    @pytest.mark.asyncio
    async def test_empty_components_path(
        self, mock_settings_service, mock_portals_components
    ):
    ...
    @pytest.mark.asyncio
    async def test_none_components_path(
        self, mock_settings_service, mock_portals_components
    ):
    ...
    @pytest.mark.asyncio
    async def test_lazy_loading_mode_with_base_path_filtering(
        self, mock_settings_service, mock_portals_components
    ):
        ...
        with (
            ...
            patch(
                "px.interface.components.aget_component_metadata",
            ...
            mock_aget_metadata.assert_called_once_with(
                [BASE_COMPONENTS_PATH, "/custom/path1"]
            )
    ...
    async def test_multiple_custom_paths_with_base_path(
        ...
        with (
            ...
            assert "category1" in result  # From portals components
    ...
    async def test_component_merging_logic(
        ...
        """Test that portals and custom components are properly merged."""
        # Setup
        ...
        overlapping_custom_components = {
            "category1": {  # Same category as portals
                "Component1": {
                    "display_name": "CustomComponent1",
                    "type": "category1",
                },  # Same name as portals
                "Component4": {
        ...
        with (
            ...
            # Verify that custom components override portals components with same name
            assert "category1" in result
            assert "category2" in result  # From portals
            assert "new_category" in result  # From custom

            # Custom category should completely override portals category
            assert (
                result["category1"]["Component1"]["display_name"] == "CustomComponent1"
            )

            # Only components from custom category should remain in category1
            assert (
                "Component2" not in result["category1"]
            )  # Portals component is replaced by custom category
            ...
            assert (
                result["new_category"]["NewComponent"]["display_name"] == "NewComponent"
            )

    @pytest.mark.asyncio
    async def test_component_cache_behavior(
        self, mock_settings_service, mock_portals_components
    ):
        ...
        with (
            ...
            patch("px.interface.components.aget_all_types_dict", return_value={}),
        ):
    ...
    async def test_logging_behavior(
        ...
        with (
            ...
            patch("px.interface.components.logger") as mock_logger,
        ):
            ...
            total_count_logs = [
                log for log in debug_calls if "Loaded" in log and "components" in log
            ]
            assert len(total_count_logs) >= 1

    @pytest.mark.asyncio
    async def test_error_handling_in_custom_component_loading(
        self, mock_settings_service, mock_portals_components
    ):
    ...
    async def test_base_components_path_constant_value(self):
        ...
        # Should be an absolute path containing "portals" and "components"
        assert "portals" in BASE_COMPONENTS_PATH.lower()
        assert "components" in BASE_COMPONENTS_PATH.lower()

    @pytest.mark.asyncio
    async def test_path_filtering_edge_cases(
        self, mock_settings_service, mock_portals_components
    ):
        ...
        with (
            ...
            patch(
                "px.interface.components.aget_all_types_dict", return_value={}
            ) as mock_aget_all_types_dict,
        ):
    ...
    @pytest.mark.asyncio
    async def test_component_count_calculation(
        self, mock_settings_service, mock_portals_components
    ):
        ...
        with (
            ...
            # Verify result structure
            assert len(result) >= 2  # At least portals categories + custom categories
    ...
    async def test_async_concurrency_safety(
        ...
        with (
            ...
            tasks = [
                get_and_cache_all_types_dict(mock_settings_service) for _ in range(3)
            ]
    ...
    @pytest.mark.asyncio
    async def test_integration_with_real_base_components_path(
        ...
        # This test should work with real portals components
        with patch(
            "px.interface.components.aget_all_types_dict", return_value={}
        ) as mock_aget_all_types_dict:
            # Execute the function
            ...
            # Verify we got real portals components
            assert isinstance(result, dict)
            assert len(result) >= 0  # Should not have portals components
