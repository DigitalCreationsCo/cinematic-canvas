...
from px.base.models.groq_model_discovery import GroqModelDiscovery


class TestGroqModelDiscoveryEdgeCases:
    """Test edge cases in model discovery."""

    @patch("px.base.models.groq_model_discovery.requests.get")
    ...
    def test_cache_directory_created_on_save(
        self, mock_api_key, temp_cache_dir, sample_models_metadata
    ):
    ...
    def test_fallback_models_structure(self, mock_api_key):
        ...
        for metadata in fallback.values():
            ...
            assert (
                metadata["tool_calling"] is True
            )  # Fallback models should support tools
