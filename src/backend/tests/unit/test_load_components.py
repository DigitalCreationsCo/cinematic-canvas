...
from px.interface.components import aget_all_types_dict, import_portals_components
...
class TestComponentLoading:
    ...
    @pytest.mark.asyncio
    async def test_import_portals_components_basic(self):
        """Test basic functionality of import_portals_components."""
    ...
    async def test_component_template_structure(self):
        ...
        portals_result = await import_portals_components()

        for category, components in portals_result["components"].items():
    ...
    async def test_concurrent_loading(self, base_components_path):
        ...
        portals_result1, all_types_result, portals_result2 = results

        assert isinstance(portals_result1, dict)
        assert isinstance(portals_result2, dict)
        assert isinstance(all_types_result, dict)

        assert "components" in portals_result1
        assert "components" in portals_result2

        categories1 = set(portals_result1["components"].keys())
        categories2 = set(portals_result2["components"].keys())

        for category in categories1.intersection(categories2):
            comps1 = set(portals_result1["components"][category].keys())
            comps2 = set(portals_result2["components"][category].keys())
            if comps1 != comps2:
    ...
    async def test_component_loading_performance(self):
        ...
        await import_portals_components()
    ...
    async def test_process_single_module_exception_handling(self):
        ...
        missing dependencies, or initialization failures), it doesn't crash Portals startup.
        """
        from unittest.mock import patch

        from px.interface.components import _process_single_module
        ...
        for exception, name in exception_cases:
            with patch("importlib.import_module", side_effect=exception):
                result = _process_single_module("px.components.test_module")
