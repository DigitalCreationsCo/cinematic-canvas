"""Comprehensive tests for px.log.logger module.
...
class TestConfigure:
    ...
    @patch.dict(os.environ, {"PORTALS_LOG_LEVEL": "WARNING"})
    def test_configure_env_variable_override(self):
        """Test configure() respects PORTALS_LOG_LEVEL environment variable."""
    ...
    @patch.dict(os.environ, {"PORTALS_LOG_FILE": "/tmp/test.log"})  # noqa: S108
    def test_configure_env_log_file_override(self):
        """Test configure() respects PORTALS_LOG_FILE environment variable."""
        configure()

        config = structlog._config
        assert config is not None

    @patch.dict(os.environ, {"PORTALS_LOG_ENV": "container"})
    def test_configure_env_log_env_override(self):
        """Test configure() respects PORTALS_LOG_ENV environment variable."""
        configure()

        config = structlog._config
        assert config is not None

    @patch.dict(os.environ, {"PORTALS_LOG_FORMAT": "custom"})
    def test_configure_env_log_format_override(self):
        """Test configure() respects PORTALS_LOG_FORMAT environment variable."""
        configure()

        config = structlog._config
        assert config is not None

    @patch.dict(os.environ, {"PORTALS_PRETTY_LOGS": "false"})
    def test_configure_env_pretty_logs_disabled(self):
        """Test configure() respects PORTALS_PRETTY_LOGS=false."""
        configure()
...
class TestInterceptHandler:
    ...
    def setup_method(self):
        ...
        self.structlog_patcher = patch(
            "structlog.get_logger", return_value=self.mock_logger
        )
...
class TestLogProcessors:
    ...
    def test_remove_exception_in_production(self):
        ...
        logger_module = sys.modules["px.log.logger"]
    ...
    def test_log_level_map_values_are_integers(self):
        ...
        for level_name, level_value in LOG_LEVEL_MAP.items():
            assert isinstance(level_value, int), (
                f"Level {level_name} value {level_value} is not an integer"
            )
...
class TestSizedLogBufferIntegration:
    ...
    def test_write_with_event_field(self):
        ...
        message = json.dumps(
            {"event": "Test event message", "timestamp": "2021-07-01T12:00:00Z"}
        )
    ...
    def test_write_with_msg_field_fallback(self):
        ...
        message = json.dumps(
            {"msg": "Test msg message", "timestamp": "2021-07-01T12:00:00Z"}
        )
    ...
    def test_write_with_iso_timestamp(self):
        ...
        message = json.dumps(
            {"event": "Test message", "timestamp": "2021-07-01T12:00:00.123Z"}
        )
    ...
    def test_log_buffer_thread_safety(self):
        ...
        def write_logs():
            try:
                for i in range(10):
                    message = json.dumps(
                        {"event": f"Thread message {i}", "timestamp": time.time() + i}
                    )
                    buffer.write(message)
        ...
        assert len(hung_threads) == 0, (
            f"Threads did not complete within timeout: {hung_threads}"
        )
            ...
            try:
                ...
                if rotating_handlers:
                    ...
                    assert handler.maxBytes == expected_bytes, (
                        f"Failed for rotation '{rotation_str}'"
                    )
...
def test_init_with_env_variable():
    with patch.dict(os.environ, {"PORTALS_LOG_RETRIEVER_BUFFER_SIZE": "100"}):
        buffer = SizedLogBuffer()
...
def test_write(sized_log_buffer):
    message = json.dumps(
        {"text": "Test log", "record": {"time": {"timestamp": 1625097600.1244334}}}
    )
...
def test_write_overflow(sized_log_buffer):
    ...
    messages = [
        json.dumps(
            {"text": f"Log {i}", "record": {"time": {"timestamp": 1625097600 + i}}}
        )
        for i in range(3)
    ]
