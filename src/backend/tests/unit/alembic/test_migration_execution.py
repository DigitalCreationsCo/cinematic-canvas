...
_SCRIPT_LOCATION = _WORKSPACE_ROOT / "src/backend/base/portals/alembic"
...
def _pg_url() -> str | None:
    ...
    url = os.environ.get("PORTALS_TEST_DATABASE_URI")
    if url is not None:
...
def db_url(request):
    ...
    if request.param == "sqlite":
        ...
        base_url = _pg_url()
        if base_url is None:
            pytest.skip("PORTALS_TEST_DATABASE_URI not set")
...
def _get_main_branch_head() -> str | None:
        ...
        try:
            result = subprocess.run(  # noqa: S603
                [
                    ...
                    "origin/main",
                    "--",
                    "src/backend/base/portals/alembic/versions/",
                ],
                capture_output=True,
                text=True,
                check=True,
    ...
    if len(heads) > 1:
        pytest.fail(
            f"origin/main has {len(heads)} head revisions — migration branches need merging: {heads}"
        )
    return None
    ...
    for key in all_fk_keys:
        ...
        if rm and add:
            rm_targets = sorted(
                ...
                for elem in rm.elements
                if elem.column is not None
            )
            add_targets = sorted(
                (elem.column.table.name, elem.column.name)
...
class _FakeFK:
    ...
    def __init__(self, parent_table, col_names, ref_table, ref_col_names):
        ...
        self.elements = [
            _FakeElement(_FakeColumn(rc, ref_table)) for rc in ref_col_names
        ]
...
def test_upgrade_from_main_branch(db_url):
    ...
    if main_head is None:
        if os.environ.get("MIGRATION_VALIDATION_CI"):
            pytest.fail(
                "Could not determine main branch head revision — ensure fetch-depth: 0 and origin/main exists"
            )
        pytest.skip(
            "Could not determine main branch head revision (shallow clone or no origin/main)"
        )
