import portals.services.utils as service_utils
...
def test_register_builtin_adapters_imports_when_feature_enabled(monkeypatch):
    ...
    assert calls == ["portals.services.adapters.deployment.watsonx_orchestrate"]
...
def test_register_builtin_deployment_mappers_imports_when_feature_enabled(monkeypatch):
    ...
    assert calls == ["portals.api.v1.mappers.deployments.watsonx_orchestrate"]
