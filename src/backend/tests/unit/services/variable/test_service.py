...
from px.services.settings.constants import VARIABLES_TO_GET_FROM_ENVIRONMENT
...
async def test_initialize_user_variables__not_found_variable(service, session: AsyncSession):
    with patch("portals.services.variable.service.DatabaseVariableService.create_variable") as m:
...
async def test_get_all_decrypted_variables__decryption_failure(service, session: AsyncSession):
    ...
    with patch("portals.services.auth.utils.decrypt_api_key") as mock_decrypt:
