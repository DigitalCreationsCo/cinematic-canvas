...
import pytest
from portals.services.auth.mcp_encryption import is_encrypted
...
@pytest.fixture
def portals_auth_service(tmp_path):
    """Use Portals AuthService for encrypt/decrypt so tests get real Fernet behavior."""
...
def use_portals_auth_for_encryption(portals_auth_service):
    """Ensure utils use Portals AuthService (real encrypt/decrypt), not PX stub."""
    with patch("portals.services.auth.utils.get_auth_service", return_value=portals_auth_service):
        yield
