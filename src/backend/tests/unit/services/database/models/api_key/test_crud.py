...
async def test_create_api_key_stores_hash(async_session, mock_settings, monkeypatch):  # noqa: ARG001
    ...
    monkeypatch.setattr(
        "portals.services.database.models.api_key.crud.auth_utils.encrypt_api_key",
    ...
    row = (
        await async_session.exec(select(ApiKey).where(ApiKey.user_id == user.id))
    ).first()
...
@pytest.mark.anyio
async def test_check_key_fallback_for_legacy_keys(
    async_session, mock_settings, monkeypatch
):
    ...
    monkeypatch.setattr(
        "portals.services.database.models.api_key.crud.auth_utils.decrypt_api_key",
...
@pytest.mark.anyio
async def test_check_key_fallback_backfills_hash(
    async_session, mock_settings, monkeypatch
):
...
@pytest.mark.anyio
async def test_check_key_no_match_returns_none(
    async_session, mock_settings, monkeypatch
):
...
@pytest.mark.anyio
async def test_check_key_skips_orphaned_encrypted_keys(
    async_session, mock_settings, monkeypatch
):
