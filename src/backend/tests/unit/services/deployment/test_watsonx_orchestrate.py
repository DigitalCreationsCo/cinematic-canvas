...
try:
    import portals.services.adapters.deployment.watsonx_orchestrate  # noqa: F401
...
class FakeWXOClients(SimpleNamespace):
    ...
    def upload_tool_artifact(
        self,
        tool_id: str,
        files: dict[str, tuple[str, io.BytesIO, str, dict[str, str]]],
    ):
...
async def test_process_config_uses_raw_payload_but_overrides_name(monkeypatch):
    from portals.services.adapters.deployment.watsonx_orchestrate.core.config import (
        process_config,
    )
    ...
    monkeypatch.setattr(
        "portals.services.adapters.deployment.watsonx_orchestrate.core.config.create_config",
        mock_create_config,
    )
...
async def test_process_config_rejects_reference_id():
    ...
    with pytest.raises(
        InvalidDeploymentOperationError,
        match="Config reference binding is not supported",
    ):
...
async def test_create_rejects_legacy_top_level_config_section(monkeypatch):
    ...
    with pytest.raises(
        InvalidDeploymentOperationError,
        match="Top-level 'snapshot' and 'config' create sections",
    ):
...
@pytest.mark.anyio
async def test_resolve_runtime_credentials_supports_variable_and_raw_sources(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        "portals.services.adapters.deployment.watsonx_orchestrate.client.resolve_variable_value",
        mock_resolve_variable_value,
    )

    from portals.services.adapters.deployment.watsonx_orchestrate.client import (
...
@pytest.mark.anyio
async def test_update_provider_data_binds_existing_tool_and_updates_agent_tools(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
...
@pytest.mark.anyio
async def test_update_provider_data_bind_unbind_and_rename_preserves_connection_deltas(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_agent = FakeAgentClient({"id": "dep-1", "tools": ["tool-1"]})
    ...
    assert result.provider_result is not None
    assert [tool_id for tool_id, _payload in fake_tool.update_calls] == [
        "tool-1",
        "tool-1",
    ]
    ...
    assert rename_payload["binding"]["portals"]["connections"] == {
        "cfg-keep": "conn-keep",
        "cfg-add": "conn-add",
    }
...
@pytest.mark.anyio
async def test_update_provider_data_binds_existing_tool_using_provider_app_id_for_raw_connection(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_agent = FakeAgentClient({"id": "dep-1", "tools": ["tool-1"]})
...
@pytest.mark.anyio
async def test_update_provider_data_mixed_operations_preserve_encounter_order(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_agent = FakeAgentClient({"id": "dep-1", "tools": ["tool-1", "tool-2"]})
    ...
    assert list(tool3_payload["binding"]["portals"]["connections"]) == [
        "cfg-2",
        "cfg-1",
    ]
    assert tool3_payload["binding"]["portals"]["connections"] == {
        "cfg-2": "conn-cfg-2",
        "cfg-1": "conn-cfg-1",
    }

    tool1_payload = update_calls_by_id["tool-1"]
    assert tool1_payload["binding"]["portals"]["connections"] == {}
...
def test_build_provider_update_plan_preserves_operation_encounter_order():
    ...
    assert [item.operation_app_id for item in plan.raw_connections_to_create] == [
        "cfg-raw-1",
        "cfg-raw-2",
    ]
...
def test_build_provider_update_plan_creates_unbound_raw_tools_alongside_bound_raw_tools():
    ...
    assert [item.raw_name for item in plan.raw_tools_to_create] == [
        "snapshot-bound",
        "snapshot-unbound",
    ]
...
@pytest.mark.anyio
async def test_apply_provider_create_plan_binds_raw_tools_with_provider_app_ids(
    monkeypatch,
):
    provider_create = payloads_module.WatsonxDeploymentCreatePayload.model_validate(
        {
...
@pytest.mark.anyio
async def test_apply_provider_create_plan_rolls_back_mutated_existing_tools_with_writable_payload(
    monkeypatch,
):
    provider_create = payloads_module.WatsonxDeploymentCreatePayload.model_validate(
        {
        ...
        [
            {
                ...
                "display_name": "Tool 1",
                "description": "desc",
                "binding": {"portals": {"connections": {"old": "conn-old"}}},
    ...
    assert first_payload["binding"]["portals"]["connections"]["cfg-1"] == "conn-new"
    assert rollback_payload["binding"]["portals"]["connections"] == {"old": "conn-old"}
...
@pytest.mark.anyio
async def test_resolve_connections_for_operations_wraps_validation_failure_with_rollback_metadata(
    monkeypatch,
):
    raw_payload = payloads_module.WatsonxConnectionRawPayload.model_validate(
        {
...
async def test_apply_provider_create_plan_rolls_back_successfully_created_raw_connections_on_partial_batch_failure(
    ...
    monkeypatch.setattr(
        ...
        mock_rollback_created_resources,
    )
...
@pytest.mark.anyio
async def test_create_provider_data_prefixes_tool_and_deployment_names_but_not_connection_app_ids(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_clients = FakeWXOClients(
                    ...
                    "raw_payloads": [
                        {
                            ...
                            "provider_data": {
                                "project_id": "project-1",
                                "source_ref": "fv-create-service-1",
                            },
                        }
                    ]
                },
...
@pytest.mark.anyio
async def test_update_provider_data_maps_raw_connection_conflict_to_deployment_conflict(
    monkeypatch,
):
...
@pytest.mark.anyio
async def test_create_provider_data_maps_raw_connection_conflict_to_deployment_conflict(
    monkeypatch,
):
    from ibm_watsonx_orchestrate_clients.tools.tool_client import ClientAPIException
                        ...
                        "raw_payloads": [
                            {
                                ...
                                "provider_data": {
                                    "project_id": "project-1",
                                    "source_ref": "fv-create-conflict-1",
                                },
                            }
                        ]
                    },
...
@pytest.mark.anyio
async def test_update_provider_data_rolls_back_mutated_tools_with_writable_payload(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
                        ...
                        "raw_payloads": [
                            {
                                ...
                                "provider_data": {
                                    "project_id": "project-1",
                                    "source_ref": "fv-create-rollback-1",
                                },
                            }
                        ]
                    },
...
async def test_process_raw_flows_with_app_id_awaits_connection_validation(monkeypatch):
    from portals.services.adapters.deployment.watsonx_orchestrate.core import (
        tools as tools_core_module,
    )
    ...
    monkeypatch.setattr(
        "portals.services.adapters.deployment.watsonx_orchestrate.core.config.validate_connection",
...
@pytest.mark.anyio
async def test_process_raw_flows_with_app_id_rejects_plain_dict_provider_data(
    monkeypatch,
):
    from portals.services.adapters.deployment.watsonx_orchestrate.core import (
        tools as tools_core_module,
...
def test_create_wxo_flow_tool_keeps_load_from_db_global_values_unprefixed(monkeypatch):
    ...
    def mock_create_portals_tool(*, tool_definition, connections, show_details):  # noqa: ARG001
        assert show_details is False
        captured_tool_definition.update(tool_definition)
        return fake_tool

    monkeypatch.setattr(tools_module, "create_portals_tool", mock_create_portals_tool)
    monkeypatch.setattr(
        tools_module,
        "build_portals_artifact_bytes",
        lambda **kwargs: b"artifact",  # noqa: ARG005
    )

    from portals.services.adapters.deployment.watsonx_orchestrate.core.tools import (
...
def test_create_wxo_flow_tool_normalizes_name_for_raw_payload(monkeypatch):
    ...
    assert tool_payload["binding"]["portals"]["project_id"] == "project-123"
        ...
        [
            {
                "id": "tool-1",
                "name": "tool-one",
                "binding": {"portals": {"connections": {"cfg-1": "conn-1"}}},
            },
            {
                "id": "tool-2",
                "name": "tool-two",
                "binding": {"portals": {"connections": {"cfg-2": "conn-2"}}},
            },
        ]
    )
    connections_client = FakeConnectionsClient()
...
@pytest.mark.anyio
async def test_list_configs_deployment_scope_fails_fast_when_type_enrichment_fails(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_agent = FakeAgentClient({"id": "dep-1", "tools": ["tool-1"]})
...
@pytest.mark.anyio
async def test_list_configs_deployment_scope_accepts_schema_compatible_detailed_connection(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_agent = FakeAgentClient({"id": "dep-1", "tools": ["tool-1"]})
    ...
    assert result.configs[0].provider_data == {
...
@pytest.mark.anyio
async def test_list_configs_deployment_scope_raises_on_non_dict_agent_payload(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_clients = SimpleNamespace(
...
@pytest.mark.anyio
async def test_list_configs_deployment_scope_uses_latest_binding_for_same_app(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    connections_client = FakeConnectionsClient()
    ...
    fake_clients = SimpleNamespace(
        ...
        tool=FakeToolClient(
            [
                ...
                {
                    "id": "tool-2",
                    "binding": {"portals": {"connections": {"cfg-1": "conn-2"}}},
                },
            ]
        ),
        connections=connections_client,
...
@pytest.mark.anyio
async def test_list_configs_deployment_scope_skips_enrichment_when_no_connections(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
...
@pytest.mark.anyio
async def test_list_configs_deployment_scope_raises_on_malformed_detailed_connection(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_tool = FakeToolClient(
...
async def test_list_configs_tenant_scope_preserves_duplicates(monkeypatch):
    ...
    result = await service.list_configs(user_id="user-1", db=object(), params=None)
    assert [config.id for config in result.configs] == [
        "conn-dup",
        "conn-dup",
        "conn-unique",
    ]
...
@pytest.mark.anyio
async def test_list_snapshots_snapshot_ids_trusts_verified_results_without_revalidation(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_clients = SimpleNamespace(
...
@pytest.mark.anyio
async def test_list_snapshots_snapshot_names_ignored_when_deployment_ids_present(
    monkeypatch,
):
            ...
            [
                {
                    "id": "tool-1",
                    "name": "Tool One",
                    "binding": {"portals": {"connections": ["not-a-dict"]}},
                }
            ]
        )
    )
            ...
            [
                {
                    "id": "tool-1",
                    "name": "Tool One",
                    "binding": {"portals": {"connections": {"cfg-1": "   "}}},
                }
            ]
        )
    )
...
async def test_retry_with_backoff_succeeds_on_first_try():
    from portals.services.adapters.deployment.watsonx_orchestrate.core.retry import (
        retry_with_backoff,
    )
...
@pytest.mark.anyio
async def test_get_deployment_includes_tool_ids_and_environment_in_provider_data(
    monkeypatch,
):
    service = WatsonxOrchestrateDeploymentService(DummySettingsService())
    fake_clients = SimpleNamespace(
...
async def test_delete_only_deletes_agent_not_tools_or_configs(monkeypatch):
    ...
    fake_tool = FakeToolClient(
        [
            {
                "id": "tool-1",
                "binding": {"portals": {"connections": {"app-1": {}}}},
            },
            {
                "id": "tool-2",
                "binding": {"portals": {"connections": {"app-2": {}}}},
            },
        ]
    )
...
def test_get_authenticator_unknown_url():
    ...
    from px.services.adapters.deployment.exceptions import AuthSchemeError
...
@pytest.mark.anyio
async def test_get_provider_clients_uses_request_scoped_context_memoization(
    monkeypatch,
):
    resolve_calls = 0
    ...
    monkeypatch.setattr(
        ...
        mock_resolve_wxo_client_credentials,
    )
...
@pytest.mark.anyio
async def test_deployment_provider_scope_resets_wxo_context_between_provider_scopes(
    monkeypatch,
):
    resolve_calls = 0
...
@pytest.mark.anyio
async def test_deployment_provider_scope_rejects_mixed_users_within_same_scope(
    monkeypatch,
):
    resolve_calls = 0
...
@pytest.mark.anyio
async def test_resolve_wxo_client_credentials_reads_provider_url_from_account(
    monkeypatch,
):
...
@pytest.mark.anyio
async def test_create_and_upload_wxo_flow_tools_with_bindings_journals_created_ids_on_upload_failure(
    monkeypatch,
):
...
def test_build_config_list_item_valid():
    ...
    from portals.services.adapters.deployment.watsonx_orchestrate.payloads import (
...
def test_build_config_list_item_missing_environment_for_key_value_creds():
    ...
    with pytest.raises(
        InvalidContentError,
        match="key_value_creds connection without a required environment",
    ):
        build_config_list_item(
            config_item_data_slot=slot,
            connection_id="conn-1",
...
def test_resolve_execution_message_string():
    from portals.services.adapters.deployment.watsonx_orchestrate.core.execution import (
...
def test_get_agent_environments_dedupes_preserving_order():
    from portals.services.adapters.deployment.watsonx_orchestrate.core.status import (
...
def test_build_portals_artifact_bytes_structure():
    from portals.services.adapters.deployment.watsonx_orchestrate.core.tools import (
        build_portals_artifact_bytes,
    )
    ...
    tool = SimpleNamespace(
        ...
        requirements=["px>=0.3.0"],
    )

    artifact_bytes = build_portals_artifact_bytes(
        tool=tool,
...
async def test_credential_resolution_catches_arbitrary_exceptions(monkeypatch):
    ...
    from px.services.adapters.deployment.exceptions import CredentialResolutionError
    ...
    monkeypatch.setattr(
        "portals.services.adapters.deployment.watsonx_orchestrate.client.get_provider_account_by_id",
        mock_get_provider,
    )
...
# ---------------------------------------------------------------------------
# Test Coverage Gap #4: extract_portals_artifact_from_zip — all error paths
# ---------------------------------------------------------------------------


def test_extract_portals_artifact_from_zip_success():
    """extract_portals_artifact_from_zip returns parsed JSON from a valid zip."""
    import json

    from portals.services.adapters.deployment.watsonx_orchestrate.core.tools import (
        extract_portals_artifact_from_zip,
    )
    ...
    result = extract_portals_artifact_from_zip(buf.getvalue(), snapshot_id="snap-1")
    assert result == flow_data


def test_extract_portals_artifact_from_zip_no_json():
    """extract_portals_artifact_from_zip raises InvalidContentError when no JSON in zip."""
    from portals.services.adapters.deployment.watsonx_orchestrate.core.tools import (
        extract_portals_artifact_from_zip,
    )
    ...
    with pytest.raises(InvalidContentError, match="does not include a flow JSON"):
        extract_portals_artifact_from_zip(buf.getvalue(), snapshot_id="snap-1")


def test_extract_portals_artifact_from_zip_bad_zip():
    """extract_portals_artifact_from_zip raises InvalidContentError for invalid zip data."""
    from portals.services.adapters.deployment.watsonx_orchestrate.core.tools import (
        extract_portals_artifact_from_zip,
    )

    with pytest.raises(InvalidContentError, match="not a valid zip"):
        extract_portals_artifact_from_zip(b"not a zip file", snapshot_id="snap-1")


def test_extract_portals_artifact_from_zip_invalid_utf8():
    """extract_portals_artifact_from_zip raises InvalidContentError for non-UTF-8 content."""
    from portals.services.adapters.deployment.watsonx_orchestrate.core.tools import (
        extract_portals_artifact_from_zip,
    )
...
def test_extract_portals_artifact_from_zip_invalid_json():
    """extract_portals_artifact_from_zip raises InvalidContentError for malformed JSON."""
    from portals.services.adapters.deployment.watsonx_orchestrate.core.tools import (
        extract_portals_artifact_from_zip,
    )
...
def test_api_execution_create_schema_parses_all_explicit_fields():
    ...
    from portals.api.v1.mappers.deployments.watsonx_orchestrate.payloads import (
...
def test_api_execution_schemas_omit_portals_owned_fields():
    """deployment_id (Portals DB UUID) belongs on the top-level response, not in provider_data."""
    from portals.api.v1.mappers.deployments.watsonx_orchestrate.payloads import (
        WatsonxApiAgentExecutionCreateResultData,
        WatsonxApiAgentExecutionStatusResultData,
    )
...
def test_shape_execution_create_result_maps_all_fields():
    ...
    from portals.api.v1.mappers.deployments.watsonx_orchestrate.mapper import (
        WatsonxOrchestrateDeploymentMapper,
    )
...
async def test_verify_credentials_invalid_key_raises(monkeypatch):
    ...
    from px.services.adapters.deployment.exceptions import AuthenticationError
    from px.services.adapters.deployment.schema import VerifyCredentials
...
async def test_verify_credentials_instance_probe_forbidden(monkeypatch):
    ...
    from ibm_watsonx_orchestrate_clients.tools.tool_client import ClientAPIException
    from px.services.adapters.deployment.exceptions import AuthorizationError
    from px.services.adapters.deployment.schema import VerifyCredentials
...
async def test_verify_credentials_malformed_key_from_authenticator_constructor_raises():
    ...
    from px.services.adapters.deployment.exceptions import InvalidContentError
    from px.services.adapters.deployment.schema import VerifyCredentials
...
async def test_verify_credentials_missing_provider_data_raises():
    ...
    from px.services.adapters.deployment.schema import VerifyCredentials
    from px.services.adapters.payload import AdapterPayloadMissingError
...
@pytest.mark.anyio
async def test_verify_credentials_authenticator_construction_unexpected_error(
    monkeypatch,
):
    """verify_credentials raises DeploymentError when get_authenticator() throws unexpectedly."""
    from px.services.adapters.deployment.exceptions import DeploymentError
    from px.services.adapters.deployment.schema import VerifyCredentials
...
# ---------------------------------------------------------------------------
# Ownership checks: binding.portals verification
# ---------------------------------------------------------------------------
...
def _make_external_tool(tool_id: str) -> dict[str, Any]:
    """Build a tool dict that is NOT Portals-managed (no binding.portals)."""
    return {
        "id": tool_id,
...
@pytest.mark.anyio
async def test_update_connection_deltas_rejects_non_portals_tool():
    """_update_existing_tool_connection_deltas must refuse to modify tools without binding.portals."""
    ...
    with pytest.raises(InvalidContentError, match="does not have a Portals binding"):
...
@pytest.mark.anyio
async def test_update_connection_deltas_accepts_portals_tool():
    """_update_existing_tool_connection_deltas succeeds for tools with binding.portals."""
    _update_deltas = update_core_module._update_existing_tool_connection_deltas

    lf_tool = _make_portals_tool("lf-1")
...
@pytest.mark.anyio
async def test_bind_existing_tools_for_create_rejects_non_portals_tool():
    """_bind_existing_tools_for_create must refuse to modify tools without binding.portals."""
...
@pytest.mark.anyio
async def test_bind_existing_tools_for_create_accepts_portals_tool():
    """_bind_existing_tools_for_create succeeds for tools with binding.portals."""
    _bind_existing = create_core_module._bind_existing_tools_for_create
...
@pytest.mark.anyio
async def test_update_existing_tool_connection_bindings_rejects_non_portals_tool():
    """update_existing_tool_connection_bindings must refuse to modify tools without binding.portals."""
...
@pytest.mark.anyio
async def test_apply_tool_renames_succeeds_for_portals_tool():
    """_apply_tool_renames renames a Portals-owned tool on the agent."""
...
@pytest.mark.anyio
async def test_apply_tool_renames_rejects_non_portals_tool():
    """_apply_tool_renames must refuse to rename tools without binding.portals."""
    _apply_renames = update_core_module._apply_tool_renames
...
async def test_apply_tool_renames_preserves_latest_connections_when_original_already_captured():
    ...
    assert payload["name"] == "new_name"
    assert payload["display_name"] == "new_name"
    assert payload["binding"]["portals"]["connections"] == {
        "app-1": "conn-1",
        "app-2": "conn-2",
    }
...
# ---------------------------------------------------------------------------
# WXO_PX_REQUIREMENT_OVERRIDE
# ---------------------------------------------------------------------------


def test_resolve_px_requirement_uses_override(monkeypatch):
    """_resolve_px_requirement returns the env var value when set."""
    _resolve = tools_module._resolve_px_requirement

    monkeypatch.setenv("WXO_PX_REQUIREMENT_OVERRIDE", "px-nightly==0.4.0.dev32")
    assert _resolve() == "px-nightly==0.4.0.dev32"


def test_resolve_px_requirement_ignores_blank_override(monkeypatch):
    """_resolve_px_requirement ignores empty/whitespace-only override."""
    _resolve = tools_module._resolve_px_requirement

    monkeypatch.setenv("WXO_PX_REQUIREMENT_OVERRIDE", "   ")
...
def test_flow_version_list_item_includes_tool_name_in_provider_data():
    ...
    from portals.api.v1.schemas.deployments import DeploymentFlowVersionListItem
