...
def mock_settings_service():
    ...
    settings_service.settings.config_dir = "/tmp/portals_test"  # noqa: S108
...
class TestLocalStorageServiceListOperations:
    ...
    async def test_list_files_excludes_directories(self, local_storage_service):
        ...
        await local_storage_service.save_file(
            f"{flow_id}/subdir", "nested.txt", b"content"
        )
...
class TestLocalStorageServicePathTraversal:
    ...
    async def test_get_file_rejects_traversal(
        self, local_storage_service, malicious_filename
    ):
    ...
    async def test_delete_file_rejects_traversal(
        self, local_storage_service, malicious_filename
    ):
        with pytest.raises(ValueError, match="Invalid file"):
    ...
    async def test_get_file_size_rejects_traversal(
        self, local_storage_service, malicious_filename
    ):
        with pytest.raises(ValueError, match="Invalid file"):
    ...
    async def test_get_file_stream_rejects_traversal(self, local_storage_service):
        with pytest.raises(ValueError, match="Invalid file"):
            ...
            async for _ in local_storage_service.get_file_stream(
                "flow_a", "../escape.txt"
            ):
                pass
...
class TestLocalStorageServiceEdgeCases:
    ...
    async def test_concurrent_write_then_read(self, local_storage_service):
        ...
        aiofiles backend. See https://github.com/portals-ai/portals/issues/12414
        """
