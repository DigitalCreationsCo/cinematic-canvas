...
- API_KEY_SOURCE='env': Validates against PORTALS_API_KEY environment variable
"""
...
import pytest
from portals.services.database.models.api_key.crud import (
    _check_key_from_db,
    _check_key_from_env,
    check_key,
)
...
def mock_settings_service_db():
    ...
    settings_service.auth_settings.SUPERUSER = "portals"
    settings_service.auth_settings.SECRET_KEY.get_secret_value.return_value = (
...
class TestCheckKeyRouting:
    ...
    @pytest.mark.asyncio
    async def test_check_key_routes_to_db_by_default(
        self, mock_session, mock_settings_service_db
    ):
        """check_key should route to _check_key_from_db when API_KEY_SOURCE='db'."""
        with (
            patch(
                "portals.services.database.models.api_key.crud.get_settings_service",
                return_value=mock_settings_service_db,
            ),
            patch(
                "portals.services.database.models.api_key.crud._check_key_from_db",
                new_callable=AsyncMock,
            ) as mock_db_check,
            patch(
                "portals.services.database.models.api_key.crud._check_key_from_env",
                new_callable=AsyncMock,
            ) as mock_env_check,
        ):
    ...
    @pytest.mark.asyncio
    async def test_check_key_routes_to_env_when_configured_and_succeeds(
        self, mock_session, mock_settings_service_env
    ):
    ...
    @pytest.mark.asyncio
    async def test_check_key_falls_back_to_db_when_env_fails(
        self, mock_session, mock_settings_service_env
    ):
    ...
    @pytest.mark.asyncio
    async def test_check_key_returns_none_when_both_env_and_db_fail(
        self, mock_session, mock_settings_service_env
    ):
...
class TestCheckKeyFromEnv:
    ...
    async def test_valid_key_returns_superuser(
        ...
        monkeypatch.setenv("PORTALS_API_KEY", "sk-test-env-key")

        with patch(
            "portals.services.database.models.user.crud.get_user_by_username",
            new_callable=AsyncMock,
        ) as mock_get_user:
            mock_get_user.return_value = mock_superuser

            result = await _check_key_from_env(
                mock_session, "sk-test-env-key", mock_settings_service_env
            )

            assert result == mock_superuser
            mock_get_user.assert_called_once_with(mock_session, "portals")

    @pytest.mark.asyncio
    async def test_invalid_key_returns_none(
        self, mock_session, mock_settings_service_env, monkeypatch
    ):
        """Invalid API key not matching env var should return None."""
        monkeypatch.setenv("PORTALS_API_KEY", "sk-test-env-key")

        result = await _check_key_from_env(
            mock_session, "sk-wrong-key", mock_settings_service_env
        )

        assert result is None

    @pytest.mark.asyncio
    async def test_no_env_api_key_configured_returns_none(
        self, mock_session, mock_settings_service_env, monkeypatch
    ):
        """When PORTALS_API_KEY is not set, should return None."""
        monkeypatch.delenv("PORTALS_API_KEY", raising=False)

        result = await _check_key_from_env(
            mock_session, "sk-any-key", mock_settings_service_env
        )

        assert result is None

    @pytest.mark.asyncio
    async def test_empty_env_api_key_returns_none(
        self, mock_session, mock_settings_service_env, monkeypatch
    ):
        """When PORTALS_API_KEY is empty string, should return None."""
        monkeypatch.setenv("PORTALS_API_KEY", "")
    ...
    @pytest.mark.asyncio
    async def test_superuser_not_found_returns_none(
        self, mock_session, mock_settings_service_env, monkeypatch
    ):
    ...
    @pytest.mark.asyncio
    async def test_case_sensitive_key_comparison(
        self, mock_session, mock_settings_service_env, monkeypatch
    ):
        """API key comparison should be case-sensitive."""
        monkeypatch.setenv("PORTALS_API_KEY", "sk-Test-Key")

        # Different case should not match
        result = await _check_key_from_env(
            mock_session, "sk-test-key", mock_settings_service_env
        )
        assert result is None
    ...
    @pytest.mark.asyncio
    async def test_whitespace_in_key_not_trimmed(
        self, mock_session, mock_settings_service_env, monkeypatch
    ):
    ...
    async def test_special_characters_in_key(
        ...
        monkeypatch.setenv("PORTALS_API_KEY", special_key)
        ...
        with patch(
            ...
            result = await _check_key_from_env(
                mock_session, special_key, mock_settings_service_env
            )

            assert result == mock_superuser

    @pytest.mark.asyncio
    async def test_unicode_in_key(
        self, mock_session, mock_superuser, mock_settings_service_env, monkeypatch
    ):
        """API key with unicode characters should work correctly."""
        unicode_key = "sk-тест-キー-密钥"
        monkeypatch.setenv("PORTALS_API_KEY", unicode_key)
        ...
        with patch(
            ...
            result = await _check_key_from_env(
                mock_session, unicode_key, mock_settings_service_env
            )
    ...
    async def test_very_long_key(
        ...
        long_key = "sk-" + "a" * 1000
        monkeypatch.setenv("PORTALS_API_KEY", long_key)
        ...
        with patch(
            ...
            result = await _check_key_from_env(
                mock_session, long_key, mock_settings_service_env
            )
...
class TestCheckKeyEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_none_api_key_raises_or_returns_none(
        self, mock_session, mock_settings_service_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_custom_superuser_name(
        self, mock_session, mock_superuser, mock_settings_service_env, monkeypatch
    ):
...
class TestCheckKeyIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_flow_env_mode_valid_key(
        self, mock_session, mock_superuser, monkeypatch
    ):
        """Full flow test: env mode with valid key."""
        monkeypatch.setenv("PORTALS_API_KEY", "sk-env-secret")

        mock_settings = MagicMock()
        mock_settings.auth_settings.API_KEY_SOURCE = "env"
        mock_settings.auth_settings.SUPERUSER = "portals"
    ...
    @pytest.mark.asyncio
    async def test_full_flow_env_mode_invalid_key_not_in_db(
        self, mock_session, monkeypatch
    ):
        """Full flow test: env mode with invalid key that's also not in db returns None."""
        monkeypatch.setenv("PORTALS_API_KEY", "sk-correct-key")
        ...
        mock_settings.auth_settings.API_KEY_SOURCE = "env"
        mock_settings.auth_settings.SUPERUSER = "portals"
        mock_settings.auth_settings.SECRET_KEY.get_secret_value.return_value = (
            "test-secret-key-for-unit-tests"
        )
