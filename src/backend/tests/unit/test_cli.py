...
import pytest
import typer
from portals.__main__ import _create_superuser, app, get_number_of_workers
from px.services import deps
...
class TestSuperuserCommand:
    ...
    async def test_additional_superuser_requires_auth_production(self, client, active_super_user):
        ...
        with (
            patch("portals.services.deps.get_settings_service") as mock_settings,
            patch("portals.__main__.get_settings_service") as mock_settings2,
        ):
    ...
    async def test_failed_auth_token_validation(self, client, active_super_user):  # noqa: ARG002
        ...
        with (
            ...
            patch("portals.__main__.get_settings_service") as mock_settings2,
            patch(
                "portals.__main__.get_current_user_from_access_token",
                side_effect=Exception("Invalid token"),
            ),
            patch("portals.__main__.check_key", return_value=None),
        ):
            # Configure settings for production mode (AUTO_LOGIN=False)
            mock_auth_settings = type(
                "MockAuthSettings",
...
def test_get_number_of_workers():
    ...
    with (
        patch("portals.__main__.platform.system", return_value="Linux"),
        patch("portals.__main__.cpu_count", return_value=4),
    ):
