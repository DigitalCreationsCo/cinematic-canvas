...
import anyio
import pytest
from portals.services.event_manager import WebhookEventManager
...
def test_webhook_auth_enable_defaults_to_true():
    ...
    directly so a stray PORTALS_WEBHOOK_AUTH_ENABLE env var can't mask a regression.
    """
...
async def test_webhook_invalid_api_key(client, added_webhook_test):
    ...
    with patch("portals.api.v1.endpoints.get_auth_service", return_value=mock_auth_service):
        endpoint_name = added_webhook_test["endpoint_name"]
        endpoint = f"api/v1/webhook/{endpoint_name}"
        payload = {"test": "data"}
...
async def test_vertex_builds_endpoint_returns_empty_for_new_flow(client, logged_in_headers):
    ...
    from portals.services.database.models.flow.model import FlowCreate
...
class TestGetVertexIdsFromFlow:
    ...
    def test_returns_empty_list_when_flow_data_is_none(self):
        ...
        from portals.api.v1.endpoints import _get_vertex_ids_from_flow
...
class TestSimpleRunFlowTask:
    ...
    async def test_emits_vertices_sorted_event_when_emit_events_true(self):
        ...
        from portals.api.v1.endpoints import simple_run_flow_task
        ...
        with (
            patch("portals.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run,
            patch("portals.api.v1.endpoints.webhook_event_manager") as mock_manager,
        ):
    ...
    async def test_emits_error_event_on_exception(self):
        ...
        with (
            patch("portals.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run,
            patch("portals.api.v1.endpoints.webhook_event_manager") as mock_manager,
            patch("portals.api.v1.endpoints.logger") as mock_logger,
        ):
    ...
    async def test_logs_telemetry_on_success(self):
        ...
        with patch("portals.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run:
            mock_run.return_value = {"result": "success"}
...
class TestWebhookEventsStreamAuth:
    ...
    async def test_raises_403_when_user_does_not_own_flow(self):
        ...
        from fastapi import HTTPException
        from portals.api.v1.endpoints import webhook_events_stream
