"""Sync HTTP client for the Portals REST API.
...
The async counterpart lives in :mod:`portals_sdk._async_client`.
"""
...
# Re-export async client so that existing ``from portals_sdk.client import ...``
...
class PortalsClient(_ClientCommon):
    """Synchronous client for the Portals REST API.
    ...
    def pull_project(
        ...
        *,
        ...
        return self._write_project_flows(
            self.download_project(project_id), output_dir=output_dir
        )
...
#: Short alias for :class:`PortalsClient`.
#:
#: Example::
#:
#:     from portals_sdk import Client
...
__all__ = ["AsyncClient", "AsyncPortalsClient", "Client", "PortalsClient"]
