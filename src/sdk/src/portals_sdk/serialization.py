"""Git-friendly flow serialization for portals-sdk.

``normalize_flow`` transforms a raw Portals flow dict into a stable,
...
def normalize_flow(
    ...
    *,
    ...
    ----------
    flow:
        Raw flow dict as returned by the Portals API or read from a ``.json``
        file.
    strip_volatile:
