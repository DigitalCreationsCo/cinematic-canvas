"""Environment configuration for portals-sdk.

Loads named environment definitions from a TOML file so teams can switch
between Portals instances (dev / staging / production) without code changes.

Config file lookup order
------------------------
1. Path given explicitly to ``load_environments()`` or ``get_client()``.
2. The ``PORTALS_ENVIRONMENTS_FILE`` environment variable.
3. ``portals-environments.toml`` in the current working directory.
4. ``~/.config/portals/environments.toml``
...
.. code-block:: toml
    ...
    api_key_env = "PORTALS_STAGING_API_KEY"   # env-var that holds the key  # pragma: allowlist secret

    [environments.production]
    url = "https://portals.example.com"
    api_key_env = "PORTALS_PROD_API_KEY"  # pragma: allowlist secret
...
from portals_sdk.exceptions import EnvironmentConfigError, EnvironmentNotFoundError
...
_ENV_VAR = "PORTALS_ENVIRONMENTS_FILE"
_LOCAL_NAME = "portals-environments.toml"
_USER_PATH = Path.home() / ".config" / "portals" / "environments.toml"

_EXAMPLE_CONFIG = """\
# portals-environments.toml
#
# Define named Portals environments.  The api_key_env field is the *name*
# of an environment variable that holds the API key for that instance.
#
# [environments.staging]
# url = "https://staging.portals.example.com"
# api_key_env = "PORTALS_STAGING_API_KEY"  # pragma: allowlist secret
#
# [environments.production]
# url = "https://portals.example.com"
# api_key_env = "PORTALS_PROD_API_KEY"  # pragma: allowlist secret
...
def _load_toml(path: Path) -> dict[str, Any]:
    try:
        ...
        raise EnvironmentConfigError(
            f"Cannot read environments file {path}: {exc}"
        ) from exc
...
def _parse_env(raw: dict[str, Any], file_path: Path, name: str) -> EnvironmentConfig:
    if "url" not in raw:
        raise EnvironmentConfigError(
            f"Environment {name!r} in {file_path} is missing the required 'url' field."
        )
    url: str = raw["url"]
...
def load_environments(
    ...
    ----------
    config_file:
        Explicit path to a ``portals-environments.toml`` file. If omitted,
    ...
    if file_path is None:
        raise EnvironmentConfigError(
            "No portals-environments.toml found. "
    ...
    if not isinstance(raw_envs, dict):
        raise EnvironmentConfigError(
            f"Expected [environments] to be a TOML table in {file_path}"
        )
    ...
    for name, env_data in raw_envs.items():
        if not isinstance(env_data, dict):
            raise EnvironmentConfigError(
                f"Environment {name!r} in {file_path} must be a TOML table."
            )
...
def get_client(
    ...
    *,
    ...
    Example::
        ...
        from portals_sdk import get_client
...
def get_async_client(
    ...
    *,
    ...
    Example::
        ...
        from portals_sdk import get_async_client
