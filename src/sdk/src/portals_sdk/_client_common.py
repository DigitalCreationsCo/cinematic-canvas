"""Shared helpers used by both sync and async Portals SDK clients."""
...
from portals_sdk._http import _HTTP_201_CREATED, _logger
from portals_sdk.models import FlowCreate, RunRequest, StreamChunk
...
class _ClientCommon:
    ...
    def _validate_model_list(
        cls, model_type: type[_ModelT], payload: list[Any]
    ) -> list[_ModelT]:
        ...
        with zipfile.ZipFile(io.BytesIO(content)) as zf:
            ...
            for info in entries:
                ...
                if len(raw) > cls._MAX_ENTRY_BYTES:
                    _logger.warning(
                        "Skipping ZIP entry %r: actual size %d exceeds limit",
                        info.filename,
                        len(raw),
                    )
                    continue
    ...
    def _load_flow_file(path: str | Path) -> tuple[Any, FlowCreate]:
        ...
        if not flow_id:
            msg = (
                f"Flow file {str(path)!r} does not contain an 'id' field; cannot upsert"
            )
            raise ValueError(msg)
        flow_create = FlowCreate.model_validate(
