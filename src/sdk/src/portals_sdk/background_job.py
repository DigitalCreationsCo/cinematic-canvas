...
from portals-ai/sdk PR #1 (Janardan Singh Kavia, IBM Corp., Apache 2.0)
adapted for the Portals V1 ``/api/v1/run/{id}`` endpoint.

Typical usage::

    async with AsyncClient("https://portals.example.com", api_key="...") as client:
...
class BackgroundJob:
    """Non-blocking handle for an in-flight :meth:`AsyncPortalsClient.run` call.

    Returned by :meth:`AsyncPortalsClient.run_background`.  The underlying
    network request runs in an :class:`asyncio.Task` so the caller's event
    loop remains free.

    Adapted from ``BackgroundJob`` in portals-ai/sdk PR #1
        ...
        *,
        ...
        Args:
            timeout: Maximum seconds to wait.  ``None`` (default) means wait
                     indefinitely.  Raises :exc:`PortalsTimeoutError` on
                     expiry.
        ...
        Raises:
            PortalsTimeoutError: If *timeout* elapses before the run finishes.
        ...
        try:
            ...
            raise PortalsTimeoutError(msg) from exc
