...
import httpx
import pytest
from portals_sdk import AsyncClient, Client, StreamChunk
...
def test_stream_chunk_add_message_text() -> None:
    chunk = StreamChunk(
        event="add_message", data={"message": {"text": "Final answer", "sender": "AI"}}
    )
...
def test_sync_stream_skips_blank_lines() -> None:
    ...
    body = (
        b"\n\n"
        + json.dumps({"event": "token", "data": {"chunk": "A"}}).encode()
        + b"\n\n\n\n"
    )
...
def test_sync_stream_raises_auth_error_on_401() -> None:
    ...
    client = _sync_client(
        _MockTransport(
            status=401, content=body, headers={"content-type": "application/json"}
        )
    )
...
def test_sync_stream_raises_http_error_on_500() -> None:
    ...
    client = _sync_client(
        _MockTransport(
            status=500, content=body, headers={"content-type": "application/json"}
        )
    )
    with pytest.raises(PortalsHTTPError):
        list(client.stream(_FLOW_ID, input_value="hi"))
    client.close()
...
def test_sync_stream_raises_connection_error() -> None:
    ...
    client = Client(_BASE_URL, httpx_client=http)
    with pytest.raises(PortalsConnectionError):
        list(client.stream(_FLOW_ID, input_value="hi"))
    client.close()
...
# ---------------------------------------------------------------------------
# PortalsClient.run() convenience tests
# ---------------------------------------------------------------------------
...
def test_sync_run_returns_run_response() -> None:
    ...
    client = _sync_client(
        _MockTransport(content=body, headers={"content-type": "application/json"})
    )
...
# ---------------------------------------------------------------------------
# AsyncPortalsClient.stream() tests
# ---------------------------------------------------------------------------
...
async def test_async_stream_skips_blank_lines() -> None:
    body = (
        b"\n\n"
        + json.dumps({"event": "token", "data": {"chunk": "X"}}).encode()
        + b"\n\n"
    )
...
# ---------------------------------------------------------------------------
# AsyncPortalsClient.run() convenience tests
# ---------------------------------------------------------------------------
...
async def test_async_run_returns_run_response() -> None:
    ...
    client = _async_client(
        _AsyncMockTransport(content=body, headers={"content-type": "application/json"})
    )
...
def test_stream_chunk_importable_from_package() -> None:
    ...
    assert portals_sdk.StreamChunk is StreamChunk
