...
import httpx
import pytest
from portals_sdk import AsyncClient, Client
from portals_sdk.exceptions import PortalsHTTPError
from portals_sdk.models import Flow
...
class _MockTransport(httpx.BaseTransport):
    ...
    def __init__(
        self, *, status: int = 200, body: Any = None, created: bool = False
    ) -> None:
        self._status = status
        self._body = body
...
def test_push_project_pushes_all_json_files(tmp_path: Path) -> None:
    for name in ("flow-a", "flow-b"):
        data = {
            **_SAMPLE_FLOW,
            "id": f"00000000-0000-0000-0000-{name[-1] * 12}",
            "name": name,
        }
