...
import pytest
import respx
from portals_sdk.client import PortalsClient
from portals_sdk.exceptions import PortalsHTTPError, PortalsNotFoundError
...
def test_upsert_flow_create():
    ...
    from portals_sdk.models import FlowCreate
...
# ---------------------------------------------------------------------------
# upsert_flow -- 404 raises PortalsNotFoundError
# ---------------------------------------------------------------------------
...
def test_upsert_flow_not_found_raises():
    ...
    client = _client()
    with pytest.raises(PortalsNotFoundError):
        client.upsert_flow(_FLOW_ID, FlowCreate(name="Test Flow"))


# ---------------------------------------------------------------------------
# upsert_flow -- 409 conflict raises PortalsHTTPError
# ---------------------------------------------------------------------------
...
def test_upsert_flow_conflict_raises():
    ...
    client = _client()
    with pytest.raises(PortalsHTTPError) as exc_info:
        client.upsert_flow(_FLOW_ID, FlowCreate(name="Test Flow"))
...
def test_push_command_creates_flow(tmp_path: Path):
    ...
    respx.get(f"{_BASE}/api/v1/projects/").mock(
...
def test_push_command_dry_run_makes_no_requests(tmp_path: Path):
    ...
    flow_file.write_text(
        json.dumps(
            {"id": str(_FLOW_ID), "name": "F", "data": {"nodes": [], "edges": []}}
        ),
        encoding="utf-8",
    )
...
def test_push_command_project_dir(tmp_path: Path):
    ...
    respx.post(f"{_BASE}/api/v1/projects/").mock(
        return_value=httpx.Response(201, json=project_payload)
    )

    for fid in flow_ids:
        payload = {**_FLOW_PAYLOAD, "id": str(fid)}
        respx.put(f"{_BASE}/api/v1/flows/{fid}").mock(
