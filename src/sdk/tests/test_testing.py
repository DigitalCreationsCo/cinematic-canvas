"""Unit tests for portals_sdk.testing and RunResponse helpers.

Uses respx to mock HTTP so no live Portals instance is needed.
"""
...
from portals_sdk.client import AsyncPortalsClient, PortalsClient
from portals_sdk.models import RunOutput, RunResponse
from portals_sdk.testing import AsyncFlowRunner, FlowRunner, pytest_addoption

_BASE = "http://portals.test"

# ---------------------------------------------------------------------------
# Shared fixtures
# ---------------------------------------------------------------------------

# Minimal RunResponse payload matching the standard Portals chat shape
_CHAT_RUN_RESPONSE = {
    "session_id": "sess-abc",
    "outputs": [
        {
...
def _sync_client() -> PortalsClient:
    return PortalsClient(base_url=_BASE, api_key="test-key")  # pragma: allowlist secret


def _async_client() -> AsyncPortalsClient:
    return AsyncPortalsClient(
        base_url=_BASE, api_key="test-key"
...
def test_run_output_first_text_none_when_no_text_key():
    ...
    out = RunOutput(
        outputs=[{"results": {"other": "value"}, "artifacts": {}, "outputs": {}}]
    )
    assert out.first_text() is None
    ...
    resp = RunResponse.model_validate(
        {
            ...
            "outputs": [
                ...
                # block without text
                {
                    ...
                    "artifacts": {},
                    "outputs": [
                        {"results": {"other": 42}, "artifacts": {}, "outputs": {}}
                    ],
                    "session_id": None,
                    "timedelta": None,
                },
...
def test_flow_runner_calls_run_flow():
    ...
    respx.post(f"{_BASE}/api/v1/run/my-endpoint").mock(
        return_value=httpx.Response(200, json=_CHAT_RUN_RESPONSE)
    )
...
def test_flow_runner_passes_tweaks():
    ...
    route = respx.post(f"{_BASE}/api/v1/run/ep").mock(
        return_value=httpx.Response(200, json=_CHAT_RUN_RESPONSE)
    )
    runner = FlowRunner(_sync_client())
...
async def test_async_flow_runner_calls_run_flow():
    ...
    respx.post(f"{_BASE}/api/v1/run/async-ep").mock(
        return_value=httpx.Response(200, json=_TEXT_RUN_RESPONSE)
    )
...
async def test_async_flow_runner_multi_output():
    ...
    respx.post(f"{_BASE}/api/v1/run/multi-ep").mock(
        return_value=httpx.Response(200, json=_MULTI_RUN_RESPONSE)
    )
    runner = AsyncFlowRunner(_async_client())
...
def test_flow_runner_accepts_uuid():
    ...
    respx.post(f"{_BASE}/api/v1/run/{flow_id}").mock(
        return_value=httpx.Response(200, json=_CHAT_RUN_RESPONSE)
    )
    runner = FlowRunner(_sync_client())
...
def test_flow_runner_and_async_runner_are_importable():
    """FlowRunner and AsyncFlowRunner can be imported from portals_sdk.testing."""
    from portals_sdk.testing import AsyncFlowRunner as ImportedAsync
    from portals_sdk.testing import FlowRunner as ImportedSync
...
def test_pytest_addoption_registers_options(pytestconfig):
    """The plugin registers all --portals-* options (verified via public getoption API)."""
    ...
    for name in (
        ...
        assert pytestconfig.getoption(name) is None, (
            f"Option {name!r} missing or has unexpected default"
        )
