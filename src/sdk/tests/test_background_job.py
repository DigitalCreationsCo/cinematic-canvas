"""Unit tests for BackgroundJob and AsyncPortalsClient.run_background.

All tests run entirely in-process; no real Portals instance required.
...
from portals_sdk.exceptions import PortalsTimeoutError
...
class TestWaitForCompletion:
    ...
    @pytest.mark.asyncio
    async def test_raises_portals_timeout_error_on_expiry(self):
        task = asyncio.create_task(_slow_success(5.0))
        job = BackgroundJob(task)
        with pytest.raises(PortalsTimeoutError):
...
# ---------------------------------------------------------------------------
# AsyncPortalsClient.run_background integration
# ---------------------------------------------------------------------------
...
class TestRunBackground:
    ...
    async def test_returns_background_job_instance(self):
        from portals_sdk.client import AsyncPortalsClient

        client = AsyncPortalsClient(
            "http://portals.test", api_key="test-key"
        )  # pragma: allowlist secret
        with patch.object(
            client, "run", new_callable=AsyncMock, return_value=_RUN_RESPONSE
        ):
    ...
    async def test_run_background_exported_from_package(self):
        from portals_sdk import BackgroundJob as ExportedBackgroundJob

        assert ExportedBackgroundJob is BackgroundJob

    @pytest.mark.asyncio
    async def test_portals_timeout_error_exported_from_package(self):
        from portals_sdk import PortalsTimeoutError as ExportedError

        assert ExportedError is PortalsTimeoutError
