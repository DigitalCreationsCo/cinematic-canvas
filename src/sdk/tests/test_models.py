...
def test_load_environments(tmp_path: Path):
    config = tmp_path / "portals-environments.toml"
    config.write_text(
        textwrap.dedent("""\
    ...
    from portals_sdk.environments import get_environment, load_environments
    ...
    try:
        ...
        assert (
            envs["production"].api_key == "not-a-real-secret"
...
def test_environment_not_found(tmp_path: Path):
    ...
    from portals_sdk.environments import get_environment
    from portals_sdk.exceptions import EnvironmentNotFoundError
...
def test_missing_url_raises(tmp_path: Path):
    ...
    from portals_sdk.environments import load_environments
    from portals_sdk.exceptions import EnvironmentConfigError
