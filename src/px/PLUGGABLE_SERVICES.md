...
PX now supports a pluggable service architecture that allows you to customize and extend service implementations without modifying core code.
...
PX also supports **adapter registries** -- collections of swappable implementations that share the same protocol.
...
Each adapter type exposes a typed accessor in `px.services.deps`:

```python
from px.services.deps import get_deployment_adapter
...
```python
from px.services import register_adapter
...
`px.toml` (preferred when both files exist):
...
Entry-point groups follow the naming convention `px.<adapter_type>.adapters` (e.g. `px.deployment.adapters`).

```toml
[project.entry-points."px.deployment.adapters"]
local = "my_package.deployment:LocalAdapter"
remote = "my_package.deployment:RemoteAdapter"
```
...
Entry-point groups and config section paths are derived from the `AdapterType` value automatically (e.g. `px.deployment.adapters` and `[deployment.adapters]`).
...
When adapters are part of core PX, prefer namespacing them under `services/adapters`:
...
- **PX owns shared slot primitives and slot taxonomy**:
  - `px.services.adapters.payload` defines shared primitives (`PayloadSlot`, `ProviderPayloadSchemas`)
  - each adapter domain in px defines canonical slot names once (for example deployment slots in
    `px.services.adapters.deployment.payloads`)
- **Adapters (PX side) own adapter-facing payload models**:
  - adapters populate their `*PayloadSchemas` registry with adapter-side models
- **API hosts (for example Portals) own API-facing payload models**:
...
- PX defines deployment slot taxonomy and adapter registry (`DeploymentPayloadFields` / `DeploymentPayloadSchemas`)
- Portals defines deployment API registry (`DeploymentApiPayloads`) and mapper behavior
...
Create an `px.toml` in your project root:

```toml
[services]
database_service = "portals.services.database.service:DatabaseService"
storage_service = "portals.services.storage.local:LocalStorageService"
cache_service = "portals.services.cache.service:ThreadingInMemoryCache"
```

Then run:
```bash
px serve my_flow.json
```

Services will be automatically discovered from:
1. The current working directory (looking for `px.toml` or `pyproject.toml`)
...
```python
from px.services import register_service
...
```toml
[project.entry-points."px.services"]
database_service = "my_plugin.services:MyDatabaseService"
storage_service = "my_plugin.services:MyStorageService"
```

When your package is installed, PX will automatically discover these services.

## Configuration Details

### Config File Locations

PX searches for configuration in this order:

1. `./px.toml` (project-specific config)
2. `./pyproject.toml` under `[tool.px.services]` (Python project integration)
...
```python
import pytest
from px.services.manager import get_service_manager
from px.services.schema import ServiceType
...
3. Ensure config file is in the correct location:
   - If settings service exists: `{settings.config_dir}/px.toml`
   - Otherwise: `./px.toml` or `./pyproject.toml` in current directory
4. Enable debug logging: `export PORTALS_LOG_LEVEL=DEBUG`

### Import Errors

**Error:** `ModuleNotFoundError: No module named 'portals'`

**Solutions:**
1. Ensure portals is installed: `pip install portals`
2. Check if module path in config is correct
3. Verify the package is importable: `python -c "import portals.services.database"`
...
See:
- `px.toml.example` - Example configuration file showing Portals service registration
- `src/px/services/` - Minimal built-in service implementations (auth, telemetry, tracing, variable, storage, etc.)
  - Auth: `px.services.auth.base` (BaseAuthService) and `px.services.auth.service` (AuthService). Use `get_auth_service()` from `px.services.deps`. Override with `auth_service = "portals.services.auth.service:AuthService"` in config for full JWT/API key auth.
- `src/backend/base/portals/services/` - Full-featured Portals services
