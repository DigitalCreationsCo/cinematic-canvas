...
The CLI's ``px run`` (``run_flow``) and ``px serve``/``execute_graph_with_capture``
both need to:
...
3. Propagate ``session_id`` to ``Memory``/``MessageHistory`` vertices the way
   ``portals.api.utils.flow_utils.build_graph_from_data`` does — preserving
   any value already pinned in the flow JSON.
4. Resolve ``fallback_to_env_vars`` from settings (mirrors the portals API
   path in ``processing.process.run_graph_internal``).

Both call sites used to inline these steps; consolidating them here prevents
behavior drift between ``px run`` and ``px serve``.
"""
    ...
    *,
    ...
    if not user_id:
        ...
        logger.debug(
            ...
            "px's variable service is env-backed, so user_id is not used for variable scoping."
        )
    ...
    # Mirror portals's build_graph_from_data: only write when raw_params has no
...
def resolve_fallback_to_env_vars() -> bool:
    ...
    Mirrors the portals API path (``processing.process.run_graph_internal``):
