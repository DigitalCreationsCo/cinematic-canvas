...
from px.template.frontend_node.constants import FORCE_SHOW_FIELDS
from px.utils import constants
...
def format_dict(
    dictionary: dict[str, Any], class_name: str | None = None
) -> dict[str, Any]:
...
def check_list_type(type_: str, value: dict[str, Any]) -> str:
    ...
    if any(list_type in type_ for list_type in ["List", "Sequence", "Set"]):
        type_ = (
            type_.replace("List[", "").replace("Sequence[", "").replace("Set[", "")[:-1]
        )
        value["list"] = True
    else:
        value["list"] = False
...
def add_options_to_field(
    value: dict[str, Any], class_name: str | None, key: str
) -> None:
...
async def update_settings(
    *,
    ...
    if webhook_polling_interval is not None:
        ...
        settings_service.settings.update_settings(
            webhook_polling_interval=webhook_polling_interval
        )
