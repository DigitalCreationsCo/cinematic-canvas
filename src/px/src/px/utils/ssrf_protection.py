...
Configuration:
    PORTALS_SSRF_PROTECTION_ENABLED: Enable/disable SSRF protection (default: true)
    PORTALS_SSRF_ALLOWED_HOSTS: Comma-separated list of allowed hosts/CIDR ranges
...
def is_ssrf_protection_enabled() -> bool:
    ...
    env_value = os.getenv("PORTALS_SSRF_PROTECTION_ENABLED")
...
def get_allowed_hosts() -> list[str]:
    ...
    env_value = os.getenv("PORTALS_SSRF_ALLOWED_HOSTS", "")
    if env_value:
...
def _validate_direct_ip_address(hostname: str) -> bool:
    ...
    if is_ip_blocked(ip_obj):
        msg = (
            ...
            "To allow this IP, add it to PORTALS_SSRF_ALLOWED_HOSTS environment variable."
        )
        raise SSRFProtectionError(msg)
...
def _validate_hostname_resolution(hostname: str) -> None:
    ...
    if blocked_ips:
        msg = (
            ...
            "To allow this hostname, add it to PORTALS_SSRF_ALLOWED_HOSTS environment variable."
        )
        raise SSRFProtectionError(msg)
