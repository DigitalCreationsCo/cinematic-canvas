...
_PORTALS_IS_INSTALLED = False

with contextlib.suppress(ImportError):
    import portals  # noqa: F401

    _PORTALS_IS_INSTALLED = True
...
def validate_code(code):
    ...
    # Evaluate the function definition with portals context
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            ...
            try:
                # Create execution context with common portals imports
                exec_globals = _create_portals_execution_context()
...
def _create_portals_execution_context():
    """Create execution context with common portals imports."""
    context = {}

    # Import common portals types that are used in templates
    try:
    ...
    try:
        from px.io import HandleInput, Output, TabInput
            ...
            for alias in node.names:
                try:
                    ...
                    if alias.asname:
                        ...
                        exec_globals[variable_name] = sys.modules.get(
                ...
                try:
                    if isinstance(node, ast.ImportFrom):
                        ...
                        if alias.asname:
                            ...
                            exec_globals[top_level] = sys.modules.get(
                                top_level, imported
                            )
                except ModuleNotFoundError as e:
                    msg = f"Module {alias.name} not found. Please install it and try again."
...
def create_class(code, class_name):
    ...
    code = code.replace(
        "from portals import CustomComponent",
        "from portals.custom import CustomComponent",
    )
    code = code.replace(
        "from portals.interface.custom.custom_component import CustomComponent",
        "from portals.custom import CustomComponent",
    )
    ...
    try:
        ...
        error_message = "\n".join(
            [message[1] if len(message) > 1 else message[0] for message in messages]
        )
...
def _get_module_fallbacks(module_name: str) -> list[str]:
    ...
    Handles portals -> px and langchain -> langchain_classic remapping at the
    ...
    """
    names = [module_name]
    if module_name.startswith("portals."):
        names.append(module_name.replace("portals.", "px.", 1))
    if module_name.startswith("langchain."):
...
def prepare_global_scope(module):
    ...
    for node in module.body:
        if isinstance(node, ast.Import):
            ...
            node, ast.ClassDef | ast.FunctionDef | ast.Assign | ast.AnnAssign
        ):
        ...
        for alias in node.names:
            ...
            if module_obj is None:
                if sys.platform == "win32":
                    ...
                    logger.debug(
                        "Module '%s' unavailable on Windows — inserted placeholder",
                        module_name,
                    )
                    continue
    ...
    for node in import_froms:
        ...
        for module_name in module_names_to_try:
            try:
                ...
                _handle_module_attributes(
                    imported_module, node, module_name, exec_globals
                )
...
def get_default_imports(code_string):
    ...
    portals_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, portals_imports)
    portals_module = importlib.import_module("px.field_typing")
    default_imports.update(
        {name: getattr(portals_module, name) for name in necessary_imports}
    )
