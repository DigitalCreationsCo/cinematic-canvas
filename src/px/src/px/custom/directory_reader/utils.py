...
from px.custom.directory_reader.directory_reader import DirectoryReader
from px.log.logger import logger
from px.template.frontend_node.custom_components import CustomComponentFrontendNode
...
async def abuild_custom_component_list_from_path(path: str):
    ...
    valid_components, invalid_components = await abuild_and_validate_all_files(
        reader, file_list
    )
...
def build_menu_items(menu_item):
    ...
    for component_name, component_template, component in menu_item["components"]:
        try:
            ...
            logger.exception(
                f"Error while building custom component {component['output_types']}"
            )
    return menu_items
