...
from px.base.tools.constants import (
    TOOL_OUTPUT_DISPLAY_NAME,
    TOOL_OUTPUT_NAME,
    TOOLS_METADATA_INFO,
    TOOLS_METADATA_INPUT_NAME,
)
from px.custom.tree_visitor import RequiredInputsVisitor
from px.exceptions.component import StreamingError
from px.field_typing import Tool  # noqa: TC001

# Lazy import to avoid circular dependency
# from px.graph.state.model import create_state_model
# Lazy import to avoid circular dependency
# from px.graph.utils import has_chat_output
from px.helpers.custom import format_type
from px.memory import astore_message, aupdate_messages, delete_message
from px.schema.artifact import get_artifact_type, post_process_raw
from px.schema.data import Data
from px.schema.log import Log
from px.schema.message import ErrorMessage, Message
from px.schema.properties import Source, Usage
from px.schema.token_usage import accumulate_usage, extract_usage_from_chunk
from px.serialization.serialization import serialize
from px.template.field.base import UNDEFINED, Input, Output
from px.template.frontend_node.custom_components import ComponentFrontendNode
from px.utils.async_helpers import run_until_complete
from px.utils.util import find_closest_match
...
if TYPE_CHECKING:
    ...
    from px.base.tools.component_tool import ComponentToolkit
    ...
    from px.inputs.inputs import InputTypes
    from px.schema.dataframe import DataFrame
    from px.schema.log import LoggableType
...
class Component(CustomComponent):
    ...
    def _should_track_input(self, input_obj: InputTypes) -> bool:
        ...
        from px.inputs.input_mixin import SENSITIVE_FIELD_TYPES
    ...
    async def _build_tools_metadata_input(self):
        try:
            from px.inputs.inputs import ToolsInput
        except ImportError as e:
            msg = "Failed to import ToolsInput from px.inputs.inputs"
    ...
    def is_connected_to_chat_input(self) -> bool:
        # Lazy import to avoid circular dependency
        from px.graph.utils import has_chat_input
    ...
    def _ensure_message_required_fields(self, message: Message) -> None:
        ...
        """
        from px.utils.constants import MESSAGE_SENDER_AI, MESSAGE_SENDER_NAME_AI
    ...
    async def send_message(self, message: Message, id_: str | None = None, *, skip_db_update: bool = False):
        ...
        This is the central method for sending messages in Portals. It handles:
