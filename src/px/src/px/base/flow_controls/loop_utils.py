...
def get_loop_body_vertices(
    ...
    start_edges = [
        e for e in vertex.outgoing_edges if e.source_handle.name == loop_output_name
    ]
    if not start_edges:
        return set()
...
def get_loop_body_start_vertex(
    vertex: "Vertex", loop_output_name: str = "item"
) -> str | None:
