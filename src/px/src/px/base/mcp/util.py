    ...
    if expected_type in (dict, list):
        ...
        if isinstance(value, str):
            try:
                ...
                raise _err(
                    expected_type_desc,
                    f"but received string: {value!r}; could not convert.",
                ) from e
...
async def get_flow_snake_case(flow_name: str, user_id: str, session, *, is_action: bool | None = None):
    try:
        ...
        msg = "Portals Flow model is not available. This feature requires the full Portals installation."
                ...
                # If no aclose, try regular close method
                    ...
                    try:
                        ...
                        if inspect.iscoroutinefunction(session.close):
                            ...
                            await session.close()
                            await logger.adebug(
                                "Successfully closed session %s using async close()",
                                session_id,
                            )
                        else:
                            ...
                            if inspect.isawaitable(close_result):
                                await close_result
                                await logger.adebug(
                                    "Successfully closed session %s using awaitable close()",
                                    session_id,
                                )
                            else:
                                # It's a synchronous close
                                await logger.adebug(
                                    "Successfully closed session %s using sync close()",
                                    session_id,
                                )
                    except Exception as e:  # noqa: BLE001
    ...
    async def _connect_to_server(self, command_str: str, env: dict[str, str] | None = None) -> list[StructuredTool]:
        ...
        if platform.system() == "Windows":
            ...
            server_params = StdioServerParameters(
                ...
                args=[
                    "-c",
                    f"exec {command_str} || echo 'Command failed with exit code $?' >&2",
                ],
                env=env_data,
            )
