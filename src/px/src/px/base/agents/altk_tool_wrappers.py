...
from px.base.agents.altk_base_agent import ALTKBaseTool, BaseToolWrapper
    ...
    if "type" in param_info:
        ...
        if schema_type == "array":
            ...
            if "items" in param_info:
                items_schema = _convert_pydantic_type_to_json_schema_type(
                    param_info["items"]
                )
    ...
    def _validate_and_run(self, *args, **kwargs) -> str:
        ...
        if (
            ...
            self.conversation_context = [
                self._custom_message_to_dict(msg) for msg in self.conversation_context
            ]
        ...
        try:
            ...
            logger.debug(
                f"SPARC validation result for tool {self.name}: {result.output.reflection_result}"
            )
...
class PreToolValidationWrapper(BaseToolWrapper):
    ...
    def wrap_tool(self, tool: BaseTool, **kwargs) -> BaseTool:
        ...
        if not agent:
            logger.warning(
                "Cannot wrap tool with PreToolValidationWrapper: missing 'agent'"
            )
            return tool
            ...
            try:
                ...
                if isinstance(args_dict, dict):
                    for param_name, param_info in args_dict.items():
                        ...
                        param_spec = _convert_pydantic_type_to_json_schema_type(
                            param_info
                        )
