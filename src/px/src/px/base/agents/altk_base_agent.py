...
from px.base.agents.callback import AgentAsyncHandler
from px.base.agents.events import ExceptionWithMessageError, process_agent_events
from px.base.agents.utils import data_to_messages, get_chat_output_sender_name
from px.components.models_and_agents import AgentComponent
from px.log.logger import logger
from px.memory import delete_message
...
def normalize_message_content(message: BaseMessage) -> str:
    ...
    Note:
        This addresses the inconsistency in px.schema.data.Data.to_lc_message() where:
...
class ALTKBaseTool(BaseTool):
    ...
    def _get_altk_llm_object(self, *, use_output_val: bool = True) -> Any:
        ...
        if isinstance(llm_object, ChatAnthropic):
            ...
            logger.info(
                "ALTK currently only supports OpenAI and Anthropic models through Portals."
            )
            llm_client_obj = None
...
class ALTKBaseAgentComponent(AgentComponent):
    ...
    def build_conversation_context(self) -> list[BaseMessage]:
        ...
        if hasattr(self, "chat_history") and self.chat_history:
            if isinstance(self.chat_history, Data):
                ...
                error_message = f"chat_history must be a Data object, list of Data/Message objects, or None. Got: {type_name}"
    ...
    async def run_agent(
        ...
        if hasattr(self, "chat_history") and self.chat_history:
            ...
            # Handle both px.schema.message.Message and portals.schema.message.Message types
            if all(
