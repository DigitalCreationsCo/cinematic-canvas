...
from px.inputs.input_mixin import FieldTypes
...
def _resolve_input_type(
    annotation: Any, *, required: bool
) -> tuple[type[InputTypes], bool, list[Any] | None]:
    """Resolve a Pydantic annotation into a Portals input type."""
    ann = annotation
...
def _get_portals_input_default(model_field: Any, input_cls: type[InputTypes]) -> Any:
    """Return a Portals-safe default value for a Pydantic model field."""
...
def flatten_schema(root_schema: dict[str, Any]) -> dict[str, Any]:
    ...
    if not defs and all(
        "$ref" not in v and v.get("type") not in ("object", "array")
    ...
    def _walk(
        ...
        *,
        ...
        while "$ref" in schema:
            ...
            if resolved is None:
                logger.warning(
                    "Flattening schema: definition '%s' not found, skipping field '%s'",
                    ref_name,
                    name,
                )
                return
...
def schema_to_portals_inputs(schema: type[BaseModel]) -> list[InputTypes]:
    inputs: list[InputTypes] = []

    for field_name, model_field in schema.model_fields.items():
        required = model_field.is_required()
        lf_cls, is_list, options = _resolve_input_type(
            model_field.annotation, required=required
        )
        input_kwargs = {
        ...
        default = _get_portals_input_default(model_field, lf_cls)
...
def create_input_schema_from_dict(
    inputs: list[dotdict], param_key: str | None = None
) -> type[BaseModel]:
    if not isinstance(inputs, list):
        msg = "inputs must be a list of Inputs"
        raise TypeError(msg)
