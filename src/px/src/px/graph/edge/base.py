...
from px.graph.edge.schema import (
    EdgeData,
...
def types_compatible(output_types: list[str], input_types: list[str]) -> bool:
    ...
    for output_type in output_types:
        ...
        for input_type in input_types:
            ...
            if input_type in {output_type, migrated_output} or migrated_input in {
                output_type,
                migrated_output,
            }:
                return True
...
class Edge:
                ...
                try:
                    if "name" in self._target_handle:
                        self.target_handle: TargetHandle = (
                            TargetHandle.from_loop_target_handle(
                    ...
                    if (
                        "inputTypes" in self._target_handle
                        and self._target_handle["inputTypes"] is None
                    ):
                        # Check if self._target_handle['fieldName']
                        if hasattr(target, "custom_component"):
                            display_name = getattr(
                                target.custom_component, "display_name", ""
                            )
                            msg = (
    ...
    def _validate_handles(self, source, target) -> None:
        if self.target_handle.input_types is None:
            ...
            self.valid_handles = types_compatible(
                self.source_handle.output_types,
                [self.target_handle.type] if self.target_handle.type else [],
            )
    ...
    def _legacy_validate_handles(self, source, target) -> None:
        if self.target_handle.input_types is None:
            # Backward compatibility: old flows may have Data/DataFrame types
            self.valid_handles = types_compatible(
                self.source_handle.base_classes,
                [self.target_handle.type] if self.target_handle.type else [],
            )
        else:
            # Backward compatibility: old flows may have Data/DataFrame types
    ...
    def _validate_edge(self, source, target) -> None:
        ...
        self.source_types = [
            ...
            if output["name"] == self.source_handle.name
        ]

        # Check if this is an loop input (loop target handle with output_types)
        is_loop_input = (
            hasattr(self.target_handle, "input_types")
            and self.target_handle.input_types
        )
        loop_input_types = []
        ...
        if is_loop_input:
            ...
            # Backward compatibility: old flows may have Data/DataFrame types
            self.valid = any(
                types_compatible(output["types"], loop_input_types)
    ...
    def _legacy_validate_edge(self, source, target) -> None:
        ...
        self.matched_type = next(
            (
                output
                for output in self.source_types
                if types_compatible([output], self.target_reqs)
            ),
            None,
        )
        no_matched_type = self.matched_type is None
