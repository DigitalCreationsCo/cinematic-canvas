...
from px.interface.utils import extract_input_variables_from_prompt
from px.log.logger import logger
from px.schema.data import Data
from px.schema.message import Message

# Database imports removed - px should be lightweight
...
async def emit_vertex_build_event(
    *,
    ...
    try:
        ...
        from portals.services.event_manager import webhook_event_manager
        ...
        pass  # portals not available (standalone px usage)
...
async def log_transaction(
    ...
    Uses the pluggable TransactionService to log transactions. When running within portals,
    the concrete TransactionService implementation persists to the database.
    When running standalone (px only), transactions are not persisted.
    ...
    try:
        ...
        from px.services.deps import get_transaction_service
...
async def log_vertex_build(
    *,
    ...
    When running within portals, it will use portals's database service to persist the build.
    When running standalone (px only), it will only log debug messages.
    """
    try:
        # Try to use portals's services if available (when running within portals)
        try:
            from portals.services.deps import get_db_service as portals_get_db_service
            from portals.services.deps import get_settings_service as portals_get_settings_service

            settings_service = portals_get_settings_service()
            ...
            from portals.services.database.models.vertex_builds.model import VertexBuildBase
            ...
            db_service = portals_get_db_service()
            ...
            # Fallback for standalone px usage (without portals)
            ...
            # Log basic vertex build info - concrete implementation is in portals
...
def has_chat_output(vertices: dict[Vertex, int]):
    from px.graph.schema import InterfaceComponentTypes
