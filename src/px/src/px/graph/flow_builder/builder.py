...
from px.graph.flow_builder.component import add_component, configure_component
from px.graph.flow_builder.connect import add_connection
from px.graph.flow_builder.flow import empty_flow
from px.graph.flow_builder.layout import layout_flow
from px.graph.flow_builder.spec import parse_flow_spec, validate_spec_references
from px.log.logger import logger

_INDEX_PATH = (
    Path(__file__).resolve().parent.parent.parent / "_assets" / "component_index.json"
)
...
def load_local_registry() -> dict[str, dict]:
    ...
    try:
        ...
        msg = f"Component registry not found at {_INDEX_PATH}. The px package may be installed incorrectly."
...
def build_flow_from_spec(spec: str) -> dict[str, Any]:
    ...
    for node in parsed["nodes"]:
        try:
            ...
            return {
                "error": f"Failed to add component '{node['type']}' (node '{node['id']}')",
                "details": str(e),
            }
    ...
    for edge in parsed["edges"]:
        ...
        try:
            ...
            return {
                "error": f"Failed to connect {src_out} -> {tgt_in}",
                "details": str(e),
            }
