...
- Concrete ``*PayloadFields`` classes (for example deployment payload fields)
  define canonical slot names in px so both layers share one structure.
- Adapter integrations populate adapter-side registries in px
  (``*PayloadSchemas`` subclasses).
- Portals integrations populate API-side registries in Portals
  (for example ``DeploymentApiPayloads``), including Portals-specific
