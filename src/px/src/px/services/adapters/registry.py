...
class AdapterRegistry(Generic[T]):
    ...
    def discover(self, config_dir: Path) -> None:
        ...
        Args:
            config_dir: Directory to search for ``px.toml`` / ``pyproject.toml``.
        """
        with self._lock:
            if self._discovered:
                logger.debug(
    ...
    def _discover_from_config(self, *, config_dir: Path) -> None:
        source = get_preferred_config_source(
            config_dir,
            px_root_path=self._config_section_path,
            pyproject_root_path=("tool", "px", *self._config_section_path),
        )
        if source is None:
            return
    ...
    def _load_config_file(self, *, config_path: Path, root_path: tuple[str, ...]) -> None:
        ...
        if config_path.name == "px.toml" or section:
...
def get_adapter_registry(
    ...
    * ``entry_point_group`` → ``"px.<adapter_type>.adapters"``
