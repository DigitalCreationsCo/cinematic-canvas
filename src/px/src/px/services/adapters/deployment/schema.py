...
from px.services.adapters.deployment.payloads import (
    T_ConfigItemData,
    T_ConfigListParams,
    T_ConfigListResult,
    T_DeploymentConfig,
...
from px.services.adapters.payload import AdapterPayload
...
class DeploymentType(str, Enum):
    """Core deployment types recognized by PX contracts.

    **Cross-package coupling** — portals's ``Deployment`` database model
    (``portals.services.database.models.deployment.model``) persists this
...
class BaseFlowArtifact(BaseModel, Generic[T_FlowProviderData]):
    ...
    model_config = ConfigDict(
        extra="allow"
    )  # e.g., viewport - good for viewing the flow in the UI
...
class SnapshotItems(BaseModel, Generic[T_FlowProviderData]):
    ...
    raw_payloads: (
        Annotated[list[BaseFlowArtifact[T_FlowProviderData]], Field(min_length=1)]
        | None
    ) = Field(
        None,
...
class SnapshotDeploymentBindingUpdate(BaseModel, Generic[T_FlowProviderData]):
    ...
    def validate_operations(self):
        ...
        # add_raw_payloads carry flow-artifact IDs (Portals domain),
...
class ConfigListItem(BaseModel, Generic[T_ConfigItemData]):
    ...
    created_at: datetime.datetime | None = Field(
        None, description="The created timestamp of the config item"
    )
    updated_at: datetime.datetime | None = Field(
        None, description="The last updated timestamp of the config item"
    )
    provider_data: T_ConfigItemData | None = Field(
        None, description="Provider-specific data for the config item"
    )
...
class ProviderDataModel(BaseModel, Generic[T_ProviderData]):
    ...
    provider_data: T_ProviderData | None = Field(
        None, description="The data from the provider"
    )
...
class ProviderResultModel(BaseModel, Generic[T_ProviderResult]):
    ...
    provider_result: T_ProviderResult | None = Field(
        None, description="The result from the provider"
    )
...
class ItemResult(ProviderDataModel[T_DeploymentItemData]):
    ...
    type: DeploymentType = Field(description="The type of the deployment")
    created_at: datetime.datetime | None = Field(
        None, description="The created timestamp of the deployment"
    )
    updated_at: datetime.datetime | None = Field(
        None, description="The last updated timestamp of the deployment"
    )
...
class ConfigListResult(
    ...
    configs: list[ConfigListItem[T_ConfigItemData]] = Field(
...
class SnapshotListResult(
    ...
    snapshots: list[SnapshotItem[T_SnapshotItemData]] = Field(
...
class _BaseListParams(BaseModel, Generic[T_ListParamsPayload]):
    ...
    @staticmethod
    def _normalize_filter_id_values(
        value: list[IdLike] | None, *, field_name: str
    ) -> list[str] | None:
        if value is None:
            return None
...
class DeploymentListParams(_BaseListParams[T_DeploymentListParams]):
    ...
    @classmethod
    def validate_deployment_types(
        cls, value: list[DeploymentType] | None
    ) -> list[DeploymentType] | None:
        if value is None:
            return None
        # Keep first occurrence order while removing duplicates.
    ...
    @classmethod
    def validate_entity_filter_ids(
        cls, value: list[IdLike] | None, info
    ) -> list[str] | None:
        return cls._normalize_filter_id_values(value, field_name=info.field_name)
...
class DeploymentCreate(BaseModel):
    ...
    snapshot: SnapshotItems | None = Field(
        None, description="The snapshots of the deployment"
    )
...
class DeploymentUpdate(BaseModel):
    ...
    spec: BaseDeploymentDataUpdate | None = Field(
        None, description="The metadata of the deployment"
    )
    snapshot: SnapshotDeploymentBindingUpdate | None = Field(
        None, description="The snapshot of the deployment"
    )
    config: ConfigDeploymentBindingUpdate | None = Field(
        None, description="The config of the deployment"
    )
...
class ExecutionCreate(BaseModel):
    ...
    deployment_id: IdLike = Field(
        description="The id of the deployment to create an execution for."
    )
...
class ExecutionResultBase(ProviderResultModel[T_ExecutionResult]):
    ...
    deployment_id: IdLike = Field(
        description="The id of the deployment that was executed."
    )
...
class DeploymentListTypesResult(ProviderResultModel[AdapterPayload]):
    ...
    deployment_types: list[DeploymentType] = Field(
...
def _normalize_and_validate_id_list(values: list[str], *, field_name: str) -> list[str]:
    ...
    return [
        _normalize_and_validate_id(value, field_name=field_name) for value in values
    ]


def _normalize_and_dedupe_id_list(
    values: list[IdLike], *, field_name: str
) -> list[str]:
