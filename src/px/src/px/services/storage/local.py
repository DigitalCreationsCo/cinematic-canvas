"""Local file-based storage service for px package."""
...
class LocalStorageService(StorageService, Service):
    ...
    async def _validated_path(self, flow_id: str, file_name: str) -> anyio.Path:
        ...
        try:
            ...
            await logger.aerror(
                f"Error validating file path for flow_id='{flow_id}': {type(e).__name__}"
            )
    ...
    def parse_file_path(self, full_path: str) -> tuple[str, str]:
        ...
        if full_path.startswith(data_dir_str):
            ...
            path_without_prefix = (
                full_path[len(data_dir_str) :].lstrip("/").lstrip("\\")
            )
    ...
    async def save_file(
        ...
        try:
            ...
            await logger.ainfo(
                f"File {file_name} {action} successfully in flow {flow_id}."
            )
        except Exception:
    ...
    async def delete_file(self, flow_id: str, file_name: str) -> None:
        ...
        if await file_path.exists():
            ...
            await logger.ainfo(
                f"File {file_name} deleted successfully from flow {flow_id}."
            )
        else:
    ...
    async def get_file_size(self, flow_id: str, file_name: str) -> int:
        ...
        try:
            ...
            logger.exception(
                f"Error getting size of file {file_name} in flow {flow_id}"
            )
            raise
        else:
