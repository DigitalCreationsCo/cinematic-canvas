"""Service dependency functions for px package."""
...
from px.services.config_discovery import resolve_config_dir
...
if TYPE_CHECKING:
    ...
    from px.services.adapters.registry import AdapterRegistry
    from px.services.interfaces import (
        AuthServiceProtocol,
...
def get_shared_component_cache_service() -> CacheServiceProtocol | None:
    ...
    from px.services.shared_component_cache.factory import (
...
def get_auth_service() -> AuthServiceProtocol | None:
    ...
    Returns the pluggable auth service (minimal PX or full Portals when configured).
    """
    from px.services.schema import ServiceType
...
def _get_deployment_registry() -> AdapterRegistry[DeploymentServiceProtocol]:
    ...
    """
    from px.services.adapters.registry import get_adapter_registry
...
def get_deployment_adapter(
    ...
    if instance is None:
        logger.warning(
            ...
            f"Check your px.toml or adapter registration."
        )
    return instance
