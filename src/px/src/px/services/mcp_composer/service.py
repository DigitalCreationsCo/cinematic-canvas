...
class MCPComposerService(Service):
    ...
    async def _ensure_port_available(self, port: int, current_project_id: str) -> None:
        ...
        if not is_port_available:
            ...
            if port_owner_project == current_project_id:
                ...
                await logger.aerror(
                    f"Port {port} is in use by an unknown process (not owned by Portals). "
    ...
    def _normalize_oauth_callback_aliases(cls, auth_config: dict[str, Any] | None) -> dict[str, Any]:
        ...
        `oauth_callback_url` is the canonical field name in Portals. For compatibility,
    ...
    async def start_project_composer(
        ...
        *,
        ...
        Args:
            project_id: The project ID
            streamable_http_url: Streamable HTTP endpoint for the remote Portals MCP server
