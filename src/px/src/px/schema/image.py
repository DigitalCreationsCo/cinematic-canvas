...
def get_file_paths(files: list[str | dict]):
    ...
    for file in files:
        ...
        file_paths.append(
            storage_service.build_full_path(flow_id=flow_id, file_name=file_name)
        )
    return file_paths
...
async def get_files(
    ...
    *,
    ...
    for file in file_paths:
        ...
        try:
            file_object = await storage_service.get_file(
                flow_id=flow_id, file_name=file_name
            )
            if convert_to_base64:
