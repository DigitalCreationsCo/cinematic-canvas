"""Lightweight JSON class for px package - contains only methods with no portals dependencies.

This module provides the JSON class (formerly Data) as the base type for Portals data structures.
...
from px.utils.constants import MESSAGE_SENDER_AI, MESSAGE_SENDER_USER
...
class JSON(CrossModuleModel):
    ...
    This is the base type for Portals data structures, replacing the legacy Data class.
    Data is maintained as an alias for backwards compatibility.
    ...
    def validate_data(cls, values):
        ...
        if not isinstance(values["data"], dict):
            msg = (
                ...
                " This will raise an error in version portals==1.3.0."
            )
            logger.warning(msg)
        # Any other keyword should be added to the data dictionary
        for key in values:
            if key not in values["data"] and key not in {
                "text_key",
                "data",
                "default_value",
            }:
    ...
    def serialize_model(self):
        return {
            k: v.to_json() if hasattr(v, "to_json") else v for k, v in self.data.items()
        }
    ...
    def to_lc_message(
        ...
        if sender == MESSAGE_SENDER_USER:
            if files:
                from px.schema.image import get_file_paths

                resolved_file_paths = get_file_paths(files)
                contents = [
                    create_image_content_dict(file_path)
    ...
    def filter_data(self, filter_str: str) -> JSON:
        ...
        """
        from px.template.utils import apply_json_filter

        return apply_json_filter(self.data, filter_str)

    def to_message(self) -> Message:
        from px.schema.message import Message  # Local import to avoid circular import
    ...
    def to_dataframe(self) -> Table:
        from px.schema.dataframe import (
            DataFrame,  # Local import to avoid circular import
        )
