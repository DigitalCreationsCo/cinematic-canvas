...
def create_input_schema_from_json_schema(schema: dict[str, Any]) -> type[BaseModel]:
    ...
    def resolve_ref(s: dict[str, Any] | None) -> dict[str, Any]:
        ...
        while "$ref" in s:
            ...
            if ref_name in visited:
                logger.warning(
                    "Parsing input schema: Circular $ref detected for '%s', treating as string",
                    ref_name,
                )
                return {"type": "string"}
            ...
            if s is None:
                logger.warning(
                    "Parsing input schema: Definition '%s' not found", ref_name
                )
                return {"type": "string"}
        return s
    ...
    def parse_type(s: dict[str, Any] | None) -> Any:
        ...
        if "anyOf" in s:
            ...
            non_null_types = [
                t for t in subtypes if t is not None and t is not type(None)
            ]
            if non_null_types:
    ...
    def _build_model(name: str, subschema: dict[str, Any]) -> type[BaseModel]:
        ...
        if "$ref" in subschema:
            ...
            if refname in building:
                logger.warning(
                    "Parsing input schema: Self-referential $ref '%s' detected, treating as dict",
                    refname,
                )
        ...
        if name in building:
            logger.warning(
                "Parsing input schema: Self-referential model '%s' detected, treating as dict",
                name,
            )
            return dict  # type: ignore[return-value]
        ...
        try:
            ...
            extra_mode = (
                "ignore" if subschema.get("additionalProperties") is False else "allow"
            )
            model_cls = create_model(
                name, __config__=ConfigDict(extra=extra_mode), **fields
            )
        finally:
    ...
    top_extra_mode = (
        "ignore" if schema.get("additionalProperties") is False else "allow"
    )
    return create_model(
        "InputSchema", __config__=ConfigDict(extra=top_extra_mode), **top_fields
    )
