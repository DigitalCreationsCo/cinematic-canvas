"""Logging configuration for Portals using structlog."""
...
class SizedLogBuffer:
    ...
    def __init__(
        ...
        The buffer can be overwritten by an env variable PORTALS_LOG_RETRIEVER_BUFFER_SIZE
    ...
    def max(self) -> int:
        ...
        if self._max == 0:
            env_buffer_size = os.getenv("PORTALS_LOG_RETRIEVER_BUFFER_SIZE", "0")
...
def remove_exception_in_production(
    _logger: Any, _method_name: str, event_dict: dict[str, Any]
) -> dict[str, Any]:
...
def configure(
    *,
    ...
    if (
        os.getenv("PORTALS_LOG_LEVEL", "").upper() in VALID_LOG_LEVELS
        and log_level is None
    ):
        log_level = os.getenv("PORTALS_LOG_LEVEL")

    log_level_str = os.getenv("PORTALS_LOG_LEVEL", "ERROR")
    ...
    if log_file is None:
        env_log_file = os.getenv("PORTALS_LOG_FILE", "")
        log_file = Path(env_log_file) if env_log_file else None

    if log_env is None:
        log_env = os.getenv("PORTALS_LOG_ENV", "")

    # Get log format from env if not provided
    if log_format is None:
        log_format = os.getenv("PORTALS_LOG_FORMAT")
    ...
    # Add callsite information only when PORTALS_DEV is set
    if DEV:
        processors.append(
    ...
    if log_env.lower() == "container" or log_env.lower() == "container_json":
        ...
        processors.append(
            structlog.processors.KeyValueRenderer(
                key_order=key_order, drop_missing=True
            )
        )
    else:
        # Use rich console for pretty printing based on environment variable
        log_stdout_pretty = os.getenv("PORTALS_PRETTY_LOGS", "true").lower() == "true"
    ...
    if log_file:
        if not log_file.parent.exists():
            ...
            log_file = cache_dir / "portals.log"
