...
class SmartRouterComponent(Component):
    ...
    def update_outputs(
        ...
        if field_name in {"routes", "enable_else_output", "model"}:
            ...
            routes_data = (
                field_value if field_name == "routes" else getattr(self, "routes", [])
            )
    ...
    def _get_categorization(self) -> str:
        ...
        if custom_prompt and custom_prompt.strip():
            ...
            simple_routes = ", ".join(
                [
                    f'"{cat.get("route_category", f"Category {i + 1}")}"'
    ...
    def process_case(self) -> Message:
        ...
        for i, category in enumerate(categories):
            ...
            if categorization.lower() == route_category.lower():
                matched_category = i
                self.status = (
                    f"MATCH FOUND! Category {i + 1} matched with '{categorization}'"
                )
                break
        ...
        if matched_category is not None:
            ...
            if (
                override_output
                and isinstance(override_output, str)
                and override_output.strip()
            ):
            ...
            if (
                custom_output
                and str(custom_output).strip()
                and str(custom_output).strip().lower() != "none"
            ):
