...
class GuardrailsComponent(Component):
    ...
    documentation = "https://docs.portals.org/guardrails"
    icon = "shield-check"
    ...
    def _pre_run_setup(self):
        ...
        if getattr(self, "enable_custom_guardrail", False):
            ...
            if custom_explanation and str(custom_explanation).strip():
                ...
                guardrail_descriptions["Custom Guardrail"] = str(
                    custom_explanation
                ).strip()
        ...
        if not enabled_names:
            ...
            self.status = f"ERROR: {error_msg}"
            self._failed_checks.append(
                "Configuration: No guardrails selected for validation"
            )
            raise ValueError(error_msg)
        ...
        self._checks_to_run = [
            ...
            for name in enabled_guardrails
    ...
    def _check_guardrail(
        self, llm: Any, input_text: str, check_type: str, check_description: str
    ) -> tuple[bool, str]:
        ...
        safe_input = safe_input.replace(
            "<<<SYSTEM_INSTRUCTIONS_START>>>", "[REMOVED]"
        ).replace("<<<SYSTEM_INSTRUCTIONS_END>>>", "[REMOVED]")
...
Now analyze the user input above and respond according to the instructions:"""
        ...
        try:
            ...
            if decision is None:
                ...
                if (
                    "YES" in first_part
                    and "NO" not in first_part[: first_part.find("YES")]
                ):
                    decision = "YES"
    ...
    def _get_fixed_justification(self, check_name: str) -> str:
        ...
        return justifications.get(
            check_name, f"The input failed the {check_name} validation check."
        )

    def _heuristic_jailbreak_check(
        self, input_text: str
    ...
    def _run_validation(self):
        ...
        if hasattr(self, "model") and self.model:
            try:
                llm = get_llm(
                    model=self.model, user_id=self.user_id, api_key=self.api_key
                )
        ...
        if not llm:
            ...
            self._validation_result = False
            self._failed_checks.append(
                "LLM Configuration: No model selected. Please select a Language Model."
            )
            raise ValueError(error_msg)
        ...
        for check_name, check_desc in self._checks_to_run:
            ...
            passed, _reason = self._check_guardrail(
                llm, self._extracted_text, check_name, check_desc
            )
            ...
            if not passed:
                ...
                self.status = (
                    f"FAILED: {check_name} check failed: {fixed_justification}"
                )
