...
from px.base.models.model_utils import get_model_name
...
from px.schema.token_usage import accumulate_usage, extract_usage_from_message
...
class LLMSelectorComponent(Component):
    ...
    documentation: str = "https://docs.portals.org/llm-selector"
    icon = "git-branch"
    ...
    async def _fetch_openrouter_models_data(self) -> None:
        ...
        try:
            ...
            self.log(
                "Fetching all model specifications from OpenRouter API: https://openrouter.ai/api/v1/models"
            )
            async with (
                ...
                if response.status == http.HTTPStatus.OK:
                    ...
                    for model_data in models_list:
                        ...
                        if api_model_name:
                            ...
                            _model_name_to_api_id_temp[simplified_api_name] = (
                                api_model_id
                            )
                        ...
                        if "/" in api_model_id:
                            try:
                                ...
                                if model_name_part_of_id:
                                    ...
                                    _model_name_to_api_id_temp[simplified_part_id] = (
                                        api_model_id
                                    )
                            except IndexError:
                    ...
                    self.log(
                        f"Failed to fetch OpenRouter models: HTTP {response.status} - {err_text}"
                    )
    ...
    def _get_api_model_id_for_portals_model(
        self, portals_model_name: str
    ) -> str | None:
        """Attempt to find the OpenRouter API ID for a given Portals model name."""
        if not portals_model_name:
            return None

        potential_names_to_check = [
            portals_model_name,
            self._simplify_model_name(portals_model_name),
        ]

        if portals_model_name.startswith("models/"):
            name_without_prefix = portals_model_name[len("models/") :]
        ...
        elif portals_model_name.startswith("community_models/"):
            name_without_prefix = portals_model_name[len("community_models/") :]
            potential_names_to_check.append(name_without_prefix)
        ...
        self.log(
            f"Could not map Portals model name '{portals_model_name}' "
            f"(tried variants: {unique_names_to_check}) to an OpenRouter API ID."
        )
        return None

    def _get_model_specs_dict(self, portals_model_name: str) -> dict[str, Any]:
        """Get a dictionary of relevant model specifications for a given Portals model name."""
        if not self.use_openrouter_specs or not self._models_api_cache:
            return {
                "id": portals_model_name,
                "name": portals_model_name,
                "description": "Specifications not available.",
            }

        api_model_id = self._get_api_model_id_for_portals_model(portals_model_name)

        if not api_model_id or api_model_id not in self._models_api_cache:
            log_msg = (
                f"No cached API data found for Portals model '{portals_model_name}' "
        ...
        specs = {
            ...
            "max_completion_tokens": (
                top_provider_data.get("max_completion_tokens")
...
If no model seems suitable, pick the first model in the list (index 0) as a fallback."""
    ...
    async def route_to_model(self) -> Message:
        ...
        try:
            self.log(
                f"Starting model routing with {len(self.models)} available Portals models."
            )
            ...
            for i, portals_model_instance in enumerate(self.models):
                portals_model_name = get_model_name(portals_model_instance)
                if not portals_model_name:
                    self.log(
                        f"Warning: Could not determine name for model at index {i}. Using placeholder.",
                        "warning",
                    )
                    spec_dict = {
                    ...
                    spec_dict = self._get_model_specs_dict(portals_model_name)

                model_specs_for_judge.append(
                    {
                        "index": i,
                        "portals_name": portals_model_name,
                        "specs": spec_dict,
                    }
                )
                self.log(
                    f"Prepared specs for Portals model {i} ('{portals_model_name}'): {spec_dict.get('name', 'N/A')}"
                )
...
Return ONLY the index number:"""
            ...
            self._token_usage = accumulate_usage(
            ...
            if self._selected_model_name:
                self._selected_api_model_id = (
                    self._get_api_model_id_for_portals_model(self._selected_model_name)
                    or self._selected_model_name
                )
            else:
            ...
            specs_source = (
                ...
                else "Basic (Portals model names only)"
            )
            self._routing_decision = f"""Model Selection Decision:
- Selected Model Index: {selected_index}
- Selected Portals Model Name: {self._selected_model_name}
...
- Specifications Source: {specs_source}"""
            ...
            log_msg = (
                ...
                f"Portals Name: '{self._selected_model_name}', API ID: '{self._selected_api_model_id}'"
            )
            self.log(log_msg)
            ...
            raw_result = get_chat_result(
                ...
                token_usage_callback=lambda msg: setattr(
                    ...
                    "_token_usage",
                    accumulate_usage(
                        self._token_usage, extract_usage_from_message(msg)
                    ),
                ),
            )
            result = (
                Message(text=str(raw_result))
            ...
            self.log(
                "Detailed routing error occurred. Check logs for details.", "error"
            )
            self.status = error_msg
            ...
            if self.fallback_to_first and self.models:
                ...
                self._selected_model_name = get_model_name(chosen_model_instance)
                if self._selected_model_name:
                    mapped_id = self._get_api_model_id_for_portals_model(
...
- Fallback Model Portals Name: {self._selected_model_name}
...
- Reason: Automatic fallback enabled"""
                ...
                self.log(
                    "No fallback model available or fallback disabled. Raising error.",
                    "error",
                )
                raise
    ...
    def _parse_judge_response(self, response_content: str) -> tuple[int, Any]:
        ...
        try:
            ...
            self.log(
                f"Error parsing judge response '{response_content}': {e!s}. Defaulting to index 0.",
                "error",
            )
            return 0, self.models[0]
    ...
    def get_selected_model_info(self) -> list[Data]:
        ...
        if self._selected_model_name:
            ...
            if "portals_name" not in specs_dict:
                specs_dict["portals_model_name_used_for_lookup"] = (
                    self._selected_model_name
                )
            if (
                self._selected_api_model_id
                and specs_dict.get("id") != self._selected_api_model_id
            ):
        ...
        data_output = [
            Data(data={"info": "No model selected yet - run the router first."})
        ]
        self.status = data_output
        return data_output
    ...
    def get_routing_decision(self) -> Message:
        ...
        message_output = Message(
            text="No routing decision made yet - run the router first."
        )
        self.status = message_output
        return message_output
