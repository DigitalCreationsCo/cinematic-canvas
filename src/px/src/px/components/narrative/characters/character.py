"""Character component — reads character data from DB and generates persona-driven LLM responses.

Outputs
-------
character_data : Data
    The selected character's database record, returned as structured Data for downstream
    narrative components. Cached per entity name within a single component execution.

character_response : Message
    A persona-driven LLM response. The component constructs a system prompt from the
    character's profile (name, physical traits, narrative state, aliases) and invokes
    the connected language model with the user's input message.

Caching
-------
Database records are cached in an instance-level dict keyed by entity name. If both
``character_data`` and ``character_response`` outputs are connected for the same
character, only one database read occurs.
"""
