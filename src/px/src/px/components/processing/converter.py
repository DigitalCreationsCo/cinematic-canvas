...
from px.io import BoolInput, HandleInput, Output, TabInput
from px.schema import Data, DataFrame, Message
from px.schema.data import JSON
...
def convert_to_dataframe(
    v: Table | Data | Message | dict, *, auto_parse: bool
) -> Table:
    ...
    if isinstance(v, Message):
        data = Data(data={"text": v.data["text"]})
        return (
            parse_structured_data(data).to_dataframe()
            if auto_parse
            else data.to_dataframe()
        )
...
class TypeConverterComponent(Component):
    ...
    documentation: str = "https://docs.portals.org/type-convert"
    icon = "repeat"
    ...
    async def update_frontend_node(
        ...
        output_type = (
            new_frontend_node.get("template", {})
            .get("output_type", {})
            .get("value", "Message")
        )
