...
from portals.services.auth.utils import decrypt_api_key, encrypt_api_key
...
from px.base.models.unified_models import get_embedding_model_options, get_embeddings
...
class KnowledgeIngestionComponent(Component):
    """Create or append to Portals Knowledge from a DataFrame."""

    # ------ UI metadata ---------------------------------------------------
    display_name = "Knowledge Ingestion"
    description = "Create or update knowledge in Portals."
    icon = "upload"
            ...
            default_factory=lambda: {
                "data": {
                    "node": {
                        ...
                        "description": "Create new knowledge in Portals.",
                        ...
                        "template": {
                            ...
                            "02_embedding_model": ModelInput(
                                ...
                                info=(
                                    ...
                                    "Portals uses the configured credentials for that model provider."
                                ),
                                required=True,
    ...
    def _build_embedding_metadata(
        ...
        """
        model_dict = (
            model_selection[0] if isinstance(model_selection, list) else model_selection
        )
    ...
    def _update_metadata_metrics(self, kb_path: Path, chroma: Chroma) -> None:
        ...
        from portals.api.utils.kb_helpers import KBAnalysisHelper, KBStorageHelper
    ...
    async def _convert_df_to_data_objects(
        ...
        for _, row in df_source.iterrows():
            ...
            identifier_parts = [
                ...
                if col in row and self._scalar_notna(row[col])
            ]
            ...
            for col in df_source.columns:
                if (
                    col not in content_cols
                    and col in row
                    and self._scalar_notna(row[col])
                ):
        ...
        try:
            ...
            if metadata_path.exists():
                ...
                if model_selection:
                    ...
                    embedding_provider = stored_metadata.get(
                        ...
                        try:
                            ...
                            match = next(
                                (
                                    ...
                                    if o.get("name") == embedding_model_name
                                ),
                                None,
                            )
                            if match:
            ...
            # Create vector store following Local DB component pattern
            chroma = await self._create_vector_store(
