...
from px.inputs.inputs import MultilineInput, SecretStrInput, StrInput
...
class AddContentToPage(LCToolComponent):
    ...
    def _add_content_to_page(
        self, markdown_text: str, block_id: str
    ) -> dict[str, Any] | str:
        try:
    ...
    def process_node(self, node):
        ...
        if isinstance(node, str):
            ...
            blocks.append(
                self.create_block(
                    "bookmark", node.get_text(strip=True), link_url=node.get("href")
                )
            )
    ...
    def create_block(self, block_type: str, content: str, **kwargs) -> dict[str, Any]:
        ...
        if block_type in {
            ...
            block[block_type] = {
                "type": "external",
                "external": {"url": kwargs.get("image_url", "")},
            }
