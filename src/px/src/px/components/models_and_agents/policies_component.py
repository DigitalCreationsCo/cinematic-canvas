...
from px.base.models import LCModelComponent
from px.base.models.unified_models import (
    get_language_model_options,
    get_llm,
    update_model_options_in_build_config,
)
from px.components.models_and_agents.policies.module_utils import unload_module
from px.field_typing import LanguageModel, Tool
from px.io import (
    BoolInput,
    HandleInput,
    ModelInput,
...
BUILDTIME_MODELS = [
    "gpt-5",
    "claude-sonnet",
]  # currently inactive, we recommend but do not enforce
STEP1 = "Step_1"
STEP2 = "Step_2"
...
_TOOLGUARD_INSTALL_HINT = (
    ...
    "Install the optional extra: `pip install 'portals-base[toolguard]'`."
)
...
class PoliciesComponent(LCModelComponent):
    ...
    `toolguard` is an optional extra (`portals-base[toolguard]`); imports happen
...
Powered by [ALTK ToolGuard](https://github.com/AgentToolkit/toolguard )"""
    ...
    def _import_toolguard():
        ...
        try:
            ...
            from px.components.models_and_agents.policies.guard_sync_utils import (
                sync_generated_guard_code_inputs,
            )
            from px.components.models_and_agents.policies.guarded_tool import (
                GuardedTool,
            )
            from px.components.models_and_agents.policies.llm_wrapper import (
    ...
    async def _generate_guard_code(
        self, specs: list[ToolGuardSpec]
    ) -> ToolGuardsCodeGenerationResult:
        tg = self._import_toolguard()
    ...
    def make_toolguard_result(self) -> ToolGuardsCodeGenerationResult:
        ...
        result.domain.app_types.content = attrs.get(
            str(result.domain.app_types.file_name)
        )["value"]
        result.domain.app_api.content = attrs.get(str(result.domain.app_api.file_name))[
            "value"
        ]
        result.domain.app_api_impl.content = attrs.get(
            str(result.domain.app_api_impl.file_name)
        )["value"]
    ...
    async def guard_tools(self) -> list[Tool]:
        if self.enabled:
            ...
            if mode == MODE_GENERATE:
                ...
                await self.generate()
                self.log(
                    f"Policies code generation saved to {self.work_dir}", name="info"
                )
                self.log(
                    "Review the generated files in the details panel on the right.",
                    name="info",
                )
                ...
                try:
                    ...
                    guarded_tools = [
                        tg["GuardedTool"](tool, self.in_tools, tg_runtime)
                        for tool in self.in_tools
                    ]
