...
from px.base.agents.events import ExceptionWithMessageError
...
from px.components.agentics.helpers.model_config import validate_model_selection
...
from px.custom.custom_component.component import get_component_toolkit
from px.field_typing.range_spec import RangeSpec
from px.helpers.base_model import build_model_from_schema
from px.inputs.inputs import BoolInput, DropdownInput, ModelInput, StrInput
from px.io import (
    IntInput,
    MessageTextInput,
    MultilineInput,
...
class AgentComponent(ToolCallingAgentComponent):
    ...
    documentation: str = "https://docs.portals.org/agents"
    icon = "bot"
    beta = False
    name = "Agent"
    ...
    def _resolve_selected_model(self):
        ...
        for option in options:
            if (
                option.get("provider") == legacy_provider
                and option.get("name") == legacy_model_name
            ):
                return [option]
    ...
    async def update_build_config(
        ...
        build_config = handle_model_input_update(
            ...
            get_options_func=lambda user_id=None: get_language_model_options(
                user_id=user_id, tool_calling=True
            ),
        )
