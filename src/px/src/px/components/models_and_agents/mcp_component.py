...
from px.base.agents.utils import maybe_unflatten_dict, safe_cache_get, safe_cache_set
from px.base.mcp.util import (
    MCPStdioClient,
    MCPStreamableHttpClient,
    update_tools,
)
from px.custom.custom_component.component_with_cache import ComponentWithCache
from px.inputs.inputs import InputTypes  # noqa: TC001
from px.io import (
    BoolInput,
    DictInput,
    DropdownInput,
...
from px.io.schema import schema_to_portals_inputs
from px.log.logger import logger
from px.schema.dataframe import DataFrame
from px.schema.message import Message
from px.services.deps import get_storage_service, session_scope
...
class MCPToolsComponent(ComponentWithCache):
    ...
    documentation: str = "https://docs.portals.org/mcp-tools"
    icon = "Mcp"
    name = "MCPTools"
    ...
    async def _validate_schema_inputs(self, tool_obj) -> list[InputTypes]:
        ...
        try:
            ...
            schema_inputs = schema_to_portals_inputs(input_schema)
    ...
    async def update_tool_list(self, mcp_server_value=None):
        ...
        mcp_server = (
            mcp_server_value
            if mcp_server_value is not None
            else getattr(self, "mcp_server", None)
        )
        server_name = None
        ...
        if cached is not None:
            try:
                ...
                if (
                    isinstance(current_servers_cache, dict)
                    and server_name in current_servers_cache
                ):
                    current_servers_cache.pop(server_name)
                    safe_cache_set(
                        self._shared_component_cache, "servers", current_servers_cache
                    )
            else:
                return self.tools, {
                    "name": server_name,
        ...
        try:
            ...
            try:
                from portals.api.v2.mcp import get_server
                ...
                msg = (
                    "Portals MCP server functionality is not available. "
                    "This feature requires the full Portals installation."
                )
            ...
            has_headers = (
                ...
                and len(server_config.get("headers", {})) > 0
            )
            if not request_variables and has_headers:
                try:
                    from px.services.deps import get_variable_service

                    variable_service = get_variable_service()
                    if variable_service:
                        async with session_scope() as db:
                            request_variables = (
                                await variable_service.get_all_decrypted_variables(
                                    user_id=self.user_id, session=db
                                )
                            )
                except Exception as e:  # noqa: BLE001
                    await logger.awarning(
                        f"Failed to load global variables for MCP component: {e}"
                    )
    ...
    async def update_build_config(
        ...
        try:
            if field_name == "tool":
                ...
                server_changed = bool(
                    _last_selected_server
                    and current_server_name != _last_selected_server
                )
                ...
                if not use_cache and current_server_name:
                    ...
                    if (
                        isinstance(servers_cache, dict)
                        and current_server_name in servers_cache
                    ):
                        servers_cache.pop(current_server_name)
                        safe_cache_set(
                            self._shared_component_cache, "servers", servers_cache
                        )
    ...
    def _should_include_tool_argument(cls, model_field: Any, value: Any) -> bool:
        ...
        return not (
            ...
            and cls._is_object_like_annotation(model_field.annotation)
        )
    ...
    def get_inputs_for_all_tools(self, tools: list) -> dict:
        ...
        for tool in tools:
            if not tool or not hasattr(tool, "name"):
                continue
            try:
                portals_inputs = schema_to_portals_inputs(tool.args_schema)
                inputs[tool.name] = portals_inputs
    ...
    async def build_output(self) -> DataFrame:
        ...
        try:
            ...
            if self.tool != "":
                # Set session context for persistent MCP sessions using Portals session ID
    ...
    def _get_session_context(self) -> str | None:
        """Get the Portals session ID for MCP session caching."""
