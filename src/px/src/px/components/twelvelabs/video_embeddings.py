...
from px.io import DropdownInput, IntInput, SecretStrInput
...
class TwelveLabsVideoEmbeddings(Embeddings):
    ...
    def embed_video(
        ...
        if (
            hasattr(result.video_embedding, "segments")
            and result.video_embedding.segments
        ):
...
class TwelveLabsVideoEmbeddingsComponent(LCEmbeddingsModel):
    ...
    inputs = [
        SecretStrInput(
            name="api_key", display_name="TwelveLabs API Key", required=True
        ),
        DropdownInput(
            name="model_name",
            display_name="Model",
