...
from px.inputs.inputs import BoolInput, MessageTextInput, SecretStrInput
...
class YouTubeChannelComponent(Component):
    ...
    outputs = [
        Output(
            name="channel_df", display_name="Channel Info", method="get_channel_info"
        ),
    ]
    ...
    def _get_channel_playlists(
        self, youtube: Any, channel_id: str
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for item in playlists_response.get("items", []):
                playlist_data = {
                    ...
                    "playlist_thumbnail_url": item["snippet"]["thumbnails"]["default"][
                        "url"
                    ],
                }
    ...
    def get_channel_info(self) -> DataFrame:
        ...
        try:
            ...
            channel_response = (
                youtube.channels().list(part=",".join(parts), id=channel_id).execute()
            )
                ...
                channel_data.update(
                    {
                        ...
                        "brand_description": [
                            branding.get("channel", {}).get("description", "")
                        ],
                        "brand_keywords": [
                            branding.get("channel", {}).get("keywords", "")
                        ],
                        "brand_banner_url": [
