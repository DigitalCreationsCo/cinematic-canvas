...
from px.io import DropdownInput, MessageTextInput, Output
...
class GitLoaderComponent(Component):
    ...
    def check_file_patterns(file_path: str | Path, patterns: str) -> bool:
        ...
        return any(
            fnmatch(path_str, pattern) or fnmatch(file_name, pattern)
    ...
    def build_combined_filter(
        ...
        def combined_filter(file_path: str) -> bool:
            try:
                ...
                if file_filter_patterns and not self.check_file_patterns(
                    path, file_filter_patterns
                ):
                    return False
                ...
                return not (
                    ...
                    and not self.check_content_pattern(path, content_filter_pattern)
                )
            except Exception:  # noqa: BLE001
                return False
    ...
    async def temp_clone_dir(self):
        ...
        temp_dir = None
        try:
            temp_dir = tempfile.mkdtemp(prefix="portals_clone_")
            yield temp_dir
        finally:
            if temp_dir:
