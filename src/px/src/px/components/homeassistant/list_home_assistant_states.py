...
from px.inputs.inputs import SecretStrInput, StrInput
...
class ListHomeAssistantStates(LCToolComponent):
    ...
    # 1) Define fields to be received in Portals UI
    inputs = [
        SecretStrInput(
            name="ha_token",
    ...
    def _make_data_response(self, result: list[Any] | str | dict) -> Data:
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                return Data(
                    data=result, text=json.dumps(result, indent=2, ensure_ascii=False)
                )
            ...
            return Data(
                data={}, text=f"Error: Failed to process response. Details: {e!s}"
            )
