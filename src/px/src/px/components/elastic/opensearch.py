...
class OpenSearchVectorStoreComponent(LCVectorStoreComponent):
    ...
    icon: str = "OpenSearch"
    description: str = "Store and search documents using OpenSearch with hybrid semantic and keyword search capabilities."
    ...
    def _add_documents_to_vector_store(self, client: OpenSearch) -> None:
        ...
        try:
            if not client.indices.exists(index=self.index_name):
                self.log(
                    f"Creating index '{self.index_name}' with KNN mapping (index.knn: true)"
                )
    ...
    def search_documents(self) -> list[Data]:
        ...
        configured search_query and returns results in Portals's Data format.
