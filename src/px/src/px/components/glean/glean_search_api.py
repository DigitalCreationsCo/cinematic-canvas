...
class GleanAPIWrapper(BaseModel):
    ...
    act_as: str = "portals-component@datastax.com"  # TODO: Detect this
...
class GleanSearchAPIComponent(LCToolComponent):
    ...
    documentation: str = "https://docs.portals.org/bundles-glean"
    icon: str = "Glean"

    outputs = [
        Output(
            display_name="Table", name="dataframe", method="fetch_content_dataframe"
        ),
    ]
    ...
    inputs = [
        ...
        SecretStrInput(
            name="glean_access_token", display_name="Glean Access Token", required=True
        ),
        MultilineInput(
            name="query", display_name="Query", required=True, tool_mode=True
        ),
        IntInput(name="page_size", display_name="Page Size", value=10),
        NestedDictInput(
            name="request_options", display_name="Request Options", required=False
        ),
    ]
    ...
    def fetch_content(self) -> list[Data]:
        ...
        # Build the data
        data = [
            Data(data=result, text=result["snippets"][0]["text"]) for result in results
        ]
