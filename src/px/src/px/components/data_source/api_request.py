...
from px.base.curl.parse import parse_context
from px.custom.custom_component.component import Component
from px.inputs.inputs import TabInput
from px.io import (
    BoolInput,
    DataInput,
    DropdownInput,
...
from px.utils.component_utils import (
    set_current_fields,
    set_field_advanced,
    set_field_display,
)
...
from px.utils.ssrf_protection import SSRFProtectionError, validate_and_resolve_url
from px.utils.ssrf_transport import create_ssrf_protected_client
...
class APIRequestComponent(Component):
    ...
    documentation: str = "https://docs.portals.org/api-request"
    icon = "Globe"
    name = "APIRequest"
    ...
    inputs = [
        ...
        TableInput(
            ...
            value=[{"key": "User-Agent", "value": "Portals/1.0"}],
            advanced=True,
            input_types=["Data", "JSON"],
            real_time_refresh=True,
        ),
        ...
        try:
            for item in body:
                ...
                if hasattr(item, "data"):
                    ...
                    if isinstance(
                        unwrapped_data, dict
                    ) and not self._is_valid_key_value_item(unwrapped_data):
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                and processed_body is not None
            ):
    ...
    async def make_api_request(self) -> Data:
        ...
        try:
            ...
            if validated_ips:
                self.log(
                    f"SSRF Protection: Using DNS pinning with {len(validated_ips)} validated IP(s)"
                )
        ...
        # ============================================================================
        from px.utils.ssrf_protection import is_ssrf_protection_enabled
