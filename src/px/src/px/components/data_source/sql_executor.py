...
from px.io import BoolInput, MessageTextInput, MultilineInput, Output
...
class SQLComponent(ComponentWithCache):
    ...
    documentation: str = "https://docs.portals.org/sql-database"
    icon = "database"
    name = "SQLComponent"
    ...
    inputs = [
        MessageTextInput(
            name="database_url", display_name="Database URL", required=True
        ),
        MultilineInput(
            name="query", display_name="SQL Query", tool_mode=True, required=True
        ),
        BoolInput(
    ...
    outputs = [
        Output(
            display_name="Result Table", name="run_sql_query", method="run_sql_query"
        ),
    ]
