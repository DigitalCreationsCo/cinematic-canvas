"""px export -- serialize flows to git-friendly JSON.
...
Remote (--env / --flow-id / --project-id)
    Pull flows directly from a running Portals instance using the
    ``portals-sdk`` HTTP client, normalize them, and write to disk.

Examples:
--------
Normalize a local file in-place::

    px export my_flow.json --in-place

Normalize and write to a new file::

    px export my_flow.json -o my_flow.normalized.json

Pull a single flow from staging and write to the current directory::

    px export --flow-id <uuid> --env staging

Export an entire project from staging into ./flows/::

    px export --project-id <uuid> --env staging --output-dir ./flows/
"""
...
from px.cli.common import load_sdk, safe_filename
...
def export_command(
    ...
    *,
    ...
    # ------------------------------------------------------------------
    # Remote mode: pull from a live Portals instance
    # ------------------------------------------------------------------
    if flow_id or project_id:
        if not env and not target:
            console.print(
                "[red]Error:[/red] --env or --target is required for remote export."
            )
            raise typer.Exit(1)
        ...
        if flow_id:
            ...
            normalized = sdk.normalize_flow(
                flow_obj.model_dump(mode="json"), **normalize_kwargs
            )
            ...
            out_path.write_text(
                sdk.flow_to_json(normalized, indent=indent), encoding="utf-8"
            )
        ...
        dest_dir = (
            Path(output_dir) if output_dir else Path.cwd() / safe_filename(project.name)
        )
        dest_dir.mkdir(parents=True, exist_ok=True)
    ...
    if not flow_paths:
        console.print(
            "[red]Error:[/red] Provide at least one flow JSON file, or use --flow-id / --project-id."
        )
        raise typer.Exit(1)

    if output and len(flow_paths) > 1:
        console.print(
            "[red]Error:[/red] --output can only be used with a single input file."
        )
        raise typer.Exit(1)
