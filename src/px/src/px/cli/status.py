"""px status -- compare local flow files against a remote Portals instance.
...
Examples::
    ...
    px status                          # scans flows/ in cwd, uses [defaults] env
    px status --env staging            # compare against staging
    px status --dir ./my-flows/        # specify a custom flows directory
    px status --env prod --remote-only # also show server flows not tracked locally
"""
...
def _load_sdk() -> tuple[object, object, object, type]:
    """Return (normalize_flow, flow_to_json, Client, PortalsNotFoundError) from portals_sdk."""
    sdk = load_sdk("status")
    from portals_sdk.exceptions import PortalsNotFoundError
    from portals_sdk.serialization import flow_to_json, normalize_flow

    return normalize_flow, flow_to_json, sdk.Client, PortalsNotFoundError
...
def status_command(
    ...
    *,
    ...
    try:
        ...
        console.print(
            f"[red]Error:[/red] Could not create client for {env_cfg.url!r}: {exc}"
        )
        raise typer.Exit(1) from exc
    ...
    try:
        ...
        if not local_files and not show_remote_only:
            console.print(
                "[yellow]No flow files found.[/yellow] Use [bold]--dir[/bold] to specify a directory."
            )
            raise typer.Exit(0)
        ...
        for path in local_files:
            ...
            try:
                ...
                statuses.append(
                    FlowStatus(
                        name=path.name, status=_STATUS_ERROR, path=path, detail=str(exc)
                    )
                )
                continue
            ...
            if not raw_id:
                statuses.append(
                    FlowStatus(
                        ...
                        path=path,
                        detail="run px export --env <env> first to assign a stable id",
                    )
                )
                continue
            ...
            try:
                ...
                statuses.append(
                    FlowStatus(
                        name=name, status=_STATUS_NEW, path=path, flow_id=flow_id
                    )
                )
                continue
            ...
            remote_hash = _flow_hash(
                remote_flow.model_dump(mode="json"), normalize_flow, flow_to_json
            )

            if local_hash == remote_hash:
                statuses.append(
                    FlowStatus(
                        name=name, status=_STATUS_SYNCED, path=path, flow_id=flow_id
                    )
                )
            else:
                ...
                statuses.append(
                    FlowStatus(name=name, status=status, path=path, flow_id=flow_id)
                )
        ...
        if show_remote_only:
            try:
                ...
                console.print(
                    f"[yellow]Warning:[/yellow] Could not list remote flows: {exc}"
                )
    finally:
