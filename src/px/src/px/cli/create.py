"""px create -- scaffold a new flow JSON from a built-in template.
...
Examples::
    ...
    px create my-chatbot
    px create my-rag --template hello-world
    px create my-flow --output-dir ./flows --overwrite
    px create --list
"""
...
# Descriptions shown in ``px create --list``
...
def _is_px_project() -> bool:
    """Return True if cwd (or a parent up to .git) contains .px/environments.yaml."""
    cwd = Path.cwd()
    for directory in (cwd, *cwd.parents):
        if (directory / ".px" / "environments.yaml").is_file():
            return True
        if (directory / ".px" / "environments.yml").is_file():
            return True
...
def create_command(
    ...
    *,
    ...
    """
    if not _is_px_project():
        console.print(
            "[yellow]Warning:[/yellow] No .px/environments.yaml found in this project. "
            "Run [bold]px init[/bold] first to scaffold a project."
        )

    available = list_templates()
    if not available:
        console.print("[red]Error:[/red] No flow templates found in the px package.")
        raise typer.Exit(1)
    ...
    console.print(f"  [bold]px validate {dest}[/bold]")
    console.print(f"  [bold]px serve {dest}[/bold]")
