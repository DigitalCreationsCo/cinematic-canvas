...
from px.cli.serve_app import FlowMeta, create_multi_serve_app
...
async def serve_command(
    ...
    *,
    ...
    Examples:
        ...
        # Serve inline JSON
        px serve --flow-json '{"nodes": [...], "edges": [...]}'

        # Serve from stdin
        cat my_flow.json | px serve --stdin
        echo '{"nodes": [...]}' | px serve --stdin
    """
    ...
    os.environ["PORTALS_PRETTY_LOGS"] = "false"
    ...
    try:
        ...
        console.print(
            Panel.fit(
                ...
                title="[bold blue]PX Server[/bold blue]",
                border_style="blue",
            )
        )
        console.print()
