...
def register(app: typer.Typer) -> None:
    ...
    @app.command(
        name="create",
        help="Create a new flow JSON from a built-in template",
        rich_help_panel="Authoring",
    )
    def create_command_wrapper(
        name: str = typer.Argument(
            help="Display name for the new flow (also used as the filename)."
        ),
        ...
        *,
        ...
        """Scaffold a new Portals flow JSON from a built-in template (lazy-loaded)."""
        from pathlib import Path

        from px.cli.create import create_command, print_templates
    ...
    def requirements_command_wrapper(
        flow_path: str = typer.Argument(help="Path to the Portals flow JSON file"),
        ...
        px_package: str = typer.Option(
            "px",
            "--px-package",
            help="Name of the PX package (default: px)",
        ),
        *,
        no_px: bool = typer.Option(
            False,
            "--no-px",
            help="Exclude the PX package from output",
        ),
        ...
        """Generate requirements.txt from a Portals flow JSON (lazy-loaded)."""
        import json
        from pathlib import Path

        from px.utils.flow_requirements import generate_requirements_txt
    ...
    @app.command(
        name="validate",
        help="Validate one or more flow JSON files",
        rich_help_panel="Authoring",
    )
    def validate_command_wrapper(
        flow_paths: list[str] = typer.Argument(
            default=None,
            help="Path(s) to Portals flow JSON file(s) or directories to validate. Defaults to flows/.",
        ),
        ...
        """Validate Portals flow JSON files without executing them (lazy-loaded)."""
        from px.cli.validate import validate_command
