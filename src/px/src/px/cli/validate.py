"""px validate -- structural and semantic validation of Portals flow JSON.
...
Validation levels (each level implies all levels below it):
    ...
    Level 1 - structural
        ...
        (nodes built with a different Portals version than the one installed).

    Level 2 - components
        Every node's ``data.type`` references a component type that exists in
        the px component registry.
...
``px.cli.validation`` subpackage so that all existing imports continue
to work unchanged.
"""
...
# This keeps ``from px.cli.validate import ...`` working for all consumers
# (the CLI entry point in __main__.py, tests, etc.).
from px.cli.validation import (  # noqa: F401
    ValidationIssue,
    ValidationResult,
...
from px.cli.validation.core import (  # noqa: F401
    _LEVEL_COMPONENTS,
    _LEVEL_EDGE_TYPES,
