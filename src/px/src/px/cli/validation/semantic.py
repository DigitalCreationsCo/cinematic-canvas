"""Semantic validation checks for Portals flow JSON.
...
# ---------------------------------------------------------------------------
# Level 2 - component existence (loads px component registry)
# ---------------------------------------------------------------------------


def _check_component_existence(flow: dict[str, Any], result: ValidationResult) -> None:
    try:
        from px.interface.utils import (
            initialize_components,  # type: ignore[import-untyped]
        )
        ...
        if component_type not in component_registry:
            result.issues.append(
                _make_issue(
                    ...
                    message=(
                        f"Unknown component type '{component_type}'. This component may be missing or outdated."
                    ),
                )
            )
        ...
        if not src_node or not tgt_node:
            result.issues.append(
                _make_issue(
                    ...
                    node_name=None,
                    message=(
                        f"Edge references non-existent node(s): source={src_id!r}, target={tgt_id!r}"
                    ),
                )
            )
            continue
    ...
    *and* the field has no incoming edge, a warning is emitted so the user
    ...
    for node in data.get("nodes", []):
        ...
        for field_name, field_def in template.items():
            ...
            is_credential = field_def.get("password", False) or field_def.get(
