"""Level 1 structural validation checks for Portals flow JSON.
...
if TYPE_CHECKING:
    from px.cli.validation.core import ValidationResult
...
def _make_issue(
    ...
    from px.cli.validation.core import ValidationIssue
...
def _node_display_name(node: dict[str, Any]) -> str | None:
    from px.cli.validation.core import _node_display_name as _ndn
...
def _check_orphaned_nodes(flow: dict[str, Any], result: ValidationResult) -> None:
    ...
    nodes: list[dict[str, Any]] = [
        n for n in data.get("nodes", []) if isinstance(n, dict) and "id" in n
    ]
    edges: list[dict[str, Any]] = [
        e for e in data.get("edges", []) if isinstance(e, dict)
    ]
...
def _check_version_mismatch(flow: dict[str, Any], result: ValidationResult) -> None:
    """Warn when nodes were built with a different Portals version.

    Each unique ``lf_version`` embedded in the node metadata that differs from
    the currently installed Portals version triggers a single warning covering
    all affected nodes.  If Portals is not installed the check is skipped
    silently (px can run standalone).
    """
    from px.cli.validation.core import _get_lf_version

    installed = _get_lf_version()
    if installed is None:
        return  # Portals not installed; skip silently

    nodes: list[dict[str, Any]] = [
        n for n in flow.get("data", {}).get("nodes", []) if isinstance(n, dict)
    ]
    ...
    for node in nodes:
        ...
        if lf_version and lf_version != installed:
            version_to_nodes.setdefault(lf_version, []).append(
                _node_display_name(node) or node.get("id") or "?"
            )

    _max_sample = 3
    for built_version, node_names in sorted(version_to_nodes.items()):
        count = len(node_names)
        sample = ", ".join(node_names[:_max_sample]) + (
            " ..." if count > _max_sample else ""
        )
        result.issues.append(
            _make_issue(
                level=_LEVEL_STRUCTURAL,
                ...
                node_name=None,
                message=(
                    f"{count} component(s) built with Portals {built_version} "
