"""px login -- validate credentials against a remote Portals instance.
...
Validate a named environment::
    ...
    px login --env staging

Validate inline credentials::

    px login --target https://portals.example.com --api-key sk-abc123
...
def _api_key_env_name(env_name: str, environments_file: str | None) -> str | None:
    ...
    try:
        from px.config.environments import _find_config_file, _load_config
        ...
        from px.config.environments import _load_raw
...
def _probe_connection(client: Any, sdk: Any) -> tuple[bool, str, int]:
    ...
    try:
    ...
    except sdk.PortalsAuthError:
        return False, "auth", 0
    except sdk.PortalsConnectionError as exc:
        ...
        # local Portals version exactly.  Treat this as a successful probe.
    ...
    *,
    ...
    if env_cfg.api_key is None:
        ...
        if key_env_name:
            ...
            warning_parts.append("  Add [bold]api_key_env: PORTALS_<ENV>_API_KEY[/bold] to your config,")
    ...
    if not ok:
        if msg == "auth":
            ...
            if key_env_name:
                err_console.print("  1. Open Portals → Settings → API Keys → Create a new key")
        ...
        if msg.startswith("connection:"):
            ...
            err_console.print()
            err_console.print("[bold]How to fix:[/bold]")
            err_console.print("  • Make sure your Portals instance is running")
            err_console.print("  • Check the URL in your .px/environments.yaml")
            err_console.print("  • If running locally: [bold]portals run[/bold] or [bold]px serve <flow.json>[/bold]")
            raise typer.Exit(1)
    ...
    elif os.environ.get("PORTALS_API_KEY") == env_cfg.api_key and env_cfg.api_key:
        key_source = "  [dim]from PORTALS_API_KEY[/dim]"
    ...
    console.print(
        Panel.fit(
            ...
            title="[bold blue]px login[/bold blue]",
            border_style="green",
        )
    )
    ...
    if env_cfg.name in ("__inline__", "__env__") and env_cfg.api_key:
        console.print()
        console.print("[dim]Tip: to avoid passing credentials each time, add to .px/environments.yaml:[/dim]")
        ...
        console.print(f"[dim]      api_key_env: PORTALS_{env_display.upper()}_API_KEY[/dim]")
