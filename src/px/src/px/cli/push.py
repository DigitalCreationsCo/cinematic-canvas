"""px push -- push normalized flow JSON to a remote Portals instance.
...
Push a single flow to staging::
    ...
    px push my_flow.json --env staging

Push several flows at once::

    px push flows/*.json --env staging

Push all flows in a directory and place them in a named project::

    px push --dir ./flows/ --env staging --project "My RAG Pipeline"

Dry-run to see what would happen::

    px push ./flows/ --env production --dry-run
"""
...
from px.cli.common import load_sdk
...
def _extract_flow_id(flow: dict[str, Any], path: Path) -> UUID:
    ...
    if not raw_id:
        console.print(
            f"[red]Error:[/red] {path} has no 'id' field. Run [bold]px export[/bold] first."
        )
        raise typer.Exit(1)
    try:
...
def _flow_to_create(sdk: Any, flow: dict[str, Any], folder_id: UUID | None) -> Any:
    ...
    return sdk.FlowCreate(
        ...
        tags=flow.get("tags"),
        folder_id=folder_id
        or (UUID(flow["folder_id"]) if flow.get("folder_id") else None),
        icon=flow.get("icon"),
    ...
    *,
    ...
    if local_file_content is not None:
        try:
            ...
            from portals_sdk.exceptions import PortalsNotFoundError
            from portals_sdk.serialization import flow_to_json, normalize_flow
        ...
        except PortalsNotFoundError:
            pass  # Flow doesn't exist yet — fall through to create it
        except Exception:  # noqa: BLE001
            import logging

            logging.getLogger(__name__).debug(
                "Remote comparison failed; proceeding with push", exc_info=True
            )
    ...
    except sdk.PortalsHTTPError as exc:
        return PushResult(
            path=path,
            flow_id=flow_id,
            flow_name=flow_name,
...
def _find_project_root() -> Path | None:
    """Return the px project root (directory containing .px/), or None.

    Only the ``.px`` marker is used — ``.git`` is intentionally excluded so
    that the containment check doesn't reject paths when running inside a
    larger monorepo or outside any px project.
    """
    cwd = Path.cwd()
    for directory in (cwd, *cwd.parents):
        if (directory / ".px").is_dir():
            return directory
...
def _collect_flow_files(sources: list[str], dir_path: str | None) -> list[Path]:
    ...
    ``flows/`` — mirroring the behaviour of ``px pull``.
    """
    paths: list[Path] = []
    root = _find_project_root()

    # Default to flows/ when nothing is specified, just like px pull does.
...
def _render_results(results: list[PushResult], *, dry_run: bool) -> None:
    ...
    for r in results:
        ...
        url_cell = (
            f"[blue]{r.flow_url}[/blue]" if r.flow_url and r.ok else (r.flow_url or "")
        )
        table.add_row(
            str(r.path),
            r.flow_name,
...
def push_command(
    ...
    *,
    ...
    from px.config import ConfigError, resolve_environment
    ...
    if not paths:
        console.print(
            ...
            "Run [bold]px pull[/bold] first, or pass explicit file paths."
        )
        raise typer.Exit(1)
    ...
    if project_id:
        ...
        target_folder_id = _find_or_create_project(
            client, sdk, project, dry_run=dry_run
        )
    ...
    for path in paths:
        ...
        if dry_run:
            console.print(
                f"[yellow]DRY-RUN[/yellow] Would push {flow_name!r} ({flow_id})"
            )
            ...
            console.print(
                f"[green]Created[/green]  {flow_name!r} ({flow_id}){url_hint}"
            )
            ...
            console.print(
                f"[red]Failed[/red]   {flow_name!r} ({flow_id}): {result.error}"
            )
