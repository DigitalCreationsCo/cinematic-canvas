...
from px.load import load_flow_from_json
from px.run._defaults import apply_run_defaults, resolve_fallback_to_env_vars
...
_PORTALS_NAMESPACE_UUID = uuid.UUID("3c091057-e799-4e32-8ebc-27bc31e1108c")
...
def get_api_key() -> str:
    ...
    Used by ``px serve`` to set the superuser key on the local server.
    For *remote* commands (push, pull, login, …), the per-environment key
    is resolved via :func:`px.config.resolve_environment` and the
    ``api_key_env`` field in ``.px/environments.yaml``.
    """
    api_key = os.getenv("PORTALS_API_KEY") or os.getenv("PX_API_KEY")
    if not api_key:
        msg = "PORTALS_API_KEY environment variable is not set"
...
def download_script_from_url(url: str, verbose_print) -> Path:
    ...
    try:
        with httpx.Client(timeout=30.0) as client:
            ...
            verbose_print(
                f"✓ Script downloaded successfully to temporary file: {temp_path}"
            )
            return temp_path
...
def validate_script_path(script_path: Path | str, verbose_print) -> tuple[str, Path]:
    ...
    if isinstance(script_path, str) and is_url(script_path):
        ...
        if file_extension != ".py":
            verbose_print(
                f"Error: URL must point to a Python script (.py file), got: {file_extension}"
            )
            raise typer.Exit(1)
...
async def load_graph_from_path(
    script_path: Path, file_extension: str, verbose_print, *, verbose: bool = False
):
...
async def execute_graph_with_capture(
    graph, input_value: str | None, session_id: str | None = None
):
    ...
    Args:
        ...
        session_id: Optional session ID. ``None`` auto-generates one so that
            ...
            shell/env-var typos (see ``px.run._defaults.validate_provided_id``).
    ...
    apply_run_defaults(
        graph, session_id=session_id, user_id=None, overwrite_user_id=False
    )
    ...
    try:
        ...
        results = [
            ...
            async for result in graph.async_start(
                inputs, fallback_to_env_vars=fallback_to_env_vars
            )
        ]
...
def _needs_install(requirement: str) -> bool:
    ...
    from packaging.requirements import (
        Requirement,  # locally imported to avoid hard dep if unused
    )
...
def ensure_dependencies_installed(dependencies: list[str], verbose_print) -> None:
    ...
    verbose_print(
        f"Installing missing dependencies with {tool_name}: {', '.join(missing)}"
    )
    try:
...
def flow_id_from_path(file_path: Path, root_dir: Path) -> str:
    ...
    return str(uuid.uuid5(_PORTALS_NAMESPACE_UUID, relative))
...
_GITHUB_RE_REPO = re.compile(
    r"https?://(?:www\.)?github\.com/([\w.-]+)/([\w.-]+)(?:\.git)?/?$"
)
_GITHUB_RE_TREE = re.compile(
    r"https?://(?:www\.)?github\.com/([\w.-]+)/([\w.-]+)/tree/([\w\/-]+)"
)
_GITHUB_RE_RELEASE = re.compile(
    r"https?://(?:www\.)?github\.com/([\w.-]+)/([\w.-]+)/releases/tag/([\w\/-]+)"
)
_GITHUB_RE_COMMIT = re.compile(
    r"https?://(?:www\.)?github\.com/([\w.-]+)/([\w.-]+)/commit/(\w+)(?:/)?$"
)
...
def detect_github_url_sync(url: str, *, timeout: float = 15.0) -> str:
    ...
    if match := _GITHUB_RE_REPO.match(url):
        ...
        with httpx.Client(
            timeout=timeout, follow_redirects=True, headers=_github_headers()
        ) as client:
...
def download_and_extract_repo(
    url: str, verbose_print, *, timeout: float = 60.0
) -> Path:
...
def load_sdk(command_name: str) -> Any:
    """Lazily import ``portals_sdk`` to keep CLI startup fast.
    ...
    try:
        ...
        msg = f"portals-sdk is required for px {command_name}. Install it with: pip install portals-sdk"
