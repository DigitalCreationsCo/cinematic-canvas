...
async def load_registry(client: PortalsClient) -> dict[str, dict]:
...
def search_registry(
    ...
    for name, tmpl in sorted(registry.items()):
        ...
        if (
            ...
            and query.lower() not in cat.lower()
        ):
            continue
        if output_type:
...
def describe_component(
    registry: dict[str, dict], component_type: str
) -> dict[str, Any]:
