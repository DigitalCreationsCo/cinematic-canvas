"""Async HTTP client for Portals REST API.
...
class PortalsClient:
    """Async HTTP client for Portals's REST API.
    ...
    def __init__(
        ...
        self.server_url = (
            server_url or os.environ.get("PORTALS_SERVER_URL", "http://localhost:7860")
        ).rstrip("/")
        self.api_key = api_key or os.environ.get("PORTALS_API_KEY")
    ...
    async def post(
        self, path: str, json_data: Any = None, timeout: float = 30.0, **kwargs: Any
    ) -> Any:
        url = self._url(path)
        try:
            client = await self._client()
            resp = await client.post(
                url, headers=self._headers(), json=json_data, timeout=timeout, **kwargs
            )
            resp.raise_for_status()
            return resp.json()
        except httpx.HTTPStatusError as exc:
    ...
    async def stream_post(
        ...
        async with client.stream(
            "POST", url, headers=self._headers(), json=json_data, timeout=timeout
        ) as resp:
            resp.raise_for_status()
            async for raw_line in resp.aiter_lines():
                ...
                try:
                    ...
                    logger.debug(
                        "stream_post: skipping non-JSON SSE line: %s", line[:200]
                    )
                    continue
    ...
    async def patch(self, path: str, json_data: Any = None, **kwargs: Any) -> Any:
        ...
        try:
            ...
            resp = await client.patch(
                url, headers=self._headers(), json=json_data, timeout=30.0, **kwargs
            )
            resp.raise_for_status()
            return resp.json()
        except httpx.HTTPStatusError as exc:
    ...
    async def post_event(
        self, flow_id: str, event_type: str, summary: str = ""
    ) -> None:
        """Post an event to the flow events queue. Best-effort -- does not raise on failure."""
        try:
            await self.post(
                f"/flows/{flow_id}/events",
                json_data={"type": event_type, "summary": summary},
            )
        except Exception:  # noqa: BLE001
            logger.warning(
                "Failed to post flow event (flow_id=%s, type=%s)",
                flow_id,
                event_type,
                exc_info=True,
            )
