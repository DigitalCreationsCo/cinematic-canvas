"""FastMCP server exposing Portals operations as MCP tools.

Connects to a running Portals server via REST API. Flow data is never
...
from px.graph.flow_builder import (
...
from px.graph.flow_builder.spec import validate_spec_references
from px.log.logger import logger
from px.mcp.client import PortalsClient
from px.mcp.registry import (
...
from px.services.telemetry import MCPToolPayload, TelemetryService

# Session state. Module-level singletons for stdio (single agent), with
# contextvars overlay for SSE (multiple concurrent agents).
_client_var: contextvars.ContextVar[PortalsClient | None] = contextvars.ContextVar(
    "_client", default=None
)
_registry_var: contextvars.ContextVar[dict[str, dict] | None] = contextvars.ContextVar(
    "_registry", default=None
)
_shared_client: PortalsClient | None = None
...
mcp = FastMCP(
    "portals-mcp",
    instructions=(
        "Portals MCP server -- build and run AI flows on a Portals instance.\n"
        "\n"
        "Typical workflow:\n"
        "  1. login (or set PORTALS_API_KEY env var)\n"
...
def _get_client() -> PortalsClient:
    ...
    if _shared_client is None:
        _shared_client = PortalsClient()
    return _shared_client


def _set_client(client: PortalsClient) -> None:
    global _shared_client  # noqa: PLW0603
...
async def _patch_flow(flow_id: str, flow: dict) -> dict:
    ...
    return await _get_client().patch(
        f"/flows/{flow_id}", json_data={"data": flow["data"]}
    )
...
@_tracked
async def login(
    username: str, password: str, server_url: str | None = None
) -> dict[str, str]:
    """Authenticate with a Portals server. Call this first.
    ...
    Args:
        ...
        server_url: Server URL (defaults to PORTALS_SERVER_URL env var or http://localhost:7860).
    """
    old_client = _client_var.get() or _shared_client
    if old_client is not None:
        await old_client.close()
    client = PortalsClient(server_url=server_url)
    _set_client(client)
...
async def create_flow(
    name: str = "Untitled Flow", description: str = ""
) -> dict[str, Any]:
...
async def _create_prompt_template_vars(
    flow_id: str, parsed: dict, id_map: dict[str, str]
) -> None:
...
async def create_flow_from_spec(spec: str, *, validate: bool = True) -> dict[str, Any]:
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            await _get_client().post_event(
                flow_id, "flow_settled", "Failed, rolling back"
            )
...
@mcp.tool()
@_tracked
async def use_starter_project(
    starter_name: str, new_name: str | None = None
) -> dict[str, Any]:
...
async def add_component(flow_id: str, component_type: str) -> dict[str, Any]:
    ...
    await _patch_flow(flow_id, flow)
    await _get_client().post_event(
        flow_id, "component_added", f"Added {component_type}"
    )
    return result
...
async def remove_component(flow_id: str, component_id: str) -> dict[str, str]:
    ...
    await _patch_flow(flow_id, flow)
    await _get_client().post_event(
        flow_id, "component_removed", f"Removed {component_id}"
    )
...
async def configure_component(
    ...
    await _patch_flow(flow_id, flow)
    await _get_client().post_event(
        flow_id, "component_configured", f"Configured {component_id}"
    )
    result: dict[str, Any] = {
...
async def get_component_info(
    ...
    from px.mcp.redact import is_sensitive_field
...
async def search_component_types(
    ...
    return search_registry(
        registry, query=query, category=category, output_type=output_type
    )
...
async def connect_components(
    ...
    await _patch_flow(flow_id, flow)
    await _get_client().post_event(
        flow_id, "connection_added", f"Connected {source_id} to {target_id}"
    )
    return {
        "source_id": source_id,
...
async def disconnect_components(
    ...
    """
    flow = await _get_flow(flow_id)
    removed = fb_remove_connection(
        flow, source_id, target_id, source_output, target_input
    )
    if removed == 0:
    ...
    await _patch_flow(flow_id, flow)
    await _get_client().post_event(
        flow_id, "connection_removed", f"Disconnected {source_id} from {target_id}"
    )
...
async def run_flow(
    ...
    result: dict[str, Any] = {}
    token_count = 0
    async for event in client.stream_post(
        f"/run/{flow_id}?stream=true", json_data=request
    ):
    ...
    if not result:
        logger.warning(
            "Streaming produced no result for flow %s, falling back to synchronous execution",
            flow_id,
        )
...
async def validate_flow(flow_id: str) -> dict[str, Any]:
    ...
    try:
        async for event in _get_client().stream_post(
            f"/build/{flow_id}/flow?event_delivery=direct"
        ):
            event_type = event.get("event", "")
...
async def export_flow(flow_id: str) -> dict[str, Any]:
    ...
    Args:
        flow_id: The flow UUID.
    """
    from px.mcp.redact import redact_node
    ...
    if "nodes" in data:
        data = {
            **data,
            "nodes": [
                {**n, "data": redact_node(n.get("data", {}))} for n in data["nodes"]
            ],
        }
    return {
        "id": flow_id,
...
async def _set_frozen(
    flow_id: str, component_id: str, *, frozen: bool
) -> dict[str, str]:
...
async def freeze_component(flow_id: str, component_id: str) -> dict[str, str]:
    ...
    await _get_client().post_event(
        flow_id, "component_configured", f"Froze {component_id}"
    )
    return result
...
async def unfreeze_component(flow_id: str, component_id: str) -> dict[str, str]:
    ...
    await _get_client().post_event(
        flow_id, "component_configured", f"Unfroze {component_id}"
    )
    return result
