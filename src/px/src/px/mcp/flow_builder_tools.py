...
from px.graph.flow_builder.builder import build_flow_from_spec, load_local_registry
from px.graph.flow_builder.component import add_component as fb_add_component
from px.graph.flow_builder.component import configure_component as fb_configure
from px.graph.flow_builder.component import remove_component as fb_remove_component
from px.graph.flow_builder.connect import add_connection as fb_add_connection
from px.graph.flow_builder.flow import empty_flow
from px.graph.flow_builder.layout import layout_flow
from px.io import MessageTextInput, Output
from px.log.logger import logger
from px.mcp.redact import is_sensitive_field
from px.mcp.registry import describe_component, search_registry
...
def _save_working_flow() -> None:
    ...
    try:
        ...
        async def _save():
            from portals.services.database.models.flow import Flow
...
class SearchComponentTypes(Component):
    ...
    description = (
        "Search available Portals component types by name, category, or output type."
    )
    icon = "Search"
...
class DescribeComponentType(Component):
    ...
    outputs = [
        Output(
            name="description", display_name="Description", method="describe_component"
        ),
    ]
    ...
    def get_field_value(self) -> Data:
        ...
        if self.field_name not in template:
            available = [
                ...
                if isinstance(v, dict) and k not in ("code", "_type")
            ]
            return Data(
                data={
                    "error": f"Field '{self.field_name}' not found. Available: {available}"
                }
            )
...
class ProposeFieldEdit(Component):
    ...
    def propose_field_edit(self) -> Data:
        ...
        if flow is None:
            return Data(
                data={
                    "error": "No flow loaded. Cannot propose edits on an empty canvas."
                }
            )
        ...
        template = node.get("data", {}).get("node", {}).get("template", {})
        if self.field_name not in template or not isinstance(
            template.get(self.field_name), dict
        ):
            available = [
                k
                for k, v in template.items()
        ...
        if node_idx is None:
            return Data(
                data={"error": f"Could not resolve index for '{self.component_id}'"}
            )
        ...
        try:
            ...
            patched_val = patched_node["data"]["node"]["template"][self.field_name][
                "value"
            ]
            if patched_val != self.new_value:
                return Data(
                    data={"error": "Patch dry run: value did not change as expected"}
                )
...
class AddComponent(Component):
    ...
    description = (
        "Add a component to the user's flow canvas. Returns the component's id."
    )
    icon = "Plus"
    name = "AddComponent"
    ...
    def add_component(self) -> Data:
        ...
        try:
            ...
            return Data(
                data={"id": result["id"], "type": self.component_type, "text": text}
            )
...
class RemoveComponent(Component):
    ...
    def remove_component(self) -> Data:
        ...
        try:
            ...
            return Data(
                data={
                    "removed": self.component_id,
                    "text": f"Removed {self.component_id}",
                }
            )
        except (ValueError, KeyError) as e:
...
class ConnectComponents(Component):
    ...
    inputs = [
        MessageTextInput(
            name="source_id", display_name="Source ID", required=True, tool_mode=True
        ),
        MessageTextInput(
            name="source_output",
        ...
        MessageTextInput(
            name="target_id", display_name="Target ID", required=True, tool_mode=True
        ),
        MessageTextInput(
            name="target_input",
            display_name="Target Input",
            required=True,
            tool_mode=True,
        ),
    ]
...
class ConfigureComponent(Component):
    ...
    def configure_component(self) -> Data:
        ...
        try:
            ...
            return Data(
                data={
                    "text": f"Set {self.param_name}={self.param_value} on {self.component_id}"
                }
            )
        except (ValueError, KeyError) as e:
