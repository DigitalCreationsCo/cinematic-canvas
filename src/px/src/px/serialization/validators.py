...
def timestamp_to_str(timestamp: datetime | str) -> str:
    ...
    if isinstance(timestamp, str):
        ...
        for fmt in formats:
            try:
                parsed = datetime.strptime(timestamp.strip(), fmt).replace(
                    tzinfo=timezone.utc
                )
...
def str_to_timestamp(timestamp: str | datetime) -> datetime:
    ...
    if isinstance(timestamp, str):
        try:
            return datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S %Z").replace(
                tzinfo=timezone.utc
            )
...
timestamp_with_fractional_seconds_validator = BeforeValidator(
