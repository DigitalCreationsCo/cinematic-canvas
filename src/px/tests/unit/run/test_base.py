...
import pytest
from px.interface.components import component_cache
from px.run.base import RunError, output_error, run_flow
...
class TestRunFlowJsonInput:
    ...
    async def test_valid_json_creates_temp_file_and_loads_graph(self):
        ...
        with (
            patch("px.load.aload_flow_from_json") as mock_load,
            patch("px.run.base.validate_global_variables_for_env") as mock_validate,
            patch("px.run.base.extract_structured_result") as mock_extract,
        ):
...
class TestRunFlowSessionId:
    ...
    async def test_session_id_autogenerated_when_not_provided(self, tmp_path):
        ...
        with (
            patch("px.run.base.find_graph_variable") as mock_find,
            patch("px.run.base.load_graph_from_script") as mock_load,
            patch("px.run.base.validate_global_variables_for_env") as mock_validate,
            patch("px.run.base.extract_structured_result") as mock_extract,
        ):
...
class TestRunFlowSessionIdPropagation:
    """Session_id must reach Memory/MessageHistory components on the px run path.

    px run uses ``graph.async_start`` (not ``graph.arun``), so it bypasses the
    ...
    ``portals/api/utils/flow_utils.build_graph_from_data`` does for the playground.
    """
    ...
    def _patches():
        return (
            patch("px.run.base.find_graph_variable"),
            patch("px.run.base.load_graph_from_script"),
            patch("px.run.base.validate_global_variables_for_env"),
            patch("px.run.base.extract_structured_result"),
        )
    ...
    async def test_session_id_does_not_overwrite_hardcoded_vertex_value(self, tmp_path):
        ...
        Matches Portals's playground behavior: ``build_graph_from_data`` only writes
...
class TestRunFlowUserId:
    """user_id auto-generation on the px run path.
    ...
    px run has no notion of authenticated users, but the precheck still has to
    ...
    for variable scoping in px (env vars are process-global) but exists to keep
...
class TestRunFlowFallbackToEnvVars:
    ...
    async def test_respects_settings_when_disabled(self, tmp_path):
        """When PORTALS_FALLBACK_TO_ENV_VAR=false, the flag plumbs through as False."""
        script_path = tmp_path / "test.py"
        script_path.write_text("graph = None")
        captured: dict = {}
        mock_graph = self._mock_graph_capturing_kwargs(captured)
        ...
        with (
            ...
            patch("px.run.base.validate_global_variables_for_env") as mock_validate,
            patch("px.run.base.extract_structured_result") as mock_extract,
            patch("px.run._defaults.get_settings_service", return_value=mock_settings),
        ):
            mock_find.return_value = {"line_number": 1, "type": "Graph", "source_line": "graph = Graph(...)"}
            mock_load.return_value = mock_graph
            mock_validate.return_value = []
...
class TestRunFlowOutputFormats:
    ...
    async def test_result_format_extracts_text(self, tmp_path, mock_successful_execution):
        ...
        with (
            ...
            patch("px.run.base.load_graph_from_script") as mock_load,
            patch("px.run.base.validate_global_variables_for_env") as mock_validate,
            patch("px.run.base.extract_text_from_result") as mock_extract_text,
        ):
            mock_find.return_value = {"line_number": 1, "type": "Graph", "source_line": "graph = ..."}
            mock_load.return_value = mock_successful_execution
            mock_validate.return_value = []
...
class TestRunFlowVerbosity:
    ...
    async def test_verbose_false_configures_critical_logging(self, tmp_path):
        ...
        log_module = sys.modules["px.log.logger"]
...
class TestRunFlowInputValueHandling:
    ...
    async def test_input_value_takes_precedence(self, tmp_path):
        ...
        with (
            ...
            patch("px.run.base.validate_global_variables_for_env") as mock_validate,
            patch("px.run.base.extract_structured_result") as mock_extract,
            patch("px.run.base.InputValueRequest") as mock_input_request,
        ):
            mock_find.return_value = {"line_number": 1, "type": "Graph", "source_line": "graph = ..."}
            mock_load.return_value = mock_graph
            mock_validate.return_value = []
...
class TestRunFlowJsonFileExecution:
    ...
    async def test_json_file_calls_aload_flow_from_json(self, simple_json_flow):
        ...
        with (
            patch("px.load.aload_flow_from_json") as mock_load_json,
            patch("px.run.base.validate_global_variables_for_env") as mock_validate,
            patch("px.run.base.extract_structured_result") as mock_extract,
        ):
...
from px.template.field.base import Output, Input
