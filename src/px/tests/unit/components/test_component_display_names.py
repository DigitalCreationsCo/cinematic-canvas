...
def _discover_all_component_classes() -> list[tuple[str, type]]:
    """Discover all component classes from the px.components package.

    Finds all .py files under px/components, imports them, and collects
    ...
    components_pkg_path = Path(px.components.__path__[0])
    components_pkg_name = px.components.__name__
