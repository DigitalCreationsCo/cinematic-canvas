...
because some flows (notably ``px run`` without --session-id, before run_flow
...
try:
    from px.components.cuga import cuga_agent
except Exception as exc:
    pytest.skip(
        f"cuga_agent module not importable in this env: {exc}", allow_module_level=True
    )
...
@pytest.mark.asyncio
async def test_session_id_falls_back_to_string_uuid_when_graph_session_id_empty(
    captured_message_kwargs,
):
...
@pytest.mark.asyncio
async def test_session_id_falls_back_when_graph_session_id_none(
    captured_message_kwargs,
):
