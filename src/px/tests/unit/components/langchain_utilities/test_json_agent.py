...
class TestJsonAgentComponent:
    ...
    def test_get_local_path_with_local_json_file(self, component_class):
        ...
        try:
            ...
            with patch("px.components.langchain_utilities.json_agent.get_settings_service") as mock_get_settings:
    ...
    def test_get_local_path_with_s3_json_file(self, component_class):
        ...
        with (
            patch("px.components.langchain_utilities.json_agent.get_settings_service") as mock_get_settings,
            patch(
                "px.components.langchain_utilities.json_agent.read_file_bytes", new_callable=AsyncMock
            ) as mock_read_bytes,
        ):
            mock_settings = MagicMock()
    ...
    def test_build_agent_with_local_yaml_file(self, component_class, mock_langchain_community):
        ...
        try:
            ...
            with (
                ...
                patch("px.components.langchain_utilities.json_agent.yaml.safe_load") as mock_yaml_load,
            ):
                mock_settings = MagicMock()
                mock_settings.settings.storage_type = "local"
                mock_get_settings.return_value = mock_settings
