...
from px.components.input_output import ChatInput, ChatOutput, TextOutputComponent
from px.components.langchain_utilities.tool_calling import ToolCallingAgentComponent
from px.components.processing.combine_text import CombineTextComponent
from px.graph import Graph
from px.graph.graph.constants import Finish
...
async def test_graph_with_edge():
    ...
    graph.add_component_edge(
        input_id, (chat_input.outputs[0].name, chat_input.inputs[0].name), output_id
    )
    graph.prepare()
...
async def test_graph_functional_async_start():
    ...
    assert all(
        result.vertex.id in ids for result in results if hasattr(result, "vertex")
    )
...
def test_get_terminal_nodes():
    ...
    assert "chat_input" not in terminal_nodes, (
        "Input node should not be terminal (has outgoing edges)"
    )
    assert "text_output" not in terminal_nodes, (
        "Text node should not be terminal (has outgoing edges)"
    )
...
def test_get_terminal_nodes_with_agent_branches():
    ...
    assert "llm_agent" not in terminal_nodes, (
        "Agent node should not be terminal (has outgoing edges)"
    )
    assert "text_edit" not in terminal_nodes, (
        "Text edit node should not be terminal (has outgoing edges)"
    )
...
def test_get_terminal_nodes_single_node():
    ...
    assert terminal_nodes == ["single_node"], (
        f"Expected ['single_node'], got {terminal_nodes}"
    )
...
# TODO: Move to Portals tests
@pytest.mark.skip(reason="Temporarily disabled")
def test_graph_set_with_valid_component():
    from px.components.langchain_utilities.tool_calling import (
        ToolCallingAgentComponent,
    )
    from px.components.tools.yahoo_finance import YfinanceToolComponent
