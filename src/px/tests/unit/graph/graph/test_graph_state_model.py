...
@pytest.mark.xfail(
    reason="These components trigger aiohttp import. Should refactor LLMRouter"
)
def test_graph_state_model():
    from px.components.openai.openai_chat_model import OpenAIModelComponent
    from px.components.processing import PromptComponent
...
def test_graph_state_model_json_schema():
    ...
    for prop in ["chat_input", "chat_output"]:
        ...
        assert json_schema["properties"][prop]["allOf"][0]["$ref"].startswith(
            "#/$defs/"
        )
        assert json_schema["properties"][prop]["readOnly"] is True

    # Test $defs
    assert set(json_schema["$defs"].keys()) == {
        "ChatInputStateModel",
        "ChatOutputStateModel",
        "Image",
        "Message",
    }
    ...
    for model in ["ChatInputStateModel", "ChatOutputStateModel"]:
        ...
        assert (
            json_schema["$defs"][model]["properties"]["message"]["allOf"][0]["$ref"]
            == "#/$defs/Message"
        )
