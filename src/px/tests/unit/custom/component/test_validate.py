...
def test_importing_portals_module_in_px():
    code = dedent("""from portals.custom import   Component
...
def test_importing_portals_logging_in_px():
    """Test that portals.logging can be imported in px context without errors."""
    code = dedent("""
...
class TestGetModuleFallbacks:
    ...
    def test_portals_falls_back_to_px(self):
        result = _get_module_fallbacks("portals.custom")
        assert result == ["portals.custom", "px.custom"]

    def test_portals_deep_path(self):
        result = _get_module_fallbacks("portals.custom.validate")
        assert result == ["portals.custom.validate", "px.custom.validate"]
    ...
    def test_bare_portals_no_fallback(self):
        assert _get_module_fallbacks("portals") == ["portals"]

    def test_only_first_occurrence_replaced(self):
        result = _get_module_fallbacks("langchain.langchain.nested")
        assert result == [
            "langchain.langchain.nested",
            "langchain_classic.langchain.nested",
        ]

    def test_original_always_first(self):
        """The original module is always tried first."""
        for name in ["portals.custom", "langchain.agents", "requests"]:
