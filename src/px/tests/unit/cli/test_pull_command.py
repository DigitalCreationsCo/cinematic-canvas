"""Unit tests for px pull -- pull_command and helpers.
...
def _fake_flow_obj(
    ...
    """Return a MagicMock that looks like a portals_sdk Flow object."""
    flow = MagicMock()
    flow.id = flow_id
    flow.name = name
...
def _fake_project(
    ...
    """Return a MagicMock that looks like a portals_sdk Project object."""
    proj = MagicMock()
    proj.name = name
    proj.id = project_id
...
def _make_sdk_mock(
    ...
    """Return a mock portals_sdk module wired to client_mock."""
...
def _run_pull(
    *,
    ...
    from px.cli.pull import pull_command
    ...
    with (
        patch("px.cli.pull.load_sdk", return_value=mock_sdk),
        patch("px.config.resolve_environment", return_value=mock_cfg),
    ):
        pull_command(
            env=env,
...
class TestSafeFilename:
    def test_alphanumeric_passthrough(self):
        from px.cli.common import safe_filename as _safe_filename
...
class TestWriteFlow:
    def test_status_created_when_file_does_not_exist(self, tmp_path: Path):
        from px.cli.pull import _write_flow

        flow = _fake_flow_obj()
        sdk = _make_sdk_mock()
        result = _write_flow(
            flow, sdk=sdk, dest_dir=tmp_path, strip_secrets=False, indent=2
        )
        assert result.status == "created"

    def test_status_updated_when_file_exists_with_different_content(
        self, tmp_path: Path
    ):
        from px.cli.pull import _write_flow
    ...
    def test_written_file_has_correct_content(self, tmp_path: Path):
        ...
        result = _write_flow(
            flow, sdk=sdk, dest_dir=tmp_path, strip_secrets=False, indent=4
        )
    ...
    def test_logs_debug_when_write_fails(self, tmp_path: Path):
        ...
        with patch("px.cli.pull.logger") as mock_logger:
            _write_flow(flow, sdk=sdk, dest_dir=tmp_path, strip_secrets=False, indent=2)

        mock_logger.debug.assert_called_once_with(
            "Failed to write flow %s", _FLOW_ID, exc_info=True
        )
...
class TestPullResult:
    def test_created_is_ok(self, tmp_path: Path):
        from px.cli.pull import PullResult

        r = PullResult(
            flow_id=_FLOW_ID, flow_name="F", path=tmp_path / "f.json", status="created"
        )
        assert r.ok is True
    ...
    def test_updated_is_ok(self, tmp_path: Path):
        ...
        r = PullResult(
            flow_id=_FLOW_ID, flow_name="F", path=tmp_path / "f.json", status="updated"
        )
        assert r.ok is True
...
class TestPullCommandProjectByName:
    def test_resolves_project_from_list_projects(self, tmp_path: Path):
        ...
        client = _make_client_mock()
        client.list_projects.return_value = [
            _fake_project_summary(name="My Project", project_id=proj.id)
        ]
    ...
    def test_exits_1_if_project_name_not_found(self, tmp_path: Path):
        client = _make_client_mock()
        client.list_projects.return_value = [
            _fake_project_summary(name="Other Project")
        ]
        sdk = _make_sdk_mock(client_mock=client)
        with pytest.raises(typer.Exit) as exc_info:
            _run_pull(
    ...
    def test_uses_first_matching_project(self, tmp_path: Path):
        proj1 = _fake_project(
            name="My Project", project_id=_PROJECT_ID, flows=[_fake_flow_obj()]
        )
        proj2 = _fake_project(
            name="My Project",
    ...
    def test_exits_1_if_get_project_for_name_raises(self, tmp_path: Path):
        client = _make_client_mock()
        client.list_projects.return_value = [
            _fake_project_summary(name="My Project", project_id=_PROJECT_ID)
        ]
...
class TestPullCommandAllFlows:
    def test_calls_list_flows_with_correct_args(self, tmp_path: Path):
        ...
        client.list_flows.assert_called_once_with(
...
class TestPullCommandErrorHandling:
    def test_config_error_exits_1(self, tmp_path: Path):
        ...
        sdk = _make_sdk_mock()
        with (
            patch("px.cli.pull.load_sdk", return_value=sdk),
            patch(
    ...
    def test_client_constructed_with_resolved_url_and_key(self, tmp_path: Path):
        ...
        cfg = _fake_env_config(
            url="http://custom.server", api_key="custom-key"
        )  # pragma: allowlist secret
        _run_pull(
            tmp_path=tmp_path, output_dir=str(tmp_path), sdk_mock=sdk, env_cfg=cfg
        )
...
class TestPullCommandOutputDir:
    def test_defaults_to_flows_dir_when_not_specified(self, tmp_path: Path):
        ...
        try:
            with (
                patch("px.cli.pull.load_sdk", return_value=sdk),
                patch("px.config.resolve_environment", return_value=cfg),
            ):
                pull_command(
                    env=None,
                    output_dir=None,
...
class TestRenderResults:
    def test_does_not_crash_with_mixed_statuses(self, tmp_path: Path):
        from px.cli.pull import PullResult, _render_results
    ...
    def test_does_not_crash_with_empty_results(self):
        from px.cli.pull import _render_results
