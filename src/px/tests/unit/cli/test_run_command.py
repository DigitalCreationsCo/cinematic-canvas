...
class TestRunCommand:
    ...
    def simple_chat_script(self, tmp_path):
        ...
        script_content = '''"""A simple chat flow example for Portals.
...
'''
    ...
    def test_session_id_cli_flag_plumbs_through_to_run_flow(self, simple_chat_script):
        ...
        Locks in the typer Option name so a rename or wiring typo at px.cli.run.run
        ...
        with patch("px.cli.run.run_flow", new=AsyncMock(side_effect=fake_run_flow)):
            run(
                script_path=simple_chat_script,
                input_value="hi",
                input_value_option=None,
