"""Unit tests for PX CLI script loader."""
...
class TestGraphValidation:
    ...
    def test_validate_graph_instance_wrong_type(self):
        ...
        with pytest.raises(TypeError, match="Graph object is not a PX Graph instance"):
...
class TestLoadGraphFromScript:
    ...
    async def test_load_graph_from_script_no_graph_variable(self):
        ...
        try:
            with pytest.raises(
                RuntimeError,
                match="No 'graph' variable or 'get_graph\\(\\)' function found",
            ):
    ...
    async def test_load_graph_from_script_with_async_get_graph(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".py", delete=False) as f:
            f.write("from px.components.input_output import ChatInput, ChatOutput\n")
            f.write("from px.graph import Graph\n")
            f.write("\n")
...
class TestResultExtraction:
    ...
    def test_extract_text_from_result_no_text_attribute(self):
        ...
        result_data = ResultData(
            results={"message": "Plain string message"},
            component_display_name="Chat Output",
            component_id="test-123",
        )
