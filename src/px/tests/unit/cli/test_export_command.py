"""Unit tests for px export -- export_command and helpers.
...
def _make_client_mock(
    ...
    client.get_flow.return_value = (
        flow_obj if flow_obj is not None else _make_flow_obj()
    )
    client.get_project.return_value = (
        project_obj if project_obj is not None else _make_project_obj()
    )
    return client


def _make_sdk_mock(client_mock: MagicMock | None = None) -> MagicMock:
    """Return a mock portals_sdk module wired up for export tests."""
...
class TestWriteFlow:
    def test_in_place_writes_to_source_path(self, tmp_path):
        from px.cli.export import _write_flow

        src = tmp_path / "flow.json"
        src.write_text("{}", encoding="utf-8")
        sdk = _make_sdk_mock()
        result = _write_flow(
            _FLOW_DICT, sdk=sdk, output=None, in_place=True, source_path=src, indent=2
        )
        assert result == src
    ...
    def test_output_given_writes_to_output(self, tmp_path):
        ...
        sdk = _make_sdk_mock()
        result = _write_flow(
            _FLOW_DICT, sdk=sdk, output=out, in_place=False, source_path=None, indent=2
        )
        assert result == out
    ...
    def test_in_place_false_with_source_path_uses_output(self, tmp_path):
        ...
        sdk = _make_sdk_mock()
        result = _write_flow(
            _FLOW_DICT, sdk=sdk, output=out, in_place=False, source_path=src, indent=2
        )
        assert result == out
        assert out.exists()
    ...
    def test_sdk_flow_to_json_called_with_indent(self, tmp_path):
        ...
        sdk = _make_sdk_mock()
        _write_flow(
            _FLOW_DICT, sdk=sdk, output=out, in_place=False, source_path=None, indent=4
        )
...
class TestExportCommandLocalMode:
    ...
    def test_indent_passed_to_flow_to_json(self, tmp_path):
        ...
        sdk.flow_to_json.assert_called_with(
            sdk.normalize_flow_file.return_value, indent=4
        )
...
class TestExportCommandRemoteFlowId:
    def test_missing_env_and_target_raises_exit_1(self):
        ...
        with (
            patch("px.cli.export.load_sdk", return_value=sdk),
    ...
    def test_flow_id_uses_cwd_when_output_dir_not_specified(
...
class TestExportCommandRemoteProject:
    def test_project_exports_all_flows(self, tmp_path):
        flows = [
            _make_flow_obj(
                name=f"Flow {i}", flow_id=f"aaaaaaaa-0000-0000-0000-{i:012d}"
            )
            for i in range(3)
        ]
    ...
    def test_project_creates_dir_named_after_project_when_output_dir_not_given(
        self, tmp_path, monkeypatch
    ):
        monkeypatch.chdir(tmp_path)
    ...
    def test_project_config_error_raises_exit_1(self, tmp_path):
        ...
        with (
            patch("px.cli.export.load_sdk", return_value=sdk),
            patch(
                "px.config.resolve_environment", side_effect=ConfigError("cfg error")
            ),
            pytest.raises(typer.Exit) as exc_info,
        ):
            export_command(
    ...
    def test_project_target_used_without_env_name(self, tmp_path):
        ...
        with (
            patch("px.cli.export.load_sdk", return_value=sdk),
            patch("px.config.resolve_environment", return_value=env_cfg),
        ):
            export_command(
                flow_paths=[],
                output=None,
