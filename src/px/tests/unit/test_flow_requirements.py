...
import pytest
from px.utils.flow_requirements import (
    MODULE_EXTRA_DEPS,
    _detect_providers_from_template,
    _extract_component_requirements,
    _extract_imports,
    _get_import_to_dist_map,
    _get_px_provided_imports,
    _get_px_transitive_dists,
    _import_to_package,
    _pin_version,
...
"""
    ...
    def test_px_imports(self):
        code = "from px.schema.message import Message"
        result = _extract_imports(code)
        assert "px" in result
...
class TestImportToPackage:
    ...
    def test_fallback_underscore_to_hyphen(self):
        ...
        assert (
            _import_to_package("totally_unknown_pkg_xyz") == "totally-unknown-pkg-xyz"
        )
...
class TestDynamicResolution:
    ...
    def test_px_transitive_dists_includes_px(self):
        dists = _get_px_transitive_dists()
        assert "px" in dists

    def test_px_transitive_dists_includes_langchain(self):
        dists = _get_px_transitive_dists()
        assert "langchain" in dists
        assert "langchain-core" in dists

    def test_px_transitive_dists_includes_pydantic(self):
        dists = _get_px_transitive_dists()
        assert "pydantic" in dists

    def test_px_provided_imports_includes_expected(self):
        provided = _get_px_provided_imports()
        ...
        assert len(resolvable) > 0, (
            "No expected imports are resolvable in this environment"
        )
        for imp in resolvable:
            assert imp in provided, f"{imp} should be provided by px"

    def test_px_provided_imports_excludes_optional(self):
        """Packages not in px's dep tree should NOT be in provided."""
        provided = _get_px_provided_imports()
        # langchain-openai is an optional provider, not a core px dep
...
class TestDetectProviders:
    ...
    def test_multiple_providers(self):
        template = {
            ...
            "embeddings_model": {
                "value": [
                    {"provider": "Google Generative AI", "name": "embedding-001"}
                ],
            },
        }
        result = _detect_providers_from_template(template)
...
class TestExtractComponentRequirements:
    def test_px_only_component(self):
        code = """
...
"""
    ...
    def test_px_provided_filtered(self):
        """Imports that are transitively provided by px should not appear as requirements.
        ...
        if not resolvable:
            pytest.skip("None of the test imports are px-provided in this environment")
...
"""
    ...
    def test_module_extra_deps(self):
        ...
        whether it's transitively provided by px, but the extra runtime deps
...
"""
    ...
    def test_portals_imports_filtered(self):
        """Components with portals imports should NOT list portals as a dep.

        px provides the portals interfaces at runtime, so portals/portals_base
        should be filtered out just like px itself.
        """
        code = """
from portals.custom import Component
from portals.io import MessageTextInput
"""
        node = _make_node("LegacyComponent", code)
        packages, _ = _extract_component_requirements(node)
        assert "portals" not in packages
        assert "portals-base" not in packages
...
class TestGenerateRequirementsFromFlow:
    ...
    def test_include_px_false(self):
        flow = _make_flow()
        result = generate_requirements_from_flow(
            flow, include_px=False, pin_versions=False
        )
        assert "px" not in result

    def test_custom_px_package_name(self):
        flow = _make_flow()
        result = generate_requirements_from_flow(
            flow, px_package="px-nightly", pin_versions=False
        )
        assert "px-nightly" in result
        assert "px" not in result
    ...
    def test_results_sorted(self):
        ...
        result = generate_requirements_from_flow(flow, pin_versions=False)
        # px should be first, then sorted extras
        assert result[0] == "px"
        extras = result[1:]
    ...
    def test_amazon_bedrock_provider_adds_package(self):
        node = _make_node(
            ...
            template_extra={
                "model": {
                    "value": [
                        {"provider": "Amazon Bedrock", "name": "anthropic.claude-3"}
                    ]
                },
            },
        )
...
class TestVersionPinning:
    ...
    def test_pinned_output_includes_versions_for_installed_deps(self):
        ...
        # px is installed, so it should be pinned
        assert result[0].startswith("px==")
        # langchain-openai may or may not be installed depending on env;
        # just verify it appears in the output
        langchain_openai_entries = [
            r for r in result if r.startswith("langchain-openai")
        ]
...
class TestStarterProjects:
    ...
    def test_basic_prompting_includes_px(self, basic_prompting_flow):
        """Basic Prompting (ships with no pre-selected model) should still need px.
        ...
        """
        result = generate_requirements_from_flow(
            basic_prompting_flow, pin_versions=False
        )
        assert "px" in result
    ...
    def test_basic_prompting_from_file(self):
        ...
        generated requirements should include ``px`` but none of the
    ...
    def test_px_nightly_package_name(self, basic_prompting_flow):
        """Test specifying px-nightly as the package name."""
        result = generate_requirements_from_flow(
            basic_prompting_flow,
            px_package="px-nightly",
            pin_versions=False,
        )
        assert result[0] == "px-nightly"
        assert "px" not in result
...
class TestResolveProviderPackages:
    ...
    def _skip_if_provider_not_loaded(provider_name: str):
        ...
        """
        try:
            from px.base.models.model_input_constants import MODEL_PROVIDERS_DICT
        except ImportError:
    ...
    def test_all_registered_providers_resolve_to_packages(self):
        ...
        for provider_name in MODEL_PROVIDERS_DICT:
            ...
            assert len(packages) > 0, (
                f"Provider {provider_name} resolved to no packages"
            )
...
class TestResolveEmbeddingProviderPackages:
    ...
    def test_all_embedding_providers_resolve(self):
        ...
        from px.base.models.unified_models import EMBEDDING_PROVIDER_CLASS_MAPPING

        for provider in EMBEDDING_PROVIDER_CLASS_MAPPING:
            packages = _resolve_embedding_provider_packages(provider)
            assert len(packages) > 0, (
                f"Embedding provider '{provider}' resolved to no packages"
            )
    ...
    def test_embedding_only_flow(self):
        ...
        node = _make_node(
            "EmbeddingModel",
            "from px.base.embeddings.model import LCEmbeddingsModel",
            template_extra={
                "model": {
                    "value": [{"provider": "OpenAI", "name": "text-embedding-3-small"}]
                },
            },
        )
        flow = _make_flow(node)
...
class TestErrorHandling:
    ...
    def test_generate_requirements_from_file_wrong_structure(self, tmp_path):
        """A valid JSON file that isn't a flow should still produce a result (just px)."""
...
# ===================================================================
# Typer CLI tests: px requirements
# ===================================================================


class TestTyperRequirementsCommand:
    """Tests for the typer-based ``px requirements`` CLI command."""
    ...
    def flow_file(self, tmp_path):
        flow = _make_flow(_make_node("Simple", "import px"))
    ...
    def test_output_flag_writes_file(self, runner, app, flow_file, tmp_path):
        ...
        result = runner.invoke(
            app, ["requirements", str(flow_file), "-o", str(out), "--no-pin"]
        )
        assert result.exit_code == 0
        assert out.exists()
        assert "px" in out.read_text(encoding="utf-8")
        assert "Requirements written to" in result.output

    def test_no_px_flag(self, runner, app, flow_file):
        result = runner.invoke(
            app, ["requirements", str(flow_file), "--no-px", "--no-pin"]
        )
        assert result.exit_code == 0
        # With --no-px and only px imports, no packages should appear after header
        lines = [
            line
            for line in result.output.strip().split("\n")
            if line and not line.startswith("#")
        ]
        assert "px" not in lines
    ...
    def test_px_package_flag(self, runner, app, flow_file):
        result = runner.invoke(
            app,
            [
                "requirements",
        ...
        assert result.exit_code == 0
        assert "px-nightly" in result.output
        # Should not contain bare "px" as a separate line
        lines = [
            line
            for line in result.output.strip().split("\n")
            if line and not line.startswith("#")
