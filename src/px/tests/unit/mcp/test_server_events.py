...
def _patch_server(mock_client, mock_registry, mock_flow):
    ...
    def ctx():
        with (
            _patch("px.mcp.server._get_client", return_value=mock_client),
            _patch("px.mcp.server._get_registry", new_callable=AsyncMock, return_value=mock_registry),
            _patch("px.mcp.server._get_flow", new_callable=AsyncMock, return_value=mock_flow),
            _patch("px.mcp.server._patch_flow", new_callable=AsyncMock, return_value=mock_flow),
        ):
            yield
...
async def _call_add_component(flow_id, _flow):
    from px.mcp.server import add_component

    with (
        patch("px.mcp.server.fb_add_component", return_value={"id": "X-1", "display_name": "X"}),
        patch("px.mcp.server.layout_flow"),
    ):
        await add_component(flow_id, "ChatInput")


async def _call_remove_component(flow_id, _flow):
    from px.mcp.server import remove_component

    with patch("px.mcp.server.fb_remove_component"), patch("px.mcp.server.layout_flow"):
        await remove_component(flow_id, "X-1")


async def _call_configure_component(flow_id, flow):
    from px.mcp.server import configure_component

    flow["data"]["nodes"] = [_node("X-1")]
    with patch("px.mcp.server.fb_configure"), patch("px.mcp.server.needs_server_update", return_value=False):
        await configure_component(flow_id, "X-1", {"input_value": "hello"})


async def _call_connect_components(flow_id, _flow):
    from px.mcp.server import connect_components

    with patch("px.mcp.server.fb_add_connection"), patch("px.mcp.server.layout_flow"):
        await connect_components(flow_id, "A-1", "message", "B-1", "input_value")


async def _call_disconnect_components(flow_id, _flow):
    from px.mcp.server import disconnect_components

    with patch("px.mcp.server.fb_remove_connection", return_value=1), patch("px.mcp.server.layout_flow"):
        await disconnect_components(flow_id, "A-1", "B-1")


async def _call_freeze_component(flow_id, flow):
    from px.mcp.server import freeze_component
...
async def _call_unfreeze_component(flow_id, flow):
    from px.mcp.server import unfreeze_component
...
async def _call_layout_flow_tool(flow_id, _flow):
    from px.mcp.server import layout_flow_tool

    with patch("px.mcp.server.layout_flow"):
        await layout_flow_tool(flow_id)


async def _call_update_flow_from_spec(flow_id, flow):
    from px.mcp.server import update_flow_from_spec

    parsed = {"name": "T", "description": "", "nodes": [{"id": "A", "type": "ChatInput"}], "edges": [], "config": {}}
    with (
        patch("px.mcp.server.parse_flow_spec", return_value=parsed),
        patch("px.mcp.server.validate_spec_references"),
        patch("px.mcp.server.empty_flow", return_value=flow),
        patch("px.mcp.server.fb_add_component", return_value={"id": "X-1"}),
        patch("px.mcp.server.layout_flow"),
        patch("px.mcp.server.fb_spec_summary", return_value="A: ChatInput"),
    ):
...
class TestCreateFlowFromSpecEmitsSettled:
    async def test_emits_flow_settled_after_batch(self, mock_client, mock_registry):
        from px.mcp.server import create_flow_from_spec
        ...
        with (
            patch("px.mcp.server._get_client", return_value=mock_client),
            patch("px.mcp.server._get_registry", new_callable=AsyncMock, return_value=mock_registry),
            patch("px.mcp.server.parse_flow_spec", return_value=parsed),
            patch("px.mcp.server.validate_spec_references"),
            patch("px.mcp.server.create_flow", new_callable=AsyncMock, return_value=created),
            patch("px.mcp.server.add_component", new_callable=AsyncMock, return_value={"id": "X-1"}),
            patch("px.mcp.server.build_flow", new_callable=AsyncMock),
            patch("px.mcp.server.get_flow_info", new_callable=AsyncMock, return_value={"id": "flow-new"}),
        ):
...
class TestNotifyDone:
    async def test_emits_flow_settled_and_returns_ok(self, mock_client):
        from px.mcp.server import notify_done

        with patch("px.mcp.server._get_client", return_value=mock_client):
...
class TestCreateFlowFromSpecRollback:
    async def test_emits_settle_on_rollback(self, mock_client, mock_registry):
        ...
        with (
            ...
            patch("px.mcp.server.validate_spec_references"),
            patch("px.mcp.server.create_flow", new_callable=AsyncMock, return_value=created),
            patch("px.mcp.server.add_component", new_callable=AsyncMock, side_effect=RuntimeError("Build failed")),
            patch("px.mcp.server.delete_flow", new_callable=AsyncMock),
