"""Tests for px.mcp.server.validate_flow.
...
def _end_vertex_event(
    vertex_id: str, *, valid: bool, error_message: str | None = None
) -> dict:
    outputs: dict = {}
    if not valid and error_message is not None:
        outputs = {
            "output": {
                "message": {"errorMessage": error_message, "stackTrace": ""},
                "type": "error",
            }
        }
    return {
...
def _patch_validate(*, flow: dict, client: MagicMock):
    ...
    def ctx():
        with (
            patch("px.mcp.server._get_client", return_value=client),
            patch("px.mcp.server._get_flow", new_callable=AsyncMock, return_value=flow),
        ):
            yield
...
class TestValidateFlow:
    async def test_empty_flow_returns_valid(self) -> None:
        from px.mcp.server import validate_flow
    ...
    async def test_fails_fast_when_a_component_build_fails(self) -> None:
        ...
        events = [
            _end_vertex_event(
                "A-1", valid=False, error_message="Missing required field: api_key"
            ),
            trailing_marker,
        ]
        client = _stream_client(events)
        ...
        assert result["errors"] == [
            {"component_id": "A-1", "error": "Missing required field: api_key"}
        ]
    ...
    async def test_top_level_error_event_returns_failure(self) -> None:
        ...
        assert result["valid"] is False
        assert result["errors"] == [
            {"component_id": "flow", "error": "Graph has a cycle"}
        ]
