...
def _make_vertex_stub():
    ...
    return SimpleNamespace(
        graph=SimpleNamespace(session_id=None, flow_id=None, user_id=None)
    )
...
class TestAgentTokenCallbackWiring:
    ...
    async def test_token_usage_stored_on_component_when_handler_returns_usage(self):
        # Arrange
        from px.base.agents.agent import LCAgentComponent

        usage = Usage(input_tokens=50, output_tokens=30, total_tokens=80)
        result_message = Message(text="Agent response", id="msg-id")

        with (
            patch("px.base.agents.agent.TokenUsageCallbackHandler") as mock_handler_cls,
            patch(
                "px.base.agents.agent.process_agent_events",
            ...
            agent_component._update_stored_message = AsyncMock(
                return_value=result_message
            )
    ...
    async def test_token_usage_handler_included_in_callbacks(self):
        ...
        assert any(
            isinstance(cb, TokenUsageCallbackHandler) for cb in captured_callbacks
        )
