...
a placeholder Message via send_message. With no session_id (e.g. ``px run`` with
...
async def test_handle_stream_falls_back_to_invoke_when_no_event_manager():
    """Without an event_manager (e.g. px run) the chunk iterator would never be drained.
