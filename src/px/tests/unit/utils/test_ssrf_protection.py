...
import pytest
from px.utils.ssrf_protection import (
    SSRFProtectionError,
    get_allowed_hosts,
    is_host_allowed,
    is_ip_blocked,
...
def mock_ssrf_settings(*, enabled=False, allowed_hosts=None):
    ...
    with patch("px.utils.ssrf_protection.get_settings_service") as mock_get_settings:
...
class TestSSRFProtectionConfiguration:
    ...
    def test_ssrf_protection_disabled_by_default(self):
        ...
        with patch(
            "px.utils.ssrf_protection.get_settings_service"
...
class TestURLValidation:
    ...
    def test_valid_schemes_allowed(self):
        ...
        with (
            ...
            patch("px.utils.ssrf_protection.resolve_hostname") as mock_resolve,
        ):
    ...
    def test_direct_ip_blocking(self):
        ...
        with mock_ssrf_settings(enabled=True):
            ...
            with pytest.raises(SSRFProtectionError, match="blocked"):
                validate_url_for_ssrf(
                    "http://169.254.169.254/latest/meta-data/", warn_only=False
                )
    ...
    def test_public_hostnames_allowed(self):
        ...
        with mock_ssrf_settings(enabled=True):
            ...
            with patch("px.utils.ssrf_protection.resolve_hostname") as mock_resolve:
    ...
    def test_localhost_hostname_blocked(self):
        ...
        with (
            mock_ssrf_settings(enabled=True),
            pytest.raises(SSRFProtectionError, match="blocked IP address"),
        ):
    ...
    def test_warn_only_mode_logs_warnings(self):
        ...
        with (
            mock_ssrf_settings(enabled=True),
            patch("px.utils.ssrf_protection.logger") as mock_logger,
        ):
    ...
    def test_missing_hostname_blocked(self):
        ...
        with (
            mock_ssrf_settings(enabled=True),
            pytest.raises(SSRFProtectionError, match="valid hostname"),
        ):
...
class TestIntegrationScenarios:
    ...
    def test_legitimate_api_allowed(self):
        ...
        with (
            ...
            validate_url_for_ssrf(
                "https://api.openai.com/v1/chat/completions", warn_only=False
            )
            validate_url_for_ssrf(
                "https://api.github.com/repos/portals-ai/portals", warn_only=False
            )
            validate_url_for_ssrf(
                "https://www.googleapis.com/auth/cloud-platform", warn_only=False
            )
    ...
    def test_docker_internal_networking_requires_allowlist(self):
        ...
        with (
            mock_ssrf_settings(
                enabled=True, allowed_hosts=["database", "*.internal.local"]
            ),
            patch("px.utils.ssrf_protection.resolve_hostname") as mock_resolve,
        ):
            mock_resolve.return_value = ["172.18.0.2"]  # Docker bridge network IP
