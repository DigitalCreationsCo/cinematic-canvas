"""Unit tests for PX flow validation helpers."""
...
@pytest.mark.asyncio
async def test_ensure_component_hash_lookups_loaded_requires_settings_service(
    monkeypatch,
):
    """Hash warmup should fail loudly when the settings service is unavailable."""
    monkeypatch.setattr("px.services.deps.get_settings_service", lambda: None)
...
@pytest.mark.asyncio
async def test_ensure_component_hash_lookups_loaded_surfaces_loader_failures(
    monkeypatch,
):
    ...
    with (
        patch(
            "px.interface.components.get_and_cache_all_types_dict",
