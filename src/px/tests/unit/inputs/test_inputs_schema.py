...
class TestInput:
    ...
    def test_post_process_type_function(self):
        ...
        assert set(post_process_type(Union[int, list[str], list[float]])) == {
            int,
            str,
            float,
        }  # noqa: UP007
        ...
        assert set(post_process_type(Union[SequenceABC[int | str], list[float]])) == {
            int,
            str,
            float,
        }  # noqa: UP007
        ...
        assert set(post_process_type(Data | list[int] | None)) == {
            Data,
            int,
            type(None),
        }
        assert set(post_process_type(Data | Union[float, None])) == {
            Data,
            float,
            type(None),
        }  # noqa: UP007
        ...
        assert set(post_process_type(Union[Data | float | str, int])) == {
            Data,
            int,
            float,
            str,
...
def test_schema_to_portals_inputs():
    ...
    # Convert schema to Portals inputs
    inputs = schema_to_portals_inputs(TestSchema)
...
def test_schema_to_portals_inputs_preserves_optional_defaults_and_nullable_objects():
    schema = {
        "type": "object",
        "properties": {
            "task": {"type": "string"},
    ...
    inputs = {input_.name: input_ for input_ in schema_to_portals_inputs(model)}
...
def test_schema_to_portals_inputs_invalid_type():
    ...
    with pytest.raises(TypeError, match="Unsupported field type:"):
        schema_to_portals_inputs(InvalidSchema)
