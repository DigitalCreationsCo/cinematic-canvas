"""Tests for minimal service implementations in PX."""
...
class TestTelemetryService:
    ...
    async def test_enqueue_skipped_when_do_not_track(self, telemetry):
        ...
        from px.services.telemetry.schema import MCPToolPayload
...
class TestVariableService:
    ...
    async def test_get_variable_is_coroutine(self, variables):
        """get_variable must be async to match the portals call site.
        ...
        TypeErrors. px's implementation does no I/O — the async wrapper exists
        purely to match the interface portals's DatabaseVariableService uses.
        """
        import inspect
    ...
    async def test_get_variable_absorbs_extra_kwargs(self, variables):
        """Extra kwargs from the portals call site must not crash the px implementation.

        Portals calls ``await variable_service.get_variable(user_id=..., name=...,
        field=..., session=...)``. px's implementation must accept and ignore the
        extras so flows behave identically as long as the variable can be resolved
        by name.
        """
        os.environ["PX_KWARG_TEST"] = "value-from-env"
        try:
            value = await variables.get_variable(
                user_id="random-uuid", name="PX_KWARG_TEST", field="value", session=None
            )
            assert value == "value-from-env"
        finally:
            del os.environ["PX_KWARG_TEST"]
