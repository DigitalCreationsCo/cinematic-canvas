...
class TestParseDevMode:
    ...
    def test_dev_mode_not_set(self, monkeypatch):
        """Test default behavior when PX_DEV is not set."""
    ...
    def test_dev_mode_enabled_with_1(self, monkeypatch):
        """Test dev mode enabled with PX_DEV=1."""
        monkeypatch.setenv("PX_DEV", "1")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
        assert modules is None  # Load all modules

    def test_dev_mode_enabled_with_true(self, monkeypatch):
        """Test dev mode enabled with PX_DEV=true."""
        monkeypatch.setenv("PX_DEV", "true")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
        assert modules is None

    def test_dev_mode_enabled_with_yes(self, monkeypatch):
        """Test dev mode enabled with PX_DEV=yes."""
        monkeypatch.setenv("PX_DEV", "yes")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
        assert modules is None

    def test_dev_mode_disabled_with_0(self, monkeypatch):
        """Test dev mode disabled with PX_DEV=0."""
        monkeypatch.setenv("PX_DEV", "0")
        enabled, modules = _parse_dev_mode()
        assert enabled is False
        assert modules is None

    def test_dev_mode_disabled_with_false(self, monkeypatch):
        """Test dev mode disabled with PX_DEV=false."""
        monkeypatch.setenv("PX_DEV", "false")
        enabled, modules = _parse_dev_mode()
        assert enabled is False
        assert modules is None
    ...
    def test_dev_mode_single_module(self, monkeypatch):
        ...
        monkeypatch.setenv("PX_DEV", "mistral")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
    ...
    def test_dev_mode_multiple_modules(self, monkeypatch):
        ...
        monkeypatch.setenv("PX_DEV", "mistral,openai,anthropic")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
    ...
    def test_dev_mode_modules_case_normalized(self, monkeypatch):
        ...
        monkeypatch.setenv("PX_DEV", "Mistral,OpenAI")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
...
class TestReadComponentIndex:
    ...
    def test_read_index_file_not_found(self):
        ...
        with patch("px.interface.components.Path") as mock_path_class:
    ...
    def test_read_index_valid(self, tmp_path):
        ...
        with (
            patch("px.interface.components.inspect.getfile") as mock_getfile,
            patch("importlib.metadata.version") as mock_version,
        ):
            mock_getfile.return_value = str(tmp_path / "px" / "__init__.py")
    ...
    def test_read_index_package_not_found(self, tmp_path):
        """Test reading index when px package metadata is unavailable (e.g. Docker workspace install)."""
        ...
        with (
            ...
            mock_getfile.return_value = str(tmp_path / "px" / "__init__.py")
            mock_version.side_effect = PackageNotFoundError("px")
...
@pytest.mark.asyncio
class TestImportPortalsComponents:
    """Tests for import_portals_components() async function."""

    async def test_import_with_dev_mode(self, monkeypatch):
        """Test import in dev mode (dynamic loading)."""
        monkeypatch.setenv("PX_DEV", "1")

        with patch("px.interface.components._process_single_module") as mock_process:
            mock_process.return_value = ("category1", {"comp1": {"template": {}}})

            with (
                patch("px.interface.components.pkgutil.walk_packages") as mock_walk,
                patch("px.interface.components._save_generated_index") as mock_save,
            ):
                mock_walk.return_value = [
                    (None, "px.components.category1", False),
                ]

                result = await import_portals_components()
    ...
    async def test_import_with_missing_index_creates_cache(self, tmp_path, monkeypatch):
        ...
        with (
            patch("px.interface.components._read_component_index") as mock_read,
            patch("px.interface.components._process_single_module") as mock_process,
            patch("px.interface.components.pkgutil.walk_packages") as mock_walk,
            patch("importlib.metadata.version", return_value="0.1.12"),
        ):
            ...
            mock_process.return_value = ("category1", {"comp1": {"template": {}}})
            mock_walk.return_value = [(None, "px.components.category1", False)]
    ...
    async def test_import_with_custom_path_from_settings(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            result = await import_portals_components(mock_settings)
    ...
    async def test_import_handles_import_errors(self, monkeypatch):
        ...
        with (
            ...
            mock_walk.return_value = [(None, "px.components.broken", False)]
