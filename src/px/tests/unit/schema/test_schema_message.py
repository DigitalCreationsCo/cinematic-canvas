...
@pytest.fixture
def portals_cache_dir(tmp_path):
    """Create a temporary portals cache directory."""
    cache_dir = tmp_path / "portals"
    cache_dir.mkdir(parents=True)
    return cache_dir


@pytest.fixture
def sample_image(portals_cache_dir):
    """Create a sample image file for testing."""
    # Create the test_flow directory in the cache
    flow_dir = portals_cache_dir / "test_flow"
    ...
    real_cache_dir = Path(user_cache_dir("portals"))
...
def test_message_prompt_serialization():
    template = "Hello, {name}!"
    message = Message.from_template(template, name="Portals")
    assert message.text == "Hello, Portals!"

    # The base Message class in px doesn't support prompt serialization
    # This functionality is only available in the enhanced message class
    pytest.skip("Prompt serialization not supported in px base Message class")
...
def test_message_with_multiple_images(sample_image, portals_cache_dir):
    ...
    real_cache_dir = Path(user_cache_dir("portals")) / "test_flow"
    real_cache_dir.mkdir(parents=True, exist_ok=True)
