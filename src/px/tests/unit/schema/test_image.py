...
async def test_get_files_with_storage_service_local_path():
    ...
    with patch("px.schema.image.get_storage_service", return_value=mock_storage):
        ...
        mock_storage.get_file.assert_called_once_with(
            flow_id="flow_123", file_name="image.png"
        )
...
async def test_get_files_with_storage_service_s3_path():
    ...
    with patch("px.schema.image.get_storage_service", return_value=mock_storage):
        ...
        mock_storage.parse_file_path.assert_called_once_with(
            "files/flow_456/document.pdf"
        )
        # Verify get_file was called with parsed flow_id and file_name
        mock_storage.get_file.assert_called_once_with(
            flow_id="flow_456", file_name="document.pdf"
        )
        # Verify result contains the file content
...
async def test_get_files_with_storage_service_multiple_files():
    ...
    mock_storage = MagicMock()
    mock_storage.parse_file_path = MagicMock(
        side_effect=[("flow_1", "file1.txt"), ("flow_2", "file2.txt")]
    )
