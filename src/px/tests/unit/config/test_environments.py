"""Unit tests for px.config.environments — environment resolution.
...
from px.config.environments import (
    ConfigError,
    PortalsEnvironment,
    _find_config_file,
    _load_config,
...
class TestPortalsEnvironment:
    def test_fields_stored(self):
        e = PortalsEnvironment(
            name="staging", url="https://x.com", api_key="key123"
    ...
    def test_api_key_may_be_none(self):
        e = PortalsEnvironment(name="local", url="http://localhost:7860", api_key=None)
...
class TestParseEnvBlock:
    ...
    def test_missing_env_var_returns_none(self, tmp_path, monkeypatch):
        ...
        path = tmp_path / "e.yaml"
        result = _parse_env_block(
            "local", {"url": "http://x", "api_key_env": "MISSING_KEY"}, path
        )
...
class TestFindConfigFile:
    ...
    def test_finds_px_yaml_in_cwd(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        p = _write(tmp_path, ".px/environments.yaml", _MINIMAL_YAML)
        assert _find_config_file(None) == p

    def test_finds_px_yml_in_cwd(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        p = _write(tmp_path, ".px/environments.yml", _MINIMAL_YAML)
        assert _find_config_file(None) == p

    def test_walks_up_to_parent(self, tmp_path, monkeypatch):
        parent_yaml = _write(tmp_path, ".px/environments.yaml", _MINIMAL_YAML)
    ...
    def test_stops_at_git_boundary(self, tmp_path, monkeypatch):
        ...
        _write(tmp_path, ".px/environments.yaml", _MINIMAL_YAML)
    ...
    def test_toml_fallback_when_no_yaml(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        p = _write(tmp_path, "portals-environments.toml", _MINIMAL_TOML)
        assert _find_config_file(None) == p
...
class TestResolveConfigMode:
    ...
    def test_no_default_and_no_env_name_raises(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        _write(tmp_path, ".px/environments.yaml", _NO_DEFAULT_YAML)
    ...
    def test_toml_file_also_works(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        monkeypatch.setenv("MY_LOCAL_KEY", "local-secret")  # pragma: allowlist secret
        _write(tmp_path, "portals-environments.toml", _MINIMAL_TOML)
        result = resolve_environment("local")
        assert result.url == "http://localhost:7860"
        assert result.api_key == "local-secret"  # pragma: allowlist secret
...
class TestResolveNoConfigFallbacks:
    def test_portals_url_env_var_used_as_fallback(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / ".git").mkdir()  # stop file walk here
        monkeypatch.setenv("PORTALS_URL", "http://fallback:7860")
        monkeypatch.setenv(
            "PORTALS_API_KEY", "fallback-key"
        )  # pragma: allowlist secret
        result = resolve_environment(None)
    ...
    def test_px_url_env_var_used_as_fallback(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / ".git").mkdir()
        monkeypatch.delenv("PORTALS_URL", raising=False)
        monkeypatch.setenv("PX_URL", "http://px-fallback:7860")
        monkeypatch.setenv("PX_API_KEY", "px-key")  # pragma: allowlist secret
        result = resolve_environment(None)
        assert result.url == "http://px-fallback:7860"
        assert result.api_key == "px-key"  # pragma: allowlist secret

    def test_named_env_without_config_raises_clear_error(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / ".git").mkdir()
        monkeypatch.delenv("PORTALS_URL", raising=False)
        monkeypatch.delenv("PX_URL", raising=False)
...
class TestErrorMessages:
    ...
    def test_no_config_message_suggests_init(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("PORTALS_URL", raising=False)
        monkeypatch.delenv("PX_URL", raising=False)
        with pytest.raises(ConfigError, match="px init"):
            resolve_environment("staging")
