"""Integration tests for px CLI with Simple Agent flow.

These tests verify that the px CLI can properly load and execute the
Simple Agent starter project, addressing the bug where px serve/run
...
- px requires langchain-core>=0.3.66,<1.0.0
...
class TestSimpleAgentFlowLoading:
    ...
    def test_simple_agent_flow_loads_via_cli(self, simple_agent_flow_path: Path):
        """Test that px run can load the Simple Agent flow without critical errors."""
        ...
        critical_errors = [
            "No module named 'px.components",
            "No module named 'portals",
    ...
    def test_simple_agent_flow_json_output(self, simple_agent_flow_path: Path):
        """Test that px run produces valid JSON output."""
        result = runner.invoke(
            px_app,
            [
                "run",
...
class TestSimpleAgentServe:
    ...
    def test_serve_help(self):
        ...
        result = runner.invoke(px_app, ["serve", "--help"])
    ...
    def test_serve_requires_api_key(self, simple_agent_flow_path: Path, monkeypatch):
        """Test serve requires PORTALS_API_KEY."""
        ...
        assert result.exit_code != 0 or "PORTALS_API_KEY" in result.output
    ...
    def test_serve_starts_server_no_asyncio_error(self, simple_agent_flow_path: Path):
        """Regression test: px serve should not fail with asyncio error.

        This test verifies the fix for the issue where px serve failed with:
        ...
        proc = subprocess.Popen(  # noqa: S603
            ...
            env={
                **os.environ,
                "PORTALS_API_KEY": "test-key-12345",
...
class TestAllStarterProjectsLoad:
    """Test that all starter projects can load without px-specific module errors."""
    ...
    def test_all_projects_load(self, starter_projects_path: Path):
        """Test all starter project JSONs can load without px-specific errors.

        Note: This test only fails on px-specific module errors (px.components.*),
        ...
        for json_file in json_files:
            try:
                ...
                error_str = str(e)
                # Only track px-specific errors, not external dependency errors
                if "No module named 'px." in error_str:
                    px_module_errors.append((json_file.name, str(e)))
                elif "resolve_component_path" in error_str:
                    px_module_errors.append((json_file.name, f"Storage error: {e}"))
