...
import type { LocationWithAssets } from "#shared/types/workflow.types.js";
...
import { useAssetStore, useLocationAssets } from "#client/store/useAssetStore.js";
import { usePipelineStore } from "#client/store/usePipelineStore.js";
import { resolvePublicUrl } from "#shared/utils/utils.js";

interface LocationDetailPanelProps {
  location: Omit<LocationWithAssets, "assets">;
  projectId: string;
  isLoading?: boolean;
  onNext?: () => void;
  onPrevious?: () => void;
  ...
  const handleRegenerateClick = async () => {
    ...
    try {
      ...
      await generateLocationImage([
        { locationId: location.id, prompt: description, numberOfOutputs: 1 },
      ]);
  ...
  return (
    <>
      ...
      <div
        ...
        data-testid={`panel-location-detail-${location.id}`}
      >
            ...
            <div className="flex flex-col gap-3">
              <FramePreview
                ...
                onDelete={() =>
                  handleDeleteAsset(
                    "location_image",
                    assets["location_image"]?.version || 0,
                  )
                }
                ...
                priority={true}
                metadata={{
                  width: assets["location_image"]?.metadata?.width,
                  height: assets["location_image"]?.metadata?.height,
                }}
              />
            </div>
                ...
                <Card>
                  ...
                  <CardContent className="p-3 pt-0 space-y-3">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {[...location.naturalElements, ...location.manMadeObjects].map(
                          (item, i) => (
                  ...
                  <CardContent className="p-3 pt-0 space-y-3">
                    <div className="grid grid-cols-1 gap-2 ">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-medium capitalize">
                          {location.state.groundCondition.wetness}
                        </span>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-1 gap-2 text-muted-foreground">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-medium text-foreground">
                            {location.state.lighting.direction.keyLightPosition}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          Quality:{" "}
                          <span className="font-medium text-foreground">
                            {location.state.lighting.quality.hardness}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          Source:{" "}
                          <span className="font-medium text-foreground">
                            {location.state.lighting.motivatedSources.primaryLight}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          Color:{" "}
                          <span className="font-medium text-foreground">
                            {location.state.lighting.quality.colorTemperature}
                          </span>
                        </div>
                      </div>
                    </div>
