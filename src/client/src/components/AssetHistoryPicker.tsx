...
import { Clock, Play, Filter, SortDesc, CheckCircle2 } from "lucide-react";
import { VideoPlayer } from "#client/components/ui/video-player.js";
import { AssetKey, AssetVersion } from "#shared/types/assets.types.js";
import { EntityPrimitiveType } from "#shared/types/entity.types.js";
import { useProjectStore, selectCurrentScene } from "#client/store/useProjectStore.js";
...
export const AssetCard = memo(function AssetCard({
  ...
  return (
    <div
      ref={hoverRef}
      className={`group relative overflow-hidden cursor-pointer hover: ${
        isCurrent ? "" : ""
      }`}
      onClick={onClick}
    >
...
export function AssetHistoryPicker({
  ...
  const sceneIdsToPreload = useMemo(() => {
    ...
    return [
      currentScene.id,
      ...viewedScenesHistory.filter((id) => id !== currentScene.id).slice(-5),
    ];
