...
function CanvasContextMenuHandler({
  isOpen,
  screenPosition,
  onPositionUpdate,
}: CanvasContextMenuHandlerProps) {
  ...
  useEffect(() => {
    if (
      isOpen &&
      (screenPosition.x !== prevPosition.current.x ||
        screenPosition.y !== prevPosition.current.y)
    ) {
...
export interface NodeGraphProps {
  ...
  onSelectionDragStop?: (event: React.MouseEvent, nodes: CanvasNode[]) => void;
    ...
    >
      <ReactFlow
        ...
        onConnectEnd={() => {
          ...
          setTimeout(
            () => useCanvasInteractionStore.getState().setInitiatorNodeId(null),
            100,
          );
        }}
