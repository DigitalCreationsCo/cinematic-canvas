  ...
  return (
    <div
      ...
      className={cn(
        ...
        image.useType ? `border-b-2 ${selectedOption?.activeBorder}` : "border-border",
      )}
    >
      {/* Thumbnail */}
...
export function BulkFilesStagingPanel({
  ...
  const handlePlaceAll = useCallback(async () => {
    ...
    const createdNodes: string[] = []; // Track IDs for cleanup on failure
    ...
    try {
      ...
      const entityTypeImages = toPlace.filter(
        (img) => img.useType === "character" || img.useType === "location",
      );

      // 1. Process uploads in parallel
      const entityPayloads = await Promise.all(
      ...
      // 2. Bulk create all entities in one request
      if (entityPayloads.length > 0) {
        await api.entities.create.mutate(entityPayloads.map((p) => p.payload));
        ...
        entityPayloads.forEach(({ img, entityId }) => {
          ...
          createdNodes.push(node.id, pendingNode.id);
        });
      }
      ...
      console.error("[BulkFilesStagingPanel] Bulk placement failed:", error);

      // Rollback local UI nodes if they were added before the crash
      createdNodes.forEach((id) => nodeStore.permanentlyDeleteNode(id));

      addNotification({
        id: generateId(),
        type: "error",
        message: `Failed to place all items. Please try again.`,
        timestamp: new Date(),
      });
    } finally {
      setIsPlacing(false);
    }
  }, [readyImages, onPlace, _projectId, setStagedFiles, addNotification]);
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
            ...
            {USE_TYPE_OPTIONS.map((opt) => {
              ...
              return (
                <Button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 text-xs border-border text-muted-foreground hover:bg-muted hover:text-foreground transition-colors h-7"
                >
                  <Icon className={cn("w-3 h-3", opt.colorClass)} />
                  <span>All to {opt.label}</span>
                </Button>
              );
            })}
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={readyImages.length === 0 || isPlacing}
            className={cn(
