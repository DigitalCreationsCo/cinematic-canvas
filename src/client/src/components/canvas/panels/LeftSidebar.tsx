  ...
  return (
    <div>
      ...
      >
        ...
        <span
          className={cn(
            "text-xs uppercase tracking-wider flex-1 font-mono transition-colors duration-50",
            isOpen
              ? "text-foreground font-medium"
              : "text-muted-foreground group-hover:text-foreground group-hover:underline",
          )}
        >
          {section.label}
        </span>
      ...
      >
        <div className="collapsible-inner">
          {headerContent && <div className="collapsible-header">{headerContent}</div>}
          <div className="collapsible-content">{children}</div>
          {isOpen && <div className="border-b border-border mx-2" />}
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex flex-col min-w-0 flex-1 overflow-hidden">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[11px] font-medium truncate text-foreground/90 group-hover:text-foreground leading-tight">
            {name}{" "}
            {isWorldEntity && (
              <span className="text-[9px] text-primary ml-1">(World)</span>
            )}
          </span>
        </div>
        {isOnCanvas && (
...
export function LeftSidebar({ contextId, contextType }: CombinedSidebarProps) {
  ...
  const [modalType, setModalType] = useState<"character" | "location" | "scene">(
    "character",
  );
  ...
  const handleAudioFileDrop = async (file: File) => {
    ...
    const displayName =
      file.name.replace(/\.[^/.]+$/, "").replace(/[-_]/g, " ") || "Imported Audio";
  ...
  return (
    ...
    >
      <div className="p-4 border-b flex items-center justify-between shrink-0 min-h-[52px]">
        <span className="text-xs tracking-wide font-heading font-normal text-muted-foreground uppercase">
          Workspace
        </span>
        <div className="flex items-center gap-1">
          <Button
            ...
            style={{
              visibility: Object.values(openSections).some(Boolean)
                ? "visible"
                : "hidden",
            }}
          >
            ...
            <div className="space-y-3 px-4">
              ...
              {sequenceMode === "explicit" ? (
                ...
                <div className="text-[10px] text-muted-foreground p-2">
                  Scenes are ordered by canvas edge connections. Enable "Linear List" to
                  manually reorder.
                </div>
              )}
            </div>
