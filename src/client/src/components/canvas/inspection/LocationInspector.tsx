import React from 'react';
import type { CanvasNode } from '../../../domain/canvas/NodeTypes.js';
import { useProjectStore } from '../../../store/useProjectStore.js';
import { useLocationAssets } from '../../../store/useAssetStore.js';
import { RbacBanner } from './RbacBanner.js';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '../../ui/tabs.js';
import { Label } from '../../ui/label.js';
import { Textarea } from '../../ui/textarea.js';
import { Input } from '../../ui/input.js';

export function LocationInspector({ node }: { node: CanvasNode; }) {
  const location = useProjectStore((state) => state.locations.get(node.data.entityId));
  const updateLocation = useProjectStore((state) => state.updateLocation);
  const { assets } = useLocationAssets(node.data.entityId);
  const isLocked = node.data.isLocked;

  if (!location) return <div className="p-4 text-gray-500">Location not found</div>;

  return (
    <div className="p-4 flex flex-col h-full bg-gray-950 text-gray-200">
      <div className="mb-4">
        <h2 className="text-xl font-bold flex items-center gap-2 text-orange-100">
          { location.name || 'Unnamed Location' }
        </h2>
        { node.data.scope === 'world' && <span className="text-xs text-indigo-400 uppercase font-semibold mt-1">World Library Entity</span> }
      </div>

      <RbacBanner isLocked={ isLocked } entityType="location" />

      <Tabs defaultValue="attributes" className="flex-1 overflow-auto">
        <TabsList className="w-full grid border-b border-gray-800 bg-transparent rounded-none h-10 p-0">
          <TabsTrigger value="attributes" className="flex-1 data-[state=active]:bg-gray-800 data-[state=active]:text-white rounded-none border-t border-x border-transparent data-[state=active]:border-gray-700">Attributes</TabsTrigger>
          <TabsTrigger value="gen" className="flex-1 data-[state=active]:bg-gray-800 data-[state=active]:text-white rounded-none border-t border-x border-transparent data-[state=active]:border-gray-700">Gen Status</TabsTrigger>
        </TabsList>

        <TabsContent value="attributes" className="p-2 pt-4 flex flex-col gap-4">
          <div className="space-y-2">
            <Label className="text-gray-400 text-xs uppercase">Name</Label>
            <Input
              className="bg-gray-900 border-gray-700 disabled:opacity-60"
              value={ location.name }
              onChange={ (e) => updateLocation(location.id, { name: e.target.value }) }
              disabled={ isLocked }
            />
          </div>

          <div className="space-y-2">
            <Label className="text-gray-400 text-xs uppercase">Description</Label>
            <Textarea
              className="resize-none h-32 bg-gray-900 border-gray-700 disabled:opacity-60"
              value={ assets?.location_description?.versions?.[ 0 ]?.data || '' }
              onChange={ (e) => { } /* Needs patchAsset endpoint implementation to save */ }
              disabled={ true }
              placeholder="Architecture, layout, environment details..."
            />
          </div>

          <div className="space-y-2 pt-2 border-t border-gray-800">
            <h4 className="text-xs font-semibold uppercase text-gray-500 mb-2">Environment State</h4>
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                <Label className="text-gray-400 text-[10px] uppercase">Time of Day</Label>
                <Input disabled={ isLocked } placeholder="e.g. Golden hour" className="h-8 bg-gray-900 border-gray-800 text-sm" value={ location.timeOfDay || '' } />
              </div>
              <div className="space-y-1">
                <Label className="text-gray-400 text-[10px] uppercase">Weather</Label>
                <Input disabled={ isLocked } placeholder="e.g. Light rain" className="h-8 bg-gray-900 border-gray-800 text-sm" value={ location.weather || '' } />
              </div>
            </div>
          </div>
        </TabsContent>

        <TabsContent value="gen" className="p-2 pt-4">
          <div className="p-4 bg-gray-900 border border-gray-800 rounded-lg text-center text-sm text-gray-400">
            Establishment shot generation history.
          </div>
        </TabsContent>
      </Tabs>
    </div>
  );
};
