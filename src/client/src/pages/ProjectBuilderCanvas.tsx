...
} from "#client/domain/canvas/CoordinateSystem.js";
import {
...
export default function ProjectBuilderCanvas() {
  ...
  useEffect(() => {
    ...
    console.debug("[ProjectBuilderCanvas] Canvas cleared for project", {
      projectId,
    });
    ...
    return () => {
      ...
      console.debug("[ProjectBuilderCanvas] Canvas cleanup on unmount", {
        projectId,
      });
    };
  }, [projectId]);
  ...
  const handleSaveResult = useCallback(
    async (result: { success: boolean; error?: string; timestamp: Date }) => {
      if (result.success) {
        ...
        if (result.error?.includes("OCC conflict")) {
          ...
          try {
            ...
            layouts.forEach((layout) => {
              store.updateNodeData(layout.idEntity, {
                idxVersion: layout.idxVersion,
              });
              store.updateNodePosition(layout.idEntity, {
                x: layout.valPosX,
                y: layout.valPosY,
              });
            });
        ...
        >
          <AnimatePresence>
            ...
            {stagedFiles.length === 0 && (
              <DropFilesOverlay
                isDraggingFileOverCanvas={isDraggingFileOverCanvas}
