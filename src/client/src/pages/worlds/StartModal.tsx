...
const ActionButton: React.FC<ActionButtonProps> = React.memo(
  ({ label, action, image, hoverImage, onSelectAction, posImage, ...props }) => {
    ...
    return (
      ...
      `}</style>
        ...
        <img
          ...
          alt={label}
          onAnimationEnd={handleAnimationEnd}
          className={`mask-layer absolute inset-0 h-full w-full object-cover -z-10 ${posImage} ${
...
export const StartModal: React.FC<StartModalProps> = React.memo(
  ({ isOpen, onSelectAction }) => {
    return (
      <Dialog open={isOpen}>
        <DialogContent
        ...
        >
          <DialogHeader className="my-8 items-center text-center">
            <DialogTitle className="text-3xl uppercase font-heading mb-2 text-foreground/80">
              Welcome to Cinematic Canvas
            </DialogTitle>
