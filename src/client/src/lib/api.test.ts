import type * as ApiModule from "#client/lib/api.js";
import { describe, it, expect, vi, beforeEach, beforeAll } from "vitest";

const { mockMutate } = vi.hoisted(() => ({ mockMutate: vi.fn().mockResolvedValue({}) }));
const { mockQuery } = vi.hoisted(() => ({ mockQuery: vi.fn().mockResolvedValue({}) }));
...
beforeAll(async () => {
  api = await import("#client/lib/api.js");
});
...
describe("api.ts - Pipeline Functions", async () => {
  beforeEach(() => {
    vi.clearAllMocks();
