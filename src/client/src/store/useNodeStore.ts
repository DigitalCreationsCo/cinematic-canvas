    ...
    temporal(
      (set, get) => ({
        ...
        onConnect: (connection) => set({
          edges: addEdge(connection, get().edges) as CanvasEdge[]
        }),
      ...
      {
        partialize: (state) => {
          // Only track nodes and edges in temporal history (not viewport)
          return { nodes: state.nodes, edges: state.edges };
        },
        equality: (pastState, currentState) => {
          // Very rough equality check to avoid saving no-ops
          return pastState.nodes === currentState.nodes && pastState.edges === currentState.edges;
        },
        limit: 50,
      }
    )
