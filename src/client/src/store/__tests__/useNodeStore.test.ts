...
describe.skip("useNodeStore", () => {
  ...
  describe.skip("addNode", () => {
    it("should add a node to existing nodes", () => {
      ...
      expect(result.current.nodes).toContainEqual(expect.objectContaining({ id: "1", type: "scene" }));
      expect(result.current.nodes).toContainEqual(expect.objectContaining({ id: "2", type: "scene" }));
    });
  ...
  describe.skip("permanentlyDeleteNode", () => {
  ...
  describe.skip("isNodeSoftDeleted", () => {
  ...
  describe.skip("getConnectedEdges", () => {
...
describe.skip("onNodesChange", () => {
  it("applies a position change to a node", () => {
    useNodeStore.getState().addNode(makeNode("n1"));
    useNodeStore.getState().onNodesChange([{ type: "position", id: "n1", position: { x: 99, y: 88 } }]);
...
describe.skip("promotePendingNode", () => {
  ...
  it("only modifies the specified node", () => {
    ...
    expect(useNodeStore.getState().nodes[0].data.isPending).toBe(false);
    expect(useNodeStore.getState().nodes[1].data.isPending).toBe(false);
  });
});
