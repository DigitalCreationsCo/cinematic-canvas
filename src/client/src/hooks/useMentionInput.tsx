...
/**
 * Checks if the editor DOM already contains a mention chip with the given handle.
 */
function hasExistingMention(editor: HTMLElement, handle: string): boolean {
  return !!editor.querySelector(`[data-handle="${CSS.escape(handle)}"]`);
}

/**
 * Deletes the @query text from the caret position and inserts a mention chip,
 * then positions the cursor immediately after the chip.
 *
 * @param savedSelection — optional selection to restore if the current one is
 *   empty (needed when clicking a portal popover that blurs the editor).
 */
function insertMentionChip(
  ...
  // If the selection was cleared by clicking a portal element, restore it
  if ((!sel || sel.rangeCount === 0) && savedSelection) {
    sel?.removeAllRanges();
  ...
  // 4. Position caret immediately after chip (no trailing characters inserted)
...
export function useMentionInput({
  ...
  const selectSuggestion = useCallback(
    (suggestion: MentionSuggestion) => {
      ...
      // Save the current selection before the popover click might clear it
      // (the popover is a portal to document.body, so clicking it can blur
      //  the contentEditable and destroy the selection).
      const savedSelection = window.getSelection()?.getRangeAt(0) ?? null;

      // Prevent duplicate mentions — skip if the handle already exists
      if (editorRef.current && hasExistingMention(editorRef.current, suggestion.handle)) {
        closeSuggestions();
        return;
      }

      insertMentionChip(suggestion, triggerState.query.length, savedSelection);
      closeSuggestions();
