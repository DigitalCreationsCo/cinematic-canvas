...
} from "#client/domain/canvas/ConnectionValidator.js";
...
export function useCanvasConnections(nodes: CanvasNode[]) {
  const nodeTypeMap = useCallback(
    (id: string): CanvasNodeType | undefined => nodes.find((n) => n.id === id)?.type as CanvasNodeType | undefined,
    [nodes],
  );
  ...
  const onConnect = useCallback(
    (connection: Connection) => {
      ...
      if (rule.oneToOne) {
        const edgeConflicting = edges.find((e) => e.target === target && e.type === rule.edgeType);
        if (edgeConflicting) {
          addPendingChange({
            sourceType,
            targetType,
...
function bumpPendingCount(nodeId: string, updateNodeData: (id: string, data: any) => void): void {
...
function decrementPendingCount(nodeId: string, updateNodeData: (id: string, data: any) => void): void {
  const node = useNodeStore.getState().nodes.find((n) => n.id === nodeId);
  if (!node) return;
