  ...
  describe("Q2: Asset updates during generation", () => {
    it("updates assets via ENTITY_UPDATED event flow", () => {
      ...
      act(() => {
        const { assets: sceneAssets, ...rest } = incomingScene;
        if (sceneAssets)
          useAssetStore.getState().mergeAssets(incomingScene.id, sceneAssets);
    ...
    it("merges assets via NEW_ASSETS_BATCH event", () => {
      ...
      act(() => {
        useAssetStore
          .getState()
          .mergeAssetHistories([
            { entityId: "scene-3", assetKey: "scene_end_frame", history: newHistory },
          ]);
      });
      ...
      expect(registry?.scene_end_frame?.versions[0].data).toBe("gs://bucket/new.png");
    });
  });
...
describe("Integration: Complete pipeline", () => {
  it("handles full lifecycle from creation to display", () => {
    ...
    act(() => {
      useProjectStore
        .getState()
        .updateScene("integration-scene", { status: "generating" });
    });

    expect(useProjectStore.getState().scenes.get("integration-scene")?.status).toBe(
      "generating",
    );
    ...
    expect(
      useAssetStore.getState().assets.get("integration-scene")?.scene_start_frame,
    ).toBeDefined();
    ...
    act(() => {
      ...
      if (sceneAssets)
        useAssetStore.getState().mergeAssets(completeScene.id, sceneAssets);
    ...
    expect(
      useAssetStore.getState().assets.get("integration-scene")?.scene_video,
    ).toBeDefined();
