...
// Desktop Portals Assistant avatar bug is fixed: the component must render
...
jest.mock("@/assets/portals_assistant.svg", () => "portals-icon.svg");
...
describe("AssistantMessageItem", () => {
  describe("user messages", () => {
    ...
    // Regression guard: Portals Desktop shipped with a broken user avatar in
  ...
  describe("assistant messages", () => {
    it("should render assistant label with Portals icon", () => {
      const message = createMessage({
        role: "assistant",
      ...
      expect(screen.getByText("Portals Assistant")).toBeInTheDocument();
      expect(screen.getByAltText("Portals Assistant")).toBeInTheDocument();
    });
  });
  ...
  describe("plain text response", () => {
    it("should render markdown for regular text content", () => {
      const message = createMessage({
        role: "assistant",
        content: "Portals is a visual flow builder.",
        status: "complete",
      });
      ...
      expect(screen.getByTestId("markdown-content")).toHaveTextContent(
        "Portals is a visual flow builder.",
      );
    });
    ...
    it("should_render_markdown_when_qa_response_contains_example_component_code", () => {
      ...
      const qaWithExampleCode = [
        ...
        "```\n\n",
        "Save the file and restart Portals.",
      ].join("");
