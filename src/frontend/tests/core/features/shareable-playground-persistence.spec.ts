...
// Set PORTALS_AUTO_LOGIN=false in your .env to run these tests.
...
async function setupAutoLoginOff(page: any) {
  ...
  await page.goto("/");
  await page.waitForSelector("text=sign in to portals", { timeout: 30000 });
...
test(
  ...
  async ({ page, context }) => {
    ...
    test.skip(
      process?.env?.PORTALS_AUTO_LOGIN !== "false",
