...
import { addFlowToTestOnEmptyPortals } from "../../utils/add-flow-to-test-on-empty-portals";
...
test(
  ...
  async ({ page }) => {
    ...
    const firstRunPortals = await page
      .getByTestId("empty-project-description")
      .count();

    if (firstRunPortals > 0) {
      await addFlowToTestOnEmptyPortals(page);
    }
