...
withEventDeliveryModes(
  ...
  async ({ page }) => {
    ...
    let portalsCount = await page
      .locator('[data-testid="portals-0-option"]')
      .count();
    ...
    while (portalsCount === 0) {
      ...
      portalsCount = await page
        .locator('[data-testid="portals-0-option"]')
        .count();
    }

    await page.locator('[data-testid="portals-0-option"]').nth(0).waitFor({
      timeout: 15000,
      state: "visible",
    });

    await page.getByTestId("portals-0-option").nth(0).click();
