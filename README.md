...
The Portals production infrastructure can be deployed to AWS using Terraform. The deployment provisions Lore Server, Backend API, and all supporting infrastructure including VPC, S3, DynamoDB, EFS, Secrets Manager, ECR, ALB, NLB, and Route53.

- **Location**: `infra/terraform-aws/`
- **Documentation**: See [infra/terraform-aws/README.md](infra/terraform-aws/README.md) for complete deployment instructions
- **Architecture**: ECS Fargate with Application Load Balancer (HTTP/HTTPS) and Network Load Balancer (QUIC/gRPC)
- **Prerequisites**: AWS account, Terraform >= 1.5.0, Supabase database URL

Quick start:
```bash
cd infra/terraform-aws/environments/production
cp terraform.tfvars.example terraform.tfvars
# Edit terraform.tfvars with your database_url and domain configuration
terraform init
terraform plan
terraform apply
```
